...
-- ============================================================
-- ============================================================
-- FARM LOGIC (REWRITE BERSIH)
-- ============================================================

-- Helper: cari target mob dari workspace
local function FindMobTarget()
    local selectedDict = Options.SelectedMob.Value or {}
    local char = GetCharacter()
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return nil end
    ...
    for _, obj in ipairs(npcFolder:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
            ...
            if hum.Health > 0 then
                local isSelected = false
                if Toggles.AllMobFarm.Value then
                    isSelected = true
                elseif selectedDict[obj.Name] then
                    isSelected = true
                end
                if isSelected then
...
-- Helper: cari target boss
local function FindBossTarget()
    local selectedDict = Options.SelectedBosses.Value or {}
    local char = GetCharacter()
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return nil end
    ...
    for _, obj in ipairs(npcFolder:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
            ...
            if hum.Health > 0 then
                local isSelected = false
                if Toggles.AllBossesFarm.Value then
                    -- boss = nama mengandung "Boss"
                    if obj.Name:find("Boss") then isSelected = true end
                elseif selectedDict[obj.Name] then
                    isSelected = true
                end
                if isSelected then
...
-- Helper: equip weapon (melee/sword/power)
local function DoEquipWeapon()
    local char = GetCharacter()
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if not hum then return end

    local enabledTypes = Options.SelectedWeaponType.Value or {}
    -- fallback: kalau tidak ada pilihan, equip apapun
    local hasType = false
    for _, v in pairs(enabledTypes) do if v then hasType = true break end end

    local containers = { Plr.Backpack, char }
    for _, container in ipairs(containers) do
        for _, tool in ipairs(container:GetChildren()) do
            if tool:IsA("Tool") then
                local toolType = GetToolTypeFromModule and GetToolTypeFromModule(tool.Name) or "Melee"
                if not hasType or enabledTypes[toolType] then
...
-- Helper: move ke target (teleport atau tween)
local function MoveToTarget(target)
    ...
    if (root.Position - finalPos).Magnitude < 2 then return end
    ...
    if moveType == "Teleport" then
        ...
        local speed = tonumber(Options.TweenSpeed and Options.TweenSpeed.Value) or 160
...
-- Helper: attack target (M1)
local function DoAttack()
    pcall(function() Remotes.M1:FireServer() end)
end

-- Helper: gunakan skill ke target
...
-- MAIN FARM LOOP (bersih, sederhana)
local _farmThread = nil
local function StartFarmLoop()
    if _farmThread then pcall(task.cancel, _farmThread) end
    _farmThread = task.spawn(function()
        while getgenv().celina_Running do
            task.wait(0.1)

            -- Cek apakah ada farm mode yang aktif
            ...
            local isLvlFarm  = Toggles.LevelFarm.Value

            if not (isMobFarm or isBossFarm or isLvlFarm) then
                Shared.Target = nil
                task.wait(0.3)
                continue
            end
            ...
            -- Cari target berdasarkan priority: Boss > Mob
            local target = nil
            if isBossFarm then target = FindBossTarget() end
            if not target and isMobFarm then target = FindMobTarget() end

            -- Jika tidak ada target, tunggu spawn
            if not target then
                Shared.Target = nil
                task.wait(0.5)
                continue
            ...
            -- Target valid: set Shared.Target
            Shared.Target = target
            ...
            -- 2. Island TP jika perlu
            if Toggles.IslandTP and Toggles.IslandTP.Value then
                local island = GetNearestIsland and GetNearestIsland(target:GetPivot().Position, target.Name)
                if island and island ~= "" and island ~= Shared.Island then
                    Shared.Island = island
                    pcall(function() Remotes.TP_Portal:FireServer(island) end)
                    task.wait(tonumber(Options.IslandTPCD and Options.IslandTPCD.Value) or 0.8)
                end
            end

            -- Cek lagi target masih valid setelah TP
            ...
            -- 3. Gerak ke target
            pcall(MoveToTarget, target)

            -- Cek lagi setelah gerak
            hum = target:FindFirstChildOfClass("Humanoid")
            if not hum or hum.Health <= 0 or not target.Parent then
                Shared.Target = nil
                continue
            ...
            -- 5. Skills (jika AutoSkill ON)
            if Toggles.AutoSkill and Toggles.AutoSkill.Value then
                pcall(DoSkills)
            end

            task.wait(tonumber(Options.M1Speed and Options.M1Speed.Value) or 0.05)
        end
    end)
end

-- Kill Aura saja (gabungkan Auto Attack + Kill Aura jadi satu)
local _auraThread = nil
local function StartKillAuraLoop()
    if _auraThread then pcall(task.cancel, _auraThread) end
    _auraThread = task.spawn(function()
        while getgenv().celina_Running do
            task.wait(tonumber(Options.KillAuraCD and Options.KillAuraCD.Value) or 0.1)
            if not Toggles.KillAura.Value then task.wait(0.3); continue end
