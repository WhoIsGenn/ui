...
-- ================================================================
-- AUTO PARRY SYSTEM (Hybrid Detection)
-- ================================================================
...
-- ================================================================
-- HYBRID ATTACK DETECTION (Dot Product + Trace Ray)
-- ================================================================
local function IsKillerAttackingMe(killerChar)
    ...
    -- ========== METHOD 1: Dot Product (Arah Hadap) ==========
    local killerForward = killerRoot.CFrame.LookVector
    local toPlayer = (myRoot.Position - killerRoot.Position).Unit
    local dot = killerForward:Dot(toPlayer)
    local angle = math.deg(math.acos(math.clamp(dot, -1, 1)))
    
    -- Kalo arah hadap terlalu jauh (> 60°), skip (biar ga kena gocek)
    if angle > 60 then
        if Config.PARRY_Debug then
            print(string.format("[Parry] Angle: %.1f° > 60°, skip (gocek?)", angle))
        end
        return false
    end
    
    -- ========== METHOD 2: Trace Ray (Cek Halangan) ==========
    local attackOrigin = killerRoot.Position
    local rightHand = killerChar:FindFirstChild("RightHand")
    if rightHand then
        attackOrigin = rightHand.Position
    end
    
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Blacklist
    params.FilterDescendantsInstances = {killerChar, LocalPlayer.Character}
    
    local direction = (myRoot.Position - attackOrigin)
    local ray = workspace:Raycast(attackOrigin, direction, params)
    local hasObstacle = ray and ray.Instance and ray.Instance.Parent ~= LocalPlayer.Character
    
    if hasObstacle then
        if Config.PARRY_Debug then
            print(string.format("[Parry] Obstacle: %s, skip (tembok/pallet)", ray.Instance.Name))
        end
        return false
    end
    
    if Config.PARRY_Debug then
        print(string.format("[Parry] ✅ Attack confirmed! Angle: %.1f° | Dist: %.1f", angle, dist))
    end
...
-- ================================================================
-- AUTO PARRY HOOK KILLER CHARACTER (with Hybrid Detection)
-- ================================================================
local function AutoParry_HookKillerChar(char, p)
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            -- ========== HYBRID DETECTION CHECK ==========
            if not IsKillerAttackingMe(char) then
                return
            end
        ...
        -- Method 2: Heartbeat GetPlayingAnimationTracks (backup)
