...
local ShortMap = {
    ["DamagePercent"]    = "DMG%",  ["CritChance"]       = "CC%",
    ["CritDamage"]       = "CD%",   ["BonusDropChance"]  = "Drop%",
    ["ExecuteBonus"]     = "BonusDMG%", ["ExecuteThreshold"] = "Exec%",
    ["Damage"]           = "DMG",   ["Crit Chance"]      = "CC",
    ["Crit Damage"]      = "CD",    ["Luck"]             = "Luck"
}
...
local function Shorten(text)
    for _, long in ipairs(SortedShortKeys) do
        text = text:gsub(long, ShortMap[long]:gsub("%%", "%%%%"))
    end
    return text
end
...
local function UpdatePassiveSliders()
    ...
    for _, slider in pairs(Shared.SpecStatsSlider or {}) do
        pcall(function() slider:SetVisible(false) end)
    end
        ...
        if isSelected then
            ...
            if data and data.Buffs then
                for statKey, range in pairs(data.Buffs) do
                    ...
                    if not (Shared.SpecStatsSlider and Shared.SpecStatsSlider[sliderId]) then
                        S(sfEN_Cfg_ref, sliderId, { Text = Shorten(passiveName).." ["..Shorten(statKey).."]", Default = range[1], Min = range[1], Max = range[2], Rounding = 1 })
                        if Shared.SpecStatsSlider then Shared.SpecStatsSlider[sliderId] = Options[sliderId] end
                    end
                    if Shared.SpecStatsSlider and Shared.SpecStatsSlider[sliderId] then
...
for name, tab in pairs(Tabs) do
    if tab == nil then warn("[celina] Tab nil: " .. name) end
end

-- Helper: AddSection dengan guard
local function Sec(tab, cfg)
    if not tab then warn("[celina] Sec() tab nil for: " .. (cfg and cfg.Title or "?")); return setmetatable({},{__index=function() return function() end end}) end
    local ok, result = pcall(function() return tab:AddSection(cfg) end)
    if not ok or not result then
        warn("[celina] AddSection fail: " .. tostring(result))
...
local function D(sec, id, cfg)
    ...
    local el = sec:AddDropdown({
        ...
        Multi = cfg.Multi or false, Default = cfg.Default, Locked = cfg.Disabled or false,
        Callback = function(v)
            flag.Value = v
            for _, f in ipairs(flag._cb) do pcall(f, v) end
            if cfg.Callback then pcall(cfg.Callback, v) end
...
AddSliderToggle = function(cfg)
    local toggleFlag = T(cfg.Group, cfg.Id, {
        Text = cfg.Text, Default = cfg.DefaultToggle or false, Disabled = cfg.Disabled or false,
    })
    local sliderFlag = S(cfg.Group, cfg.Id .. "Value", {
        Text = cfg.Text, Default = cfg.Default, Min = cfg.Min, Max = cfg.Max,
        Rounding = cfg.Rounding or 0, Disabled = cfg.Disabled or false,
    })
    toggleFlag:OnChanged(function(v)
        -- tidak bisa SetVisible per elemen di Brigida, skip
    end)
    return toggleFlag, sliderFlag
end
...
-- INFORMATION TAB
-- ============================================================
local secInfoUser   = Sec(Tabs.Information, { Title = "User",   Open = false })
local secInfoGame   = Sec(Tabs.Information, { Title = "Game",   Open = false })
local secInfoOthers = Sec(Tabs.Information, { Title = "Others", Open = false })
...
-- PRIORITY TAB
-- ============================================================
local secPriority = Sec(Tabs.Priority, { Title = "Config", Open = false })
...
-- MAIN TAB
-- ============================================================
local secFarm = Sec(Tabs.Main, { Title = "Farm", Open = false })
...
local secBoss = Sec(Tabs.Main, { Title = "Boss", Open = false })
...
local secAltHelp = Sec(Tabs.Main, { Title = "Alt Help", Open = false })
...
local secFarmCfg = Sec(Tabs.Main, { Title = "Farm Config", Open = false })
...
local secHaki = Sec(Tabs.Main, { Title = "Haki", Open = false })
...
local secSkill = Sec(Tabs.Main, { Title = "Skill", Open = false })
...
local secCombo = Sec(Tabs.Main, { Title = "Combo", Open = false })
...
local secSwitch = Sec(Tabs.Main, { Title = "Switch - Title", Open = false })
local secSwitchRune  = Sec(Tabs.Main, { Title = "Switch - Rune",  Open = false })
local secSwitchBuild = Sec(Tabs.Main, { Title = "Switch - Build", Open = false })
...
-- AUTOMATION TAB
-- ============================================================
local secAscend = Sec(Tabs.Automation, { Title = "Ascend", Open = false })
...
local secRolls = Sec(Tabs.Automation, { Title = "Rolls", Open = false })
...
local secTrade = Sec(Tabs.Automation, { Title = "Trade", Open = false })
...
local secStatLevel = Sec(Tabs.Automation, { Title = "Stats - Level", Open = false })
...
local secStatGem = Sec(Tabs.Automation, { Title = "Stats - Gem", Open = false })
...
local secStatMisc = Sec(Tabs.Automation, { Title = "Stats - Misc", Open = false })
...
local secEnchant = Sec(Tabs.Automation, { Title = "Enchant", Open = false })
...
local secPassive = Sec(Tabs.Automation, { Title = "Passive", Open = false })
...
sfEN_Cfg_ref = Sec(Tabs.Automation, { Title = "Passive Config", Open = false })
L(sfEN_Cfg_ref, "Advanced users only. Will stop once passive reaches min value.")

-- ============================================================
-- ARTIFACT TAB
-- ============================================================
local secArtStatus  = Sec(Tabs.Artifact, { Title = "Status",     Open = false })
local secArtEquip   = Sec(Tabs.Artifact, { Title = "Auto-Equip", Open = false })
local secArtUpgrade = Sec(Tabs.Artifact, { Title = "Upgrade",    Open = false })
local secArtLock    = Sec(Tabs.Artifact, { Title = "Lock",       Open = false })
local secArtDelete  = Sec(Tabs.Artifact, { Title = "Delete",     Open = false })
...
-- DUNGEON TAB
-- ============================================================
local secDungeonJoin = Sec(Tabs.Dungeon, { Title = "Auto Join", Open = false })
...
local secDungeonCfg = Sec(Tabs.Dungeon, { Title = "Config", Open = false })
...
-- PLAYER TAB
-- ============================================================
local secGeneral = Sec(Tabs.Player, { Title = "General", Open = false })
local secServer  = Sec(Tabs.Player, { Title = "Server",  Open = false })
local secGame    = Sec(Tabs.Player, { Title = "Game",    Open = false })
local secSafety  = Sec(Tabs.Player, { Title = "Safety",  Open = false })
...
-- TELEPORT TAB
-- ============================================================
local secIsland = Sec(Tabs.Teleport, { Title = "Island", Open = false })
...
local secQuestNPC = Sec(Tabs.Teleport, { Title = "Quest NPC", Open = false })
...
local secMiscNPC = Sec(Tabs.Teleport, { Title = "Misc NPC", Open = false })
...
local secMovesetNPC = Sec(Tabs.Teleport, { Title = "Moveset NPC", Open = false })
D(secMovesetNPC, "SelectedMovesetNPC", {
    Text = "Select NPC", Values = Tables.NPC_MovesetList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

local secMasteryNPC = Sec(Tabs.Teleport, { Title = "Mastery NPC", Open = false })
...
-- MISC TAB
-- ============================================================
local secMerchant = Sec(Tabs.Misc, { Title = "Regular Merchant", Open = false })
...
local secDungeonMerchant = Sec(Tabs.Misc, { Title = "Dungeon Merchant", Open = false })
...
local secValentine = Sec(Tabs.Misc, { Title = "Valentine Merchant", Open = false })
...
local secTowerMerchant = Sec(Tabs.Misc, { Title = "Tower Merchant", Open = false })
...
local secBossRushMerchant = Sec(Tabs.Misc, { Title = "Boss Rush Merchant", Open = false })
...
local secChests = Sec(Tabs.Misc, { Title = "Chests", Open = false })
...
local secCraft = Sec(Tabs.Misc, { Title = "Craft", Open = false })
...
local secNotify = Sec(Tabs.Misc, { Title = "Notify", Open = false })
T(secNotify, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

local secPuzzles = Sec(Tabs.Misc, { Title = "Puzzles", Open = false })
...
local secQuestlines = Sec(Tabs.Misc, { Title = "Questlines", Open = false })
...
-- WEBHOOK TAB
-- ============================================================
local secWebhook = Sec(Tabs.Webhook, { Title = "Config", Open = false })
...
-- CONFIG TAB
-- ============================================================
local secMenu = Sec(Tabs.Config, { Title = "Menu", Open = false })
