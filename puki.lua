    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Team then
            ...
            elseif mode == "Closest" then
                -- Target siapapun yang ada di dalam FOV, tidak peduli role
                shouldTarget = true
            end
...
local AttackAnimIDs = {
    ["100092272524635"] = true, -- abys+veil:corrupt
    ...
    ["112166042383605"] = true, -- veil:kickanimation
...
local HealConn = nil
local HealOtherConn = nil

-- Fungsi untuk instant complete heal bar
-- Dari heal.txt: game track HealProgress attribute (0-100)
-- Saat >= 93, tombol bangkit muncul dan bisa di-firesignal
-- Cache tombol heal dari GUI
local HealBtnSelf = nil
local HealBtnOther = nil

local function GetHealButtons()
    local gui = LocalPlayer:FindFirstChild("PlayerGui")
    if not gui then return end
    local survivorGui = gui:FindFirstChild("Survivor-mob") or gui:FindFirstChild("Survivor")
    ...
    HealBtnSelf  = action:FindFirstChild("healyourself")
    HealBtnOther = action:FindFirstChild("healother")
end

local function FireHealBtn(btn)
...
local function InstantHeal_Setup()
    ...
    HealConn = RunService.Heartbeat:Connect(function()
        ...
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not hum then return end
        -- Saat knocked (Health < MaxHealth * 0.5) atau sedang recover
        local isKnocked = hum.Health < hum.MaxHealth * 0.5
        local isBeingHealed = char:GetAttribute("IsBeingHealed")
        if isKnocked or isBeingHealed then
            if not HealBtnSelf then GetHealButtons() end
            FireHealBtn(HealBtnSelf)
            -- Juga fire HealAnimRec
            local remotes = ReplicatedStorage:FindFirstChild("Remotes")
            local healing = remotes and remotes:FindFirstChild("Healing")
            if healing then
                local har = healing:FindFirstChild("HealAnimRec")
                if har then pcall(function() har:FireServer() end) end
            end
        end
    end)
end
...
local function InstantHealOther_Setup()
    if HealOtherConn then HealOtherConn:Disconnect() end
    GetHealButtons()
    HealOtherConn = RunService.Heartbeat:Connect(function()
        if not Config.SURV_InstantHealOther then return end
        if GetRole() ~= "Survivor" then return end
        -- Cek apakah ada survivor downed di sekitar
        local myRoot = GetCharacterRoot()
        if not myRoot then return end
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if h and h.Health < h.MaxHealth * 0.5 then
                    -- Ada survivor downed, spam healother
                    if not HealBtnOther then GetHealButtons() end
                    FireHealBtn(HealBtnOther)
                    break
                end
            end
        end
...
local function Heal_Cleanup()
    ...
    if HealOtherConn then pcall(function() HealOtherConn:Disconnect() end); HealOtherConn = nil end
end
...
local function ThirdPerson_Remove()
    ...
    -- Re-enable Firstperson hanya saat in-game (ada round), bukan lobby
    local role = GetRole()
    local inGame = role == "Survivor" or role == "Killer"
    local char = LocalPlayer.Character
...
secSurvival:AddToggle({
    ...
    Default = Config.SURV_InstantHealOther,
    Callback = function(v)
        Config.SURV_InstantHealOther = v
        if v then task.spawn(InstantHealOther_Setup) end
        notifToggle("Instant Heal Other", v)
    end
})
secSurvival:AddToggle({
