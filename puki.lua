...
-- ================================================================
-- PARRY FOV CIRCLE (LINGKARAN BULAT DI LANTAI)
-- ================================================================
...
local ParryFOVOutline = nil   -- outline putih (tepi)
local ParryFOVCircle = nil    -- garis warna (hijau/merah)
local ParryFOVFill = nil      -- fill transparan

local function CreateParryFOVCircle()
    if ParryFOVOutline then return end
    
    -- Outline putih (paling luar, biar keliatan)
    ...
    -- Garis utama (hijau/merah tergantung cooldown)
    ...
    -- Fill (highlight isi transparan)
    ParryFOVFill = Drawing.new("Circle")
    ParryFOVFill.Filled = true
    ParryFOVFill.Thickness = 0
    ParryFOVFill.Color = Color3.fromRGB(0, 255, 50)
    ParryFOVFill.Transparency = 0.6
    ParryFOVFill.NumSides = 64
    ParryFOVFill.Visible = false
end
...
local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        ...
        if ParryFOVFill then ParryFOVFill.Visible = false end
        return
    end
    ...
    -- Posisi circle di LANTAI (Y - 3.5, tepat di bawah kaki)
    local circlePos = root.Position + Vector3.new(0, -3.2, 0)
    local screenPos, onScreen = WorldToScreen(circlePos)
    
    if onScreen then
        -- Hitung radius berdasarkan jarak kamera (semakin jauh semakin kecil)
        local dist = (cam.CFrame.Position - circlePos).Magnitude
        local radius = (Config.PARRY_Dist * 50) / math.max(dist, 1)
        radius = math.clamp(radius, 30, 300)
        
        -- Warna berdasarkan cooldown
        local color
        if ParryOnCooldown then
            color = Color3.fromRGB(255, 50, 50)  -- Merah saat cooldown
        else
            color = Color3.fromRGB(0, 255, 50)   -- Hijau saat siap
        end
        
        -- Update OUTLINE PUTIH (paling luar)
        ParryFOVOutline.Position = screenPos
        ParryFOVOutline.Radius = radius + 2
        ParryFOVOutline.Visible = true
        
        -- Update GARIS UTAMA (warna sesuai cooldown)
        ParryFOVCircle.Position = screenPos
        ParryFOVCircle.Radius = radius
        ParryFOVCircle.Color = color
        ParryFOVCircle.Visible = true
        
        -- Update FILL (highlight isi)
        ParryFOVFill.Position = screenPos
        ParryFOVFill.Radius = radius
        ParryFOVFill.Color = color
        ParryFOVFill.Transparency = 0.55
        ParryFOVFill.Visible = true
    else
        ParryFOVOutline.Visible = false
        ParryFOVCircle.Visible = false
        ParryFOVFill.Visible = false
...
-- Cleanup function untuk Parry FOV
local function ParryFOV_Cleanup()
    ...
    if ParryFOVFill then ParryFOVFill:Remove(); ParryFOVFill = nil end
end
