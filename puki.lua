...
-- Parry FOV: world-space segmented lines + Highlight indicator
local PFOV_N        = 32
local PFOVLines     = {}   -- outline tipis (putih/biru)
local PFOVFillLines = {}   -- glow tebal (biru transparan)
for i = 1, PFOV_N do
    local ol = Drawing.new("Line")
    ol.Thickness    = 1.5
    ol.Color        = Color3.fromRGB(100, 220, 255)
    ol.Transparency = 0.0  -- 0 = fully visible di Roblox API
    ol.Visible      = false
    PFOVLines[i]    = ol

    local fl = Drawing.new("Line")
    fl.Thickness    = 5
    fl.Color        = Color3.fromRGB(0, 150, 255)
    fl.Transparency = 0.55
    fl.Visible      = false
    PFOVFillLines[i] = fl
end

-- Highlight part di lantai sebagai area indicator
local ParryAreaHighlight = nil
local ParryAreaPart      = nil

local function ParryArea_Show(root)
    if not ParryAreaPart then
        ...
        ParryAreaPart.Size        = Vector3.new(0.1, Config.PARRY_Dist * 2, Config.PARRY_Dist * 2)
    ...
    if not ParryAreaHighlight then
        ParryAreaHighlight = Instance.new("SelectionBox")
        ParryAreaHighlight.Name             = "_VHParryHL"
        ParryAreaHighlight.Color3           = Color3.fromRGB(0, 180, 255)
        ParryAreaHighlight.LineThickness    = 0
        ParryAreaHighlight.SurfaceColor3    = Color3.fromRGB(0, 180, 255)
        ParryAreaHighlight.SurfaceTransparency = 0.75
    ...
    ParryAreaPart.CFrame = CFrame.new(root.Position - Vector3.new(0, 2.8, 0))
        * CFrame.Angles(0, 0, math.rad(90))
    -- Warna: hijau normal, merah saat cooldown
    local col = ParryOnCooldown and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(0, 200, 255)
    ParryAreaHighlight.Color3        = col
    ParryAreaHighlight.SurfaceColor3 = col
end
...
local function ParryArea_Hide()
    ...
    for i = 1, PFOV_N do PFOVLines[i].Visible = false; PFOVFillLines[i].Visible = false end
end

local ParryFOVCircle = nil  -- kept for unload compat

local function CreateParryFOVCircle() end  -- no-op, replaced by above
...
local function UpdateParryFOV()
    ...
    -- Update Highlight area indicator
    ParryArea_Show(root)

    -- World-space circle segments di lantai
    local r    = Config.PARRY_Dist
    local baseY = root.Position.Y - 2.8  -- tepat di kaki
    ...
    for i = 1, PFOV_N do
        ...
        local col = ParryOnCooldown and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(100, 220, 255)
        PFOVLines[i].From    = s1;  PFOVLines[i].To    = s2
        PFOVLines[i].Color   = col; PFOVLines[i].Visible = vis
        PFOVFillLines[i].From  = s1;  PFOVFillLines[i].To  = s2
        PFOVFillLines[i].Color = col; PFOVFillLines[i].Visible = vis
    end
end
...
local function Unload()
    ...
    for i = 1, PFOV_N do
        ...
        pcall(function() PFOVFillLines[i]:Remove() end)
    end
    ParryArea_Hide()
