...
-- Highlight disk: Part flat disk di-parent ke character, ikut gerak otomatis
local ParryDiskPart      = nil
local ParryDiskHighlight = nil

local function ParryDisk_Attach(root)
    -- Buat Part disk tipis, parent ke HumanoidRootPart supaya ikut gerak
    if not ParryDiskPart or not ParryDiskPart.Parent then
        ParryDiskPart = Instance.new("Part")
        ParryDiskPart.Name        = "_VHParryDisk"
        ParryDiskPart.Anchored    = false
        ParryDiskPart.CanCollide  = false
        ParryDiskPart.CanTouch    = false
        ParryDiskPart.Massless    = true
        ParryDiskPart.Transparency = 1
        ParryDiskPart.Shape       = Enum.PartType.Cylinder
        -- Diameter = Config.PARRY_Dist * 2, sangat tipis (0.05)
        local d = Config.PARRY_Dist * 2
        ParryDiskPart.Size        = Vector3.new(0.05, d, d)
        ParryDiskPart.Parent      = root.Parent  -- parent ke character

        -- Weld ke HumanoidRootPart supaya ikut bergerak
        local weld = Instance.new("WeldConstraint")
        weld.Part0  = root
        weld.Part1  = ParryDiskPart
        weld.Parent = ParryDiskPart

        -- Offset ke bawah kaki (Y-3 dari HRP, diputar agar flat horizontal)
        ParryDiskPart.CFrame = root.CFrame
            * CFrame.new(0, -3, 0)
            * CFrame.Angles(0, 0, math.rad(90))
    end

    -- Update ukuran kalau PARRY_Dist berubah
    local d = Config.PARRY_Dist * 2
    if ParryDiskPart.Size.Y ~= d then
        ParryDiskPart.Size = Vector3.new(0.05, d, d)
    end

    if not ParryDiskHighlight or not ParryDiskHighlight.Parent then
...
local function UpdateParryFOV()
    ...
    -- Update/attach disk highlight
    ParryDisk_Attach(root)

    -- World-space line segments di lantai (Y-3 persis seperti tesvide.lua)
    ...
    for i = 1, PFOV_N do
        ...
        local s1, v1 = WorldToScreen(Vector3.new(ox+math.cos(a1)*r, baseY, oz+math.sin(a1)*r))
        local s2, v2 = WorldToScreen(Vector3.new(ox+math.cos(a2)*r, baseY, oz+math.sin(a2)*r))
        local vis = v1 and v2
