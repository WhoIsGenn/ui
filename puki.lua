...
local Config = {
    ...
    AUTO_Parry = false,
    PARRY_AimStrict = true,      -- Enable Aim Strictness
    PARRY_AimStrictness = 0.5,   -- Threshold (0.3 = longgar, 0.9 = strict)
...
-- ================================================================
-- AUTO PARRY SYSTEM (with Aim Strictness)
-- ================================================================
...
-- ================================================================
-- AIM STRICTNESS - Cek apakah killer menghadap ke kita
-- ================================================================
local function IsKillerFacingMe(killerChar)
    if not killerChar then return false end
    
    -- Kalo Aim Strictness mati, selalu return true
    if not Config.PARRY_AimStrict then return true end
    ...
    -- Dapatkan arah hadap killer
    local killerLookDir = killerRoot.CFrame.LookVector
    local killerHead = killerChar:FindFirstChild("Head")
    if killerHead then
        killerLookDir = killerHead.CFrame.LookVector
    end
    
    -- Vektor dari killer ke player
    local toMe = (myRoot.Position - killerRoot.Position).Unit
    
    -- Hitung dot product (-1 = berlawanan, 0 = tegak lurus, 1 = searah)
    local dot = killerLookDir:Dot(toMe)
    
    -- Threshold: 0.5 berarti killer menghadap dalam 60 derajat ke kita
    -- Semakin tinggi (0.8) makin strict
    local threshold = Config.PARRY_AimStrictness or 0.5
    
    if Config.PARRY_Debug then
        print(string.format("[Parry] Dot: %.2f | Threshold: %.2f | Facing: %s", 
            dot, threshold, dot > threshold and "YES" or "NO"))
    end
...
-- ================================================================
-- AUTO PARRY HOOK KILLER CHARACTER (with Aim Strictness)
-- ================================================================
local function AutoParry_HookKillerChar(char, p)
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            -- ========== AIM STRICTNESS CHECK ==========
            if not IsKillerFacingMe(char) then
                return
            end
...
-- ==================== AUTO PARRY SECTION (UI LAMA - VictUI) ====================
...
secParry:AddToggle({
    ...
    Desc = "Hanya parry kalo killer ngadep ke kamu",
    Default = Config.PARRY_AimStrict or true,
    Callback = function(v)
        Config.PARRY_AimStrict = v
        notifToggle("Aim Strictness", v)
    end
})

secParry:AddSlider({
    Title = "Strictness",
    Desc = "0.3 = longgar, 0.9 = harus tepat ngadep",
    Min = 0.3,
    Max = 0.9,
    Default = Config.PARRY_AimStrictness or 0.5,
    Decimals = 2,
    Callback = function(v)
        Config.PARRY_AimStrictness = v
    end
})

secParry:AddToggle({
    Title = "Debug",
    Desc = "Print dot product ke console",
    Default = Config.PARRY_Debug or false,
    Callback = function(v)
        Config.PARRY_Debug = v
    end
})
...
secVisual:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then Fullbright_On() else Fullbright_Off() end
        notifToggle("Fullbright", state)
    end
})
