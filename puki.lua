...
-- ================================================================
-- ✅ DETEKSI ARAH WEAPON / RIGHT ARM (VALIDASI REAL ATTACK)
-- ================================================================

-- Cek apakah weapon killer ngadep ke lo
local function IsWeaponFacingMe(killerChar, myPos)
    -- Cari weapon dulu
    local weapon = killerChar:FindFirstChild("Weapon") or 
                   killerChar:FindFirstChild("Tool") or
                   killerChar:FindFirstChild("Right Arm")
    
    if not weapon then return true end -- kalo ga ada weapon, skip validasi
    
    -- Dapetin arah weapon
    local weaponPos = weapon.Position
    local weaponDir = weapon.CFrame.LookVector
    
    -- Hitung arah dari weapon ke player
    local toMe = (myPos - weaponPos).Unit
    local dotProduct = weaponDir:Dot(toMe)
    
    -- Weapon menghadap lo (dot > 0.3 = sekitar 70 derajat)
    return dotProduct > 0.3
end

-- Cek apakah killer beneran attack ke lo (pake weapon/right arm)
local function IsRealAttack(killerChar, myChar)
    ...
    if killerRoot then
        local dist = (killerRoot.Position - myPos).Magnitude
        if dist > Config.PARRY_Dist + 3 then return false end
    end
    
    -- 2. ✅ CEK ARAH WEAPON KE LO (PALING PENTING)
    if not IsWeaponFacingMe(killerChar, myPos) then
        return false  -- Weapon ga ngadep lo, fake attack
    end
...
local function AutoParry_TryFire()
    ...
    local hasRealAttack = false
    for _, player in ipairs(Players:GetPlayers()) do
        if IsKiller(player) and player.Character then
            if IsRealAttack(player.Character, LocalPlayer.Character) then
                hasRealAttack = true
                break
            end
        end
    ...
    if not hasRealAttack then return end
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and kr then
                ...
                if dist <= Config.PARRY_Dist + 2 then
                    AutoParry_TryFire()
                end
            end
        end)
        ...
        local conn2 = RunService.Heartbeat:Connect(function()
            ...
            local dist = (kr.Position - myRoot.Position).Magnitude
            if dist > Config.PARRY_Dist + 2 then return end
