...
local function Aimbot_GetTargetPart(char)
    ...
    if targetPartName == "Left Arm" then
        part = char:FindFirstChild("LeftArm"); if part then return part end
    end
    ...
    if targetPartName == "UpperTorso" then
        -- UpperTorso tidak selalu ada, fallback ke Torso lalu HRP
...
local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        for i = 1, PFOV_N do PFOVLines[i].Visible = false end; return
    end
    local myRoot = GetCharacterRoot()
    if not myRoot then for i=1,PFOV_N do PFOVLines[i].Visible=false end; return end
    ...
    for i = 1, PFOV_N do
        ...
        if on1 and on2 then PFOVLines[i].From=s1;PFOVLines[i].To=s2;PFOVLines[i].Color=col;PFOVLines[i].Visible=true
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and kr then
                ...
                if dist <= Config.PARRY_Dist then
                    -- Direction check: killer harus facing ke kita (dot > 0.3 = dalam ~72 derajat)
                    local dot = kr.CFrame.LookVector:Dot((myRoot.Position - kr.Position).Unit)
                    if dot > 0.3 then
                        AutoParry_TryFire()
                    end
                end
...
secAim:AddDropdown({ Title = "Target Part", Options = { "Left Arm", "Torso","Upper Torso","HumanoidRootPart" }, Default = Config.AIM_TargetPart, Callback = function(v) Config.AIM_TargetPart = v end })
