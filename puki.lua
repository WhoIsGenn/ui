...
local function InstantHeal_Setup()
    ...
    HealConn = RunService.Heartbeat:Connect(function()
        ...
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not hum then return end
        -- Cek knocked (health rendah) atau sedang recover
        local isKnocked = hum.Health <= hum.MaxHealth * 0.5
        local isRecover = char:GetAttribute("IsBeingHealed") or char:GetAttribute("HealProgress")
        if not isKnocked and not isRecover then return end
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        ...
        if not har then return end
        -- Spam true (sedang heal) untuk naikkan progress
        pcall(function() har:FireServer(true) end)
    end)
end
