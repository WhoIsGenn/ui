...
local HealAnimRemote = nil

local function GetHealRemotes()
    local r = ReplicatedStorage:FindFirstChild("Remotes")
    if not r then return end
    local healing = r:FindFirstChild("Healing")
    if healing then
        HealRemote = healing:FindFirstChild("HealEvent")
        HealAnimRemote = healing:FindFirstChild("HealAnimRec")
    end
end
...
local function InstantHeal_Setup()
    ...
    HealConn = RunService.Heartbeat:Connect(function()
        ...
        local char = LocalPlayer.Character
        if not char then return end
        local hp = char:GetAttribute("HealProgress")
        -- Set progress langsung ke 100
        if hp and hp > 0 and hp < 100 then
            pcall(function() char:SetAttribute("HealProgress", 100) end)
        end
        -- Fire HealAnimRec untuk server tau progress naik
        if HealAnimRemote then
            pcall(function() HealAnimRemote:FireServer() end)
        end
        -- Kalau sudah 100, fire remote bangkit
        -- Dari dex: Remotes.Mechanics.gotknocked = remote untuk revive
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum and hum.Health < hum.MaxHealth * 0.5 then
            -- Kita knocked, coba bangkit
            local r = ReplicatedStorage:FindFirstChild("Remotes")
            local mech = r and r:FindFirstChild("Mechanics")
            if mech then
                local gotknocked = mech:FindFirstChild("gotknocked")
                if gotknocked then
                    pcall(function() gotknocked:FireServer(false) end)
                end
            end
            -- Juga coba via Adrenaline Shot remote
            local items = r and r:FindFirstChild("Items")
            local adren = items and items:FindFirstChild("Adrenaline Shot")
            if adren then
                local knocked = adren:FindFirstChild("Knocked")
                if knocked then
                    pcall(function() knocked:FireServer() end)
                end
            end
        end
    end)
    ...
    HealOtherConn = RunService.Heartbeat:Connect(function()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                local c = p.Character
                local hp = c:GetAttribute("HealProgress")
                if hp and hp > 0 and hp < 100 then
                    pcall(function() c:SetAttribute("HealProgress", 100) end)
                end
                -- Fire remote bangkit untuk player lain
