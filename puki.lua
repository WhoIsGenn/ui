...
local function InstantHeal_Setup()
    ...
    HealConn = RunService.Heartbeat:Connect(function()
        ...
        local char = LocalPlayer.Character
        if not char then return end
        -- Tunggu IsBeingHealed = true, lalu set HealProgress = 100
        if not char:GetAttribute("IsBeingHealed") then return end
    ...
    HealOtherConn = RunService.Heartbeat:Connect(function()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                local c = p.Character
                if not c:GetAttribute("IsBeingHealed") then continue end
...
local function GodMode_Start()
    ...
    task.spawn(function()
        local char = LocalPlayer.Character
        if not char then LocalPlayer.CharacterAdded:Wait(); char = LocalPlayer.Character end
        ...
        if healthVal and healthVal:IsA("IntValue") then
            local waited = 0
            while healthVal.Value <= 0 and waited < 5 do task.wait(0.3); waited = waited + 0.3 end
            local maxHp = math.max(healthVal.Value, 1)
            GodModeConn2 = healthVal:GetPropertyChangedSignal("Value"):Connect(function()
                if not Config.SURV_GodMode then return end
                if healthVal.Value < maxHp then pcall(function() healthVal.Value = maxHp end) end
            end)
        end
    end)
end
