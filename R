...
local Config = {
    AutoParry = false,
    DistanceTiming = 50,
    ParryRange = 0,               -- 0 = disabled, 1-30 = fire duluan di radius ini
...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    local parryRangeStud = Config.ParryRange or 0
    ...
    for idx, Ball in ipairs(balls) do
        ...
        -- ═══════════════════════════════════════════════════════════════
        -- ═══ PARRY RANGE (BLOK TERPISAH) ═══
        -- Fire duluan kalau bola masuk radius parry_range
        -- Blok ini SELF-CONTAINED — ga nyentuh logic accuracy di bawahnya
        -- ═══════════════════════════════════════════════════════════════
        local fired_by_range = false
        if autoParryEnabled and allowedToHit and parryRangeStud > 0 then
            if Distance <= parryRangeStud then
        ...
        -- ═══ LOGIC ACCURACY (BAWAAN LU) ═══
        -- Kalau belum fire dari parry range, jalanin logic accuracy asli
        if autoParryEnabled and allowedToHit and not fired_by_range then
...
AutoParrySec:AddSlider({
    Title = "Accuracy",
    Description = "Detect bola (strikeZone radius). 1 = kecil, 100 = lebar",
    Flag = "accuracy",
    Min = 1,
    Max = 100,
    Default = 50,
...
AutoParrySec:AddSlider({
    Title = "Parry Range (stud)",
    Description = "Fire duluan kalau bola masuk radius ini (0 = off)",
    Flag = "parryrange",
    Min = 0,
    Max = 30,
    Default = 0,
    Increment = 1,
    Callback = function(value) Config.ParryRange = value end,
})
