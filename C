-- ======================================================
-- FIONZY-VD | Violence District Script
-- UI: VilarisUI (victuinew.lua)
-- ======================================================

-- LOAD UI LIB
-- Ganti URL ini dengan URL victuinew.lua lu sendiri
local Chloex = loadstring(game:HttpGet("https://raw.githubusercontent.com/WhoIsGenn/ui/refs/heads/main/victui.lua"))()
...
local Window = Chloex:Window({
    ...
    Version = "2.1.0",
    Config  = {
        ConfigFolder  = "Fionzy-VD/configs",
...
-- TABS
-- ======================================================
local TabInfo      = Window:AddTab({ Name = "Info",        Icon = "info" })
local TabESP       = Window:AddTab({ Name = "ESP",         Icon = "eye" })
local TabPlayer    = Window:AddTab({ Name = "Player",      Icon = "user" })
local TabKiller    = Window:AddTab({ Name = "Killer",      Icon = "skull" })
...
-- SECTIONS
-- ======================================================
local InfoSec     = TabInfo:AddSection({ Title = "Script Info" })
local CreditsSec  = TabInfo:AddSection({ Title = "Credits" })

local ESPSec      = TabESP:AddSection({ Title = "ESP Cham" })
local ESPStatSec  = TabESP:AddSection({ Title = "ESP Status" })

local CrosshairSec = TabPlayer:AddSection({ Title = "Crosshair" })
local AimlockSec  = TabPlayer:AddSection({ Title = "AimBot" })
local ParrySec    = TabPlayer:AddSection({ Title = "Parry" })
local AbilitySec  = TabPlayer:AddSection({ Title = "Survivor" })

local KillerSec   = TabKiller:AddSection({ Title = "Killer" })

local MoveSec     = TabMisc:AddSection({ Title = "Movement" })
local EmoteSec    = TabMisc:AddSection({ Title = "Emote" })

local VisualSec   = TabVisual:AddSection({ Title = "Graphics" })
local MorphSec    = TabVisual:AddSection({ Title = "Morph Avatar" })
local TimeSec     = TabVisual:AddSection({ Title = "Clock & Ambient" })
local ZoomSec     = TabVisual:AddSection({ Title = "Zoom Out" })

local SettingSec  = TabSettings:AddSection({ Title = "Menu" })

-- ======================================================
-- INFO TAB
-- ======================================================
InfoSec:AddParagraph({ Title = "Script", Content = "Premium" })
InfoSec:AddParagraph({ Title = "Version", Content = "2.1.0" })
InfoSec:AddParagraph({ Title = "Game", Content = "Violence District" })
InfoSec:AddParagraph({ Title = "Dev", Content = "•༶Fionzy-VD༶•" })
InfoSec:AddButton({ Title = "Copy Discord Link", Callback = function()
    setclipboard("https://discord.gg/fionzyvd")
end })

CreditsSec:AddParagraph({ Title = "Developer", Content = "• Fionzy-VD" })
CreditsSec:AddParagraph({ Title = "Library", Content = "• VilarisUI" })
CreditsSec:AddButton({ Title = "Copy Support Link", Callback = function()
    setclipboard("https://discord.gg/fionzyvd")
end })
...
-- ESP TAB
-- ======================================================
ESPSec:AddToggle({ Title = "ESP Survivor", Default = false, Callback = function(v) ESP.Survivor = v end })
ESPSec:AddColorPicker({ Title = "Survivor Color", Default = TeamColors.Survivor, Callback = function(c) TeamColors.Survivor = c end })

ESPSec:AddToggle({ Title = "ESP Killer", Default = false, Callback = function(v) ESP.Killer = v end })
ESPSec:AddColorPicker({ Title = "Killer Color", Default = TeamColors.Killer, Callback = function(c) TeamColors.Killer = c end })

ESPSec:AddToggle({ Title = "ESP Generator", Default = false, Callback = function(v) ESP.Generator = v end })
ESPSec:AddColorPicker({ Title = "Generator Color", Default = TeamColors.Generator, Callback = function(c) TeamColors.Generator = c end })

ESPSec:AddToggle({ Title = "ESP SCP", Default = false, Callback = function(v) ESP.SCP = v end })
ESPSec:AddColorPicker({ Title = "SCP Color", Default = TeamColors.SCP, Callback = function(c) TeamColors.SCP = c end })

ESPSec:AddToggle({ Title = "ESP Pallet", Default = false, Callback = function(v) ESP.Pallet = v end })
ESPSec:AddColorPicker({ Title = "Pallet Color", Default = TeamColors.Pallet, Callback = function(c) TeamColors.Pallet = c end })

ESPSec:AddToggle({ Title = "ESP Window", Default = false, Callback = function(v) ESP.Window = v end })
ESPSec:AddColorPicker({ Title = "Window Color", Default = TeamColors.Window, Callback = function(c) TeamColors.Window = c end })

ESPSec:AddSlider({ Title = "ESP Distance", Default = ESP.Distance, Min = 50, Max = 1000, Rounding = 0, Callback = function(v) ESP.Distance = v end })
ESPSec:AddButton({ Title = "Refresh ESP", Callback = function() for _, p in pairs(Players:GetPlayers()) do if p.Character then removeESP(p.Character) end end end })

ESPStatSec:AddToggle({ Title = "Enable Status", Default = false, Callback = function(v) ESPStatus.Enabled = v end })
ESPStatSec:AddToggle({ Title = "Show Name", Default = false, Callback = function(v) ESPStatus.ShowName = v end })
ESPStatSec:AddToggle({ Title = "Show Item ESP", Default = false, Callback = function(v) ESPStatus.ShowItem = v end })
ESPStatSec:AddToggle({ Title = "Show Distance", Default = false, Callback = function(v) ESPStatus.ShowDistance = v end })
ESPStatSec:AddToggle({ Title = "Show Health", Default = false, Callback = function(v) ESPStatus.ShowHealth = v end })
ESPStatSec:AddSlider({ Title = "Status Radius", Default = 20, Min = 5, Max = 100, Rounding = 0, Callback = function(v) ESPStatus.Radius = v end })

-- ======================================================
-- PLAYER TAB - CROSSHAIR
-- ======================================================
CrosshairSec:AddToggle({ Title = "Enable Crosshair", Default = false, Callback = function(v) Crosshair.Enabled = v; drawCrosshair() end })
CrosshairSec:AddColorPicker({ Title = "Crosshair Color", Default = Crosshair.Color, Callback = function(c) Crosshair.Color = c end })
CrosshairSec:AddDropdown({ Title = "Style", Values = {"Plus","Dot","Circle","X"}, Default = 1, Callback = function(v) Crosshair.Style = v end })
CrosshairSec:AddSlider({ Title = "Pos X", Default = 0, Min = -500, Max = 500, Rounding = 0, Callback = function(v) Crosshair.PosX = v end })
CrosshairSec:AddSlider({ Title = "Pos Y", Default = 0, Min = -500, Max = 500, Rounding = 0, Callback = function(v) Crosshair.PosY = v end })

-- ======================================================
-- PLAYER TAB - AIMBOT
-- ======================================================
AimlockSec:AddToggle({ Title = "Gun AimLock", Default = false, Callback = function(v) GunAim.Enabled = v end })
AimlockSec:AddDropdown({ Title = "Gun Aim Target", Values = {"Nearest","Random"}, Default = 1, Callback = function(v) GunAim.Target = v end })
AimlockSec:AddDropdown({ Title = "Gun Aim Part", Values = {"Head","HumanoidRootPart","Torso"}, Default = 1, Callback = function(v) GunAim.Part = v end })
AimlockSec:AddSlider({ Title = "Gun Aim FOV", Default = GunAim.FOV, Min = 10, Max = 500, Rounding = 0, Callback = function(v) GunAim.FOV = v end })
AimlockSec:AddSlider({ Title = "Gun Aim Predict", Default = 0, Min = 0, Max = 1, Rounding = 2, Callback = function(v) GunAim.Predict = v end })
AimlockSec:AddSeparator()
AimlockSec:AddToggle({ Title = "Silent Aim", Default = false, Callback = function(v) SilentAim.Enabled = v end })
AimlockSec:AddDropdown({ Title = "Silent Aim Target", Values = {"Nearest","Random"}, Default = 1, Callback = function(v) SilentAim.Target = v end })

-- ======================================================
-- PLAYER TAB - PARRY
-- ======================================================
ParrySec:AddToggle({ Title = "Auto Parry", Default = false, Callback = function(v) Auto.Parry = v; ParryRangeVisual.Enabled = v end })
ParrySec:AddColorPicker({ Title = "Parry Range Color", Default = Color3.fromRGB(255,80,80), Callback = function(c) ParryRangeVisual.Color = c end })
ParrySec:AddToggle({ Title = "Show Parry Range", Default = false, Callback = function(v) ParryRangeVisual.Enabled = v end })
ParrySec:AddSlider({ Title = "Parry Distance", Default = 15, Min = 1, Max = 100, Rounding = 0, Callback = function(v) Auto.ParryDistance = v end })
ParrySec:AddSlider({ Title = "Face Sensitivity", Default = 45, Min = 1, Max = 180, Rounding = 0, Callback = function(v) Auto.FaceSensitivity = v end })
ParrySec:AddToggle({ Title = "Fake Parry", Default = false, Callback = function(v) FakeParry.Enabled = v end })
ParrySec:AddDropdown({ Title = "Fake Parry Anim", Values = {"Default"}, Default = 1, Callback = function(v) FakeParry.Anim = v end })

-- ======================================================
-- PLAYER TAB - SURVIVOR ABILITIES
-- ======================================================
AbilitySec:AddToggle({ Title = "Auto Skill Check", Default = false, Callback = function(v) Auto.SkillCheck = v end })
AbilitySec:AddToggle({ Title = "Auto Flee Killer", Default = false, Callback = function(v) Auto.Flee = v end })
AbilitySec:AddToggle({ Title = "God Mode", Default = false, Callback = function(v) GodMode.Enabled = v end })
AbilitySec:AddToggle({ Title = "Fast Vault", Default = false, Callback = function(v) FastVault.Enabled = v end })
AbilitySec:AddSlider({ Title = "Vault Speed", Default = FastVault.Speed, Min = 0.1, Max = 5, Rounding = 1, Callback = function(v) FastVault.Speed = v end })
AbilitySec:AddToggle({ Title = "Moonwalk Button", Default = false, Callback = function(v) Moonwalk.ShowButton = v; if v then createMoonwalkButton() else removeMoonwalkButton() end end })
AbilitySec:AddSlider({ Title = "Moonwalk Spam Speed", Default = Moonwalk.SpamSpeed, Min = 0.01, Max = 0.5, Rounding = 2, Callback = function(v) Moonwalk.SpamSpeed = v end })
AbilitySec:AddSlider({ Title = "Moonwalk Intensity", Default = Moonwalk.Intensity, Min = 1, Max = 50, Rounding = 1, Callback = function(v) Moonwalk.Intensity = v end })
AbilitySec:AddSeparator()
AbilitySec:AddButton({ Title = "Instant Escape", Callback = function() teleportToFinishLine() end })

-- ======================================================
-- KILLER TAB
-- ======================================================
KillerSec:AddToggle({ Title = "Auto Stalk (Myers)", Default = false, Callback = function(v) AutoStalk.Enabled = v; if v then startAutoStalk() else stopAutoStalk() end end })
KillerSec:AddToggle({ Title = "Auto Kill All", Default = false, Callback = function(v) Killer.KillAll = v end })
KillerSec:AddToggle({ Title = "AimLock Attack", Default = false, Callback = function(v) AttackAim.Enabled = v; if v then startAttackAim() end end })
KillerSec:AddDropdown({ Title = "Aimlock Mode", Values = {"Normal","Spear"}, Default = 1, Callback = function(v) State.AttackAimMode = v end })
KillerSec:AddSlider({ Title = "Spear Gravity", Default = 50, Min = 10, Max = 200, Rounding = 0, Callback = function(v) SpearAim.Gravity = v end })
KillerSec:AddSlider({ Title = "Spear Speed", Default = 100, Min = 20, Max = 300, Rounding = 0, Callback = function(v) SpearAim.Speed = v end })
KillerSec:AddSeparator()
KillerSec:AddDropdown({ Title = "Select Power", Values = MaskedPowers, Default = 1, Callback = function(v) Masked.CurrentPower = v end })
KillerSec:AddButton({ Title = "Activate Power", Callback = function()
...
KillerSec:AddButton({ Title = "Deactivate Power", Callback = function()
    local Event = ReplicatedStorage:FindFirstChild("Remotes", true)
        and ReplicatedStorage.Remotes:FindFirstChild("Killers", true)
        and ReplicatedStorage.Remotes.Killers:FindFirstChild("Masked", true)
...
KillerSec:AddSeparator()
KillerSec:AddToggle({ Title = "Silent Aim Veil", Default = false, Callback = function(v) SilentAimVeil.Enabled = v; if v then StartSilentAimVeil() else StopSilentAimVeil() end end })
KillerSec:AddDropdown({ Title = "Veil Target Mode", Values = {"Nearest","Random"}, Default = 1, Callback = function(v) SilentAimVeil.TargetMode = v end })
KillerSec:AddSlider({ Title = "Veil FOV", Default = SilentAimVeil.FOV or 200, Min = 10, Max = 500, Rounding = 0, Callback = function(v) SilentAimVeil.FOV = v end })
KillerSec:AddToggle({ Title = "Veil Predict", Default = false, Callback = function(v) SilentAimVeil.Predict = v end })
KillerSec:AddSlider({ Title = "Veil Predict Strength", Default = SilentAimVeil.PredictStrength or 0.5, Min = 0, Max = 2, Rounding = 2, Callback = function(v) SilentAimVeil.PredictStrength = v end })

-- ======================================================
-- MISC TAB - MOVEMENT
-- ======================================================
MoveSec:AddToggle({ Title = "WalkSpeed", Default = false, Callback = function(v) Movement.WalkSpeedEnabled = v; applyWalkSpeed() end })
MoveSec:AddSlider({ Title = "Walk Speed Value", Default = Movement.WalkSpeedValue, Min = 1, Max = 100, Rounding = 0, Callback = function(v) Movement.WalkSpeedValue = v; if Movement.WalkSpeedEnabled then applyWalkSpeed() end end })
MoveSec:AddToggle({ Title = "No Clip", Default = false, Callback = function(v) toggleNoClip(v) end })

-- ======================================================
-- MISC TAB - EMOTE
-- ======================================================
EmoteSec:AddDropdown({ Title = "Select Emote", Values = {"Dance","Wave","Laugh","Point","Cheer"}, Default = 1, Callback = function(v) Emote.Selected = v end })
EmoteSec:AddButton({ Title = "Play Emote", Callback = function() playEmote(Emote.Selected) end })
EmoteSec:AddToggle({ Title = "Show Emote Button", Default = false, Callback = function(v) Emote.ShowButton = v end })

-- ======================================================
-- VISUAL TAB
-- ======================================================
VisualSec:AddToggle({ Title = "Fullbright", Default = false, Callback = function(v) Fullbright.Enabled = v; ApplyFullbright() end })
VisualSec:AddColorPicker({ Title = "Fullbright Color", Default = Color3.fromRGB(255,255,255), Callback = function(c) Fullbright.Color = c; ApplyFullbright() end })
VisualSec:AddToggle({ Title = "No Fog", Default = false, Callback = function(v) NoFog.Enabled = v; if v then ApplyNoFog() end end })
VisualSec:AddToggle({ Title = "Low Graphics", Default = false, Callback = function(v) if v then setLowGraphics() end end })
VisualSec:AddToggle({ Title = "No Screen Effects", Default = false, Callback = function(v) NoScreenEffects.Enabled = v end })

MorphSec:AddButton({ Title = "Copy Avatar", Callback = function() copyAvatar(AvatarStealer.TargetUsername) end })
MorphSec:AddButton({ Title = "Reset to Original Skin", Callback = function() resetAvatar() end })
MorphSec:AddButton({ Title = "Save Current as Original", Callback = function() saveOriginalAppearance() end })

ZoomSec:AddToggle({ Title = "Unlimited Zoom", Default = false, Callback = function(v) CameraZoom.UnlimitedEnabled = v; applyUnlimitedZoom() end })
ZoomSec:AddSlider({ Title = "Max Zoom Distance", Default = 400, Min = 10, Max = 1000, Rounding = 0, Callback = function(v) CameraZoom.MaxZoom = v; applyUnlimitedZoom() end })
ZoomSec:AddToggle({ Title = "Custom FOV", Default = false, Callback = function(v) CameraZoom.FOVEnabled = v end })
ZoomSec:AddSlider({ Title = "Camera FOV", Default = 70, Min = 10, Max = 120, Rounding = 0, Callback = function(v) CameraZoom.FOV = v end })

TimeSec:AddSlider({ Title = "Clock Time", Default = 14, Min = 0, Max = 24, Rounding = 1, Callback = function(v) Lighting.ClockTime = v end })
TimeSec:AddSlider({ Title = "Brightness", Default = 1, Min = 0, Max = 10, Rounding = 1, Callback = function(v) Lighting.Brightness = v end })

-- ======================================================
-- SETTINGS TAB
-- ======================================================
SettingSec:AddButton({ Title = "Join Discord", Callback = function() setclipboard("https://discord.gg/fionzyvd") end })
SettingSec:AddButton({ Title = "Unload Script", Callback = function() 
    -- cleanup
    for _, v in pairs(ESPCache.BillboardGuis) do pcall(function() v:Destroy() end) end
end })
...
ApplyFullbright()
ApplyNoFog()
if SilentAimVeil.Enabled then StartSilentAimVeil() end
