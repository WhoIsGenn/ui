...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    if workspace:FindFirstChild("TrainingBalls") then
        for _, tb in pairs(workspace.TrainingBalls:GetChildren()) do
            if tb:GetAttribute("realBall") then
                training_ball = tb
                break
            end
        end
    ...
    if #balls == 0 and not training_ball then
        shared.ManualSpamActive = false
        _clashStartTime = 0
        return
    end
    ...
    -- ═══ TRAINING BALL AUTO PARRY ═══
    if training_ball and autoParryEnabled then
        local zoomies = training_ball:FindFirstChild("zoomies")
        if zoomies then
            training_ball:GetAttributeChangedSignal('target'):Once(function() System_training_parried = false end)
            if not System_training_parried then
                local ball_target = training_ball:GetAttribute('target')
                local velocity = zoomies.VectorVelocity
                local distance = (playerPos - training_ball.Position).Magnitude
                local speed = velocity.Magnitude

                local ballDirection = speed > 1 and velocity.Unit or Vector3.new(0, -1, 0)
                local toPlayerDirection = distance > 0.1 and (playerPos - training_ball.Position).Unit or Vector3.zero
                local dotProduct = ballDirection:Dot(toPlayerDirection)

                local pingVal = pingSeconds * 10
                local pingThreshold = math.clamp(pingVal / 10, 5, 17)
                local cappedSpeedDiff = math.min(math.max(speed - 9.5, 0), 650)
                local speedDivisor = (2.4 + cappedSpeedDiff * 0.002)
                local parryAccuracy = pingThreshold + math.max(speed / speedDivisor, 9.5)

                local isTrainingTarget = (tostring(ball_target or "") == LocalPlayer.Name 
                    or tostring(ball_target or "") == tostring(LocalPlayer)
                    or training_ball:GetAttribute("training") == true)

                if isTrainingTarget and distance <= parryAccuracy then
                    if dotProduct > -0.35 or distance <= 35 then
                        if (now - lastParryTime) >= 0.003 then
                            local bTarget = training_ball:GetAttribute("target") or training_ball:GetAttribute("Target")
                            if bTarget then ZX_Parry.KeyTable = training_ball:GetAttribute("realBall") or training_ball.Name or bTarget end

                            if getgenv().AutoParryMode == "Keypress" then
                                Auto_Parry.parry_keypress and Auto_Parry.parry_keypress() or Auto_Parry.FireParryRemote(false)
                            else
                                Auto_Parry.FireParryRemote(false)
                            end
                            lastParryTime = now
                        end
                        System_training_parried = true
                        task.delay(0.5, function() System_training_parried = false end)
                    end
                end
            end
        end
...
AutoParrySec:AddToggle({
    Title = "Auto Parry",
    Description = "Automatically parries incoming balls (Real + Training)",
    Flag = "autoparry",
    Default = false,
