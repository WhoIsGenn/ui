...
local function AutoParry_HookKillerChar(char, p)
    ...
    task.spawn(function()
        ...
        -- Method 2: GetPlayingAnimationTracks per frame - lebih cepat dari event
...
-- Background: 48 triangles dari center ke setiap segment
local PFOVTris = {}
for i = 1, PFOV_N do
...
local function MainLoop()
    ...
    if Config.AUTO_Parry and Config.PARRY_FOV then
        ...
        if myRoot then
            ...
            -- Background: triangle fan dari center ke tiap segment
            local centerSP, centerOn = WorldToScreen(Vector3.new(ox, baseY, oz))
            if centerOn then
                for i = 1, PFOV_N do
                    local a1 = (i-1)/PFOV_N * math.pi * 2
                    local a2 = i/PFOV_N * math.pi * 2
                    ...
                    local s2, on2 = WorldToScreen(Vector3.new(ox+math.cos(a2)*r, baseY, oz+math.sin(a2)*r))
                    if on1 and on2 then
                        PFOVTris[i].PointA = centerSP
                        PFOVTris[i].PointB = s1
                        PFOVTris[i].PointC = s2
                        PFOVTris[i].Visible = true
                    else
                        PFOVTris[i].Visible = false
                    end
                end
            else
                for i = 1, PFOV_N do PFOVTris[i].Visible = false end
            end
        else
...
Unload = function()
    ...
    for i = 1, PFOV_N do pcall(function() PFOVTris[i]:Remove() end) end
    for i = 1, 4 do
