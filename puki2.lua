...
local HealEventRemote = nil

local function GetHealEvent()
    if HealEventRemote and HealEventRemote.Parent then return HealEventRemote end
    local r = ReplicatedStorage:FindFirstChild("Remotes")
    local healing = r and r:FindFirstChild("Healing")
    HealEventRemote = healing and healing:FindFirstChild("HealEvent")
    return HealEventRemote
end
...
local function InstantHeal_Setup()
    ...
    HealConn = RunService.Heartbeat:Connect(function()
        ...
        if not isKnocked then return end
        local healEvent = GetHealEvent()
        if not healEvent then return end
        -- Fire HealEvent(root, true) = mulai heal diri sendiri
        pcall(function() healEvent:FireServer(root, true) end)
    end)
end
    ...
    HealOtherConn = RunService.Heartbeat:Connect(function()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if h and root and h.Health <= h.MaxHealth * 0.5 then
                    pcall(function() healEvent:FireServer(root, true) end)
                end
            end
        end
