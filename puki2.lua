...
-- ================================================================
-- PARRY FOV CIRCLE (LINGKARAN + HIGHLIGHT)
-- ================================================================
...
local PFOV_N = 64  -- lebih halus
local PFOVLines = {}     -- outline garis
local PFOVFill = {}      -- fill garis tebal buat efek solid

-- Buat garis-garis lingkaran (outline)
for i = 1, PFOV_N do
    local line = Drawing.new("Line")
    line.Thickness = 3
    line.Color = Color3.fromRGB(255, 255, 255)  -- outline putih
    line.Transparency = 1
    line.Visible = false
    PFOVLines[i] = line
end

-- Buat fill (garis tebal transparan buat efek solid)
for i = 1, PFOV_N do
    local fill = Drawing.new("Line")
    fill.Thickness = 8
    fill.Color = Color3.fromRGB(0, 150, 255)  -- biru
...
local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        for i = 1, PFOV_N do
            if PFOVLines[i] then PFOVLines[i].Visible = false end
            if PFOVFill[i] then PFOVFill[i].Visible = false end
        end
        return
    end
    ...
    local outlineColor = Color3.fromRGB(255, 255, 255)  -- putih tetap
    local fillColor = ParryOnCooldown and Color3.fromRGB(255, 50, 50) or Color3.fromRGB(0, 150, 255)
    ...
    for i = 1, PFOV_N do
        ...
        if on1 and on2 then
            ...
            PFOVLines[i].From = s1
            PFOVLines[i].To = s2
            PFOVLines[i].Color = outlineColor
            PFOVLines[i].Visible = true
            
            -- Fill (biru/merah transparan)
            PFOVFill[i].From = s1
            PFOVFill[i].To = s2
            PFOVFill[i].Color = fillColor
            PFOVFill[i].Visible = true
        else
            if PFOVLines[i] then PFOVLines[i].Visible = false end
            if PFOVFill[i] then PFOVFill[i].Visible = false end
        end
...
local function ParryFOV_Cleanup()
    for i = 1, PFOV_N do
        if PFOVLines[i] then PFOVLines[i]:Remove() end
        if PFOVFill[i] then PFOVFill[i]:Remove() end
    end
    PFOVLines = {}
    PFOVFill = {}
end
