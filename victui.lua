...
local TabsModule = (function()
-- // VelarisUI | Tabs.lua
-- Modul inti: AddTab -> sidebar button + halaman tab -> AddSection -> Items (AddButton/AddToggle/dst)
-- Dibuat ulang berdasarkan kontrak pemanggilan di dist/main.lua (rekonstruksi, bukan sumber asli)

return function(
    GuiConfig, LayersFolder, LayersPageLayout, ScrollTab, NameTab,
    MoreBlur, DropdownFolder, DropdownSelect, DropPageLayout,
    ElementsRegistry, ElementsModule, KeybindModule, Mouse, TweenService,
    getIconId, CircleClick, EmptyCallback, EmptyTable, CreateColorpickerElement
)
    local GuiFunc = {}
    ...
    local _firstPage   = nil

    local ACCENT = GuiConfig.Color or Color3.fromRGB(0, 208, 255)

    local function tween(obj, props, time, style, dir)
        TweenService:Create(obj, TweenInfo.new(
            time or 0.25,
            style and Enum.EasingStyle[style] or Enum.EasingStyle.Quad,
            dir and Enum.EasingDirection[dir] or Enum.EasingDirection.Out
        ), props):Play()
    end

    local function selectTab(name)
        for n, btn in pairs(_tabButtons) do
            local isSel = (n == name)
            local fill  = btn:FindFirstChild("Fill")
            local strip = btn:FindFirstChild("AccentStrip")
            local title = btn:FindFirstChild("TabTitle")
            local icon  = btn:FindFirstChild("TabIcon")
            if fill  then tween(fill,  { BackgroundTransparency = isSel and 0.85 or 1 }, 0.2) end
            if strip then tween(strip, { BackgroundTransparency = isSel and 0 or 1 }, 0.2) end
            if title then tween(title, { TextTransparency = isSel and 0 or 0.35 }, 0.2) end
            if icon  then tween(icon,  { ImageTransparency = isSel and 0 or 0.35 }, 0.2) end
        end
        local page = _tabPages[name]
        if page then
            pcall(function() LayersPageLayout:JumpTo(page) end)
        end
        if NameTab then NameTab.Text = name end
    end

    function GuiFunc:AddTab(TabConfig)
        TabConfig = TabConfig or {}
        local Name = TabConfig.Name or ("Tab " .. (_tabCount + 1))
        _tabCount = _tabCount + 1
        local order = _tabCount
        ...
        AccentStrip.Size = UDim2.new(0, 3, 0, 16)
        AccentStrip.Parent = Btn
        StripCorner.CornerRadius = UDim.new(1, 0)
        StripCorner.Parent = AccentStrip

        TabIcon.Name = "TabIcon"
        TabIcon.BackgroundTransparency = 1
        TabIcon.Image = getIconId(TabConfig.Icon)
        ...
        TabTitle.TextTransparency = 0.35
        TabTitle.TextXAlignment = Enum.TextXAlignment.Left
        TabTitle.BackgroundTransparency = 1
        TabTitle.Position = UDim2.new(0, TabIcon.Image ~= "" and 36 or 12, 0, 0)
        TabTitle.Size = UDim2.new(1, -46, 1, 0)
        TabTitle.Parent = Btn
        ...
        TabPage.Name = Name
        TabPage.LayoutOrder = order
        TabPage.BackgroundTransparency = 1
        TabPage.BorderSizePixel = 0
        TabPage.Size = UDim2.new(1, 0, 1, 0)
        TabPage.ScrollBarThickness = 3
        TabPage.ScrollBarImageColor3 = ACCENT
        ...
        PageList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(refreshCanvas)
        refreshCanvas()

        _tabButtons[Name] = Btn
        _tabPages[Name]   = TabPage
        if not _firstPage then
            _firstPage = TabPage
            task.defer(function() selectTab(Name) end)
        end

        Click.MouseButton1Click:Connect(function()
            if CircleClick then pcall(CircleClick, Click, ACCENT) end
            selectTab(Name)
        end)
        ...
        function Sections:AddSection(SectionConfig, LegacyOpen)
            if type(SectionConfig) == "string" then
                SectionConfig = { Title = SectionConfig }
            end
            SectionConfig = SectionConfig or {}
            if SectionConfig.Open == nil and SectionConfig.open ~= nil then
                SectionConfig.Open = SectionConfig.open
            end
            if SectionConfig.Open == nil and LegacyOpen ~= nil then
                SectionConfig.Open = LegacyOpen
            end
            ...
            if SectionConfig.Title and SectionConfig.Title ~= "" then
                ...
                HeaderBar.BackgroundTransparency = 0.92
                HeaderBar.BorderSizePixel = 0
                HeaderBar.Size = UDim2.new(1, 0, 0, 34)
                HeaderBar.Parent = SectionFrame
            ...
            SectionAdd.Position = UDim2.new(0, 0, 0, headerHeight)
            ...
            ItemsList.Parent = SectionAdd

            -- // Collapse / expand — tinggi dihitung manual dari AbsoluteContentSize
            -- (bukan AutomaticSize) biar nggak nyangkut/telat refresh di dalam ScrollingFrame
            local collapsed = SectionConfig.Open == false

            local function applyCollapse()
                if collapsed then
                    SectionAdd.Visible = false
                    tween(SectionFrame, { Size = UDim2.new(1, 0, 0, headerHeight) }, 0.2)
                    if Chevron then tween(Chevron, { Rotation = -90 }, 0.2) end
                else
                    SectionAdd.Visible = true
                    tween(SectionFrame, { Size = UDim2.new(1, 0, 0, headerHeight + ItemsList.AbsoluteContentSize.Y + 10) }, 0.2)
                    if Chevron then tween(Chevron, { Rotation = 0 }, 0.2) end
                end
            end
            SectionAdd.Visible = not collapsed
            SectionFrame.Size = UDim2.new(1, 0, 0, collapsed and headerHeight or (headerHeight + ItemsList.AbsoluteContentSize.Y + 10))
            ...
            ItemsList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                if not collapsed then
                    SectionFrame.Size = UDim2.new(1, 0, 0, headerHeight + ItemsList.AbsoluteContentSize.Y + 10)
                end
            end)

            local HeaderBarObj = SectionFrame:FindFirstChild("HeaderBar")
            local HeaderClickObj = HeaderBarObj and HeaderBarObj:FindFirstChild("HeaderClick")
            if HeaderClickObj then
                HeaderClickObj.MouseButton1Click:Connect(function()
            ...
            local function updateSectionSize() end -- ukuran otomatis via AutomaticSize

            local Items = {}
            Items._sectionAdd    = SectionAdd
            Items._getCountItem  = function() return countItem end

            function Items:AddButton(Cfg)
                local ci = nextCount()
                return ElementsModule:CreateButton(SectionAdd, Cfg, ci)
            end

            function Items:AddToggle(Cfg)
                local ci = nextCount()
                return ElementsModule:CreateToggle(SectionAdd, Cfg, ci, updateSectionSize, ElementsModule)
            end

            function Items:AddSlider(Cfg)
                local ci = nextCount()
                return ElementsModule:CreateSlider(SectionAdd, Cfg, ci, updateSectionSize, ElementsModule)
            end

            function Items:AddInput(Cfg)
                local ci = nextCount()
                return ElementsModule:CreateInput(SectionAdd, Cfg, ci, updateSectionSize, ElementsModule)
            end
            ...
            function Items:AddParagraph(Cfg)
                local ci = nextCount()
                return ElementsModule:CreateParagraph(SectionAdd, Cfg, ci)
            end

            function Items:AddKeybind(Cfg)
                local ci = nextCount()
                return KeybindModule:CreateKeybind(SectionAdd, Cfg, ci, ElementsModule)
            end

            function Items:AddDivider()
                local ci = nextCount()
                return ElementsModule:CreateDivider(SectionAdd, ci)
            end

            function Items:AddSubSection(Title)
                local ci = nextCount()
                return ElementsModule:CreateSubSection(SectionAdd, Title, ci, {})
            end

            -- // AddColorpicker disuntik dari main.lua (butuh DropShadowHolder),
            -- tapi disediakan juga fallback langsung di sini kalau dipanggil sebelum wrapper terpasang.
            if CreateColorpickerElement then
                function Items:AddColorpicker(Cfg)
                    local ci = nextCount()
                    return CreateColorpickerElement(SectionAdd, Cfg, ci, ElementsModule)
                end
...
local SearchModule = (function()
-- // VelarisUI | Search.lua
-- Kotak pencarian di atas daftar tab (ScrollTab), memfilter tombol tab berdasarkan nama
-- Rekonstruksi berdasarkan kontrak pemanggilan di dist/main.lua
...
return function(GuiConfig, LayersTab, LayersFolder, LayersPageLayout, TweenService, searchOffset)
    ...
    Bar.Size = UDim2.new(1, 0, 0, math.max(searchOffset - 6, 24))
    Bar.Parent = LayersTab
    ...
    Underline.Position = UDim2.new(0, 4, 1, 0)
    Underline.Size = UDim2.new(1, -8, 0, 1)
    ...
    -- // ScrollTab baru dibuat & di-parent belakangan oleh main.lua,
    -- jadi kita tunggu sampai muncul sebagai child dari LayersTab.
    local ScrollTab = LayersTab:FindFirstChild("ScrollTab")
    if not ScrollTab then
        LayersTab.ChildAdded:Connect(function(child)
            if child.Name == "ScrollTab" then ScrollTab = child end
        end)
    end

    local function applyFilter(query)
        if not ScrollTab then ScrollTab = LayersTab:FindFirstChild("ScrollTab") end
        if not ScrollTab then return end
        query = (query or ""):lower()
        for _, child in ipairs(ScrollTab:GetChildren()) do
            if child:IsA("GuiObject") and child.Name ~= "Separator" then
                if query == "" then
                    child.Visible = true
                else
    ...
    return {
        Bar = Bar,
        Box = Box,
        Refresh = function() applyFilter(Box.Text) end,
    }
end

end)()
local _ColorpickerSetup = (function()
-- // VelarisUI | Colorpicker.lua
-- Floating color picker (HSV square + hue slider + hex input), satu panel dipakai bersama
-- Rekonstruksi berdasarkan kontrak pemanggilan di dist/main.lua

return function(TweenService, UserInputService, CoreGui, getIconId, EmptyTable, MoreBlurFlag, EmptyCallback, ElementsModule)
    local Module = {}

    -- // Panel mengambang dibuat sekali, dipakai ulang untuk semua AddColorpicker
    local Panel, SVMap, SVCursor, HueSlider, HueCursor, HexBox, PreviewSwatch
    local _currentApi = nil
    ...
    local function buildPanel()
        ...
        Panel.Name = "VelarisColorpickerPanel"
        Panel.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
        Panel.BorderSizePixel = 0
        Panel.Size = UDim2.new(0, 190, 0, 190)
        Panel.Visible = false
        Panel.ZIndex = 500
        Panel.Parent = CoreGui:FindFirstChild("VelarisUI") or CoreGui
        ...
        WhiteGrad.ZIndex = 501
        WhiteGrad.Parent = SVMap
        local WhiteUICorner = Instance.new("UICorner")
        WhiteUICorner.CornerRadius = UDim.new(0, 6)
        WhiteUICorner.Parent = WhiteGrad
        local WhiteUIGrad = Instance.new("UIGradient")
        WhiteUIGrad.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 0),
            NumberSequenceKeypoint.new(1, 1),
        })
        WhiteUIGrad.Parent = WhiteGrad
        ...
        BlackGrad.ZIndex = 502
        BlackGrad.Parent = SVMap
        local BlackUICorner = Instance.new("UICorner")
        BlackUICorner.CornerRadius = UDim.new(0, 6)
        BlackUICorner.Parent = BlackGrad
        local BlackUIGrad = Instance.new("UIGradient")
        BlackUIGrad.Rotation = 90
        BlackUIGrad.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 1),
            NumberSequenceKeypoint.new(1, 0),
        })
        BlackUIGrad.Parent = BlackGrad
        ...
        -- // Preview + hex input
        PreviewSwatch = Instance.new("Frame")
        PreviewSwatch.Name = "Preview"
        PreviewSwatch.BorderSizePixel = 0
        PreviewSwatch.Position = UDim2.new(0, 10, 0, 152)
        PreviewSwatch.Size = UDim2.new(0, 28, 0, 28)
        PreviewSwatch.ZIndex = 501
        ...
        HexBox.BackgroundTransparency = 0.92
        HexBox.Position = UDim2.new(0, 46, 0, 152)
        HexBox.Size = UDim2.new(1, -56, 0, 28)
        HexBox.ZIndex = 501
        HexBox.Parent = Panel
        ...
        local function pushColor(fromHex)
            local color = Color3.fromHSV(h, s, v)
            PreviewSwatch.BackgroundColor3 = color
            SVMap.BackgroundColor3 = Color3.fromHSV(h, 1, 1)
            if not fromHex then HexBox.Text = hexFromColor(color) end
            if _currentApi then
                _currentApi.Value = color
                if _currentApi.Config and _currentApi.Config.Callback then
                    pcall(_currentApi.Config.Callback, color)
                end
                if _currentApi.Swatch then _currentApi.Swatch.BackgroundColor3 = color end
            end
        end
        ...
        UserInputService.InputChanged:Connect(function(input)
            ...
            if draggingSV then
                local rel = SVMap.AbsolutePosition
                local size = SVMap.AbsoluteSize
                local x = clamp01((input.Position.X - rel.X) / math.max(size.X, 1))
                local y = clamp01((input.Position.Y - rel.Y) / math.max(size.Y, 1))
                s, v = x, 1 - y
                SVCursor.Position = UDim2.new(x, 0, y, 0)
                pushColor(false)
            elseif draggingHue then
                local rel = HueSlider.AbsolutePosition
        ...
        HexBox.FocusLost:Connect(function()
            local color = colorFromHex(HexBox.Text)
            if color then
                h, s, v = color:ToHSV()
                ...
                HexBox.Text = _currentApi and hexFromColor(_currentApi.Value) or "FFFFFF"
            end
        end)

        Module._setHSV = function(color)
            h, s, v = color:ToHSV()
            SVCursor.Position = UDim2.new(s, 0, 1 - v, 0)
            HueCursor.Position = UDim2.new(h, 0, 0.5, 0)
            SVMap.BackgroundColor3 = Color3.fromHSV(h, 1, 1)
            PreviewSwatch.BackgroundColor3 = color
            HexBox.Text = hexFromColor(color)
        end
        ...
        UserInputService.InputBegan:Connect(function(input, gp)
            ...
            task.defer(function()
                ...
                if not inside and _currentApi and not _currentApi._justOpened then
                    Panel.Visible = false
                end
                if _currentApi then _currentApi._justOpened = false end
            end)
        end)
    end
    ...
    function Module.MakeColorPicker(Config, SectionAdd, CountItem, AccentColor, MainDropShadow)
        ...
        Row.LayoutOrder = CountItem or 0
        ...
        SwatchStroke.Transparency = 0.75
        ...
        function api:Set(color)
            ...
            if _currentApi == self and Module._setHSV then Module._setHSV(color) end
        end

        Swatch.MouseButton1Click:Connect(function()
            _currentApi = api
            api._justOpened = true
            Module._setHSV(api.Value)
            local absPos = Swatch.AbsolutePosition
            local absSize = Swatch.AbsoluteSize
            Panel.Position = UDim2.new(0, absPos.X - 190 + absSize.X, 0, absPos.Y + absSize.Y + 6)
            Panel.Visible = true
        end)
...
local __KeySystemUiFactory__ = (function()
-- // VelarisUI | KeySystemUi.lua
-- Layar key-input sebelum window utama dibuka. Dipanggil sebagai:
--   result(ks, GuiConfig, CoreGui, TweenService, getIconId)
-- ks = GuiConfig.KeySystem, contoh: { Type = "luarmor", ScriptId = "xxxx", Note = "..." }
-- Return true kalau lolos verifikasi (window lanjut dibuat), false/hang kalau nggak.

return function(ks, GuiConfig, CoreGui, TweenService, getIconId)
    -- // Provider table di-embed langsung dari src/elements/KeySystem/*.lua
    local Providers = {}
    ...
    Providers["luarmor"] = {
        ...
        New = function(ScriptId)
            ...
            local function copyLink()
                return setclipboard and setclipboard("https://luarmor.net/")
            end
            return { Verify = validateKey, Copy = copyLink }
        end,
    }

    Providers["pandadeveloper"] = {
        Name = "Panda",
        New = function(ServiceId)
            ...
            local function copyLink()
                return setclipboard and setclipboard("https://pandadevelopment.net/getkey?service=" .. tostring(ServiceId))
            end
            return { Verify = validateKey, Copy = copyLink }
        end,
    }
    Providers["panda"] = Providers["pandadeveloper"]

    -- // Resolusi cara verifikasi: Callback custom (ks.Callback) ATAU provider (ks.Type)
    local provider
    local defaultKeyText = ""
    local providerName = "provider"

    if type(ks.Callback) == "function" then
        defaultKeyText = tostring(ks.Default or "")
        providerName = "Callback"
        provider = {
            Verify = function(key)
                local ok, result = pcall(ks.Callback, key)
                if not ok then return false, tostring(result) end
                return result == true, result == true and "Key valid!" or "Key tidak valid."
            end,
            Copy = function() end,
        ...
        local providerType = tostring(ks and ks.Type or ""):lower()
        local providerDef = Providers[providerType]
        if not providerDef then
            warn("[VelarisUI] KeySystem: tipe provider '" .. tostring(ks and ks.Type) .. "' tidak dikenal, key system dilewati.")
            return true -- fail-open, jangan kunci user gara-gara salah konfig
        end
        providerName = providerDef.Name
        local providerId = ks.ScriptId or ks.ServiceId or ks.Id
        local ok
        ok, provider = pcall(providerDef.New, providerId)
        if not ok or not provider then
            warn("[VelarisUI] KeySystem: gagal membuat provider.")
            return true
        end
    end

    -- // Palet warna, di-port langsung dari referensi KeysystemUI (bisa di-override GuiConfig.Color)
    local ACCENT       = GuiConfig.Color or Color3.fromRGB(155, 89, 255)
    local ACCENT_DARK  = Color3.fromRGB(115, 55, 215)
    local BG_PRIMARY   = Color3.fromRGB(14, 10, 22)
    local BG_SECONDARY = Color3.fromRGB(22, 16, 35)
    local BG_INPUT     = Color3.fromRGB(32, 24, 52)
    local TEXT_PRIMARY = Color3.fromRGB(230, 220, 250)
    local TEXT_DIM     = Color3.fromRGB(120, 100, 160)
    local SUCCESS      = Color3.fromRGB(140, 100, 255)
    local ERROR        = Color3.fromRGB(180, 60, 120)

    local LOADER_NAME = ks.Title or GuiConfig.Title or "VelarisUI"
    local VERSION     = ks.Version or GuiConfig.Version or "v1.0"
    ...
    ScreenGui.Name = "VelarisUI_KeySystem"
    ...
    Card.BackgroundColor3 = BG_PRIMARY
    ...
    TitleContainer.BackgroundColor3 = BG_SECONDARY
    ...
    TitleClip.BackgroundColor3 = BG_SECONDARY
    ...
    SubtitleLabel.Text = (ks.Note and tostring(ks.Note)) or "Enter your key to continue"
    ...
    InputFrame.BackgroundColor3 = BG_INPUT
    InputFrame.BorderSizePixel = 0
    ...
    KeyBox.Text = defaultKeyText
    KeyBox.PlaceholderText = "Paste your key here..."
    KeyBox.PlaceholderColor3 = TEXT_DIM
    ...
    KeyBox.Focused:Connect(function()
        tween(InputStroke, { Color = ACCENT }, 0.2)
    end)
    ...
    local hasCopyLink = providerName ~= "Callback"

    local SubmitBtn  = makeButton("Verify Key", 170, ACCENT, Card)
    local GetKeyBtn
    if hasCopyLink then
        GetKeyBtn = makeButton("Get Key", 220, Color3.fromRGB(45, 32, 72), Card)
    end
    local DiscordBtn
    if ks.DiscordLink then
        DiscordBtn = makeButton("Copy Discord Invite", hasCopyLink and 268 or 220, Color3.fromRGB(90, 60, 160), Card)
    end
    ...
    -- // Loading screen (disembunyikan awalnya) — cuma animasi transisi,
    -- TIDAK loadstring/HttpGet script lain, lanjut ke Window utama VelarisUI
    local LoadFrame = Instance.new("Frame")
    LoadFrame.Name = "LoadFrame"
    LoadFrame.Size = UDim2.new(0, 380, 0, 180)
    LoadFrame.Position = UDim2.new(0.5, -190, 0.5, -90)
    LoadFrame.BackgroundColor3 = BG_PRIMARY
    LoadFrame.BorderSizePixel = 0
    LoadFrame.Visible = false
    LoadFrame.Parent = ScreenGui
    Instance.new("UICorner", LoadFrame).CornerRadius = UDim.new(0, 14)
    local LoadStroke = Instance.new("UIStroke")
    LoadStroke.Color = ACCENT
    ...
    LoadStatus.Text = "Verifying license..."
    ...
    PercentLabel.Text = "0%"
    PercentLabel.TextColor3 = TEXT_DIM
    PercentLabel.Font = Enum.Font.GothamBold
    PercentLabel.TextSize = 14
    PercentLabel.Parent = LoadFrame

    local function playLoadingThenContinue()
        ...
        local steps = {
            { text = "Verifying license...", pct = 0.25 },
            { text = "Preparing interface...", pct = 0.55 },
            { text = "Almost there...", pct = 0.85 },
            { text = "Done!", pct = 1.00 },
        }
        local stepDuration = 0.4
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        ...
        local ok2, isValid, msg = pcall(provider.Verify, key)
        if not ok2 then
            showStatus("Error: " .. tostring(isValid), ERROR)
        ...
        if isValid then
            showStatus(msg or "Key verified!", SUCCESS)
            SubmitBtn.Text = "Verified!"
            tween(SubmitBtn, { BackgroundColor3 = SUCCESS }, 0.2)
            verified = true
            task.wait(1)
            playLoadingThenContinue()
        else
            showStatus(msg or "Invalid key. Try again or get a new key.", ERROR)
            SubmitBtn.Text = "Verify Key"
            shakeInput()
        end
    end)
    ...
    if GetKeyBtn then
        GetKeyBtn.MouseButton1Click:Connect(function()
            local ok2 = pcall(provider.Copy)
            if ok2 and setclipboard then
                showStatus("Key link copied to clipboard!", ACCENT)
            else
                showStatus("Couldn't copy link automatically.", ACCENT)
            end
        end)
    end
    ...
    if DiscordBtn then
        DiscordBtn.MouseButton1Click:Connect(function()
            if setclipboard then
                setclipboard(tostring(ks.DiscordLink))
                showStatus("Discord invite copied!", Color3.fromRGB(88, 101, 242))
            else
                showStatus("Join: " .. tostring(ks.DiscordLink), Color3.fromRGB(88, 101, 242))
            end
        end)
    end
    ...
    -- // Blok sampai user berhasil verifikasi (atau nutup window secara manual)
    while not verified and not destroyed do
        task.wait(0.1)
    end
...
local __ConfigSectionFactory__ = (function()
...
function Chloex:Window(GuiConfig)
    ...
    -- // ConfigSection addon (dari proyek asli, langsung di-wire — nggak fetch eksternal)
    if __ConfigSectionFactory__ then
        pcall(__ConfigSectionFactory__,
            GuiFunc,
            function() return _cfgFolder end,
            function() return GuiConfig.Version end,
            function() return _ConfigData end,
            function() return _ConfigElements end,
            function(name) return GuiFunc:LoadConfig(name) end,
            function(name) return GuiFunc:SaveConfig(name) end,
            function(msg, delay, color)
                pcall(function()
                    GuiFunc:Notify({ Title = "Config", Content = tostring(msg), Duration = delay or 3 })
                end)
            end,
            function() return GuiConfig.Config.AutoLoad end
        )
    end
