...
return function(ks, GuiConfig, CoreGui, TweenService, getIconId)
    ...
    tween(Backdrop, { BackgroundTransparency = 0.45 }, 0.4)
    ...
    tween(CardStroke, { Transparency = 0.55 }, 0.5)

    -- // Title bar (bagian atas beda warna, seolah "header" terpisah)
    ...
    TitleBarClip.Size = UDim2.new(1, 0, 0, 14)
    TitleBarClip.Position = UDim2.new(0, 0, 1, -14)
    TitleBarClip.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
    TitleBarClip.BorderSizePixel = 0
    TitleBarClip.Parent = TitleBar

    local Title = Instance.new("TextLabel")
    Title.Font = Enum.Font.GothamBold
    Title.Text = ks.Title or GuiConfig.Title or "VelarisUI"
    Title.TextSize = 18
    ...
    Subtitle.Text = (ks.Note and tostring(ks.Note)) or "Enter your key to continue"
    ...
    InputFrame.BorderSizePixel = 0
    InputFrame.Position = UDim2.new(0, 18, 0, 84)
    ...
    KeyBox.Focused:Connect(function() tween(InputStroke, { Color = ACCENT }, 0.2) end)
    KeyBox.FocusLost:Connect(function(enterPressed)
        tween(InputStroke, { Color = Color3.fromRGB(60, 60, 60) }, 0.2)
    end)
    ...
    StatusLabel.Position = UDim2.new(0, 18, 0, 132)
    StatusLabel.Size = UDim2.new(1, -36, 0, 24)
    StatusLabel.Parent = Card

    -- // Tombol dengan hover effect
    local function makeButton(text, posY, color, parent, filled)
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(1, -36, 0, 40)
        btn.Position = UDim2.new(0, 18, 0, posY)
        btn.BackgroundColor3 = color
        btn.BackgroundTransparency = filled and 0 or 0.9
        btn.Text = text
        ...
        local baseTrans = filled and 0 or 0.9
        btn.MouseEnter:Connect(function()
            tween(btn, { BackgroundTransparency = math.max(baseTrans - 0.1, 0) }, 0.15)
        end)
        btn.MouseLeave:Connect(function()
            tween(btn, { BackgroundTransparency = baseTrans }, 0.15)
        end)
        return btn
    end
    ...
    local SubmitBtn = makeButton("Verify Key", 166, ACCENT, Card, true)
    local GetKeyBtn
    if hasCopyLink then
        GetKeyBtn = makeButton("Get Key", 214, Color3.fromRGB(255, 255, 255), Card, false)
    end
    ...
    if GetKeyBtn then
        GetKeyBtn.MouseButton1Click:Connect(function()
            ...
            StatusLabel.Text = (ok2 and setclipboard) and "Key link copied to clipboard!" or "Can't copy link (executor doesn't support setclipboard)."
        end)
    end
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        ...
        local ok2, isValid, msg = pcall(provider.Verify, KeyBox.Text)
        if not ok2 then
            StatusLabel.TextColor3 = Color3.fromRGB(255, 110, 110)
            StatusLabel.Text = "Error: " .. tostring(isValid)
        ...
        if isValid then
            ...
            StatusLabel.Text = msg or "Key verified!"
            SubmitBtn.Text = "Verified!"
            tween(SubmitBtn, { BackgroundColor3 = Color3.fromRGB(80, 200, 120) }, 0.2)
            verified = true
            task.wait(0.5)
            ...
            StatusLabel.TextColor3 = Color3.fromRGB(255, 110, 110)
            StatusLabel.Text = msg or "Invalid key. Try again."
            SubmitBtn.Text = "Verify Key"
            shakeInput()
        end
    end)
