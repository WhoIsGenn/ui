...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry=RunService.PreSimulation:Connect(function()
        ...
        for _,ball in pairs(balls) do
            ...
            local range_level = System.__properties.__parry_range_offset or 0
            if range_level > 0 then
                local base_boost = range_level * 2.5
                local ping_sec = Stats.Network.ServerStatsItem['Data Ping']:GetValue() / 1000
                local reach_time = speed > 0 and (distance / speed) or math.huge
                local time_window = ping_sec * (1 + range_level * 0.15) + 0.05
                local predictive = ball_target == LocalPlayer.Name and reach_time <= time_window
                parry_accuracy = parry_accuracy + base_boost
                if predictive then parry_accuracy = parry_accuracy + base_boost end
            end
            ...
            local cooldown = range_level > 0 and 0.3 or 1
            repeat RunService.Stepped:Wait()
            until (tick()-last_parrys) >= cooldown or not System.__properties.__parried
        ...
        if training_ball then
            ...
            if zoomies then
                ...
                if not System.__properties.__training_parried then
                    ...
                    local speed_divisor=(2.4+capped_speed_diff*0.002)*System.__properties.__divisor_multiplier
                    local parry_accuracy=ping_threshold+math.max(speed/speed_divisor,9.5)
                    local t_range_level = System.__properties.__parry_range_offset or 0
                    if t_range_level > 0 then
                        local t_base_boost = t_range_level * 2.5
                        local t_ping_sec = Stats.Network.ServerStatsItem['Data Ping']:GetValue() / 1000
                        local t_reach_time = speed > 0 and (distance / speed) or math.huge
                        local t_time_window = t_ping_sec * (1 + t_range_level * 0.15) + 0.05
                        local t_predictive = ball_target == LocalPlayer.Name and t_reach_time <= t_time_window
                        parry_accuracy = parry_accuracy + t_base_boost
                        if t_predictive then parry_accuracy = parry_accuracy + t_base_boost end
                    end
                    if ball_target==LocalPlayer.Name and distance <= parry_accuracy then
                        if getgenv().AutoParryMode=="Keypress" then System.parry.keypress()
                        else System.parry.execute_action() end
