...
function Auto_Parry.CalculateParryCFrame(ball, targetPlayer)
    ...
    elseif mode == "Left" and cam then
        return targetPos + (cam.CFrame.RightVector * -55)
    elseif mode == "Right" and cam then
        return targetPos + (cam.CFrame.RightVector * 55)
    elseif mode == "Up" then
                ...
                pcall(function()
                    if _remote:IsA("RemoteEvent") then
                        _remote:FireServer(
                            ...
                            cam.CFrame, Last_Positions_Cache,
...
-- ═══ MAIN PHYSICS LOOP (CURVE MODE + ACCURACY) ═════════════════════════
...
AutoParrySec:AddDropdown({
    Title = "Curve Mode",
    Description = "Ball curve direction",
    Flag = "parrycurve",
    Options = {"Camera", "Left", "Right", "Up", "Down", "Random", "Straight"},
    Default = "Camera",
    Multi = false,
...
AppearanceSec:AddButton({
    ...
    Callback = function()
        ...
        Window:Notify({
            Title = "Headless",
            Content = isHeadless and "Enabled" or "Disabled",
            Duration = 2,
        })
    end,
})
...
AppearanceSec:AddButton({
    ...
    Callback = function()
        ...
        Window:Notify({
            Title = "Korblox",
            Content = isKorblox and "Enabled" or "Disabled",
            Duration = 2,
        })
    end,
})
