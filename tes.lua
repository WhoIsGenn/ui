...
-- Pakai Beam untuk garis luar yang di lantai, Decal untuk fill
-- Ikut karakter via RenderStepped, selalu di lantai (Y - 2.9)
-- ================================================================
local ParryFOV = {
    holder     = nil,
    fillPart   = nil,
...
local function ParryFOV_Clear()
    if ParryFOV.conn then
        ParryFOV.conn:Disconnect()
        ParryFOV.conn = nil
    end
    if ParryFOV.holder and ParryFOV.holder.Parent then
        pcall(function() ParryFOV.holder:Destroy() end)
    end
    ParryFOV.holder      = nil
    ParryFOV.fillPart    = nil
    ParryFOV.beams       = {}
    ParryFOV.attachments = {}
    ParryFOV.decal       = nil
end
...
local function ParryFOV_Create()
    ...
    ParryFOV.holder    = holder

    -- Fill: flat cylinder + Decal lingkaran
    local fillPart = Instance.new("Part")
    fillPart.Size        = Vector3.new(r * 2, 0.05, r * 2)
    ...
    fillPart.Parent      = holder
    ParryFOV.fillPart    = fillPart
    ...
    ParryFOV.decal     = decal

    -- Highlight untuk fill yang beneran circle (lebih reliable dari Decal)
    ...
    -- Outline: 48 Beam segments membentuk circle di lantai
    local beamCount = 48
    for i = 1, beamCount do
        local a1 = (i-1)/beamCount * math.pi * 2
        local a2 = i    /beamCount * math.pi * 2

        local att1 = Instance.new("Attachment")
        att1.Position = Vector3.new(math.cos(a1)*r, 0, math.sin(a1)*r)
        att1.Parent   = holder
        table.insert(ParryFOV.attachments, att1)

        local att2 = Instance.new("Attachment")
        att2.Position = Vector3.new(math.cos(a2)*r, 0, math.sin(a2)*r)
        att2.Parent   = holder
        table.insert(ParryFOV.attachments, att2)
        ...
        beam.Parent       = holder
        table.insert(ParryFOV.beams, beam)
    end

    -- Update posisi tiap RenderStepped (di lantai Y-2.9 dari HRP)
    ParryFOV.conn = RunService.RenderStepped:Connect(function()
        ...
        local px, py, pz = root.Position.X, root.Position.Y - 2.9, root.Position.Z
        holder.CFrame   = CFrame.new(px, py, pz)
        -- fillPart dirotasi agar flat (cylinder horizontal)
        fillPart.CFrame = CFrame.new(px, py, pz) * CFrame.Angles(0, 0, math.rad(90))

        -- Update warna kalau cooldown berubah
        local newCol = ParryOnCooldown
            and Color3.fromRGB(255, 60, 60)
            or  Color3.fromRGB(0, 170, 255)
        if ParryFOV.decal then
            ParryFOV.decal.Color3 = newCol
        end
        local colSeq = ColorSequence.new(newCol)
        for _, b in ipairs(ParryFOV.beams) do
            b.Color = colSeq
        end
...
local function UpdateParryFOV()
    ...
    local myRoot = GetCharacterRoot()
    if not myRoot then ParryFOV_Clear(); return end
    -- Buat circle kalau belum ada atau radius berubah
    if not ParryFOV.holder or not ParryFOV.holder.Parent then
        ParryFOV_Create()
    elseif ParryFOV.fillPart and ParryFOV.fillPart.Size.Y ~= Config.PARRY_Dist * 2 then
        -- Radius berubah, rebuild
        ParryFOV_Create()
    end
end
