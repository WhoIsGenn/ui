...
local Config = {
    ...
    AIM_Enabled = false,
    AIM_UseRMB = true,
    AIM_AutoMode = false, -- auto aim saat character Aiming (mobile)
    AIM_FOV = 120,
    AIM_Smooth = 0.3,
    AIM_TargetPart = "Head",
    AIM_VisCheck = true,
...
function Aimbot.Update(cam, screenSize, screenCenter)
    ...
    if Config.AIM_AutoMode then
        -- Mobile: aktif saat character sedang Aiming (hold tombol gun)
        local char = LocalPlayer.Character
        shouldAim = char ~= nil and char:GetAttribute("Aiming") == true
    elseif Config.AIM_UseRMB then
        -- PC: aktif saat hold RMB
        shouldAim = State.AimHolding
    else
        -- Always on
        shouldAim = true
    end
