-- ============================================
-- SAILOR PIECE SCRIPT - VELARIS UI FINAL
-- SEMUA FITUR AUTO FARM, BOSS, SUMMON, SKILL, STAT, ARTIFACT, DUNGEON, MERCHANT DLL TETAP ADA
-- OBSIDIAN UI SUDAH DIGANTI TOTAL DENGAN VELARIS UI
-- ============================================
...
local function SafeLoop(name, func)
    return function()
        ...
        if not success then
            VelarisUI:Notify({ Title = "Error", Content = "Error in ["..name.."]: "..tostring(err), Duration = 10 })
...
Remotes.StockUpdate.OnClientEvent:Connect(function(itemName, stockLeft)
    ...
    if stockLeft == 0 then
        VelarisUI:Notify({ Title = "MERCHANT", Content = "Bought: " .. tostring(itemName), Duration = 2 })
    end
end)
...
local function PanicStop()
    ...
    VelarisUI:Notify({ Title = "celina", Content = "Stopped.", Duration = 5 })
end
    ...
    task.spawn(function()
        ...
        pcall(function()
            ...
            for i, v in ipairs(wsChildren) do
                local isProtected = v.Name:find("TimedBossSpawn_") or v.Name == Plr.Name or v.Name == "Main Temple" or v.Name == "NPCs" or v.Name == "ServiceNPCs" or v.Name:find("QuestNPC") or v:IsA("Camera") or v:IsA("Terrain") or v.Name:find("Portal_")
                if not isProtected then
...
local function SendSafetyWebhook(targetPlayer, reason)
    local url = Options.WebhookURL.Value
    if url == "" or not url:find("discord.com/api/webhooks/") then return end
    local payload = { ["embeds"] = {{ ["title"] = "⚠️ Auto Kick", ["description"] = "Someone joined you blud", ["color"] = 16711680, ["fields"] = { { ["name"] = "Username", ["value"] = "`" .. targetPlayer.Name .. "`", ["inline"] = true }, { ["name"] = "Type", ["value"] = reason, ["inline"] = true }, { ["name"] = "ID", ["value"] = "```" .. game.JobId .. "```", ["inline"] = false } }, ["footer"] = { ["text"] = "celina • " .. os.date("%x %X") } }} }
    task.spawn(function()
        pcall(function()
            request({ Url = url, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = HttpService:JSONEncode(payload) })
        end)
    ...
    if kickTypes["Public Server"] then
        ...
        if success and serverType ~= "VIPServer" then
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = { ["embeds"] = {{ ["title"] = "⚠️ Auto Kick", ["description"] = "Kicked because in Public server.", ["color"] = 16753920, ["fields"] = { { ["name"] = "Username", ["value"] = "`" .. Plr.Name .. "`", ["inline"] = true }, { ["name"] = "JobId", ["value"] = "```" .. game.JobId .. "```", ["inline"] = false } }, ["footer"] = { ["text"] = "celina" } }} }
                task.spawn(function()
                    pcall(function()
                        request({ Url = url, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = HttpService:JSONEncode(payload) })
                    end)
...
local function SafeTeleportToNPC(targetName, customMap)
    ...
    if target then
        ...
        VelarisUI:Notify({ Title = "celina", Content = "NPC not found: " .. tostring(actualName), Duration = 3 })
    end
end
...
local function EvaluateArtifact2(uuid, data)
    ...
    if Toggles.ArtifactLock.Value and not data.Locked and data.Level >= (lockMinSS * 3) then
        if IsWhitelisted(Options.Lock_MS.Value, data.MainStat.Stat) and IsWhitelisted(Options.Lock_Type.Value, data.Category) and IsWhitelisted(Options.Lock_Set.Value, data.Set) then
...
local function AutoUpgradeLoop(mode)
    ...
    while toggle.Value or allToggle.Value do
        ...
        if not workDone then
            VelarisUI:Notify({ Title = "celina", Content = "Stopping..", Duration = 5 })
            toggle:SetValue(false)
...
local function UniversalPuzzleSolver(puzzleType)
    ...
    VelarisUI:Notify({ Title = "celina", Content = "Starting " .. puzzleType .. " Puzzle...", Duration = 5 })
    for i, islandOrPiece in ipairs(piecesToCollect) do
        ...
        if piece then
            ...
            if prompt then
                fireproximityprompt(prompt)
                VelarisUI:Notify({ Title = "celina", Content = string.format("Collected Piece %d/%d", i, #piecesToCollect), Duration = 2 })
                task.wait(1.5)
            else
                VelarisUI:Notify({ Title = "celina", Content = "Found piece but no interaction prompt was detected.", Duration = 3 })
            end
        else
            VelarisUI:Notify({ Title = "celina", Content = "Failed to find piece " .. i .. " on " .. tostring(tpTarget or "Island"), Duration = 3 })
        end
    end
    VelarisUI:Notify({ Title = "celina", Content = puzzleType .. " Puzzle Completed!", Duration = 5 })
end
...
local function GetOtherTarget()
    ...
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        ...
        if lowerSelected:find("strongest") then
            if name:find("strongest") and ((lowerSelected:find("history") and name:find("history")) or (lowerSelected:find("today") and name:find("today"))) then
                isMatch = true
            end
...
local function Func_KillAura()
    while Toggles.KillAura.Value do
        ...
        if target then
            ...
            pcall(function() Remotes.M1:FireServer(targetPos) end)
        end
...
local function AutoRollStatsLoop()
    ...
    if not hasStat or not hasRank then
        VelarisUI:Notify({ Title = "celina", Content = "Error: Select at least one Stat and one Rank first!", Duration = 5 })
    ...
    while Toggles.AutoRollStats.Value do
            ...
            if selectedStats[statName] then
                ...
                if currentData then
                    ...
                    if not selectedRanks[currentRank] then
                        ...
                        if not success then
                            VelarisUI:Notify({ Title = "celina", Content = "ERROR: " .. tostring(err):gsub("<", "["), Duration = 5 })
                        end
        ...
        if workDone then
            VelarisUI:Notify({ Title = "celina", Content = "Successfully rolled selected stats.", Duration = 5 })
            Toggles.AutoRollStats:SetValue(false)
            break
        end
        task.wait()
    ...
    while task.wait() do
        if Toggles.AutoTrait.Value then
            ...
            if selected[currentTrait] then
                VelarisUI:Notify({ Title = "celina", Content = "Success! Got Trait: " .. currentTrait, Duration = 5 })
        ...
        if Toggles.AutoRace.Value then
            ...
            if selected[currentRace] then
                VelarisUI:Notify({ Title = "celina", Content = "Success! Got Race: " .. currentRace, Duration = 5 })
        ...
        if Toggles.AutoClan.Value then
            ...
            if selected[currentClan] then
                VelarisUI:Notify({ Title = "celina", Content = "Success! Got Clan: " .. currentClan, Duration = 5 })
...
local function EnsureRollManager()
    Thread("UnifiedRollManager", Func_UnifiedRollManager, Toggles.AutoTrait.Value or Toggles.AutoRace.Value or Toggles.AutoClan.Value)
end
...
local function AutoSpecPassiveLoop()
    ...
    while Toggles.AutoSpec.Value do
        ...
        if not workDone then
            VelarisUI:Notify({ Title = "celina", Content = "Done", Duration = 5 })
    ...
    while Toggles.AutoInfiniteTower.Value do
        ...
        if isInTower then
            ...
            if bestTarget then
                ...
                if (root.Position - finalPos).Magnitude > 0.1 then
                    if Options.SelectedMovementType.Value == "Teleport" then
                        ...
                        local distance = (root.Position - finalPos).Magnitude
                        local speed = tonumber(Options.TweenSpeed.Value) or 180
                        game:GetService("TweenService"):Create(root, TweenInfo.new(distance/speed, Enum.EasingStyle.Linear), {CFrame = finalCFrame}):Play()
                    end
                end
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
            ...
            if PGui.InfiniteTowerUI and PGui.InfiniteTowerUI.MainFrame.Visible then
                ...
                local spawnButton = holder:FindFirstChild("SpawnInfiniteTowerPortalFrame") and holder.SpawnInfiniteTowerPortalFrame:FindFirstChild("SpawnPortalButton")
                if spawnButton then
                ...
                if Shared.Island ~= "Tower" then
                    ...
                    if npc then
                        local prompt = npc:FindFirstChild("HumanoidRootPart") and npc.HumanoidRootPart:FindFirstChild("ProximityPrompt")
                        if prompt then
                            local char = GetCharacter()
                            local root = char and char:FindFirstChild("HumanoidRootPart")
                            if root then
        ...
        if Toggles.AutoCraftItem.Value then
            ...
            for _, item in pairs(Shared.Cached.Inv) do
                if selected["DivineGrail"] and item.name == "Broken Sword" and item.quantity >= 3 then
                    ...
                    pcall(function() Remotes.GrailCraft:InvokeServer("DivineGrail", craftAmount) end)
                    task.wait(0.5)
                end
                if selected["SlimeKey"] and item.name == "Slime Shard" and item.quantity >= 2 then
                    local totalPossible = math.floor(item.quantity / 2)
                    local craftAmount = math.min(totalPossible, 99)
                    pcall(function() Remotes.SlimeCraft:InvokeServer("SlimeKey", craftAmount) end)
                end
            end
        end
...
for i = 1, #PriorityTasks do
    PrioritySec:AddDropdown({ Title = "Priority " .. i, Options = PriorityTasks, Default = DefaultPriority[i], Callback = function(val) Options["SelectedPriority_" .. i].Value = val end })
end

-- Main Tab - Autofarm
local MainTabVel = TabsVel.Main
local AutofarmSec = MainTabVel:AddSection({ Title = "Autofarm", Open = true })
AutofarmSec:AddDropdown({ Title = "Select Mob(s)", Options = Tables.MobList, Multi = true, Callback = function(v) Options.SelectedMob.Value = v end })
AutofarmSec:AddButton({ Title = "Refresh Mobs", Callback = UpdateNPCLists })
AutofarmSec:AddToggle({ Title = "Autofarm Selected Mob", Default = false, Callback = function(s) Toggles.MobFarm = { Value = s } end })
AutofarmSec:AddToggle({ Title = "Autofarm All Mobs", Default = false, Callback = function(s) Toggles.AllMobFarm = { Value = s } end })
AutofarmSec:AddToggle({ Title = "Autofarm Level", Default = false, Callback = function(s) Toggles.LevelFarm = { Value = s } end })

-- Boss Farm Section
local BossSec = MainTabVel:AddSection({ Title = "Boss Farm", Open = true })
BossSec:AddDropdown({ Title = "Select Bosses", Options = Tables.BossList, Multi = true, Callback = function(v) Options.SelectedBosses.Value = v end })
BossSec:AddToggle({ Title = "Autofarm Selected Boss", Default = false, Callback = function(s) Toggles.BossesFarm = { Value = s } end })
BossSec:AddToggle({ Title = "Autofarm All Bosses", Default = false, Callback = function(s) Toggles.AllBossesFarm = { Value = s } end })
BossSec:AddDivider()

-- Summon Section
BossSec:AddDropdown({ Title = "Select Summon Boss", Options = Tables.SummonList, Callback = function(v) Options.SelectedSummon.Value = v end })
BossSec:AddDropdown({ Title = "Summon Difficulty", Options = Tables.DiffList, Default = "Normal", Callback = function(v) Options.SelectedSummonDiff.Value = v end })
BossSec:AddToggle({ Title = "Auto Summon", Default = false, Callback = function(s) Toggles.AutoSummon = { Value = s } end })
BossSec:AddToggle({ Title = "Autofarm Summon Boss", Default = false, Callback = function(s) Toggles.SummonBossFarm = { Value = s } end })
BossSec:AddDivider()

-- Pity Label
local PityLabelObj = BossSec:AddParagraph({ Title = "Pity", Content = "0/25" })
BossSec:AddDropdown({ Title = "Select Boss [Build Pity]", Options = Tables.AllBossList, Multi = true, AllowNull = true, Callback = function(v) Options.SelectedBuildPity.Value = v end })
BossSec:AddDropdown({ Title = "Select Boss [Use Pity]", Options = Tables.AllBossList, AllowNull = true, Callback = function(v) Options.SelectedUsePity.Value = v end })
BossSec:AddToggle({ Title = "Autofarm Pity Boss", Default = false, Callback = function(s) Toggles.PityBossFarm = { Value = s } end })

-- Weapon Config Section
local WeaponSec = MainTabVel:AddSection({ Title = "Weapon Config", Open = true })
WeaponSec:AddDropdown({ Title = "Select Weapon Type", Options = Tables.Weapon, Multi = true, Callback = function(v) Options.SelectedWeaponType.Value = v end })
WeaponSec:AddSlider({ Title = "Switch Weapon Delay", Min = 1, Max = 20, Default = 4, Callback = function(v) Options.SwitchWeaponCD.Value = v end })
WeaponSec:AddToggle({ Title = "Auto Switch Weapon", Default = true, Callback = function(s) Toggles.SwitchWeapon = { Value = s } end })

-- Movement Config Section
local MoveSec = MainTabVel:AddSection({ Title = "Movement Config", Open = true })
MoveSec:AddToggle({ Title = "Island TP [Autofarm]", Default = true, Callback = function(s) Toggles.IslandTP = { Value = s } end })
MoveSec:AddSlider({ Title = "Island TP CD", Min = 0, Max = 2.5, Default = 0.67, Rounding = 2, Callback = function(v) Options.IslandTPCD.Value = v end })
MoveSec:AddDropdown({ Title = "Movement Type", Options = {"Teleport", "Tween"}, Default = "Tween", Callback = function(v) Options.SelectedMovementType.Value = v end })
MoveSec:AddDropdown({ Title = "Farm Position", Options = {"Behind", "Above", "Below"}, Default = "Behind", Callback = function(v) Options.SelectedFarmType.Value = v end })
MoveSec:AddSlider({ Title = "Farm Distance", Min = 0, Max = 30, Default = 12, Callback = function(v) Options.Distance.Value = v end })
MoveSec:AddSlider({ Title = "Tween Speed", Min = 50, Max = 500, Default = 160, Callback = function(v) Options.TweenSpeed.Value = v end })
...
HakiSec:AddToggle({ Title = "Auto Observation Haki", Default = false, Callback = function(s) Toggles.ObserHaki = { Value = s } end })
HakiSec:AddToggle({ Title = "Auto Armament Haki", Default = false, Callback = function(s) Toggles.ArmHaki = { Value = s } end })
HakiSec:AddToggle({ Title = "Auto Conqueror Haki", Default = false, Callback = function(s) Toggles.ConquerorHaki = { Value = s } end })

-- Skills Section
local SkillSec = AutoTabVel:AddSection({ Title = "Skills", Open = true })
SkillSec:AddToggle({ Title = "Auto Attack (M1)", Default = false, Callback = function(s) Toggles.AutoM1 = { Value = s } end })
SkillSec:AddToggle({ Title = "Kill Aura", Default = false, Callback = function(s) Toggles.KillAura = { Value = s } end })
SkillSec:AddSlider({ Title = "Kill Aura CD", Min = 0.05, Max = 1, Default = 0.1, Rounding = 2, Callback = function(v) Options.KillAuraCD.Value = v end })
SkillSec:AddSlider({ Title = "Kill Aura Range", Min = 50, Max = 300, Default = 200, Callback = function(v) Options.KillAuraRange.Value = v end })
SkillSec:AddDropdown({ Title = "Select Skills", Options = {"Z", "X", "C", "V", "F"}, Multi = true, Callback = function(v) Options.SelectedSkills.Value = v end })
SkillSec:AddToggle({ Title = "Auto Use Skills", Default = false, Callback = function(s) Toggles.AutoSkill = { Value = s } end })

-- Rolls Section
local RollSec = AutoTabVel:AddSection({ Title = "Rolls", Open = true })
RollSec:AddSlider({ Title = "Roll Delay", Min = 0.05, Max = 1, Default = 0.3, Rounding = 2, Callback = function(v) Options.RollCD.Value = v end })
RollSec:AddDropdown({ Title = "Select Trait(s)", Options = Tables.TraitList, Multi = true, Callback = function(v) Options.SelectedTrait.Value = v end })
RollSec:AddToggle({ Title = "Auto Roll Trait", Default = false, Callback = function(s) Toggles.AutoTrait = { Value = s } end })
RollSec:AddDropdown({ Title = "Select Race(s)", Options = Tables.RaceList, Multi = true, Callback = function(v) Options.SelectedRace.Value = v end })
RollSec:AddToggle({ Title = "Auto Roll Race", Default = false, Callback = function(s) Toggles.AutoRace = { Value = s } end })

-- Stats Section
local StatsSec = AutoTabVel:AddSection({ Title = "Stats", Open = true })
StatsSec:AddDropdown({ Title = "Select Stat(s)", Options = {"Melee", "Defense", "Sword", "Power"}, Multi = true, Callback = function(v) Options.SelectedStats.Value = v end })
StatsSec:AddToggle({ Title = "Auto UP Stats", Default = false, Callback = function(s) Toggles.AutoStats = { Value = s } end })
local StatsLabelObj = StatsSec:AddParagraph({ Title = "Gem Stats", Content = "N/A" })
StatsSec:AddDropdown({ Title = "Target Gem Stat(s)", Options = Tables.GemStat, Multi = true, Callback = function(v) Options.SelectedGemStats.Value = v end })
StatsSec:AddToggle({ Title = "Auto Roll Stats", Default = false, Callback = function(s) Toggles.AutoRollStats = { Value = s } end })

-- Player Tab
local PlayerTabVel = TabsVel.Player

-- General Section
local GeneralSec = PlayerTabVel:AddSection({ Title = "General", Open = true })
GeneralSec:AddSlider({ Title = "WalkSpeed", Min = 16, Max = 250, Default = 16, Callback = function(v) Options.WSValue = { Value = v } Toggles.WS = { Value = true } end })
GeneralSec:AddSlider({ Title = "JumpPower", Min = 0, Max = 500, Default = 50, Callback = function(v) Options.JPValue = { Value = v } Toggles.JP = { Value = true } end })
GeneralSec:AddToggle({ Title = "Noclip", Default = false, Callback = function(s) Toggles.Noclip = { Value = s } end })
GeneralSec:AddToggle({ Title = "Anti Knockback", Default = false, Callback = function(s) Toggles.AntiKnockback = { Value = s } end })
GeneralSec:AddToggle({ Title = "Anti AFK", Default = true, Callback = function(s) Toggles.AntiAFK = { Value = s } end })
GeneralSec:AddToggle({ Title = "Auto Reconnect", Default = false, Callback = function(s) Toggles.AutoReconnect = { Value = s } end })
...
IslandSec:AddDropdown({ Title = "Select Island", Options = Tables.IslandList, Callback = function(v) if v then Remotes.TP_Portal:FireServer(v) end end })

local NPCSec = TeleTabVel:AddSection({ Title = "NPC Teleport", Open = true })
NPCSec:AddDropdown({ Title = "Select NPC", Options = Tables.NPC_QuestList, Callback = function(v) if v then SafeTeleportToNPC(v) end end })

-- Misc Tab
local MiscTabVel = TabsVel.Misc

-- Merchant Section
local MerchantSec = MiscTabVel:AddSection({ Title = "Merchant", Open = true })
local MerchantTimerLabel = MerchantSec:AddParagraph({ Title = "Refresh Timer", Content = "N/A" })
MerchantSec:AddDropdown({ Title = "Select Item(s)", Options = Tables.MerchantList, Multi = true, Callback = function(v) Options.SelectedMerchantItems.Value = v end })
MerchantSec:AddToggle({ Title = "Auto Buy Selected Items", Default = false, Callback = function(s) Toggles.AutoMerchant = { Value = s } end })

-- Chest Section
local ChestSec = MiscTabVel:AddSection({ Title = "Chests", Open = true })
ChestSec:AddDropdown({ Title = "Select Chest(s)", Options = Tables.Rarities, Multi = true, Callback = function(v) Options.SelectedChests.Value = v end })
ChestSec:AddToggle({ Title = "Auto Open Chest", Default = false, Callback = function(s) Toggles.AutoChest = { Value = s } end })

-- Craft Section
local CraftSec = MiscTabVel:AddSection({ Title = "Craft", Open = true })
CraftSec:AddDropdown({ Title = "Select Item(s) to Craft", Options = Tables.CraftItemList, Multi = true, Callback = function(v) Options.SelectedCraftItems.Value = v end })
CraftSec:AddToggle({ Title = "Auto Craft Item", Default = false, Callback = function(s) Toggles.AutoCraftItem = { Value = s } end })

-- Dungeon Tab
local DungeonTabVel = TabsVel.Dungeon
local DungeonSec = DungeonTabVel:AddSection({ Title = "Dungeon Config", Open = true })
DungeonSec:AddDropdown({ Title = "Select Dungeon", Options = Tables.DungeonList, AllowNull = true, Callback = function(v) Options.SelectedDungeon.Value = v end })
DungeonSec:AddToggle({ Title = "Auto Join Dungeon", Default = false, Callback = function(s) Toggles.AutoDungeon = { Value = s } end })
DungeonSec:AddToggle({ Title = "Auto Infinite Tower", Default = false, Callback = function(s) Toggles.AutoInfiniteTower = { Value = s } end })

-- Artifact Tab
local ArtifactTabVel = TabsVel.Artifact
local StatusSec = ArtifactTabVel:AddSection({ Title = "Status", Open = true })
local ArtifactLabel = StatusSec:AddParagraph({ Title = "Status", Content = "N/A" })
local DustLabel = StatusSec:AddParagraph({ Title = "Dust", Content = "N/A" })

local EquipSec = ArtifactTabVel:AddSection({ Title = "Auto Equip", Open = true })
EquipSec:AddToggle({ Title = "Auto Equip Artifacts", Default = false, Callback = function(s) Toggles.ArtifactEquip = { Value = s } end })

local UpgradeSec = ArtifactTabVel:AddSection({ Title = "Upgrade", Open = true })
UpgradeSec:AddSlider({ Title = "Upgrade Limit", Min = 0, Max = 15, Default = 0, Callback = function(v) Options.UpgradeLimit.Value = v end })
UpgradeSec:AddToggle({ Title = "Auto Upgrade", Default = false, Callback = function(s) Toggles.ArtifactUpgrade = { Value = s } end })
...
-- Update pity label setiap detik
task.spawn(function()
    while getgenv().celina_Running do
        pcall(function()
            if PityLabelObj then
                local current, max = GetCurrentPity()
                PityLabelObj:SetContent(string.format("%d/%d", current or 0, max or 25))
            end
        end)
        task.wait(1)
    end
...
task.spawn(function()
    while getgenv().celina_Running do
        pcall(function()
            if MerchantTimerLabel and Shared.LocalMerchantTime then
                local minutes = math.floor(Shared.LocalMerchantTime / 60)
                local seconds = Shared.LocalMerchantTime % 60
                MerchantTimerLabel:SetContent(string.format("%02d:%02d", minutes, seconds))
            end
        end)
        task.wait(1)
    end
...
task.spawn(function()
    while getgenv().celina_Running do
        pcall(function()
            if StatsLabelObj and next(Shared.GemStats) then
                local text = ""
                for statName, data in pairs(Shared.GemStats) do
                    text = text .. statName .. ": " .. tostring(data.Rank) .. "\n"
                end
                StatsLabelObj:SetContent(text ~= "" and text or "N/A")
            end
        end)
        task.wait(2)
    end
...
task.spawn(Func_ArtifactAutomation)
task.spawn(Func_AutoDungeon)
task.spawn(Func_AutoInfiniteTower)
task.spawn(Func_ArtifactMilestone)
task.spawn(AutoSkillTreeLoop)
task.spawn(AutoSpecPassiveLoop)
task.spawn(AutoRollStatsLoop)
task.spawn(EnsureRollManager)
task.spawn(Func_AutoReconnect)
task.spawn(Func_NoGameplayPaused)
...
task.spawn(function()
    while getgenv().celina_Running do
        ...
        if not success then
            VelarisUI:Notify({ Title = "celina", Content = "ERROR: " .. tostring(err), Duration = 10 })
        end
    end
end)
    ...
    while getgenv().celina_Running do
        ...
        if root and not Shared.MovingIsland then
            ...
            if pos.Y > 5000 or math.abs(pos.X) > 10000 or math.abs(pos.Z) > 10000 then
                ...
                VelarisUI:Notify({ Title = "celina", Content = "Something wrong, attempt to reset..", Duration = 5 })
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
...
if not eh_success then
    VelarisUI:Notify({ Title = "ERROR", Content = tostring(err), Duration = 10 })
end
