...
-- BLATANT V3 SECTION - REWRITE PAKAI SISTEM SAMA DENGAN V2
local blatantV3Section = Tab3:AddSection("Blatant V3 (Beta)")

-- Setup networking (sama dengan yang kamu punya)
...
-- Variables untuk V3
local frameCount = 0
local isBlatantV3Running = false

-- Heartbeat loop untuk V3
RunService.Heartbeat:Connect(function()
    if not isBlatantV3Running then return end
    
    frameCount += 1
    
    local reelEvery = math.max(1, math.floor((_G.ReelDelayV3 or 0.7) / (1/60)))
    local completeEvery = math.max(1, math.floor((_G.CompleteDelayV3 or 0.3) / (1/60)))
    ...
    -- Cleanup setiap 10 detik (600 frames)
...
-- UI Elements untuk V3 (sama dengan V2 style)
blatantV3Section:AddToggle({
    Title = "Enable Blatant V3",
    Default = false,
    Callback = function(value)
        isBlatantV3Running = value
...
blatantV3Section:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num > 0 then
            _G.ReelDelayV3 = num
            notif("V3 Reel Delay: " .. num .. "s", 3, Color3.fromRGB(0, 255, 0))
        else
            notif("Invalid delay!", 3, Color3.fromRGB(255, 0, 0))
        end
    end
...
blatantV3Section:AddInput({
    Title = "Complete Delay (V3)",
    Default = "0.3",
    Callback = function(v)
        local num = tonumber(v)
        if num and num > 0 then
            _G.CompleteDelayV3 = num
            notif("V3 Complete Delay: " .. num .. "s", 3, Color3.fromRGB(0, 255, 0))
        else
            notif("Invalid delay!", 3, Color3.fromRGB(255, 0, 0))
        end
    end
