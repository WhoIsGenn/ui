...
local function StopAutoGen()
    ...
    -- Reset humanoid state agar bisa gerak lagi
    pcall(function()
        local char = LocalPlayer.Character
...
local function LeaveGenerator()
    Config.AUTO_Generator = false  -- stop loop dulu
    local char = LocalPlayer.Character
    if not char then return false end
    local root = char:FindFirstChild("HumanoidRootPart")
    local hum  = char:FindFirstChildOfClass("Humanoid")
    ...
    -- Kalau ga ada gen, tetap stop aja
    if not nearestGen then
        StopAutoGen()
        return false
    end

    -- Hitung posisi escape: 25 studs menjauhi gen, di ground level
    local genPos  = nearestGen.part.Position
    local dir     = (root.Position - genPos)
    dir = Vector3.new(dir.X, 0, dir.Z)
    if dir.Magnitude < 0.01 then dir = Vector3.new(1, 0, 0) end
    dir = dir.Unit
    local escapePos = genPos + dir * 25 + Vector3.new(0, 3, 0)

    -- Unanchor dulu agar bisa di-TP
    local wasAnchored = root.Anchored
    root.Anchored = false

    -- TP langsung ke posisi escape
...
-- ================================================================
-- AUTO PARRY
-- Deteksi killer dalam jarak serang (<=12 studs) lalu fire parry remote.
-- Tidak pakai OnClientEvent BasicAttack karena game ini remote-nya
-- dari server ke killer, bukan ke survivor.
-- Pakai Heartbeat dedicated agar reaksi instan, bukan AutoLoop 0.1s.
-- ================================================================
local LastParryTime  = 0
local ParryAttackConn = nil

local function AutoParry_Fire()
    if tick() - LastParryTime < 0.4 then return end
    LastParryTime = tick()
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        ...
        local parryRE = dagger:FindFirstChild("parry")
        if parryRE then
            parryRE:FireServer()
        end
    end)
...
local function AutoParry_Setup()
    ...
    -- Heartbeat: cek jarak killer tiap frame, fire parry kalau dalam range serang
    ParryAttackConn = RunService.Heartbeat:Connect(function()
        if not Config.AUTO_Parry then
            ParryAttackConn:Disconnect()
            ParryAttackConn = nil
            return
        end
            ...
            if player ~= LocalPlayer and IsKiller(player) then
                ...
                if char then
                    ...
                    if kr then
                        ...
                        -- Range 12 studs = jarak killer bisa kena hit
                        if dist <= 12 then
                            AutoParry_Fire()
                            return
                        end
...
local function AutoParry()
    if Config.AUTO_Parry and GetRole() == "Survivor" then
        if not ParryAttackConn then
            AutoParry_Setup()
        end
    elseif not Config.AUTO_Parry and ParryAttackConn then
        pcall(function() ParryAttackConn:Disconnect() end)
        ParryAttackConn = nil
    end
end
...
-- SKILL CHECK SYSTEM
-- Dua mode:
--   AUTO_SkillCheck = true  → perfect tap otomatis saat needle masuk goal zone
-- Bekerja untuk Gen repair DAN heal (sama-sama pakai SkillCheckPromptGui)
-- =========================================================

-- SKILL CHECK SYSTEM
-- Exact logic dari referensi: cek Line.Rotation vs Goal.Rotation
-- Sweet spot: (Goal + 104) % 360  sampai  (Goal + 114) % 360
-- Hook kedua GUI: SkillCheckPromptGui (gen) dan SkillCheckPromptGui-con (heal)

local QTE_Connections = {}  -- { HeartbeatConn, VisConn } per GUI
...
local function QTE_StopMonitoring()
    for _, c in ipairs(QTE_Connections) do
        if c.HeartbeatConn then pcall(function() c.HeartbeatConn:Disconnect() end); c.HeartbeatConn = nil end
        if c.VisConn        then pcall(function() c.VisConn:Disconnect() end);        c.VisConn = nil end
    end
    QTE_Connections = {}
end
...
local function QTE_HookGui(guiName)
    task.spawn(function()
        pcall(function()
            ...
            local gui   = pg:WaitForChild(guiName, 20)
            local check = gui:WaitForChild("Check", 10)
            local line  = check:WaitForChild("Line", 5)
            local goal  = check:WaitForChild("Goal", 5)

            local c = { HeartbeatConn = nil, VisConn = nil }
            table.insert(QTE_Connections, c)

            local function onHeartbeat()
                if not Config.AUTO_SkillCheck then
                    if c.HeartbeatConn then c.HeartbeatConn:Disconnect(); c.HeartbeatConn = nil end
                    return
                end
            ...
            local function onVisChanged()
                ...
                if check.Visible then
                    if not c.HeartbeatConn then
                        c.HeartbeatConn = RunService.Heartbeat:Connect(onHeartbeat)
                    end
                else
                    if c.HeartbeatConn then c.HeartbeatConn:Disconnect(); c.HeartbeatConn = nil end
                end
            ...
            c.VisConn = check:GetPropertyChangedSignal("Visible"):Connect(onVisChanged)
...
local function UpdateNoSlowdown()
    ...
    -- Hanya intercept kalau game sengaja nurunin speed (slowdown effect)
    -- Tapi jangan hardcode ke 16 karena killer punya base speed beda (misal 18.9, 20, dll)
    -- Simpan peak speed sebagai referensi
    if not State.KillerBaseSpeed then
        State.KillerBaseSpeed = hum.WalkSpeed
    else
        -- Update peak kalau speed naik (ability boost)
        if hum.WalkSpeed > State.KillerBaseSpeed then
            State.KillerBaseSpeed = hum.WalkSpeed
        end
        -- Kalau tiba2 turun drastis (lebih dari 20%) → restore
...
secSurvival:AddToggle({
    ...
    Callback = function(v) Config.AUTO_SkillCheck = v; notifToggle("Skill Check", v) end
})
