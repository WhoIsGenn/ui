...
local Config = {
    ...
    SURV_NoFall = false,
    SURV_GodMode = false,  -- block semua damage
    ...
    SILENT_AIM = false,  -- Silent aim untuk Twist of Fate gun
    SILENT_AIM_Part = "Head",  -- target part: Head, Torso, HumanoidRootPart, Left Arm, Right Arm, Left Leg, Right Leg: Head, Torso, HumanoidRootPart, Left Arm, Right Arm, Left Leg, Right Leg
    SILENT_AIM_Target = "Killer",  -- target: Killer, Survivor, Closest
}
...
local ParryNamecallConn = nil  -- hook __namecall untuk detect killer FireServer sebelum server proses

local function AutoParry_HookNamecall()
    -- Hook __namecall: detect saat killer fire BasicAttack/Lunge ke server
    -- Ini fire di client kita SAAT killer menekan tombol attack (pre-server)
    -- Lebih cepat dari OnClientEvent karena tidak butuh server round-trip
    ...
    pcall(function()
        ...
        -- Cache remote references untuk compare di hook
        local basicAttack = attacks:FindFirstChild("BasicAttack")
        local lungeDetect = attacks:FindFirstChild("LungeDetect")
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            -- Detect saat siapapun (killer) fire BasicAttack atau Lunge
            if method == "FireServer" then
                if self == basicAttack or self == lunge or self == lungeDetect then
                    -- Cek apakah yang fire adalah killer (bukan kita)
                    -- Kita tidak bisa tahu siapa yang fire, tapi kita bisa cek
                    -- apakah ada killer dalam jarak parry
                    task.defer(function()
                        if not Config.AUTO_Parry then return end
...
local function AutoParry_Tick()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            -- Turunkan threshold: speed > 15 (dari 25) dan dot > 0.3 (dari 0.5)
            -- Lebih sensitif untuk lunge yang belum full speed
            local isLunging = vel.Magnitude > 15 and toMe.Magnitude > 0.01 and vel.Unit:Dot(toMe.Unit) > 0.3
...
local function AutoParry_Setup()
    ...
    -- Preemptive hook: detect killer FireServer sebelum server proses
    task.spawn(AutoParry_HookNamecall)
end
...
local function UpdateGodMode()
    local role = GetRole()
    if not Config.SURV_GodMode or (role ~= "Survivor" and role ~= "Killer") then
        if GodModeConn  then pcall(function() GodModeConn:Disconnect()  end); GodModeConn  = nil end
        if GodModeConn2 then pcall(function() GodModeConn2:Disconnect() end); GodModeConn2 = nil end
        return
    end
    if GodModeConn then return end  -- sudah aktif

    -- Method 1: Hook Health IntValue di workspace (game simpan HP disini)
    -- Dari dex: Workspace.PlayerName.Health [IntValue]
    local healthVal = workspace:FindFirstChild(LocalPlayer.Name)
    healthVal = healthVal and healthVal:FindFirstChild("Health")
    if healthVal and healthVal:IsA("IntValue") then
        local maxHp = healthVal.Value  -- simpan HP saat ini sebagai max
        if maxHp <= 0 then maxHp = 100 end
        GodModeConn = healthVal:GetPropertyChangedSignal("Value"):Connect(function()
            if not Config.SURV_GodMode then return end
            if healthVal.Value < maxHp then
                healthVal.Value = maxHp
            end
        end)
    end

    -- Method 2: Humanoid backup (kalau game juga baca dari sini)
    local char = LocalPlayer.Character
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then
        hum.MaxHealth = math.huge
        hum.Health = math.huge
        GodModeConn2 = hum:GetPropertyChangedSignal("Health"):Connect(function()
            if not Config.SURV_GodMode then return end
...
local function RehookForRole(role)
    ...
    task.delay(2, function()
        ...
        -- GodMode: re-apply ke char baru
        if Config.SURV_GodMode then
            task.spawn(UpdateGodMode)
        end
...
local function ThirdPerson_Apply()
    ...
    -- Disable scripts yang override camera/rotasi
    local lockScripts = {"Firstperson", "SmoothCamera", "CamShake", "Lookscript", "Lookscriptkiller"}
    if char then
        ...
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            -- Reset CameraOffset ke 0 — kita kontrol camera secara manual
            hum.CameraOffset = Vector3.new(0, 0, 0)
            hum.AutoRotate = true
        end
    end

    -- Set ke Scriptable agar kita bisa kontrol penuh CFrame camera
    ...
    -- RenderStepped: update camera CFrame tiap frame
    -- Ini yang bikin third person smooth dan sync dengan karakter
    ...
    ThirdPersonRenderConn = RunService.RenderStepped:Connect(function()
        ...
        if h then
            if h.CameraOffset ~= Vector3.new(0, 0, 0) then
                h.CameraOffset = Vector3.new(0, 0, 0)
            end
            if not h.AutoRotate then h.AutoRotate = true end
        end

        -- Hitung posisi kamera: di belakang + kanan + atas karakter
        -- Pakai arah camera horizontal untuk follow look direction
        local camDir   = ca.CFrame.LookVector
        local flatDir  = Vector3.new(camDir.X, 0, camDir.Z)
        if flatDir.Magnitude < 0.001 then flatDir = root.CFrame.LookVector end
        flatDir = flatDir.Unit

        local rightDir = Vector3.new(-flatDir.Z, 0, flatDir.X).Unit
        local headPos  = head.Position

        -- Offset: kanan 1.5, naik 1.5, mundur 5.5
        local camPos = headPos
            + rightDir * 1.5
            + Vector3.new(0, 1.5, 0)
            - flatDir * 5.5

        -- Raycast untuk obstacle avoidance (kamera tidak masuk tembok)
        local rayParams = RaycastParams.new()
        rayParams.FilterType = Enum.RaycastFilterType.Blacklist
        rayParams.FilterDescendantsInstances = {c, ca}
        local rayDir = camPos - headPos
        local hit = workspace:Raycast(headPos, rayDir, rayParams)
        if hit then
            -- Geser kamera ke depan obstacle
            camPos = hit.Position - rayDir.Unit * 0.3
        end

        -- Set CFrame camera: posisi baru, look ke kepala
        local lookTarget = headPos + Vector3.new(0, 0.5, 0)
        ca.CFrame = CFrame.new(camPos, lookTarget)
            * CFrame.Angles(math.rad(-5), 0, 0)  -- tilt sedikit ke bawah
    end)
    ...
    ThirdPersonCharConn = LocalPlayer.CharacterAdded:Connect(function(newChar)
        ...
        local ca = workspace.CurrentCamera
        if ca then ca.CameraType = Enum.CameraType.Scriptable end
    end)
...
local function ThirdPerson_Remove()
    ...
    local cam = workspace.CurrentCamera
    if cam then
        -- Restore ke Custom (bukan Scriptable) agar game camera kembali normal
...
-- Forward declare SilentAim_Setup (didefinisikan setelah AutoLoop)
local SilentAim_Setup
local secSilentAim = TabAIM:AddSection("Silent Aim")
secSilentAim:AddToggle({
    Title = "Silent Aim (Twist of Fate)",
    Default = Config.SILENT_AIM,
    Callback = function(v)
        Config.SILENT_AIM = v
        if v then task.spawn(SilentAim_Setup) end
        notifToggle("Silent Aim", v)
    end
})
secSilentAim:AddDropdown({
    Title = "Aim Target",
    Options = {"Killer", "Survivor", "Closest"},
    Default = Config.SILENT_AIM_Target,
    Callback = function(v)
        Config.SILENT_AIM_Target = v
    end
})
secSilentAim:AddDropdown({
    Title = "Aim Target Part",
    Options = {"Head", "Torso", "HumanoidRootPart", "Left Arm", "Right Arm", "Left Leg", "Right Leg"},
    Default = Config.SILENT_AIM_Part,
    Callback = function(v)
        Config.SILENT_AIM_Part = v
    end
})
...
-- Hook __namecall: replace direction vector saat Fire:FireServer
-- Fire remote: Remotes.Items["Twist of Fate"].Fire:FireServer(gun, direction)
-- ================================================================
local SilentAimConn    = nil
local SilentAimEnabled = false

local function SilentAim_GetTarget()
    -- Twist of Fate hanya ada di Survivor
    ...
    local closest, closestDist = nil, math.huge
    local targetMode = Config.SILENT_AIM_Target or "Killer"
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if targetMode == "Killer" then
            shouldTarget = IsKiller(p)
        elseif targetMode == "Survivor" then
            shouldTarget = not IsKiller(p)
        elseif targetMode == "Closest" then
            shouldTarget = true  -- siapapun yang terdekat
        end
...
SilentAim_Setup = function()
    ...
    pcall(function()
        ...
        if not fireRemote then
            warn("[SilentAim] Fire remote NOT FOUND — path: Remotes/Items/Twist of Fate/Fire")
            return
        end
        print("[SilentAim] Fire remote found:", fireRemote:GetFullName())

        -- Simpan FireServer original via rawget agar tidak loop
        local originalFireServer = fireRemote.FireServer

        -- Override FireServer langsung di remote object
        fireRemote.FireServer = function(self, gun, direction)
            if Config.SILENT_AIM and typeof(direction) == "Vector3" then
                local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                local target = SilentAim_GetTarget()
                if target and target.Character and myRoot then
                    local partName = Config.SILENT_AIM_Part or "Head"
                    local aimPart = target.Character:FindFirstChild(partName)
                                 or target.Character:FindFirstChild("HumanoidRootPart")
                    if aimPart then
                        local cam = workspace.CurrentCamera
                        local origin = cam and cam.CFrame.Position or myRoot.Position
                        direction = (aimPart.Position - origin).Unit
                        print("[SilentAim] Redirected to", target.Name, partName, "dir:", direction)
                    else
                        print("[SilentAim] Part not found:", partName, "in", target.Name)
                    end
                else
                    print("[SilentAim] No target found (role:", GetRole(), "mode:", Config.SILENT_AIM_Target, ")")
                end
            end
            return originalFireServer(self, gun, direction)
        end
...
local function SilentAim_Cleanup()
    -- Note: tidak bisa un-hook __namecall secara aman
    -- Cukup disable via Config.SILENT_AIM = false
...
local function Init()
    ...
    task.spawn(SilentAim_Setup)   -- hook silent aim
