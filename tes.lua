...
local function SpearSilent_Hook()
    ...
    if not remote then
        -- Coba lagi setelah delay (mungkin belum jadi Veil)
        task.delay(2, function()
            if not SpearHookActive then SpearSilent_Hook() end
        end)
        return
    end

    -- Method 1: hookfunction (paling aman, hanya hook 1 function)
    if hookfunction then
        local oldFire
        oldFire = hookfunction(remote.FireServer, newcclosure(function(self, ...)
            if Config_SpearSilent and self == remote then
                local args = {...}
                ...
                if target and target.Character then
                    ...
                    if pr then
                        ...
                        if dir then
                            -- Ganti semua Vector3 arg dengan direction (biasanya arg 1 atau 2)
                            local replaced = false
                            for i, v in ipairs(args) do
                                if typeof(v) == "Vector3" and not replaced then
                                    args[i] = dir
                                    replaced = true
                                end
                            end
                            if not replaced then
                                -- Insert direction sebagai arg pertama
                                table.insert(args, 1, dir)
                            end
                        end
                    end
                end
    ...
    -- Method 2: __namecall dengan getnamecallmethod
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if Config_SpearSilent and method == "FireServer" and self == remote then
            local args = {...}
            local target = SpearSilent_GetTarget()
            if target and target.Character then
                local pr = target.Character:FindFirstChild("HumanoidRootPart")
...
-- Snapline: dari HRP kita → target terdekat yang visible
-- Hanya muncul saat ada target dalam jarak 100 studs
local function UpdateSpearSnapline(cam, screenCenter)
    ...
    -- Cek jarak maksimal (jangan tampil kalau terlalu jauh)
    local myRoot = GetCharacterRoot()
    if not myRoot then SpearSnapLine.Visible = false; return end
    local pr = target.Character:FindFirstChild("HumanoidRootPart")
    if not pr then SpearSnapLine.Visible = false; return end
    if (pr.Position - myRoot.Position).Magnitude > 100 then
        SpearSnapLine.Visible = false; return
    end

    -- From: posisi HRP kita di screen (bukan tengah layar)
    ...
    -- To: aimPos target dengan kompensasi gravity
