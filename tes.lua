...
local function setupCharacterAnimator(character)
    local humanoid = character:WaitForChild("Humanoid")
    if not humanoid then return end
    
    playerAnimator = humanoid:FindFirstChildOfClass("Animator")
    if not playerAnimator then
        playerAnimator = Instance.new("Animator")
        playerAnimator.Parent = humanoid
    end
    
    -- Hook animator untuk block rod animations
    playerAnimator.AnimationPlayed:Connect(function(track)
...
if Players.LocalPlayer.Character then
    setupCharacterAnimator(Players.LocalPlayer.Character)
end

Players.LocalPlayer.CharacterAdded:Connect(function(char)
    currentTrack = nil
    task.wait(0.2)
    setupCharacterAnimator(char)
end)
...
local function playFishAnim(animName)
    ...
    local track = playerAnimator:LoadAnimation(animObj)
    ...
    -- Reset block setelah animation selesai
