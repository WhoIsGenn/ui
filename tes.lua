...
local function QTE_HookGui(guiName)
    task.spawn(function()
        ...
        if not line or not goal then return end  -- GUI tidak ditemukan
...
local SkillcheckGenScript = nil  -- cache
...
local function AntiFailGen_Apply(char)
    ...
    if scr then
        ...
        task.spawn(function()
            local s = char:WaitForChild("Skillcheck-gen", 10)
            if s and Config.ANTI_FailGen then
                pcall(function() s.Enabled = false end)
                SkillcheckGenScript = s
            end
        end)
    end
end
...
local function Init()
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        -- Reset KillerBaseSpeed agar tidak carry-over antar round
