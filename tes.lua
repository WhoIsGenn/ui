...
local Config = {
    ...
    PARRY_Dist = 13,
    PARRY_FOV  = false,
    PARRY_FaceSens = -1,  -- -1 = all direction, 1 = exact facing
...
-- Device detection: mobile = touch enabled tapi tidak ada keyboard
...
local function AutoParry_Fire()
    if Config.PARRY_Mode == "With Animation" then
        ...
        -- PC only: simulasi klik kanan (tidak di mobile karena bikin stuck)
...
local function AutoParry_TryFire()
    ...
    local now = tick()
    if now - LastParryFireTime < 0.1 then return end  -- dikurangi dari 0.5 → 0.3
    
    if AutoParry_Fire() then
        ParryOnCooldown = true
        LastParryFireTime = now
        task.delay(0.1, function() ParryOnCooldown = false end)  -- 1.2 → 1.0
    end
end
...
local function AutoParry_HookKillerChar(char, player)
    ...
    task.spawn(function()
        ...
        -- Face check: apakah killer menghadap ke kita
        -- PARRY_FaceSens: -1 = all direction, 1 = exact facing
        local function IsKillerFacingMe(myPos)
            local head = char:FindFirstChild("Head")
                or char:FindFirstChild("HumanoidRootPart")
            if not head then return true end
            local toMe = (myPos - head.Position).Unit
            local dot  = head.CFrame.LookVector:Dot(toMe)
            return dot > Config.PARRY_FaceSens
        end

        -- METHOD 1: AnimationPlayed Event — paling cepat, fire langsung
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if IsKillerFacingMe(myRoot.Position) then
                AutoParry_TryFire()
            end
        end)
        table.insert(KillerHitConns, conn1)

        -- METHOD 2: Heartbeat — backup + pre-fire jarak dekat
        ...
        local conn2 = RunService.Heartbeat:Connect(function()
            ...
            if not IsKillerFacingMe(myRoot.Position) then return end

            for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
                if track.IsPlaying then
                    local numId = track.Animation.AnimationId:match("%d+$") or ""
                    if AttackAnimIDs[numId] and numId ~= lastFiredTrack then
                        -- Jarak sangat dekat (<=6): fire tanpa tunggu TimePosition
                        -- Jarak normal: hanya fire kalau anim baru mulai (<0.15s)
                        local shouldFire = dist <= 8 or track.TimePosition < 0.1
                        if shouldFire then
                            lastFiredTrack = numId
                            task.delay(0.1, function() lastFiredTrack = "" end)
                            AutoParry_TryFire()
                        end
                        return
                    end
...
local KillerAnimsV2 = {
    ["rbxassetid://17838788137"] = true,
    ["rbxassetid://17838777318"] = true,
    ["rbxassetid://17838777992"] = true,
    ["rbxassetid://95636719657684"] = true,
}

local hookedKillersV2 = {}
local PARRY_DEBOUNCE_V2 = 0.5
local lastParryV2 = 0
local ParryActiveV2 = false
local VeilHookState = { Hook = false, HookOld = nil, Remote = nil }


-- ================================================================
-- AUTO PARRY V2 FUNCTIONS
-- ================================================================
local function GetParryButtonV2()
...
local function pressParryButtonV2()
    if UserInputService.TouchEnabled then
        local btn = GetParryButtonV2()
        if btn and btn:IsA("GuiObject") then
            ...
            VirtualInputManager:SendTouchEvent(9933, 0, x, y)
            task.wait(0.01)
            VirtualInputManager:SendTouchEvent(9933, 2, x, y)
        end
    else
        VirtualInputManager:SendMouseButtonEvent(0, 0, 1, true, game, 0)
        task.wait(0.01)
...
local function doParryV2()
    local now = tick()
    if now - lastParryV2 < PARRY_DEBOUNCE_V2 then return end
    lastParryV2 = now
    ParryActiveV2 = true
    task.spawn(pressParryButtonV2)
    task.delay(0.3, function() ParryActiveV2 = false end)
end

local function isInParryRangeV2(killerChar)
    ...
    return (enemyRoot.Position - myRoot.Position).Magnitude <= AutoV2.ParryDistance
end

local function isFacingTargetV2(targetChar)
    if not AutoV2.RequireFacing then return true end
    local myRoot    = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    local enemyRoot = targetChar and targetChar:FindFirstChild("HumanoidRootPart")
    ...
    return dot >= AutoV2.FaceSensitivity
end

local function hookKillerV2(char)
    if hookedKillersV2[char] then return end
    hookedKillersV2[char] = true
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    local animator = hum:FindFirstChildOfClass("Animator")
    if not animator then return end
    animator.AnimationPlayed:Connect(function(track)
        if not AutoV2.Parry then return end
        local anim = track.Animation
        if not anim then return end
        local id = anim.AnimationId:match("%d+")
        if not id then return end
        if KillerAnimsV2["rbxassetid://" .. id] then
            if not isInParryRangeV2(char) then return end
            if not isFacingTargetV2(char) then return end
            doParryV2()
        end
    end)
end
...
-- AUTO SKILL CHECK V2
-- ================================================================
local SkillCheckV2Conn = nil
local function startSkillCheckV2()
    if SkillCheckV2Conn then SkillCheckV2Conn:Disconnect() end
    SkillCheckV2Conn = RunService.RenderStepped:Connect(function()
        if not AutoV2.SkillCheck then return end
        ...
        if success then
            if UserInputService.TouchEnabled then
                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                local btn = pg and pg:FindFirstChild("SkillCheckPromptGui")
                    and pg.SkillCheckPromptGui:FindFirstChild("Check")
                    and pg.SkillCheckPromptGui.Check:FindFirstChild("PressButton")
                if btn then
                    local pos  = btn.AbsolutePosition
                    local size = btn.AbsoluteSize
                    local inset = game:GetService("GuiService"):GetGuiInset()
                    local x = pos.X + size.X/2 + inset.X
                    local y = pos.Y + size.Y/2 + inset.Y
                    VirtualInputManager:SendTouchEvent(7755, 0, x, y)
                    task.wait(0.02)
                    VirtualInputManager:SendTouchEvent(7755, 2, x, y)
                end
            else
                VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.Space, false, game)
                task.wait(0.02)
...
local function stopSkillCheckV2()
    if SkillCheckV2Conn then SkillCheckV2Conn:Disconnect(); SkillCheckV2Conn = nil end
end

-- ================================================================
-- SILENT AIM VEIL + TWIST OF FATE
-- ================================================================
local function GetVeilRemote()
    if VeilHookState.Remote and VeilHookState.Remote.Parent then return VeilHookState.Remote end
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    VeilHookState.Remote = fireRemote
    return fireRemote
end
...
local function HookVeilRemote()
    ...
    if not remote then warn("[SilentAimVeil] Remote tidak ditemukan"); return false end
    local mt = getrawmetatable(game)
    ...
    setreadonly(mt, true)
    VeilHookState.HookOld = oldNamecall
    return true
end

local function StartSilentAimVeil()
    if VeilHookState.Hook then return end
    local success = HookVeilRemote()
    VeilHookState.Hook = success
end

local function StopSilentAimVeil()
    if not VeilHookState.Hook then return end
    local mt = getrawmetatable(game)
    if mt and VeilHookState.HookOld then
        setreadonly(mt, false)
        mt.__namecall = VeilHookState.HookOld
        setreadonly(mt, true)
    end
    VeilHookState.Hook = false
    VeilHookState.HookOld = nil
end
...
local function HookTwistOfFate()
    ...
    if not ok or not remote then warn("[TwistOfFate] Remote tidak ditemukan"); return end
    local mt = getrawmetatable(game)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "FireServer" and self == remote then
            local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            local target = GetVeilTarget()
            if myHRP and target and SilentAimVeil.Enabled then
                local args = {...}
                local dir = (target.Position - myHRP.Position).Unit
                if SilentAimVeil.Predict then
                    local vel = target.AssemblyLinearVelocity
                    dir = (target.Position + (vel * SilentAimVeil.PredictStrength) - myHRP.Position).Unit
                end
                args[2] = dir
...
local Window = VelarisUI:Window({
    ...
    Footer = "Gennskie Development", -- Footer text shown at the bottom
    Content = "Violence District",
    Color = "Blue", -- UI theme color (Default or custom theme)
    Version = 1.0,
...
secParry:AddSlider({
    ...
    Default  = -1,
    Increment = 0.01,
    Callback = function(v) Config.PARRY_FaceSens = v end
})
...
-- ================================================================
-- AUTO PARRY V2 SECTION
-- ================================================================
local secParryV2 = Tabs.SURVIVOR:AddSection({ Title = "Auto Parry V2", Open = false })

secParryV2:AddToggle({
    Title = "Auto Parry V2",
    Default = false,
    Callback = function(v)
        AutoV2.Parry = v
...
secParryV2:AddSlider({
    Title = "Parry Distance V2",
    Min = 5, Max = 40, Default = 15,
    Callback = function(v) AutoV2.ParryDistance = v end
})

secParryV2:AddSlider({
    Title = "Face Sensitivity V2",
    Min = -1, Max = 1, Default = 0,
    Increment = 0.01,
    Callback = function(v) AutoV2.FaceSensitivity = v end
})

secParryV2:AddToggle({
    Title = "Require Facing V2",
    Default = false,
    Callback = function(v) AutoV2.RequireFacing = v end
})

secParryV2:AddToggle({
    Title = "Show Parry Range V2",
    Default = false,
...
secParryV2:AddToggle({
    Title = "Fake Parry",
    Default = false,
    Callback = function(v) FakeParry.Enabled = v end
})

secParryV2:AddDropdown({
    Title = "Fake Parry Animation",
    Options = { "Enten", "Stopwatch" },
    Default = "Enten",
    Callback = function(v) FakeParry.Animation = v end
})

-- ================================================================
-- AUTO SKILL CHECK V2 SECTION
-- ================================================================
local secSCV2 = Tabs.SURVIVOR:AddSection({ Title = "Auto Skill Check V2", Open = false })

secSCV2:AddToggle({
    Title = "Auto Skill Check V2",
    Default = false,
    Callback = function(v)
        AutoV2.SkillCheck = v
        if v then startSkillCheckV2() else stopSkillCheckV2() end
    end
})
...
-- ================================================================
-- SILENT AIM VEIL SECTION
-- ================================================================
local secVeil = Tabs.KILLER:AddSection({ Title = "Silent Aim Veil", Open = false })
...
secVeil:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then StartSilentAimVeil() else StopSilentAimVeil() end
    end
})

secVeil:AddDropdown({
    Title = "Veil Target Mode",
    Options = { "Survivor", "Killer" },
    Default = "Survivor",
    Callback = function(v) SilentAimVeil.TargetMode = v end
})

secVeil:AddSlider({
    Title = "Veil FOV",
    Min = 10, Max = 500, Default = 200,
    Callback = function(v) SilentAimVeil.FOV = v end
})

secVeil:AddToggle({
    Title = "Veil Predict",
    Default = false,
    Callback = function(v) SilentAimVeil.Predict = v end
})

secVeil:AddSlider({
    Title = "Veil Predict Strength",
    Min = 0, Max = 2, Default = 0.5,
    Increment = 0.01,
    Callback = function(v) SilentAimVeil.PredictStrength = v end
})

-- ================================================================
-- TWIST OF FATE FULL SECTION
-- ================================================================
local secTwist = Tabs.KILLER:AddSection({ Title = "Twist of Fate Full", Open = false })

secTwist:AddToggle({
    Title = "Hook Twist of Fate Full",
    Default = false,
    Callback = function(v)
        if v then
            task.spawn(HookTwistOfFate)
        else
            -- reuse StopSilentAimVeil unhook logic
            local mt = getrawmetatable(game)
            if mt and VeilHookState.HookOld then
                setreadonly(mt, false)
                mt.__namecall = VeilHookState.HookOld
