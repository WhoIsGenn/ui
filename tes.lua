...
L(S.DungJoin, "Auto join & farm dungeon.\nTP ke island dungeon otomatis.")
D(S.DungJoin, "SelectedDungeon", { Text = "Select Dungeon", Values = {"CidDungeon","RuneDungeon","DoubleDungeon"}, Multi = false })
D(S.DungJoin, "SelectedDiff",    { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Easy", Multi = false })
T(S.DungJoin, "AutoDiff",        { Text = "Auto Select Difficulty", Default = false })
T(S.DungJoin, "AutoVoteStart",   { Text = "Auto Vote/Start", Default = false })
T(S.DungJoin, "AutoReplay",      { Text = "Auto Replay",     Default = false })
T(S.DungJoin, "DungeonAutofarm", { Text = "Autofarm Dungeon", Default = false })
T(S.DungJoin, "AutoDungeon",     { Text = "Auto Join Dungeon", Default = false })
DungeonCount = L(S.DungJoin, "Dungeon Completed: N/A")

-- Boss Rush (Sea 1 - Sailor Island, ada difficulty)
L(S.DungBR, "Sea 1 - Sailor Island.\nAda pilihan difficulty.")
D(S.DungBR, "SelectedBossRushDiff", { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Easy", Multi = false })
T(S.DungBR, "AutoVoteStart_BR",   { Text = "Auto Vote/Start", Default = false })
T(S.DungBR, "AutoReplay_BR",      { Text = "Auto Replay",     Default = false })
T(S.DungBR, "DungeonAutofarm_BR", { Text = "Autofarm",        Default = false })
T(S.DungBR, "AutoBossRush",       { Text = "Auto Boss Rush",  Default = false })

-- Infinite Tower (Sea 1 - Tower Island, tidak ada difficulty)
L(S.DungIT, "Sea 1 - Tower Island.\nTidak ada difficulty.")
T(S.DungIT, "AutoVoteStart_IT",   { Text = "Auto Vote/Start", Default = false })
T(S.DungIT, "AutoReplay_IT",      { Text = "Auto Replay",     Default = false })
T(S.DungIT, "DungeonAutofarm_IT", { Text = "Autofarm",        Default = false })
T(S.DungIT, "AutoInfiniteTower",  { Text = "Auto Infinite Tower", Default = false })

-- Crystal Defense (Sea 2 - tidak perlu TP, tidak ada difficulty)
L(S.DungCD, "Sea 2 - Tidak perlu TP.\nTidak ada difficulty.")
T(S.DungCD, "AutoVoteStart_CD",    { Text = "Auto Vote/Start",  Default = false })
T(S.DungCD, "AutoReplay_CD",       { Text = "Auto Replay",      Default = false })
T(S.DungCD, "DungeonAutofarm_CD",  { Text = "Autofarm",         Default = false })
T(S.DungCD, "AutoCrystalDefense",  { Text = "Auto Crystal Defense", Default = false })

-- Raid (Sea 2 - tidak perlu TP, ada difficulty)
L(S.DungRaid, "Sea 2 - Tidak perlu TP.\nAda pilihan difficulty.")
D(S.DungRaid, "SelectedRaidDiff",    { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Easy", Multi = false })
T(S.DungRaid, "AutoVoteStart_Raid",  { Text = "Auto Vote/Start", Default = false })
T(S.DungRaid, "AutoReplay_Raid",     { Text = "Auto Replay",     Default = false })
T(S.DungRaid, "DungeonAutofarm_Raid",{ Text = "Autofarm",        Default = false })
T(S.DungRaid, "AutoRaid",            { Text = "Auto Raid",       Default = false })
...
-- ============================================================
local function DungeonFarmLoop(toggleName)
    while Toggles[toggleName] and Toggles[toggleName].Value do
        task.wait(0.05)
        if not Toggles.DungeonAutofarm.Value then task.wait(0.3); continue end

        local char = GetCharacter()
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not char or not root then task.wait(0.3); continue end

        local npcFolder = workspace:FindFirstChild("NPCs")
        if not npcFolder then task.wait(0.5); continue end
        ...
        if best then
            ...
            if tRoot then
                ...
                local finalPos = fType == "Above" and tPos + Vector3.new(0,dist,0)
                    or fType == "Below" and tPos + Vector3.new(0,-dist,0)
                    or tPos + (root.Position - tPos).Unit * dist

                if (root.Position - finalPos).Magnitude > 3 then
                    ...
                    if Options.SelectedMovementType and Options.SelectedMovementType.Value == "Teleport" then
                        ...
                        local dur = (root.Position - finalPos).Magnitude / speed
                        game:GetService("TweenService"):Create(root, TweenInfo.new(dur, Enum.EasingStyle.Linear), {CFrame = dest}):Play()
                        task.wait(dur)
                    end
                end
                root.AssemblyLinearVelocity = Vector3.zero
                ...
                if tick() - (Shared.LastM1 or 0) >= (tonumber(Options.M1Speed and Options.M1Speed.Value) or 0.1) then
...
-- Helper: Vote/Start
local function TryVoteStart()
    pcall(function()
        -- Dungeon vote
        local dungUI = PGui:FindFirstChild("DungeonUI")
        if dungUI then
            local voted = dungUI:FindFirstChild("ContentFrame", true)
            voted = voted and voted:FindFirstChild("PeopleVoted", true)
            voted = voted and voted:FindFirstChild("ButtonFrame", true)
            voted = voted and voted:FindFirstChild("Voted")
            if voted and voted.Visible then pcall(function() fire_event(voted.MouseButton1Click) end) end
        end
        -- BossRush / Raid / Tower / CrystalDefense START button
        for _, uiName in ipairs({"BossRushUI","RaidUI","InfiniteTowerUI","CrystalDefenseUI"}) do
            local ui = PGui:FindFirstChild(uiName)
            if ui then
                local start = ui:FindFirstChild("START", true) or ui:FindFirstChildWhichIsA("TextButton", true)
                if start and (start.Text == "START" or start.Name == "START") then
                    pcall(function() fire_event(start.MouseButton1Click) end)
                end
            end
        end
    end)
...
local function TryAutoReplay()
    pcall(function()
        local dungUI = PGui:FindFirstChild("DungeonUI")
        if dungUI then
            local replayFrame = dungUI:FindFirstChild("ReplayDungeonFrameVisibleOnlyWhenClearingDungeon", true)
            if replayFrame and replayFrame.Visible then
                local playersNeeded = replayFrame:FindFirstChild("PlayersNeededToVoteInOrderToReplayTheDungeon", true)
                if playersNeeded then
                    local voteBtn = playersNeeded:FindFirstChildWhichIsA("TextButton", true)
                    if voteBtn then pcall(function() fire_event(voteBtn.MouseButton1Click) end) end
                end
            end
        end
    end)
...
-- ============================================================
-- AUTO BOSS RUSH (Sea 1 - Sailor Island)
-- ============================================================
local function Func_AutoBossRush()
    while Toggles.AutoBossRush.Value do
        task.wait(0.5)
        if Toggles.AutoVoteStart_BR.Value then TryVoteStart() end
        if Toggles.AutoReplay_BR.Value then TryAutoReplay() end
        ...
        if inDungeon then
            if Toggles.DungeonAutofarm_BR.Value then DungeonFarmLoop("AutoBossRush") end
        else
            if Shared.Island ~= "Sailor" then
                Remotes.TP_Portal:FireServer("Sailor")
                Shared.Island = "Sailor"
                task.wait(2.5)
            end
            local diff = Options.SelectedBossRushDiff and Options.SelectedBossRushDiff.Value
            if diff then pcall(function() Remotes.SetDungeonDifficulty:FireServer("BossRush", diff) end) end
            if tick() - (Shared.LastBossRush or 0) > 10 then
                Remotes.OpenDungeon:FireServer("BossRush")
                Shared.LastBossRush = tick()
                task.wait(1)
            end
            local portal = workspace:FindFirstChild("ActiveDungeonPortal")
            if portal then
                ...
                if root then
                    ...
                    local prompt = portal:FindFirstChild("JoinPrompt")
                    if prompt then pcall(function() fireproximityprompt(prompt) end); task.wait(1) end
                end
            end
            local ui = PGui:FindFirstChild("BossRushUI")
            if ui and ui:FindFirstChild("MainFrame") and ui.MainFrame.Visible then
                local spawnBtn = ui:FindFirstChild("SpawnBossRushPortalFrame", true)
                spawnBtn = spawnBtn and spawnBtn:FindFirstChild("SpawnPortalButton")
                if spawnBtn then pcall(function() fire_event(spawnBtn.MouseButton1Click) end); task.wait(1) end
            end
        end
    end
end
...
-- ============================================================
-- AUTO INFINITE TOWER (Sea 1 - Tower Island)
-- ============================================================
local function Func_AutoInfiniteTower()
    while Toggles.AutoInfiniteTower.Value do
        task.wait(0.5)
        if Toggles.AutoVoteStart_IT.Value then TryVoteStart() end
        if Toggles.AutoReplay_IT.Value then TryAutoReplay() end

        local ui = PGui:FindFirstChild("InfiniteTowerUI")
        local mainFrame = ui and ui:FindFirstChild("MainFrame")

        if mainFrame and mainFrame.Visible then
            local spawnBtn = ui:FindFirstChild("SpawnInfiniteTowerPortalFrame", true)
            spawnBtn = spawnBtn and spawnBtn:FindFirstChild("SpawnPortalButton")
            if spawnBtn then pcall(function() fire_event(spawnBtn.MouseButton1Click) end); task.wait(1) end
        else
            if Shared.Island ~= "TowerIsland" then
                Remotes.TP_Portal:FireServer("TowerIsland")
                Shared.Island = "TowerIsland"
                task.wait(2.5)
            end
            local npc = NPCs and NPCs.Merchant and NPCs.Merchant.InfiniteTower
            if npc then
                local root = GetCharacter() and GetCharacter():FindFirstChild("HumanoidRootPart")
                local npcRoot = npc:FindFirstChild("HumanoidRootPart")
                if root and npcRoot then
                    root.CFrame = npcRoot.CFrame * CFrame.new(0, 0, 3); task.wait(0.2)
                    local prompt = npcRoot:FindFirstChild("ProximityPrompt")
                    if prompt then
                        if Support.Proximity then pcall(function() fireproximityprompt(prompt) end)
        ...
        if Toggles.DungeonAutofarm_IT.Value then
            local npcFolder = workspace:FindFirstChild("NPCs")
            if npcFolder and #npcFolder:GetChildren() > 0 then DungeonFarmLoop("AutoInfiniteTower") end
        end
    end
end

-- ============================================================
-- AUTO CRYSTAL DEFENSE (Sea 2 - tidak perlu TP)
-- ============================================================
local function Func_AutoCrystalDefense()
    while Toggles.AutoCrystalDefense.Value do
        task.wait(0.5)

        if Toggles.AutoVoteStart.Value then TryVoteStart() end
        if Toggles.AutoReplay.Value then TryAutoReplay() end

        local ui = PGui:FindFirstChild("CrystalDefenseUI")
        local mainFrame = ui and ui:FindFirstChild("MainFrame")

        if mainFrame and mainFrame.Visible then
            local spawnBtn = ui:FindFirstChild("SpawnCrystalDefensePortalFrame", true)
            spawnBtn = spawnBtn and spawnBtn:FindFirstChild("SpawnPortalButton")
            if spawnBtn then pcall(function() fire_event(spawnBtn.MouseButton1Click) end); task.wait(1) end
        end
        ...
        if Toggles.DungeonAutofarm.Value then
            ...
            if npcFolder and #npcFolder:GetChildren() > 0 then
                DungeonFarmLoop("AutoCrystalDefense")
            end
        end
    end
end
...
-- ============================================================
-- AUTO RAID (Sea 2 - tidak perlu TP)
-- ============================================================
local function Func_AutoRaid()
    while Toggles.AutoRaid.Value do
        ...
        local ui = PGui:FindFirstChild("RaidUI")
        local mainFrame = ui and ui:FindFirstChild("MainFrame")

        if mainFrame and mainFrame.Visible then
            -- Pilih difficulty
            local diff = Options.SelectedRaidDiff and Options.SelectedRaidDiff.Value
            if diff then
                pcall(function() Remotes.SetDungeonDifficulty:FireServer("Raid", diff) end)
            end
            local spawnBtn = ui:FindFirstChild("SpawnRaidPortalFrame", true)
            spawnBtn = spawnBtn and spawnBtn:FindFirstChild("SpawnPortalButton")
            if spawnBtn then pcall(function() fire_event(spawnBtn.MouseButton1Click) end); task.wait(1) end
        end
...
-- ============================================================
-- AUTO VOTE START (standalone - untuk semua dungeon)
-- ============================================================
...
Toggles.AutoBossRush:OnChanged(function(state)       Thread("AutoBossRush",      Func_AutoBossRush,      state) end)
Toggles.AutoCrystalDefense:OnChanged(function(state) Thread("AutoCrystalDefense",Func_AutoCrystalDefense,state) end)
Toggles.AutoRaid:OnChanged(function(state)           Thread("AutoRaid",          Func_AutoRaid,          state) end)
Toggles.AutoVoteStart:OnChanged(function(state)      Thread("AutoVoteStart",     Func_AutoVoteStart,     state) end)
