...
local Config = {
    ...
    PARRY_Dist = 13,
    PARRY_FOV = false,
    Config.PARRY_AimStrict = true,      -- Enable aim strictness
    Config.PARRY_AimStrictness = 0.5,   -- Threshold (0-1, makin tinggi makin strict)
...
-- ================================================================
-- AIM STRICTNESS untuk Auto Parry
-- Cek arah hadap killer sebelum parry
-- ================================================================
local function IsKillerFacingMe(killerChar)
    ...
    if killerHead then
        -- Lebih akurat pake head kalo ada
    ...
    -- Strictness: dot > 0.5 berarti killer menghadap ke arah kita (sekitar 60 derajat)
    local threshold = 0.5  -- bisa disesuaikan
    
    if Config.PARRY_Debug then
        print(string.format("[Parry] Dot: %.2f | Facing: %s", dot, dot > threshold and "YES" or "NO"))
    end
...
-- Modifikasi AutoParry_HookKillerChar dengan Aim Strictness
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if not IsKillerFacingMe(char) then
                if Config.PARRY_Debug then print("[Parry] Killer not facing me, skipping") end
                return
            end
...
local function SkillCheck_Fire()
    ...
    -- Method 2: SendKeyEvent E + Space (PC)
    pcall(function()
...
secParry:AddToggle({
    ...
    Default = Config.PARRY_AimStrict,
    Callback = function(state)
        Config.PARRY_AimStrict = state
        notifToggle("Aim Strictness", state)
    end
})
...
secParry:AddSlider({
    Title = "Strictness",
    Desc = "0 = longgar, 1 = harus tepat ngadep",
    Min = 0.3,
    Max = 0.9,
