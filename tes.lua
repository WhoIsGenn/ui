...
local SkillCheckWatchConn = nil  -- watch PlayerGui untuk GUI recreate
...
local function SetupSkillCheckMonitor()
    pcall(function()
        ...
        local function tryConnect()
            ...
            if not ui or not ui.frame or not ui.needle or not ui.target then return false end
        ...
        if not tryConnect() then
            -- GUI belum ada, poll sampai muncul
            task.spawn(function()
                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                ...
                -- Juga poll setiap 1 detik selama 60 detik
                local waited = 0
                while waited < 60 and Config.AUTO_SkillCheck do
                    task.wait(1)
                    waited = waited + 1
...
local function PerfectSC_ConnectGui()
    ...
    task.spawn(function()
        pcall(function()
            ...
            -- Poll sampai GUI muncul (max 30 detik)
            local CheckGui, waited = nil, 0
            repeat
                task.wait(0.5); waited = waited + 0.5
                CheckGui = PlayerGui:FindFirstChild("SkillCheckPromptGui")
            until CheckGui or waited >= 30 or not PerfectSCActive
            if not CheckGui or not PerfectSCActive then return end

            local Check = CheckGui:FindFirstChild("Check") or CheckGui:WaitForChild("Check", 10)
                        ...
                        if inGoal then
                            pcall(function()
                                ...
                                if checkObj then
                                    ...
                                    task.wait()
                                    checkObj:SetAttribute("action", false)
                                else
                                    local pg = LocalPlayer:FindFirstChild("PlayerGui")
                                    if pg then
                                        for _, gui in ipairs(pg:GetChildren()) do
                                            ...
                                            if controls then
                                                local btn = controls:FindFirstChild("action")
                                                if btn then
...
local function ThirdPerson_Apply()
    ...
    -- Saat respawn dalam match, re-apply offset SEKALI (bukan tiap frame)
