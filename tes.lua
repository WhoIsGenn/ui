...
local function GodMode_Start()
    ...
    -- Tunggu HP terisi dulu (bisa 0 saat baru join)
    task.spawn(function()
        local healthVal = nil
        local waited = 0
        repeat
            ...
            healthVal = plrFolder and plrFolder:FindFirstChild("Health")
        until (healthVal and healthVal.Value > 0) or waited >= 10
        if not healthVal or not Config.SURV_GodMode then return end
        local maxHp = healthVal.Value
