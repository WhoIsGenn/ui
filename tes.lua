...
local function buildMonitor()
    ...
    ft.BackgroundTransparency=1 ft.Text="v39.0  •  Auto SK"
...
local function buildManual()
    ...
    hdr.Active=true  -- perlu untuk InputBegan fire, tidak block children
    ...
    hdr.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then
            _d=true _ds=i.Position _sp=panel.Position
        end
    end)
    ...
    hdr.InputChanged:Connect(function(i)
        if _d and i.UserInputType==Enum.UserInputType.MouseMovement then
            local v=i.Position-_ds
            panel.Position=UDim2.new(_sp.X.Scale,_sp.X.Offset+v.X,_sp.Y.Scale,_sp.Y.Offset+v.Y)
        end
    end)
...
print("[SK] v39.0 | No Heartbeat scan | SK Manual panel")
WindUI:Notify({Title="Auto SK v39.0",Content="Loading KBBI...",Icon="zap",Duration=3})
