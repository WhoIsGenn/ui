...
local function MainLoop()
    ...
    -- FOV Circle
    -- FOV Circle: lazy create + update tiap frame
    if Config.AIM_ShowFOV then
        if not GUI.Drawings.FOVCircle then
            pcall(function()
                local c = Drawing.new("Circle")
                c.Thickness = 2
                c.NumSides  = 64
                c.Filled    = false
                GUI.Drawings.FOVCircle = c
            end)
        end
        if GUI.Drawings.FOVCircle then
            ...
            GUI.Drawings.FOVCircle.Transparency = 1
...
local function Init()
    ...
    -- FOV Circle dibuat lazy di MainLoop
