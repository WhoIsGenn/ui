...
local Config = {
    ...
    SURV_NoFall = false,
    SURV_InstantHeal = false,
    SURV_InstantHealOther = false,
    SURV_GodMode = false,
    PARRY_FOV = false,
    PARRY_FOV_Size = 200,
    SURV_GodMode = false,
    ...
    AIM_Enabled = false,
    AIM_AutoMode = false,
    AIM_TargetMode = "Auto", -- Auto, Killer, Survivor
    AIM_FOV = 120,
    AIM_Smooth = 0.3,
    AIM_TargetPart = "Head",
    AIM_VisCheck = true,
...
function Aimbot.GetClosestTarget(cam, screenCenter)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Team then
            ...
            local mode = Config.AIM_TargetMode or "Auto"
            if mode == "Auto" then
            ...
            elseif mode == "Killer" then
                shouldTarget = IsKiller(player)
            elseif mode == "Survivor" then
                shouldTarget = not IsKiller(player) and player ~= LocalPlayer
            end
...
function Aimbot.Update(cam, screenSize, screenCenter)
    ...
    -- Cek apakah aimbot harus aktif
    local shouldAim = false
    if Config.AIM_AutoMode then
        -- Mobile: aktif saat character Aiming (hold tombol gun)
        local char = LocalPlayer.Character
        shouldAim = char ~= nil and char:GetAttribute("Aiming") == true
    else
        -- Always on saat enabled
        shouldAim = true
    end
...
local function AutoParry_TryFire()
    ...
    -- Debounce: tidak fire lebih dari sekali per 0.1s
    local now = tick()
    if now - LastParryFireTime < 0.1 then return end
...
local function AutoParry_HookKillerChar(char, p)
    ...
    task.spawn(function()
        ...
        -- Method 1: AnimationPlayed (standard)
        ...
        -- Method 2: hook LoadAnimation via __namecall - fire SEBELUM AnimationPlayed
        -- Ini yang paling early - detect saat game baru mau load anim attack
        if getrawmetatable and setreadonly and newcclosure then
            pcall(function()
                ...
                setreadonly(mt, false)
                local hooked = mt.__namecall
                -- Cek apakah sudah di-hook oleh parry namecall sebelumnya
                -- Kalau ya, tambah check di dalam hook yang ada via flag
                -- Kalau tidak, buat hook baru khusus LoadAnimation
                setreadonly(mt, true)
            end)
        end

        -- Method 3: watch AnimationTrack speed di semua track yang sedang playing
        -- Kalau ada track attack yang mulai play, fire langsung
        local trackWatchConn
        trackWatchConn = RunService.Heartbeat:Connect(function()
            if not Config.AUTO_Parry then return end
            if not IsKiller(p) then return end
            if ParryOnCooldown then return end
            local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            ...
            -- Cek semua playing tracks di animator
            for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
                local id = track.Animation.AnimationId
                local numId = id:match("%d+$") or id:match("id=(%d+)") or ""
                if AttackAnimIDs[numId] and track.IsPlaying then
                    AutoParry_TryFire()
                    return
                end
            end
        end)
        table.insert(KillerHitConns, trackWatchConn)
    end)
end
...
local function AutoParry_Tick()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            -- Lunge detection: speed > 15, mengarah ke kita
...
local GodModeFF = nil  -- ForceField instance
...
local function UpdateGodMode()
    ...
    if Config.SURV_GodMode then
        -- Pakai ForceField - block semua damage dari server
        if not GodModeFF or not GodModeFF.Parent then
            GodModeFF = Instance.new("ForceField")
            GodModeFF.Visible = false  -- tidak terlihat
            GodModeFF.Parent = char
        end
    else
        if GodModeFF then
            pcall(function() GodModeFF:Destroy() end)
            GodModeFF = nil
        end
    end
end
...
local function InstantHeal_Setup()
    if HealConn then return end
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        local healing = remotes and remotes:FindFirstChild("Healing")
        if not healing then return end
        local healEvent = healing:FindFirstChild("HealEvent")
        if not healEvent then return end
        -- Listen HealEvent dari server (saat kita mulai heal)
        HealConn = healEvent.OnClientEvent:Connect(function()
            if not Config.SURV_InstantHeal then return end
            if GetRole() ~= "Survivor" then return end
            -- Fire HealAnimRec untuk instant complete heal
            local healAnimRec = healing:FindFirstChild("HealAnimRec")
            if healAnimRec then
                pcall(function() healAnimRec:FireServer() end)
            end
        end)
    end)
end

local function InstantHealOther_Setup()
    -- Hook HealEvent dan auto-complete untuk healing other juga
    -- Dipanggil bersamaan dengan InstantHeal_Setup
end

local function Heal_Cleanup()
    if HealConn then pcall(function() HealConn:Disconnect() end); HealConn = nil end
end

-- ================================================================
-- GOD MODE - disable damage via Health IntValue
-- ================================================================
...
local function SuspendAllFeatures()
    ...
    if GodModeFF then pcall(function() GodModeFF:Destroy() end); GodModeFF = nil end
    pcall(Heal_Cleanup)
    pcall(GodMode_Stop)
...
local function RoleManager_Start()
    ...
    RoleCharConn = LocalPlayer.CharacterAdded:Connect(function(char)
        GodModeFF = nil  -- reset FF reference saat respawn
        local role = GetRole()
        if role == "Spectator" then return end
        RehookForRole(role)
    end)
...
local function ThirdPerson_Remove()
    ...
    -- Reset list tanpa re-enable (re-enable bikin lobby UI muncul)
    ThirdPersonDisabledScripts = {}
...
secAimbot:AddToggle({
    ...
    Callback = function(state)
        Config.AIM_AutoMode = state
    end
})

secAimbot:AddDropdown({
    Title = "Target Mode",
    Options = {"Auto", "Killer", "Survivor"},
    Default = Config.AIM_TargetMode,
    Callback = function(v)
        Config.AIM_TargetMode = v
    end
})
...
secSurvival:AddToggle({
    ...
    Default = Config.SURV_InstantHeal,
    Callback = function(v)
        Config.SURV_InstantHeal = v
        if v then task.spawn(InstantHeal_Setup) end
        notifToggle("Instant Self Heal", v)
    end
})
...
-- Parry FOV Circle (di kaki karakter, line-based)
local PARRY_SEGS = 36
local ParryCircleLines = {}
for i = 1, PARRY_SEGS do
    local l = Drawing.new("Line")
    l.Thickness = 2
    l.Color = Color3.fromRGB(0, 170, 255)
    l.Transparency = 1
    l.Visible = false
    ParryCircleLines[i] = l
end

-- Parry FOV Circle (line-based)
local PARRY_FOV_SEG = 48
local ParryFOVLines = {}
for i = 1, PARRY_FOV_SEG do
    local l = Drawing.new("Line")
    l.Thickness = 1.5
    l.Color = Color3.fromRGB(0, 170, 255)
    l.Transparency = 0.7
...
-- Background fill simulasi (transparent putih) - pakai filled square di tengah
local ParryFOVBg = Drawing.new("Circle")
ParryFOVBg.Filled = true
ParryFOVBg.Color = Color3.fromRGB(255, 255, 255)
ParryFOVBg.Transparency = 0.08
ParryFOVBg.NumSides = 48
ParryFOVBg.Visible = false
...
local function MainLoop()
    ...
    -- Parry FOV Circle (world-space, di lantai bawah karakter)
    if Config.AUTO_Parry and Config.PARRY_FOV then
        local myRoot = GetCharacterRoot()
        if myRoot then
            local originY = myRoot.Position.Y - 3  -- sedikit di bawah kaki
            local r = Config.PARRY_Dist
            local allOnScreen = true
            local screenPoints = {}
            -- Project 48 titik lingkaran di lantai ke screen
            for i = 1, PARRY_FOV_SEG do
                local a = (i - 0.5) / PARRY_FOV_SEG * math.pi * 2
                local worldPt = Vector3.new(
                    myRoot.Position.X + math.cos(a) * r,
                    originY,
                    myRoot.Position.Z + math.sin(a) * r
                )
                local sp, onScreen = WorldToScreen(worldPt)
            ...
            for i = 1, PARRY_FOV_SEG do
                local next = (i % PARRY_FOV_SEG) + 1
                local p1, p2 = screenPoints[i], screenPoints[next]
                if p1 and p2 then
                    ParryFOVLines[i].From = p1
                    ParryFOVLines[i].To   = p2
                    ParryFOVLines[i].Visible = true
                    anyVisible = true
                else
                    ParryFOVLines[i].Visible = false
                end
            end
            -- Background fill - pakai center karakter di screen
            local centerSP, centerOn = WorldToScreen(Vector3.new(myRoot.Position.X, originY, myRoot.Position.Z))
            if centerOn and anyVisible then
                -- Estimasi radius screen dari titik pertama
                local estR = screenPoints[1] and (screenPoints[1] - centerSP).Magnitude or 0
                ParryFOVBg.Position = centerSP
                ParryFOVBg.Radius = estR
                ParryFOVBg.Visible = true
            else
                ParryFOVBg.Visible = false
            end
        else
            ParryFOVBg.Visible = false
            for i = 1, PARRY_FOV_SEG do ParryFOVLines[i].Visible = false end
        end
    else
        ParryFOVBg.Visible = false
        for i = 1, PARRY_FOV_SEG do ParryFOVLines[i].Visible = false end
    ...
    -- Parry FOV Circle (ground level di sekitar karakter)
    if Config.AUTO_Parry then
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            local feetPos = root.Position - Vector3.new(0, 3, 0)
            local parryR = Config.PARRY_Dist
            for i = 1, PARRY_SEGS do
                local a1 = (i-1) / PARRY_SEGS * math.pi * 2
                local a2 = i     / PARRY_SEGS * math.pi * 2
                local p1 = feetPos + Vector3.new(math.cos(a1)*parryR, 0, math.sin(a1)*parryR)
                local p2 = feetPos + Vector3.new(math.cos(a2)*parryR, 0, math.sin(a2)*parryR)
                local s1 = cam:WorldToViewportPoint(p1)
                local s2 = cam:WorldToViewportPoint(p2)
                if s1.Z > 0 and s2.Z > 0 then
                    ParryCircleLines[i].From  = Vector2.new(s1.X, s1.Y)
                    ParryCircleLines[i].To    = Vector2.new(s2.X, s2.Y)
                    ParryCircleLines[i].Color = KillerAttackDetected
                        and Color3.fromRGB(255, 80, 80)
                        or  Color3.fromRGB(0, 170, 255)
                    ParryCircleLines[i].Visible = true
                else
                    ParryCircleLines[i].Visible = false
                end
            end
        else
            for i = 1, PARRY_SEGS do ParryCircleLines[i].Visible = false end
        end
    else
        for i = 1, PARRY_SEGS do ParryCircleLines[i].Visible = false end
    end
...
Unload = function()
    ...
    for i = 1, PARRY_FOV_SEG do pcall(function() ParryFOVLines[i]:Remove() end) end
    pcall(function() ParryFOVBg:Remove() end)
    for i = 1, PARRY_SEGS do pcall(function() ParryCircleLines[i]:Remove() end) end
    for i = 1, 4 do
