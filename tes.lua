...
-- ================================================================
-- ================================================================
-- PERFECT SKILL CHECK (QTE style dari tesvide.lua - lebih robust)
-- ================================================================
local function SkillCheck_Fire()
    ...
    -- Method 2: firesignal GUI button
    pcall(function()
    ...
    -- Method 3: SendTouchEvent fallback
    pcall(function()
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        if not pg then return end
        for _, gui in ipairs(pg:GetChildren()) do
    ...
    -- Method 4: SendKeyEvent Space (PC fallback)
    pcall(function()
    ...
    task.spawn(function()
        pcall(function()
            ...
            local visConn = Check:GetPropertyChangedSignal("Visible"):Connect(function()
                if not PerfectSCActive then return end
                if Check.Visible then startHB()
                else if hbConn then hbConn:Disconnect(); hbConn = nil end end
            end)
    ...
    if PlayerGui then
        PerfectSCGuiWatchConn = PlayerGui.ChildAdded:Connect(function(child)
            ...
            if child.Name == "SkillCheckPromptGui" then
                task.wait(0.3); PerfectSC_ConnectGui()
            end
        end)
    end
end
...
-- PARRY FOV: implementasi 1:1 dari tesvide.lua
-- Drawing API: Transparency=1 = OPAQUE (kebalikan dari Instance)
local PFOV_N    = 24
local PFOVLines = {}
for i = 1, PFOV_N do
    ...
    ln.Transparency = 1   -- 1 = fully opaque di Drawing API
    ln.Visible      = false
    PFOVLines[i]    = ln
end

-- Hapus Part/Highlight approach (tidak reliable)
local ParryDiskPart      = nil
local ParryDiskHighlight = nil
local function ParryDisk_Remove()
    if ParryDiskHighlight then pcall(function() ParryDiskHighlight:Destroy() end); ParryDiskHighlight = nil end
    if ParryDiskPart      then pcall(function() ParryDiskPart:Destroy() end);      ParryDiskPart = nil end
end
...
local function UpdateParryFOV()
    ...
    for i = 1, PFOV_N do
        ...
        if on1 and on2 then
            PFOVLines[i].From    = s1
            PFOVLines[i].To      = s2
            PFOVLines[i].Color   = col
            PFOVLines[i].Visible = true
        else
...
-- Path: ReplicatedStorage.Killers.Veil.Spearthrow (bukan Remotes!)
-- Hook: hookfunction pada FireServer remote langsung
-- ================================================================
...
local SpearRemote    = nil
local SpearHookConn  = nil  -- connection untuk unhook

local function SpearSilent_GetRemote()
    if SpearRemote and SpearRemote.Parent then return SpearRemote end
    -- Path dari Dex++: ReplicatedStorage > Killers > Veil > Spearthrow
    ...
    if st then SpearRemote = st end
    return SpearRemote
end

local function SpearSilent_CalcDir(targetPos)
    local root = GetCharacterRoot()
    ...
    return (aimPos - startPos).Unit, aimPos
end
...
local SpearHookActive = false
local function SpearSilent_Hook()
    if SpearHookActive then return end
    local remote = SpearSilent_GetRemote()
    if not remote then return end  -- belum jadi killer / belum load

    -- Pakai hookfunction kalau tersedia (lebih aman dari __namecall)
    if hookfunction then
        local oldFire = remote.FireServer
        hookfunction(oldFire, newcclosure(function(self, ...)
            if Config_SpearSilent then
                local args = {...}
                local target = SpearSilent_GetTarget()
                if target and target.Character then
                    local pr = target.Character:FindFirstChild("HumanoidRootPart")
                    if pr then
                        local dir, _ = SpearSilent_CalcDir(pr.Position)
                        if dir then
                            -- Cari arg yang bertipe Vector3 (direction spear) dan ganti
        ...
        SpearHookActive = true
        return
    end

    -- Fallback: __namecall tapi HANYA intercept remote ini + cek method name
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        -- Hanya intercept kalau self adalah remote spear DAN method adalah FireServer
        if Config_SpearSilent and method == "FireServer" and self == SpearSilent_GetRemote() then
            local args = {...}
            local target = SpearSilent_GetTarget()
            if target and target.Character then
                local pr = target.Character:FindFirstChild("HumanoidRootPart")
...
-- Snapline: dari posisi HRP kita di screen ke target (bukan dari tengah layar)
local function UpdateSpearSnapline(cam, screenCenter)
    ...
    -- From: posisi HRP kita di screen
    local myRoot = GetCharacterRoot()
    local fromPos = screenCenter  -- default ke tengah
    if myRoot then
        local sp, onSc = WorldToScreen(myRoot.Position)
        if onSc then fromPos = sp end
    end

    -- To: posisi aimPos target (dengan kompensasi gravity)
    local _, aimPos = SpearSilent_CalcDir(pr.Position)
    if not aimPos then SpearSnapLine.Visible = false; return end
    local toPos, onScreen = WorldToScreen(aimPos)
    if onScreen then
        SpearSnapLine.From    = fromPos
        SpearSnapLine.To      = toPos
...
secSpear:AddToggle({ Title = "Spear Silent Aim", Default = false, Callback = function(v)
    ...
    if v then SpearSilent_Hook() end
end })
...
local function Init()
    ...
    UpdateVisuals()
    SetupRoleManager()
    -- Pre-cache spear remote kalau sudah ada
    task.delay(3, function() pcall(SpearSilent_GetRemote) end)
