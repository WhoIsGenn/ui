...
local AddConfigSection = loadstring(game:HttpGet("https://raw.githubusercontent.com/nhfudzfsrzggt/brigida/refs/heads/main/addons/configsection.lua", true))()

-- ================================================================
-- VARIABLES UNTUK CONFIG
-- ================================================================
local ConfigFolder = "VictoriaHub"
local CURRENT_VERSION = "1.0"
local ConfigData = {}
local Elements = {}

local function getConfigFolder() return ConfigFolder end
local function getCURRENT_VERSION() return CURRENT_VERSION end
local function getConfigData() return ConfigData end
local function getElements() return Elements end
local function getAutoLoad() return true end
...
local function SaveConfig(name)
    -- akan ditangani oleh module
end

local function Nt(msg, delay, color)
...
-- ================================================================
-- HELPER UNTUK REGISTER ELEMENTS
-- ================================================================
local function RegisterElement(key, element)
...
local espKiller = secPlayers:AddToggle({ 
    Title = "Killer ESP", 
    Default = false, 
    Callback = function(v) Config.ESP_Killer = v end 
})
RegisterElement("ESP_Killer", espKiller)

local espSurvivor = secPlayers:AddToggle({ 
    Title = "Survivor ESP", 
    Default = false, 
    Callback = function(v) Config.ESP_Survivor = v end 
})
RegisterElement("ESP_Survivor", espSurvivor)

local espNames = secPlayers:AddToggle({ 
    Title = "Names", 
    Default = true, 
    Callback = function(v) Config.ESP_Names = v end 
})
RegisterElement("ESP_Names", espNames)

local espDistance = secPlayers:AddToggle({ 
    Title = "Distance", 
    Default = true, 
    Callback = function(v) Config.ESP_Distance = v end 
})
RegisterElement("ESP_Distance", espDistance)

local espHealth = secPlayers:AddToggle({ 
    Title = "Health", 
    Default = true, 
    Callback = function(v) Config.ESP_Health = v end 
})
RegisterElement("ESP_Health", espHealth)

local secObjects = Tabs.ESP:AddSection({ Title = "Objects", Open = false })

local espGenerator = secObjects:AddToggle({ 
    Title = "Generator", 
    Default = false, 
    Callback = function(v) Config.ESP_Generator = v; ESPRefreshMap() end 
})
RegisterElement("ESP_Generator", espGenerator)

local espGate = secObjects:AddToggle({ 
    Title = "Gate", 
    Default = false, 
    Callback = function(v) Config.ESP_Gate = v; ESPRefreshMap() end 
})
RegisterElement("ESP_Gate", espGate)

local espHook = secObjects:AddToggle({ 
    Title = "Hook", 
    Default = false, 
    Callback = function(v) Config.ESP_Hook = v; ESPRefreshMap() end 
})
RegisterElement("ESP_Hook", espHook)

local espPallet = secObjects:AddToggle({ 
    Title = "Pallet", 
    Default = false, 
    Callback = function(v) Config.ESP_Pallet = v; ESPRefreshMap() end 
})
RegisterElement("ESP_Pallet", espPallet)

local espWindow = secObjects:AddToggle({ 
    Title = "Window", 
    Default = false, 
    Callback = function(v) Config.ESP_Window = v; ESPRefreshMap() end 
})
RegisterElement("ESP_Window", espWindow)
...
local aimEnabled = secAim:AddToggle({ 
    Title = "Enable Aimbot", 
    Default = false, 
    Callback = function(v) Config.AIM_Enabled = v end 
})
RegisterElement("AIM_Enabled", aimEnabled)

local aimAutoMode = secAim:AddToggle({ 
    Title = "Auto Mode (Mobile)", 
    Default = false, 
    Callback = function(v) Config.AIM_AutoMode = v end 
})
RegisterElement("AIM_AutoMode", aimAutoMode)

local aimTargetMode = secAim:AddDropdown({ 
    Title    = "Target Mode", 
    Options  = { "Auto", "Killer", "Survivor", "Closest" }, 
    Default  = "Auto", 
    Callback = function(v) Config.AIM_TargetMode = v end 
})
RegisterElement("AIM_TargetMode", aimTargetMode)

local aimTargetPart = secAim:AddDropdown({ 
    Title    = "Target Part", 
    Options  = { "Left Arm", "Torso", "HumanoidRootPart" }, 
    Default  = "Left Arm", 
    Callback = function(v) Config.AIM_TargetPart = v end 
})
RegisterElement("AIM_TargetPart", aimTargetPart)

local aimFOV = secAim:AddSlider({ 
    Title = "FOV Size", 
    Min = 50, Max = 400, Default = 120, 
    Callback = function(v) Config.AIM_FOV = v end 
})
RegisterElement("AIM_FOV", aimFOV)

local aimSmooth = secAim:AddSlider({ 
    Title = "Smoothness", 
    Min = 1, Max = 20, Default = 6, 
    Callback = function(v) Config.AIM_Smooth = v / 20 end 
})
RegisterElement("AIM_Smooth", aimSmooth)

local aimVisCheck = secAim:AddToggle({ 
    Title = "Visibility Check", 
    Default = true, 
    Callback = function(v) Config.AIM_VisCheck = v end 
})
RegisterElement("AIM_VisCheck", aimVisCheck)

local aimPredict = secAim:AddToggle({ 
    Title = "Prediction", 
    Default = true, 
    Callback = function(v) Config.AIM_Predict = v end 
})
RegisterElement("AIM_Predict", aimPredict)

local aimShowFOV = secAim:AddToggle({ 
    Title = "Show FOV", 
    Default = false, 
    Callback = function(v) Config.AIM_ShowFOV = v end 
})
RegisterElement("AIM_ShowFOV", aimShowFOV)

local aimCrosshair = secAim:AddToggle({ 
    Title = "Crosshair", 
    Default = false, 
    Callback = function(v) Config.AIM_Crosshair = v end 
})
RegisterElement("AIM_Crosshair", aimCrosshair)

-- Spear Section
local secSpear = Tabs.AIM:AddSection({ Title = "Spear Aimbot (Veil)", Open = false })

local spearAimbot = secSpear:AddToggle({ 
    Title = "Spear Aimbot", 
    Default = false, 
    Callback = function(v) Config.SPEAR_Aimbot = v end 
})
RegisterElement("SPEAR_Aimbot", spearAimbot)

local spearSilent = secSpear:AddToggle({ 
...
RegisterElement("SPEAR_Silent", spearSilent)

local spearGravity = secSpear:AddSlider({ 
    Title = "Spear Gravity", 
    Min = 10, Max = 500, Default = 50, 
    Callback = function(v) Config.SPEAR_Gravity = v end 
})
RegisterElement("SPEAR_Gravity", spearGravity)

local spearSpeed = secSpear:AddSlider({ 
    Title = "Spear Speed", 
    Min = 50, Max = 500, Default = 100, 
    Callback = function(v) Config.SPEAR_Speed = v end 
})
RegisterElement("SPEAR_Speed", spearSpeed)

local spearSnapline = secSpear:AddToggle({ 
    Title = "Snapline", 
    Default = false, 
    Callback = function(v) Config_SpearSnapline = v end 
})
RegisterElement("SPEAR_Snapline", spearSnapline)

local spearThrough = secSpear:AddToggle({ 
    Title = "Through the Veil", 
    Default = false, 
    Callback = function(v) Config_SpearThrough = v end 
})
RegisterElement("SPEAR_Through", spearThrough)
...
local antiSkillCheck = secSurv:AddToggle({ 
...
RegisterElement("ANTI_SkillCheck", antiSkillCheck)

local perfectSC = secSurv:AddToggle({ 
...
RegisterElement("PERFECT_SkillCheck", perfectSC)

local godMode = secSurv:AddToggle({ 
    Title = "God Mode", 
    Default = false, 
    Callback = function(v) 
        Config.SURV_GodMode = v
        if v then GodMode_Start() else GodMode_Stop() end
    end 
})
RegisterElement("SURV_GodMode", godMode)

local noFall = secSurv:AddToggle({ 
    Title = "No Fall Damage", 
    Default = false, 
    Callback = function(v) Config.SURV_NoFall = v end 
})
RegisterElement("SURV_NoFall", noFall)

-- Auto Parry Section
local secParry = Tabs.SURVIVOR:AddSection({ Title = "Auto Parry", Open = false })

local autoParry = secParry:AddToggle({ 
    Title = "Auto Parry", 
    Default = false, 
    Callback = function(v) 
        Config.AUTO_Parry = v
...
local autoAttack = secCombat:AddToggle({ 
    Title = "Auto Attack", 
    Default = false, 
    Callback = function(v) Config.AUTO_Attack = v end 
})
RegisterElement("AUTO_Attack", autoAttack)

local attackRange = secCombat:AddSlider({ 
    Title = "Attack Range", 
    Min = 5, Max = 20, Default = 12, 
    Callback = function(v) Config.AUTO_AttackRange = v end 
})
RegisterElement("AUTO_AttackRange", attackRange)

local doubleTap = secCombat:AddToggle({ 
    Title = "Double Tap", 
    Default = false, 
    Callback = function(v) Config.KILLER_DoubleTap = v end 
})
RegisterElement("KILLER_DoubleTap", doubleTap)

local infiniteLunge = secCombat:AddToggle({ 
    Title = "Infinite Lunge", 
    Default = false, 
    Callback = function(v) Config.KILLER_InfiniteLunge = v end 
})
RegisterElement("KILLER_InfiniteLunge", infiniteLunge)

local autoHook = secCombat:AddToggle({ 
    Title = "Auto Hook", 
    Default = false, 
    Callback = function(v) Config.KILLER_AutoHook = v end 
})
RegisterElement("KILLER_AutoHook", autoHook)

-- Hitbox Section
local secHitbox = Tabs.KILLER:AddSection({ Title = "Hitbox", Open = false })

local hitboxEnabled = secHitbox:AddToggle({ 
    Title = "Hitbox Expander", 
    Default = false, 
    Callback = function(v) Config.HITBOX_Enabled = v end 
})
RegisterElement("HITBOX_Enabled", hitboxEnabled)

local hitboxSize = secHitbox:AddSlider({ 
    Title = "Hitbox Size", 
    Min = 5, Max = 30, Default = 15, 
    Callback = function(v) Config.HITBOX_Size = v end 
})
RegisterElement("HITBOX_Size", hitboxSize)

-- Protection Section
local secProtection = Tabs.KILLER:AddSection({ Title = "Protection", Open = false })

local noPalletStun = secProtection:AddToggle({ 
    Title = "No Pallet Stun", 
    Default = false, 
    Callback = function(v) Config.KILLER_NoPalletStun = v end 
})
RegisterElement("KILLER_NoPalletStun", noPalletStun)

local antiBlind = secProtection:AddToggle({ 
    Title = "Anti Blind", 
    Default = false, 
    Callback = function(v) Config.KILLER_AntiBlind = v end 
})
RegisterElement("KILLER_AntiBlind", antiBlind)

local noSlowdown = secProtection:AddToggle({ 
    Title = "No Slowdown", 
    Default = false, 
    Callback = function(v) Config.KILLER_NoSlowdown = v end 
})
RegisterElement("KILLER_NoSlowdown", noSlowdown)

-- Destruction Section
local secDestruction = Tabs.KILLER:AddSection({ Title = "Destruction", Open = false })

local fullGenBreak = secDestruction:AddToggle({ 
    Title = "Full Gen Break", 
    Default = false, 
    Callback = function(v) Config.KILLER_FullGenBreak = v end 
})
RegisterElement("KILLER_FullGenBreak", fullGenBreak)

local destroyPallets = secDestruction:AddToggle({ 
    Title = "Destroy All Pallets", 
    Default = false, 
    Callback = function(v) Config.KILLER_DestroyPallets = v end 
})
RegisterElement("KILLER_DestroyPallets", destroyPallets)

-- Killer Camera Section
local secKillerCamera = Tabs.KILLER:AddSection({ Title = "Camera", Open = false })

local thirdPerson = secKillerCamera:AddToggle({ 
    Title = "Third Person", 
    Default = false, 
    Callback = function(v) 
...
local noFog = secVisual:AddToggle({ 
    Title = "No Fog", 
    Default = false, 
    Callback = function(v) 
        Config.NO_Fog = v
...
RegisterElement("NO_Fog", noFog)

local fullbright = secVisual:AddToggle({ 
    Title = "Fullbright", 
    Default = false, 
    Callback = function(v) 
        Config.FULLBRIGHT = v
...
-- ================================================================
-- CONFIG TAB (PAKAI ADDCONFIGSECTION)
-- ================================================================

-- Panggil module config section
AddConfigSection(
    VelarisUI,           -- Chloex (VelarisUI instance)
    getConfigFolder,     -- function
    getCURRENT_VERSION,  -- function
    ...
    FolderName = "VictoriaHub"  -- folder custom
})
