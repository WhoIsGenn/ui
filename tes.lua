...
-- 3. HEAL AURA TEAM: heal semua teammate dalam radius
local function HealAura_Start()
    ...
    HealAuraConn = RunService.Heartbeat:Connect(function()
        ...
        if GetRole() ~= "Survivor" then return end
        for _, p in ipairs(Heal_GetSurvivors(HEAL_AuraDist, false)) do
            Heal_FireInstant(p)
...
secHeal:AddSlider({
    Title = "Heal Aura Distance",
    Min = 5, Max = 60, Default = HEAL_AuraDist,
    Callback = function(v) HEAL_AuraDist = v end
})
