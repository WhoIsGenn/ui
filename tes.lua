...
local ESPColors = {
    ...
    Window        = Color3.fromRGB(80, 160, 255),  -- biru
}
...
local function ApplyHighlight(obj, color, fillTrans, outTrans)
    ...
    h.FillColor          = color
    h.OutlineColor       = Color3.fromRGB(255, 255, 255)  -- outline selalu putih
...
local function ESPRefreshMap()
    ...
    -- Window: scan workspace (termasuk descendants Map) karena window bisa ada di mana saja
    local function applyWindowESP(obj)
        if obj.Name == "Window" then
            if Config.ESP_Window then ApplyHighlight(obj, ESPColors.Window, 0.5, 0)
            else RemoveHighlight(obj) end
        end
    end
    for _, obj in ipairs(workspace:GetDescendants()) do applyWindowESP(obj) end
...
-- ================================================================
-- PARRY FOV CIRCLE
-- Menggunakan: Highlight pada Part silinder (circle area di lantai)
--              + 32 line segments world-space (outline putih)
-- ================================================================
...
local PFOV_N        = 32
local PFOVLines     = {}  -- outline putih tipis
local PFOVGlowLines = {}  -- glow biru tebal

for i = 1, PFOV_N do
    local ol = Drawing.new("Line")
    ol.Thickness = 1.5; ol.Color = Color3.fromRGB(255,255,255); ol.Transparency = 0; ol.Visible = false
    PFOVLines[i] = ol
    local gl = Drawing.new("Line")
    gl.Thickness = 4;   gl.Color = Color3.fromRGB(80,160,255);  gl.Transparency = 0.5; gl.Visible = false
    PFOVGlowLines[i] = gl
end

-- Highlight cylinder Part (lingkaran area di lantai)
local ParryAreaPart      = nil
local ParryAreaHighlight = nil

local function ParryArea_CreateOrUpdate(root)
    local r = Config.PARRY_Dist
    if not ParryAreaPart then
        ParryAreaPart = Instance.new("Part")
        ParryAreaPart.Name         = "_VHParryArea"
        ParryAreaPart.Anchored     = true
        ParryAreaPart.CanCollide   = false
        ParryAreaPart.CanTouch     = false
        ParryAreaPart.Transparency = 1
        ParryAreaPart.Shape        = Enum.PartType.Cylinder
        ParryAreaPart.Parent       = workspace
    end
    -- Cylinder horizontal (poros X) — terlihat sebagai lingkaran dari atas
    ParryAreaPart.Size   = Vector3.new(0.05, r * 2, r * 2)
    ParryAreaPart.CFrame = CFrame.new(root.Position + Vector3.new(0, -2.8, 0))
                         * CFrame.Angles(0, 0, math.rad(90))

    if not ParryAreaHighlight then
        ParryAreaHighlight = Instance.new("Highlight")
        ParryAreaHighlight.Name               = "_VHParryHL"
        ParryAreaHighlight.DepthMode          = Enum.HighlightDepthMode.AlwaysOnTop
        ParryAreaHighlight.Adornee            = ParryAreaPart
        ParryAreaHighlight.OutlineTransparency = 1   -- tanpa outline di highlight itu sendiri
        ParryAreaHighlight.Parent             = workspace
    end
    -- Warna fill: biru normal, merah saat cooldown
    local col = ParryOnCooldown and Color3.fromRGB(255,60,60) or Color3.fromRGB(80,160,255)
    ParryAreaHighlight.FillColor         = col
    ParryAreaHighlight.FillTransparency  = 0.65
end

local function ParryArea_Hide()
    if ParryAreaHighlight then ParryAreaHighlight:Destroy(); ParryAreaHighlight = nil end
    if ParryAreaPart      then ParryAreaPart:Destroy();      ParryAreaPart = nil end
    for i = 1, PFOV_N do
        PFOVLines[i].Visible     = false
        PFOVGlowLines[i].Visible = false
    end
end

-- Dummy untuk backward compat
local ParryFOVCircle = nil
local function CreateParryFOVCircle() end

local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        ParryArea_Hide(); return
    end
    local root = GetCharacterRoot()
    if not root then ParryArea_Hide(); return end

    -- Update Highlight area
    ParryArea_CreateOrUpdate(root)

    -- World-space circle outline di lantai
    local r     = Config.PARRY_Dist
    local baseY = root.Position.Y - 2.8
    local ox, oz = root.Position.X, root.Position.Z
    local lineCol = ParryOnCooldown and Color3.fromRGB(255,80,80) or Color3.fromRGB(255,255,255)
    local glowCol = ParryOnCooldown and Color3.fromRGB(255,60,60) or Color3.fromRGB(80,160,255)
    for i = 1, PFOV_N do
        ...
        local vis = on1 and on2
        PFOVLines[i].From     = s1; PFOVLines[i].To     = s2
        PFOVLines[i].Color    = lineCol; PFOVLines[i].Visible = vis
        PFOVGlowLines[i].From = s1; PFOVGlowLines[i].To = s2
        PFOVGlowLines[i].Color= glowCol; PFOVGlowLines[i].Visible = vis
    end
end
...
local OriginalCameraMaxZoom      = 7
local OriginalCameraMinZoom      = 7
...
local function ThirdPerson_Apply()
    ...
    -- Disable Firstperson script agar tidak paksa kamera FP
    ...
    -- Heartbeat: maintain — re-disable FP kalau respawn ulang, maintain CameraOffset
    ...
    ThirdPersonRenderConn = RunService.RenderStepped:Connect(function()
        ...
        if fp2 and not fp2.Disabled then
            ...
            if not table.find(ThirdPersonDisabledScripts, fp2) then
                table.insert(ThirdPersonDisabledScripts, fp2)
            end
        end
    ...
    ThirdPersonCharConn = LocalPlayer.CharacterAdded:Connect(function(newChar)
        ...
        if hum then hum.CameraOffset = Vector3.new(2, 1, 8); hum.AutoRotate = true end
...
local function ThirdPerson_Remove()
    -- Stop heartbeat dulu sebelum restore
    ...
    -- Re-enable semua script Firstperson yang di-disable
...
local function UpdateThirdPerson()
    local role         = GetRole()
    local shouldActive = Config.CAM_ThirdPerson and role ~= "Spectator"
    if shouldActive and not ThirdPersonWasActive then
        ThirdPerson_Apply()
    elseif not shouldActive and ThirdPersonWasActive then
        ThirdPerson_Remove()
    end
end
...
local function UpdateJumpPower()
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    -- Set 0 agar tombol jump tidak muncul di mobile
    -- Hanya aktif kalau JUMP_Infinite toggle ON
    if Config.JUMP_Infinite then
...
local function Unload()
    ...
    for i = 1, PFOV_N do
        pcall(function() PFOVLines[i]:Remove() end)
        pcall(function() PFOVGlowLines[i]:Remove() end)
    end
    ParryArea_Hide()
...
secKillerCamera:AddToggle({ Title = "Third Person", Default = Config.CAM_ThirdPerson, Callback = function(v)
    Config.CAM_ThirdPerson = v
    if v then ThirdPerson_Apply() else ThirdPerson_Remove() end
end })
