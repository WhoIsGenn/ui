...
local function GetOrCreateGenTag(gen, adornee)
    ...
    if not tag then
        ...
        tag.Name            = "_VHGenTag"
        tag.AlwaysOnTop     = true
        tag.Size            = UDim2.new(0, 80, 0, 32)
        tag.StudsOffset     = Vector3.new(0, 3.5, 0)
        tag.StudsOffsetWorldSpace = Vector3.new(0, 0, 0)
        tag.SizeOffset      = Vector2.new(0, 0)
        tag.MaxDistance     = 60   -- tidak kelihatan kalau terlalu jauh
        tag.Adornee         = adornee
        tag.Parent          = gen
...
-- PARRY FOV: pure world-space Drawing lines (proven works, no SelectionSphere)
local PFOV_N = 24
local PFOVLines = {}
for i = 1, PFOV_N do
...
local function UpdateParryFOV()
    ...
    local result = workspace:Raycast(myRoot.Position, Vector3.new(0, -10, 0), params)
    if result then
        floorY = result.Position.Y + 0.05
    else
        floorY = myRoot.Position.Y - 3
    end
    ...
    for i = 1, PFOV_N do
        ...
        local s1, on1 = WorldToScreen(Vector3.new(ox+math.cos(a1)*r, floorY, oz+math.sin(a1)*r))
        local s2, on2 = WorldToScreen(Vector3.new(ox+math.cos(a2)*r, floorY, oz+math.sin(a2)*r))
        if on1 and on2 then
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and kr then
                if (kr.Position - myRoot.Position).Magnitude <= Config.PARRY_Dist then
                    AutoParry_TryFire()
                end
            end
        end)
...
-- ================================================================
-- AUTO HEAL TEAMMATE (Instant heal knocked/low HP teammate)
-- Method: intercept QTE saat healing, auto-complete tiap heartbeat
-- ================================================================
local SURV_AutoHeal = false
local AutoHealConn  = nil
local AutoHealLastTarget = nil

local function AutoHeal_GetTarget()
    local root = GetCharacterRoot()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Team == LocalPlayer.Team and p.Character then
            ...
            if pr and hum then
                ...
                -- Target: knocked (HP <= 25%) atau injured (HP <= 50%)
                if hp <= 0.5 then
                    local d = (pr.Position - root.Position).Magnitude
                    if d < bd and d < 8 then  -- hanya kalau dekat (8 studs = jangkauan heal)
                        bd = d; best = p
                    end
                end
            end
...
local function AutoHeal_Setup()
    if AutoHealConn then AutoHealConn:Disconnect(); AutoHealConn = nil end
    if not SURV_AutoHeal then return end

    -- Coba cari heal remote
    ...
    pcall(function()
        ...
        healRemote = healing:FindFirstChild("SkillCheckResultEvent")
            or healing:FindFirstChild("HealResult")
            or healing:FindFirstChild("Heal")
    end)

    AutoHealConn = RunService.Heartbeat:Connect(function()
        if not SURV_AutoHeal then
            AutoHealConn:Disconnect(); AutoHealConn = nil; return
        end
        if GetRole() ~= "Survivor" then return end
        ...
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        -- Jangan heal kalau lagi bergerak
        if hum and hum.MoveDirection.Magnitude > 0.05 then return end

        local target = AutoHeal_GetTarget()
        if not target or not target.Character then return end

        -- Method 1: FireServer heal remote langsung
        if healRemote then
            pcall(function()
                healRemote:FireServer("success", 1, target.Character)
            end)
        end

        -- Method 2: Auto QTE — kalau skill check muncul saat healing, langsung complete
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        if not pg then return end
        local prompt = pg:FindFirstChild("SkillCheckPromptGui")
        ...
        if check and check.Visible then
            -- Fire action button (complete QTE)
            for _, gui in ipairs(pg:GetChildren()) do
                local controls = gui:FindFirstChild("Controls", true)
                if controls then
                    local btn = controls:FindFirstChild("action", true)
                        or controls:FindFirstChild("Action", true)
                    if btn then
                        pcall(function()
                            firesignal(btn.MouseButton1Down)
                            firesignal(btn.MouseButton1Up)
                        ...
                        pcall(function()
                            ...
                            VirtualInputManager:SendTouchEvent(7701, 0, pos.X+ins.X, pos.Y+ins.Y)
                            task.defer(function()
                                VirtualInputManager:SendTouchEvent(7701, 2, pos.X+ins.X, pos.Y+ins.Y)
                            end)
                        end)
                        return
                    end
...
secSurv:AddToggle({
    Title = "Auto Heal Teammate",
    Default = false,
    Callback = function(v)
        SURV_AutoHeal = v
        if v then AutoHeal_Setup() else AutoHeal_Stop() end
    end
})
