...
-- STATE
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- ★ HARD WORDS: kata dari susah2.txt (berakhiran -ia, -if, -iat, -tif, -ks dll)
local HardWords = {}
local hardWordsLoaded = false
task.spawn(function()
    local ok, data = pcall(function()
    ...
    if ok and data and #data > 100 then
        ...
        if S.Debug then print("[SK] HardWords loaded: "..tostring(#HardWords)) end
    end
end)

-- Mode filter untuk SK Manual: "semua", "mudah", "normal", "hard"
...
-- ★ SLOT REFS: disimpan saat slot pertama muncul, tidak pernah scan ulang
-- _slotRefs = array of TextLabel, diurutkan by AbsolutePosition.X sekali saja
local _slotRefs = {}  -- {TextLabel, ...} sorted left→right

local function _initSlotRefs()
    -- Dipanggil SEKALI saat WordSubmit muncul
    -- Setelah itu _slotRefs tidak berubah (game tidak add/remove slot mid-game)
    ...
    for _, child in ipairs(ws:GetChildren()) do
        if child:IsA("TextLabel") then
            ...
            table.insert(tmp, {obj=child, x=ok and pos.X or 0})
        end
    end
    table.sort(tmp, function(a,b) return a.x < b.x end)
    _slotRefs = {}
    for _, s in ipairs(tmp) do table.insert(_slotRefs, s.obj) end
end

-- getWordSubmitText: baca langsung dari _slotRefs, tidak ada GetChildren, tidak ada sort
local function getWordSubmitText()
    local chars = {}
    for _, lbl in ipairs(_slotRefs) do
        local t = lbl.Text or ""
...
local function clearWordSubmit()
    for _, lbl in ipairs(_slotRefs) do pcall(function() lbl.Text = "" end) end
...
local function getWordList(prefix, maxN)
    ...
    -- Filter by mode (S_ManualMode): semua/mudah/normal/hard
    local filtered = {}
    for _,w in ipairs(avail) do
        local diff = getDiff(w)  -- "MUDAH"/"NORMAL"/"HARD"
        if S_ManualMode == "semua" then
            table.insert(filtered, w)
        elseif S_ManualMode == "mudah"  and diff == "MUDAH" then table.insert(filtered, w)
        elseif S_ManualMode == "normal" and diff == "NORMAL" then table.insert(filtered, w)
        elseif S_ManualMode == "hard"   and diff == "HARD"   then table.insert(filtered, w)
        end
    end
    -- Sort: HARD dulu (bintang ★), lalu NORMAL, lalu MUDAH; tiap grup by panjang desc
    local hard,normal,mudah2={},{},{}
    for _,w in ipairs(filtered) do
        local d=getDiff(w)
        if d=="HARD" then table.insert(hard,w)
        elseif d=="NORMAL" then table.insert(normal,w)
        else table.insert(mudah2,w) end
    end
    local function slen(t) table.sort(t,function(a,b) return #a>#b end) end
    slen(hard) slen(normal) slen(mudah2)
    -- Interleave 1 HARD : 2 NORMAL : 3 MUDAH
    local res={}
    local hi,ni,mi2=1,1,1
    while #res<maxN do
        local added=false
        ...
        if ni<=#normal then table.insert(res,normal[ni]); ni=ni+1   added=true end
        if #res>=maxN then break end
        if mi2<=#mudah2 then table.insert(res,mudah2[mi2]); mi2=mi2+1 added=true end
        if #res>=maxN then break end
        if mi2<=#mudah2 then table.insert(res,mudah2[mi2]); mi2=mi2+1 added=true end
        if #res>=maxN then break end
        if mi2<=#mudah2 then table.insert(res,mudah2[mi2]); mi2=mi2+1 added=true end
...
-- ★ Difficulty: cek HardWords set dulu, lalu huruf akhir susah, lalu rarity
-- HARD  = ada di HardWords (susah2.txt) ATAU huruf akhir f/x/q/z/v
-- NORMAL= huruf akhir b/d/g/p/c/j/y/h/w
-- MUDAH = huruf akhir a/i/u/e/o/n/r/l/t/s/k/m
local _endHard   = {f=1,x=1,q=1,z=1,v=1}
local _endNormal = {b=1,d=1,g=1,p=1,c=1,j=1,y=1,h=1,w=1}
local function isHardWord(word)
    if HardWords[word] then return true end
    local l = word:sub(-1):lower()
    return _endHard[l] ~= nil
end
local function getDiff(word)
    if isHardWord(word) then
        return "HARD",   Color3.fromRGB(160,20,20),   Color3.fromRGB(255,90,90)
    end
    local l = word:sub(-1):lower()
    if _endNormal[l] then
        return "NORMAL", Color3.fromRGB(140,90,0),    Color3.fromRGB(255,190,50)
    end
    return "MUDAH",  Color3.fromRGB(18,100,38),   Color3.fromRGB(55,215,95)
end
...
local function buildMonitor()
    ...
    ft.BackgroundTransparency=1 ft.Text="v34.0  •  Auto SK"
...
local function rebuildList(prefix)
    ...
    for idx, word in ipairs(words) do
        ...
        -- Kata (bold, warna sesuai difficulty)
        ...
        wordLbl.TextColor3=(diffTxt=="HARD") and Color3.fromRGB(255,100,100)
            or (diffTxt=="NORMAL") and Color3.fromRGB(255,200,80)
...
local function buildManual()
    ...
    MAN.awalanLabel.Size=UDim2.new(1,-20,0,16) MAN.awalanLabel.Position=UDim2.new(0,14,0,50)
    ...
    -- ── Mode filter buttons: Semua / Mudah / Normal / Hard ──
    local modeFrame=Instance.new("Frame",panel)
    modeFrame.Size=UDim2.new(1,-20,0,24) modeFrame.Position=UDim2.new(0,10,0,70)
    ...
    local modeBtns={}
    local modeColors={
        semua =Color3.fromRGB(50,80,160),
        mudah =Color3.fromRGB(18,100,38),
        normal=Color3.fromRGB(140,90,0),
        hard  =Color3.fromRGB(160,20,20),
    }
    local modeDef=Color3.fromRGB(22,26,44)
    local function setModeActive(mode)
        S_ManualMode=mode
        for m,btn in pairs(modeBtns) do
            btn.BackgroundColor3=(m==mode) and modeColors[m] or modeDef
        end
        -- Rebuild list dengan mode baru
        if manPrefix~="" then task.spawn(function() rebuildList(manPrefix) end) end
    end
    for _,mdata in ipairs({{"Semua","semua"},{"Mudah","mudah"},{"Normal","normal"},{"Hard★","hard"}}) do
        local lbl,key=mdata[1],mdata[2]
        local mb=Instance.new("TextButton",modeFrame)
        mb.Size=UDim2.new(0.23,0,1,0) mb.BackgroundColor3=modeDef mb.BorderSizePixel=0
        mb.Text=lbl mb.TextColor3=Color3.fromRGB(210,220,255) mb.TextSize=9
        mb.Font=Enum.Font.GothamBold
        Instance.new("UICorner",mb).CornerRadius=UDim.new(0,4)
        mb.MouseButton1Click:Connect(function() setModeActive(key) end)
        modeBtns[key]=mb
    end
    -- Default: Semua aktif
    modeBtns["semua"].BackgroundColor3=modeColors["semua"]

    -- ── Search bar (seperti screenshot "Cari kata... (ketik awalan)") ──
    local sbg=Instance.new("Frame",panel)
    sbg.Size=UDim2.new(1,-20,0,32) sbg.Position=UDim2.new(0,10,0,98)
...
-- EVENT HOOKS
--
-- Prefix HANYA dari WordServer.Text (akurat, tidak bisa salah)
-- Panel SK Manual filter dari SEARCH BOX (ketik manual di panel)
-- TIDAK ada event hook ke slot game → tidak ada freeze
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

-- Init slot refs SEKALI saat WordSubmit muncul (untuk typeAndSubmit & clearWordSubmit)
task.spawn(function()
    while true do
        task.wait(0.3)
        local ws = getWordSubmit()
        if ws and #_slotRefs == 0 then
            _initSlotRefs()
            -- Jika refs belum terbaca sempurna (AbsolutePosition belum tersedia)
        ...
        if #_slotRefs > 0 then break end
    end
end)

-- Hook WordServer: prefix resmi dari game
-- Update S.Prefix tiap giliran baru, ringan (1 label, 1 property)
task.spawn(function()
    while true do
        task.wait(0.3)
        local wsf = getWordServerFrame()
        if wsf then
            local wsLabel = wsf:FindFirstChild("WordServer")
            if wsLabel then
                local function onServerChanged()
                    if not kbbiLoaded then return end
                    local raw = (wsLabel.Text or ""):gsub("%s+",""):lower():gsub("[^a-z]","")
                    if raw ~= "" and raw ~= S.Prefix then
                        S.Prefix = raw
                        S.Suggestion = findBest(raw, nil) or "-"
                        -- Auto-update panel manual ke prefix resmi saat giliran baru
                ...
                wsLabel:GetPropertyChangedSignal("Text"):Connect(onServerChanged)
                onServerChanged()
                break
            end
        end
...
-- ★ Prefix scan via "Hurufnya adalah" label (dari SKTES)
-- Ini mendeteksi prefix giliran lawan juga, bukan cuma giliran sendiri
-- Scan ringan: hanya cek TextLabel yang visible, stop saat ketemu
local _lastScannedPrefix = ""
task.spawn(function()
    while true do
        task.wait(0.25)
        if not kbbiLoaded then continue end
        ...
        for _, v in ipairs(pg:GetDescendants()) do
            ...
            if v:IsA("TextLabel") and v.Visible and v.Text and v.Text:find("Hurufnya adalah") then
                -- Cari sibling TextLabel yang pendek (prefix 1-3 huruf)
                for _, sib in ipairs(v.Parent:GetChildren()) do
                    if sib:IsA("TextLabel") and sib ~= v and sib.Visible then
                        ...
                        if t:match("^[A-Z]+$") and #t >= 1 and #t <= 4 and t ~= _lastScannedPrefix then
                            _lastScannedPrefix = t
                            local raw = t:lower()
                            -- Update S.Prefix (berlaku untuk giliran siapapun)
...
-- ★ Auto-correct: kalau ada teks lebih dari prefix di TextBox, hapus via BillboardRemote
-- Sama persis logika SKTES — scan TextBox visible, kalau lebih panjang dari S.Prefix → hapus
local R_Billboard = Remotes:FindFirstChild("BillboardUpdate")
if R_Billboard then
    task.spawn(function()
        while true do
            task.wait(0.3)
            if not S.AutoAnswer or S.Busy then continue end
            local pg = LocalPlayer:FindFirstChild("PlayerGui")
            if not pg then continue end
            for _, v in ipairs(pg:GetDescendants()) do
                if v:IsA("TextBox") and v.Visible
                    and v.Parent and v.Parent.Name ~= "ChatBar"
                    and #v.Text > #S.Prefix and S.Prefix ~= "-" and S.Prefix ~= "" then
                    task.wait(1.2)
                    -- Recheck setelah delay
                    if v.Visible and #v.Text > #S.Prefix then
                        S.Busy = true
                        local ct = v.Text
                        -- Hapus huruf satu per satu sampai tinggal prefix
                        for i = #ct, #S.Prefix, -1 do
                            pcall(function() R_Billboard:FireServer(ct:sub(1,i)) end)
                            task.wait(0.06)
                        end
                        S.Busy = false
                    end
...
print("[SK] v34.0 | No Heartbeat scan | SK Manual panel")
WindUI:Notify({Title="Auto SK v34.0",Content="Loading KBBI...",Icon="zap",Duration=3})
