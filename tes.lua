...
-- Function untuk get player list (synchronous)
...
tradeSection:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            task.spawn(function()
                -- LOAD DataService di DALAM toggle (seperti contoh Auto Totem)
...
-- Load Fish Button (UNTUK UPDATE FISH DROPDOWN)
tradeSection:AddButton({
    ...
    Callback = function()
        task.spawn(function()
            ...
            table.sort(fishList)
            -- NOTE: Kamu perlu manual update dropdown fish di sini
            -- Tapi karena library-mu ga support SetOptions, kita skip
            Trade.SelectedFish = {} -- Reset selection
        end)
    end
})
