--[[
    ...
    Fixed: Generator 100% Green, Auto Parry Mode (With Animation / No Animation)
]]
...
local Colors = {
    ...
    GeneratorDone = Color3.fromRGB(100, 255, 130),  -- Hijau untuk generator selesai
...
local function UpdatePlayerESP(player)
    ...
    if hum then
        local knocked = player.Character:GetAttribute("Knocked") or (player.Character:FindFirstChild("Knocked") and player.Character.Knocked.Value)
        local hooked = player.Character:GetAttribute("IsHooked") or (player.Character:FindFirstChild("IsHooked") and player.Character.IsHooked.Value)
        if hooked then 
...
local function UpdateGeneratorESP(gen)
    ...
    -- Generator selesai (100%) -> warna HIJAU
    ...
    -- Highlight juga warna sesuai (hijau jika selesai)
...
local function UpdateObjectESP(obj, color, label, offset)
    ...
    if not Config.ESP_Generator and label == "GEN" then return end
    if not Config.ESP_Gate and label == "GATE" then return end
    if not Config.ESP_Hook and label == "HOOK" then return end
    if not Config.ESP_Pallet and label == "PALLET" then return end
    if not Config.ESP_Window and label == "WINDOW" then return end
...
-- =====================================================================
-- AUTO ATTACK KILLER (firesignal ke button)
-- =====================================================================
...
-- =====================================================================
-- AUTO PARRY (dari tesvd.lua ASLI dengan mode dropdown)
-- =====================================================================
...
-- Get remote untuk No Animation mode
...
-- Get GUI button untuk With Animation mode
...
local function AutoParry_Fire()
    if Config.PARRY_Mode == "With Animation" then
        ...
        -- No Animation: langsung FireServer
    ...
    if now - lastGenCheck >= 0.2 then
        ...
        for i = #generators, 1, -1 do
            ...
            if gen and gen.Parent then
                local isComplete = UpdateGeneratorESP(gen)
                if isComplete then
                    -- Generator selesai, tetap di list tapi udah di-mark hijau
                    -- Ga usah diremove biar tetep keliatan
                end
            else
