...
local GunFireOriginal     = nil
local GunFireRemote       = nil
...
local function GunSilentAim_Setup()
    ...
    if not hookfunction then
        warn("[SilentAim] hookfunction not available")
        return
    end

    -- Cari Fire remote dari Twist of Fate
    local rs = ReplicatedStorage
    local remote = rs:FindFirstChild("Remotes")
        and rs.Remotes:FindFirstChild("Items")
        and rs.Remotes.Items:FindFirstChild("Twist of Fate")
        and rs.Remotes.Items["Twist of Fate"]:FindFirstChild("Fire")

    if not remote then
        warn("[SilentAim] Fire remote not found")
        return
    end

    GunFireRemote = cloneref(remote)

    -- Hook FireServer method di remote
    GunFireOriginal = hookfunction(GunFireRemote.FireServer, newcclosure(function(self, gunType, dirX, dirY, dirZ)
        if Config.GUN_SilentAim and type(gunType) == "string" then
            local myChar = LocalPlayer.Character
            local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            local target = GunSilentAim_GetTarget()
            if target and myRoot then
                local dir = (target.Position - myRoot.Position).Unit
                return GunFireOriginal(self, gunType, dir.X, dir.Y, dir.Z)
            end
        end
        return GunFireOriginal(self, gunType, dirX, dirY, dirZ)
    end))

    GunSilentAimHooked = true
    print("[SilentAim] Hooked Fire remote FireServer")
end
