...
local function SuspendAllFeatures()
    ...
    -- Restore Fullbright lighting effects saat masuk lobby/spectator
...
local function RehookForRole(role)
    ...
    task.delay(2, function()
        ...
        -- Fullbright: re-apply saat ganti role/map
        if Config.FULLBRIGHT then
            pcall(Fullbright_On)
        end

        -- No Fog: force re-apply
        if Config.NO_Fog then
            State.LastFogState = nil
        end

        -- GodMode: restart hook ke humanoid baru
        if Config.SURV_GodMode and role == "Survivor" then
            pcall(GodMode_Stop)
            pcall(GodMode_Start)
        end

        -- Reset AutoAttack button cache (GUI recreated tiap match)
        AutoAttackBtn = nil
    end)
end
...
secCombat:AddToggle(secCombat:AddToggle({
    Title = "Auto Hook",
...
-- Watch map change: saat Map folder muncul di workspace, re-apply fitur visual
