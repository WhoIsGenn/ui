...
secHaki:AddToggle({
    ...
    Callback = function(v) Config.AutoHaki = v; notif("Auto Armament Haki: " .. (v and "ON" or "OFF")) end
})

secHaki:AddToggle({
    Title   = "Auto Observation Haki",
    Default = Config.AutoObservationHaki,
    Callback = function(v) Config.AutoObservationHaki = v; notif("Auto Obs Haki: " .. (v and "ON" or "OFF")) end
})
...
secWeapon:AddToggle({
    ...
    Callback = function(v) Config.AutoEquip = v; notif("Auto Equip: " .. (v and "ON" or "OFF")) end
})
...
secSkills:AddToggle({
    ...
    Callback = function(v) Config.AutoSkill.Bosses = v; notif("Skills on Bosses: " .. (v and "ON" or "OFF")) end
})
...
secSkills:AddToggle({
    ...
    Callback = function(v) Config.AutoSkill.NPCs = v; notif("Skills on NPCs: " .. (v and "ON" or "OFF")) end
})
    ...
    for _, entityName in ipairs(category.List) do
        sec:AddToggle({
            ...
            Callback = function(v)
                ...
                notif((v and "Farming " or "Skipping ") .. entityName)
            end
        })
    end
end
...
secBoss:AddToggle({
    ...
    Callback = function(v)
        ...
        notif("Auto Spawn: " .. (v and "ON" or "OFF"))
    end
})
...
for bossName, bossData in pairs(Config.Specials) do
    local sec = TabSpecials:AddSection(bossName)
    sec:AddToggle({
        Title   = "Auto Spawn " .. bossName,
        Default = bossData.Auto,
        Callback = function(v)
            Config.Specials[bossName].Auto = v
            notif(bossName .. " Spawn: " .. (v and "ON" or "OFF"))
        end
    })
    sec:AddDropdown({
        Title   = "Difficulty",
...
secAutoCraft:AddToggle({
    ...
    Callback = function(v) Config.AutoCraft.SlimeKey = v; notif("Auto Craft SlimeKey: " .. (v and "ON" or "OFF")) end
})

secAutoCraft:AddToggle({
    Title   = "Auto Craft Divine Grail",
    Default = Config.AutoCraft.DivineGrail,
    Callback = function(v) Config.AutoCraft.DivineGrail = v; notif("Auto Craft Divine Grail: " .. (v and "ON" or "OFF")) end
})
...
secUtils:AddToggle({
    ...
    Callback = function(v)
        ...
        notif("WhiteScreen: " .. (v and "ON (save CPU)" or "OFF"))
    end
})
...
secUtils:AddToggle({
    ...
    Callback = function(v)
        ...
        notif("Auto Rejoin: " .. (v and "ON" or "OFF"))
    end
})
...
secRejoin:AddToggle({
    ...
    Callback = function(v) Config.TimedRejoin = v; notif("Timed Rejoin: " .. (v and "ON" or "OFF")) end
})
...
if not _codeRedeemDone then
    ...
    task.spawn(function()
        task.wait(2)
        if RedeemCodes then RedeemCodes() end
    end)
end

notif("Victoria loaded! Refresh & select your weapon.", 5)
