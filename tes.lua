...
-- ============================================================
-- BRIGIDA (VelarisUI) - replaces Obsidian
-- Flags/Toggles/Options compat layer ada di bawah UI section
-- ============================================================
...
-- Flags system: agar semua Toggles["x"].Value dan Options["x"].Value tetap jalan
local _flags = {}
...
-- Library shim: agar Library:Notify(), Library:Unload(), dll tetap jalan
local Library = {
    Notify     = function(self, msg, dur) VelarisUI:MakeNotify({ Title = "celina", Description = msg, Time = dur or 3 }) end,
...
-- ============================================================
-- HELPER: buat flag + elemen sekaligus
-- ============================================================
local function T(sec, id, cfg) -- Toggle
    local flag = Toggles[id]
    flag.Value = cfg.Default or false
    sec:AddToggle({
...
local function D(sec, id, cfg) -- Dropdown
    local flag = Options[id]
    flag.Value = cfg.Default
...
local function S(sec, id, cfg) -- Slider
    local flag = Options[id]
    flag.Value = cfg.Default or 0
    sec:AddSlider({
        Title     = cfg.Text or id,
...
local function I(sec, id, cfg) -- Input
    local flag = Options[id]
    flag.Value = cfg.Default or ""
    sec:AddInput({
        Title    = cfg.Text or id,
        Content  = cfg.Placeholder or cfg.Content or "",
...
local function B(sec, titleOrCfg, fn) -- Button
    local title, callback
    if type(titleOrCfg) == "string" then
        title = titleOrCfg; callback = fn
    else
        title = titleOrCfg.Text or titleOrCfg.Title or "Button"
        callback = titleOrCfg.Func or titleOrCfg.Callback or fn
    end
    sec:AddButton({ Title = title, Locked = (type(titleOrCfg) == "table" and titleOrCfg.Disabled) or false, Callback = callback or function() end })
end

local function L(sec, text) -- Label → Paragraph
    if type(text) == "table" then text = text.Text or "" end
    local el = sec:AddParagraph({ Title = "", Content = text or "" })
    local obj = {
        SetText    = function(self, t) if el and el.SetContent then pcall(function() el:SetContent(t) end) end end,
...
-- INFORMATION TAB
-- ============================================================
local secInfoUser   = Tabs.Information:AddSection({ Title = "User",   Open = true })
local secInfoGame   = Tabs.Information:AddSection({ Title = "Game",   Open = true })
local secInfoOthers = Tabs.Information:AddSection({ Title = "Others", Open = true })

local executorDisplayName = (identifyexecutor and identifyexecutor() or "Unknown")
local statusText  = isLimitedExecutor and "<font color='#FFA500'>Semi-Working</font>" or "<font color='#00FF00'>Working</font>"
local extraNote   = isLimitedExecutor and "<b>NOTE:</b> May experiencing bugs for some features!" or "All features should works properly!"
local initTier, initTime = GetData()

TierLabel = L(secInfoUser, "<b>Type:</b> " .. initTier)
TimeLabel = L(secInfoUser, "<b>Time Left:</b> " .. initTime)
...
L(secInfoUser, "<b>Executor:</b> " .. executorDisplayName .. "\n<b>Status:</b> " .. statusText .. "\n" .. extraNote)
...
L(secInfoOthers, "⚠️ If some features are disabled, it is because your executor lacks the required functions.")
...
-- PRIORITY TAB
-- ============================================================
local secPriority = Tabs.Priority:AddSection({ Title = "Config", Open = true })

for i = 1, #PriorityTasks do
    D(secPriority, "SelectedPriority_" .. i, {
        Text    = "Priority " .. i,
...
-- WEBHOOK TAB
-- ============================================================
local secWebhook = Tabs.Webhook:AddSection({ Title = "Config", Open = true })

I(secWebhook, "WebhookURL",  { Text = "Webhook URL",  Default = "",  Placeholder = "Enter Webhook URL..." })
I(secWebhook, "UID",         { Text = "User ID",      Default = "",  Placeholder = "Enter UID..." })
D(secWebhook, "SelectedData", {
    Text = "Select Data (s)",
...
-- ============================================================
-- Autofarm Section
local secAutofarm = Tabs.Main:AddSection({ Title = "Autofarm", Open = true })
local tabAF_Farm  = secAutofarm:AddTabbox({ Title = "Farm",   Icon = "lucide:swords"   })
local tabAF_Boss  = secAutofarm:AddTabbox({ Title = "Boss",   Icon = "lucide:skull"    })
local tabAF_Misc  = secAutofarm:AddTabbox({ Title = "Misc",   Icon = "lucide:layers"   })
local tabAF_Cfg   = secAutofarm:AddTabbox({ Title = "Config", Icon = "lucide:settings" })

-- Farm tab
D(tabAF_Farm, "SelectedMob",  { Text = "Select Mob (s)", Values = Tables.MobList, Multi = true })
B(tabAF_Farm, "Refresh",      UpdateNPCLists)
T(tabAF_Farm, "MobFarm",      { Text = "Autofarm Selected Mob", Default = false })
T(tabAF_Farm, "AllMobFarm",   { Text = "Autofarm All Mobs",     Default = false })
D(tabAF_Farm, "AllMobType",   { Text = "Select Type [All Mob]", Values = { "Normal", "Fast" }, Default = "Normal" })
T(tabAF_Farm, "LevelFarm",    { Text = "Autofarm Level",        Default = false })

-- Boss tab
D(tabAF_Boss, "SelectedBosses",    { Text = "Select Bosses",       Values = Tables.BossList,    Multi = true })
T(tabAF_Boss, "BossesFarm",        { Text = "Autofarm Selected Boss", Default = false })
T(tabAF_Boss, "AllBossesFarm",     { Text = "Autofarm All Bosses",    Default = false })
D(tabAF_Boss, "SelectedSummon",    { Text = "Select Summon Boss",  Values = Tables.SummonList,  Multi = false })
D(tabAF_Boss, "SelectedSummonDiff",{ Text = "Select Difficulty",   Values = Tables.DiffList,    Multi = false })
T(tabAF_Boss, "AutoSummon",        { Text = "Auto Summon",          Default = false })
T(tabAF_Boss, "SummonBossFarm",    { Text = "Autofarm Summon Boss", Default = false })
PityLabel = L(tabAF_Boss, "<b>Pity:</b> 0/25")
D(tabAF_Boss, "SelectedBuildPity", { Text = "Select Boss [Build Pity]", Values = Tables.AllBossList, Multi = true })
D(tabAF_Boss, "SelectedUsePity",   { Text = "Select Boss [Use Pity]",   Values = Tables.AllBossList, Multi = false })
D(tabAF_Boss, "SelectedPityDiff",  { Text = "Select Difficulty [Use Pity]", Values = Tables.DiffList, Multi = false })
T(tabAF_Boss, "PityBossFarm",      { Text = "Autofarm Pity Boss",   Default = false })
D(tabAF_Boss, "SelectedOtherSummon",    { Text = "Select Summon Boss (Other)", Values = Tables.OtherSummonList, Multi = false })
D(tabAF_Boss, "SelectedOtherSummonDiff",{ Text = "Select Difficulty",          Values = Tables.DiffList,        Multi = false })
T(tabAF_Boss, "AutoOtherSummon",   { Text = "Auto Summon",                   Default = false })
T(tabAF_Boss, "OtherSummonFarm",   { Text = "Autofarm Summon Boss",          Default = false })

-- Misc tab (Alt Help)
D(tabAF_Misc, "SelectedAltBoss", { Text = "Select Boss",       Values = Tables.AllBossList, Multi = false })
D(tabAF_Misc, "SelectedAltDiff", { Text = "Select Difficulty", Values = Tables.DiffList,    Multi = false })
for i = 1, 5 do
    D(tabAF_Misc, "SelectedAlt_" .. i, { Text = "Select Alt #" .. i, Values = {}, Multi = false })
end
T(tabAF_Misc, "AltBossFarm", { Text = "Auto Help Alt", Default = false })

-- Config tab
D(tabAF_Cfg, "SelectedWeaponType",   { Text = "Select Weapon Type",     Values = Tables.Weapon,                    Multi = true })
S(tabAF_Cfg, "SwitchWeaponCD",       { Text = "Switch Weapon Delay",    Default = 4,    Min = 1,   Max = 20,  Rounding = 0 })
T(tabAF_Cfg, "SwitchWeapon",         { Text = "Auto Switch Weapon",     Default = true })
T(tabAF_Cfg, "IslandTP",             { Text = "Island TP [Autofarm]",   Default = true })
S(tabAF_Cfg, "IslandTPCD",           { Text = "Island TP CD",           Default = 0.67, Min = 0,   Max = 2.5, Rounding = 2 })
S(tabAF_Cfg, "TargetTPCD",           { Text = "Target TP CD",           Default = 0,    Min = 0,   Max = 5,   Rounding = 2 })
S(tabAF_Cfg, "TargetDistTP",         { Text = "Target Distance TP",     Default = 0,    Min = 0,   Max = 100, Rounding = 0 })
S(tabAF_Cfg, "M1Speed",              { Text = "M1 Attack Cooldown",     Default = 0,    Min = 0,   Max = 1,   Rounding = 2 })
D(tabAF_Cfg, "SelectedMovementType", { Text = "Select Movement Type",   Values = { "Teleport", "Tween" },     Default = "Tween"   })
D(tabAF_Cfg, "SelectedFarmType",     { Text = "Select Farm Type",       Values = { "Behind", "Above", "Below" }, Default = "Behind" })
S(tabAF_Cfg, "Distance",             { Text = "Farm Distance",          Default = 12,   Min = 0,   Max = 30,  Rounding = 0 })
S(tabAF_Cfg, "TweenSpeed",           { Text = "Tween Speed",            Default = 160,  Min = 0,   Max = 500, Rounding = 0 })
T(tabAF_Cfg, "InstaKill",            { Text = "Instant Kill",           Default = false })
D(tabAF_Cfg, "InstaKillType",        { Text = "Select Type",            Values = { "V1", "V2" }, Default = "V1" })
S(tabAF_Cfg, "InstaKillHP",          { Text = "HP% For Insta-Kill",     Default = 90,   Min = 1,   Max = 100, Rounding = 0 })
I(tabAF_Cfg, "InstaKillMinHP",       { Text = "Min MaxHP for Insta-Kill", Default = "100000", Placeholder = "Number.." })

-- MiscAuto Section
local secMiscAuto = Tabs.Main:AddSection({ Title = "Misc Auto", Open = true })
local tabMA_Haki  = secMiscAuto:AddTabbox({ Title = "Haki",  Icon = "lucide:flame" })
local tabMA_Skill = secMiscAuto:AddTabbox({ Title = "Skill", Icon = "lucide:zap"   })
local tabMA_Combo = secMiscAuto:AddTabbox({ Title = "Combo", Icon = "lucide:list"  })

T(tabMA_Haki, "ObserHaki",    { Text = "Auto Observation Haki", Default = false })
T(tabMA_Haki, "ArmHaki",      { Text = "Auto Armament Haki",    Default = false })
T(tabMA_Haki, "ConquerorHaki",{ Text = "Auto Conqueror Haki",   Default = false })

L(tabMA_Skill, "Autofarm already has <b>auto-M1 built in</b>. Enable this only if you have issues with autofarm M1.")
T(tabMA_Skill, "AutoM1",   { Text = "Auto Attack", Default = false })
T(tabMA_Skill, "KillAura", { Text = "Kill Aura",   Default = false })
S(tabMA_Skill, "KillAuraCD",    { Text = "CD",    Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
S(tabMA_Skill, "KillAuraRange", { Text = "Range", Default = 200, Min = 0,   Max = 200, Rounding = 0 })
L(tabMA_Skill, "Mode:\n- Normal: Check skill cooldowns\n- Instant: No check (may affect performance)")
D(tabMA_Skill, "SelectedSkills",  { Text = "Select Skills", Values = { "Z", "X", "C", "V", "F" }, Multi = true })
D(tabMA_Skill, "AutoSkillType",   { Text = "Select Mode",   Values = { "Normal", "Instant" }, Default = "Normal" })
T(tabMA_Skill, "OnlyTarget",          { Text = "Target Only",       Default = false })
T(tabMA_Skill, "AutoSkill_BossOnly",  { Text = "Use On Boss Only",  Default = false })
S(tabMA_Skill, "AutoSkill_BossHP",    { Text = "Boss HP%",          Default = 100, Min = 1, Max = 100, Rounding = 0 })
T(tabMA_Skill, "AutoSkill",           { Text = "Auto Use Skills",   Default = false })

L(tabMA_Combo, "Example: Z > X > C > 0.5 > V")
L(tabMA_Combo, "Mode:\n- Normal: Wait input CD\n- Instant: Ignore input CD")
I(tabMA_Combo, "ComboPattern", { Text = "Combo Pattern", Default = "Z > X > C > V > F", Placeholder = "combo.." })
D(tabMA_Combo, "ComboMode",    { Text = "Select Mode", Values = { "Normal", "Instant" }, Default = "Normal" })
T(tabMA_Combo, "ComboBossOnly",{ Text = "Boss Only",     Default = false })
T(tabMA_Combo, "AutoCombo",    { Text = "Auto Skill Combo", Default = false,
...
-- Switch Section
local secSwitch = Tabs.Main:AddSection({ Title = "Switch", Open = true })
local tabSW_Title = secSwitch:AddTabbox({ Title = "Title", Icon = "lucide:tag"     })
local tabSW_Rune  = secSwitch:AddTabbox({ Title = "Rune",  Icon = "lucide:diamond" })
local tabSW_Build = secSwitch:AddTabbox({ Title = "Build", Icon = "lucide:wrench"  })

CreateSwitchGroup(tabSW_Title, "Title", "Title", CombinedTitleList)
CreateSwitchGroup(tabSW_Rune,  "Rune",  "Rune",  Tables.RuneList)
CreateSwitchGroup(tabSW_Build, "Build", "Build", Tables.BuildList)

-- ============================================================
-- AUTOMATION TAB
-- ============================================================
local secMisc1  = Tabs.Automation:AddSection({ Title = "Misc",  Open = true })
local tabMI_Asc = secMisc1:AddTabbox({ Title = "Ascend", Icon = "lucide:arrow-up" })
local tabMI_Rll = secMisc1:AddTabbox({ Title = "Rolls",  Icon = "lucide:dices"    })
local tabMI_Trd = secMisc1:AddTabbox({ Title = "Trade",  Icon = "lucide:repeat"   })

T(tabMI_Asc, "AutoAscend", { Text = "Auto Ascend", Default = false })
for i = 1, 10 do
    Tables.AscendLabels[i] = L(tabMI_Asc, "")
    Tables.AscendLabels[i]:SetVisible(false)
end

L(tabMI_Rll, "⚠️ Increase delay based on your ping.\n⚠️ Low delay settings are not recommended.")
S(tabMI_Rll, "RollCD", { Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2 })
D(tabMI_Rll, "SelectedTrait", { Text = "Select Trait (s)", Values = Tables.TraitList, Multi = true })
T(tabMI_Rll, "AutoTrait",     { Text = "Auto Roll Trait",  Default = false })
D(tabMI_Rll, "SelectedRace",  { Text = "Select Race (s)",  Values = Tables.RaceList,  Multi = true })
T(tabMI_Rll, "AutoRace",      { Text = "Auto Roll Race",   Default = false })
D(tabMI_Rll, "SelectedClan",  { Text = "Select Clan (s)",  Values = Tables.ClanList,  Multi = true })
T(tabMI_Rll, "AutoClan",      { Text = "Auto Roll Clan",   Default = false })

D(tabMI_Trd, "SelectedTradePlr",   { Text = "Select Player",   Values = {}, Multi = false })
D(tabMI_Trd, "SelectedTradeItems", { Text = "Select Item (s)", Values = Tables.OwnedItem, Multi = true })
T(tabMI_Trd, "ReqTrade",       { Text = "Auto Send Request",  Default = false })
T(tabMI_Trd, "ReqTradeAccept", { Text = "Auto Accept Request",Default = false })
T(tabMI_Trd, "AutoAccept",     { Text = "Auto Accept Trade",  Default = false })

local secStats  = Tabs.Automation:AddSection({ Title = "Stats", Open = true })
local tabST_Lv  = secStats:AddTabbox({ Title = "Level", Icon = "lucide:bar-chart-2"  })
local tabST_Gem = secStats:AddTabbox({ Title = "Gem",   Icon = "lucide:gem"          })
local tabST_Mi  = secStats:AddTabbox({ Title = "Misc",  Icon = "lucide:more-horizontal" })

D(tabST_Lv, "SelectedStats", { Text = "Select Stat (s)", Values = { "Melee", "Defense", "Sword", "Power" }, Multi = true })
T(tabST_Lv, "AutoStats",     { Text = "Auto UP Stats",   Default = false })

L(tabST_Gem, "⚠️ Increase delay based on your ping.\n⚠️ Low delay recommended.\n- Reroll once for this to work.")
StatsLabel = L(tabST_Gem, "N/A")
D(tabST_Gem, "SelectedGemStats", { Text = "Select Stat (s)",  Values = Tables.GemStat, Multi = true })
D(tabST_Gem, "SelectedRank",     { Text = "Select Rank (s)",  Values = Tables.GemRank, Multi = true })
S(tabST_Gem, "StatsRollCD",      { Text = "Roll Delay",       Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(tabST_Gem, "AutoRollStats",    { Text = "Auto Roll Stats",  Default = false })

T(tabST_Mi, "AutoSkillTree",    { Text = "Auto Skill Tree",          Default = false })
T(tabST_Mi, "ArtifactMilestone",{ Text = "Auto Artifact Milestone",  Default = false })

local secEnchant  = Tabs.Automation:AddSection({ Title = "Enchant", Open = true })
local tabEN_Enc   = secEnchant:AddTabbox({ Title = "Enchant", Icon = "lucide:sparkles" })
local tabEN_Pas   = secEnchant:AddTabbox({ Title = "Passive", Icon = "lucide:shield"   })
local tabEN_Cfg   = secEnchant:AddTabbox({ Title = "Config",  Icon = "lucide:settings" })

D(tabEN_Enc, "SelectedEnchant",  { Text = "Select Enchant",  Values = Tables.OwnedAccessory, Multi = true })
T(tabEN_Enc, "AutoEnchant",      { Text = "Auto Enchant",     Default = false })
T(tabEN_Enc, "AutoEnchantAll",   { Text = "Auto Enchant All", Default = false })
D(tabEN_Enc, "SelectedBlessing", { Text = "Select Blessing",  Values = Tables.OwnedWeapon,    Multi = true })
T(tabEN_Enc, "AutoBlessing",     { Text = "Auto Blessing",     Default = false })
T(tabEN_Enc, "AutoBlessingAll",  { Text = "Auto Blessing All", Default = false })

SpecPassiveLabel = L(tabEN_Pas, "N/A")
D(tabEN_Pas, "SelectedPassive", { Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons, Multi = true })
D(tabEN_Pas, "SelectedSpec",    { Text = "Target Passives",   Values = Tables.SpecPassive,     Multi = true })
S(tabEN_Pas, "SpecRollCD",      { Text = "Roll Delay",        Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(tabEN_Pas, "AutoSpec",        { Text = "Auto Reroll Passive", Default = false })

L(tabEN_Cfg, "⚠️ Advanced users only.\nWill stop once passive reaches min value.")

-- ============================================================
-- ARTIFACT TAB
-- ============================================================
local secArtStatus  = Tabs.Artifact:AddSection({ Title = "Status",     Open = true })
local secArtEquip   = Tabs.Artifact:AddSection({ Title = "Auto-Equip", Open = true })
local secArtUpgrade = Tabs.Artifact:AddSection({ Title = "Upgrade",    Open = true })
local secArtLock    = Tabs.Artifact:AddSection({ Title = "Lock",       Open = true })
local secArtDelete  = Tabs.Artifact:AddSection({ Title = "Delete",     Open = true })
...
-- DUNGEON TAB
-- ============================================================
local secDungeonAuto = Tabs.Dungeon:AddSection({ Title = "Auto Join", Open = true })
local tabDG_Join     = secDungeonAuto:AddTabbox({ Title = "Autojoin", Icon = "lucide:door-open" })
local tabDG_Cfg      = secDungeonAuto:AddTabbox({ Title = "Config",   Icon = "lucide:settings"  })

L(tabDG_Join, "BossRush & Infinite Tower Supported.")
D(tabDG_Join, "SelectedDungeon",    { Text = "Select Dungeon", Values = Tables.DungeonList, Multi = false })
T(tabDG_Join, "AutoDungeon",        { Text = "Auto Join Dungeon",   Default = false })
T(tabDG_Join, "AutoInfiniteTower",  { Text = "Auto Infinite Tower", Default = false })

DungeonCount = L(tabDG_Cfg, "Dungeon Completed: N/A")
D(tabDG_Cfg, "SelectedDiff",   { Text = "Select Difficulty", Values = { "Easy", "Medium", "Hard", "Extreme" }, Multi = false })
T(tabDG_Cfg, "AutoDiff",       { Text = "Auto Select Difficulty", Default = false })
T(tabDG_Cfg, "AutoReplay",     { Text = "Auto Replay",           Default = false })
T(tabDG_Cfg, "DungeonAutofarm",{ Text = "Autofarm Dungeon",       Default = false })

-- ============================================================
-- PLAYER TAB
-- ============================================================
local secGeneral  = Tabs.Player:AddSection({ Title = "General",  Open = true })
local secServer   = Tabs.Player:AddSection({ Title = "Server",   Open = true })
local secGame     = Tabs.Player:AddSection({ Title = "Game",     Open = true })
local secSafety   = Tabs.Player:AddSection({ Title = "Safety",   Open = true })
...
L(secSafety, "Panic"):AddKeyPicker("PanicKeybind", { Default = "P", Text = "Panic" })
...
-- TELEPORT TAB
-- ============================================================
local secWaypoint = Tabs.Teleport:AddSection({ Title = "Waypoint", Open = true })
local tabWP_Isl   = secWaypoint:AddTabbox({ Title = "Island", Icon = "lucide:map"            })
local tabWP_Qst   = secWaypoint:AddTabbox({ Title = "Quest",  Icon = "lucide:scroll"         })
local tabWP_Msc   = secWaypoint:AddTabbox({ Title = "Misc",   Icon = "lucide:more-horizontal" })

D(tabWP_Isl, "SelectedIsland", {
    Text = "Select Island", Values = Tables.IslandList, Multi = false,
    Callback = function(a)
        if a then Remotes.TP_Portal:FireServer(a)
        else Library:Notify("Please select an island to teleport.", 2) end
    end,
})

D(tabWP_Qst, "SelectedQuestNPC", {
...
B(tabWP_Qst, "Teleport to Level Based Quest", function()
...
D(tabWP_Msc, "SelectedMiscAllNPC", {
...
local secNPCs   = Tabs.Teleport:AddSection({ Title = "NPCs", Open = true })
local tabNPC_MV = secNPCs:AddTabbox({ Title = "Moveset", Icon = "lucide:sword" })
local tabNPC_MS = secNPCs:AddTabbox({ Title = "Mastery", Icon = "lucide:star"  })

D(tabNPC_MV, "SelectedMovesetNPC", {
    Text = "Select NPC", Values = Tables.NPC_MovesetList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})
D(tabNPC_MS, "SelectedMasteryNPC", {
...
-- MISC TAB
-- ============================================================
local secMerchant  = Tabs.Misc:AddSection({ Title = "Merchant",   Open = true })
local tabMC_Reg    = secMerchant:AddTabbox({ Title = "Regular",   Icon = "lucide:shopping-cart" })
local tabMC_Dun    = secMerchant:AddTabbox({ Title = "Dungeon",   Icon = "lucide:door-open"     })
local tabMC_Val    = secMerchant:AddTabbox({ Title = "Valentine", Icon = "lucide:heart"         })

MerchantTimerLabel = L(tabMC_Reg, "Refresh: N/A")
D(tabMC_Reg, "SelectedMerchantItems", { Text = "Select Item (s)", Values = Tables.MerchantList, Multi = true })
T(tabMC_Reg, "AutoMerchant",          { Text = "Auto Buy Selected Items", Default = false })

L(tabMC_Dun, "Dungeon Merchant")
D(tabMC_Dun, "SelectedDungeonMerchantItems", { Text = "Select Item (s)", Values = Tables.DungeonMerchantList or {}, Multi = true })
T(tabMC_Dun, "AutoDungeonMerchant",          { Text = "Auto Buy Dungeon Items", Default = false })

L(tabMC_Val, "Able to cause performance issues.")
ValentineLabel = L(tabMC_Val, "Hearts: N/A")
D(tabMC_Val, "SelectedValentineMerchantItems", { Text = "Select Item (s)", Values = Tables.ValentineMerchantList, Multi = true })
ValentinePriceLabel = L(tabMC_Val, "Price: N/A")
T(tabMC_Val, "AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

local secMerchant2 = Tabs.Misc:AddSection({ Title = "Merchant 2", Open = true })
local tabMC2_Twr   = secMerchant2:AddTabbox({ Title = "Tower",     Icon = "lucide:tower" })
local tabMC2_BR    = secMerchant2:AddTabbox({ Title = "Boss Rush",  Icon = "lucide:skull" })

L(tabMC2_Twr, "Tower Merchant")
D(tabMC2_Twr, "SelectedTowerMerchantItems", { Text = "Select Item (s)", Values = Tables.InfiniteTowerMerchantList or {}, Multi = true })
T(tabMC2_Twr, "AutoTowerMerchant",          { Text = "Auto Buy Tower Items", Default = false })

L(tabMC2_BR, "Boss Rush Merchant")
D(tabMC2_BR, "SelectedBossRushMerchantItems", { Text = "Select Item (s)", Values = Tables.BossRushMerchantList or {}, Multi = true })
T(tabMC2_BR, "AutoBossRushMerchant",          { Text = "Auto Buy Boss Rush Items", Default = false })

local secMisc2  = Tabs.Misc:AddSection({ Title = "Misc", Open = true })
local tabMI2_Ch = secMisc2:AddTabbox({ Title = "Chests", Icon = "lucide:package" })
local tabMI2_Cr = secMisc2:AddTabbox({ Title = "Craft",  Icon = "lucide:hammer"  })
local tabMI2_Nt = secMisc2:AddTabbox({ Title = "Notify", Icon = "lucide:bell"    })

D(tabMI2_Ch, "SelectedChests", { Text = "Select Chest (s)", Values = Tables.Rarities, Multi = true })
T(tabMI2_Ch, "AutoChest",      { Text = "Auto Open Chest",  Default = false })

D(tabMI2_Cr, "SelectedCraftItems", { Text = "Select Item (s) To Craft", Values = Tables.CraftItemList, Multi = true })
T(tabMI2_Cr, "AutoCraftItem",      { Text = "Auto Craft Item",          Default = false })

T(tabMI2_Nt, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

-- Quests/Puzzles
local secQuests  = Tabs.Misc:AddSection({ Title = "Quests", Open = true })
local tabQS_Puz  = secQuests:AddTabbox({ Title = "Puzzles",    Icon = "lucide:puzzle" })
local tabQS_Qln  = secQuests:AddTabbox({ Title = "Questlines", Icon = "lucide:scroll" })

B(tabQS_Puz, { Text = "Complete Dungeon Puzzle",   Disabled = not Support.Proximity, Func = function()
...
B(tabQS_Puz, { Text = "Complete Slime Key Puzzle", Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Slime") end })
B(tabQS_Puz, { Text = "Complete Valentine Puzzle", Disabled = not Support.Proximity, Func = function()
    local Char = GetCharacter()
    ...
    for i = 1, 3 do
        ...
        if item then
            ...
            if prompt then
                Char.HumanoidRootPart.CFrame = item.CFrame * CFrame.new(0, 3, 0)
                task.wait(0.2); fireproximityprompt(prompt); task.wait(0.5)
            else Library:Notify("No prompt found in Heart" .. i, 2) end
        else Library:Notify("Heart" .. i .. " not found!", 2) end
    end
end })
B(tabQS_Puz, { Text = "Complete Demonite Puzzle",  Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Demonite") end })
B(tabQS_Puz, { Text = "Complete Hogyoku Puzzle",   Disabled = not Support.Proximity, Func = function()
    local lv = Plr.Data.Level.Value
    if lv >= 8500 then UniversalPuzzleSolver("Hogyoku")
    else Library:Notify("Level 8500 required! Current: " .. lv, 4) end
end })

L(tabQS_Qln, "⚠️ Experimental feature.\n⚠️ Store your race & clan before using.\n- Dungeon tasks only make you join dungeon.\n- Report bugs on discord!")
D(tabQS_Qln, "SelectedQuestline",        { Text = "Select Questline",       Values = Tables.QuestlineList,   Multi = false })
D(tabQS_Qln, "SelectedQuestline_Player", { Text = "Select Player",          Values = {},                     Multi = false })
D(tabQS_Qln, "SelectedQuestline_DMGTaken",{ Text = "Select Mob [Take Damage]", Values = Tables.AllEntitiesList, Multi = false })
B(tabQS_Qln, "Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)
T(tabQS_Qln, "AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })

-- ============================================================
-- CONFIG TAB
-- ============================================================
local secMenu = Tabs.Config:AddSection({ Title = "Menu", Open = true })

T(secMenu, "AutoShowUI", { Text = "Auto Show UI", Default = true })
T(secMenu, "ShowCustomCursor", { Text = "Custom Cursor", Default = false,
    Callback = function(v) Library.ShowCustomCursor = v end })
D(secMenu, "NotificationSide", {
    Text = "Notification Side", Values = { "Left", "Right" }, Default = "Right",
    Callback = function(v) Library:SetNotifySide(v) end,
})
D(secMenu, "DPIDropdown", {
    Text = "DPI Scale",
...
-- ============================================================
-- POST SETUP: SetValues setelah semua elemen dibuat
-- ============================================================
