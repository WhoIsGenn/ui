...
-- IslandCrystals: scan workspace otomatis untuk semua island
local IslandCrystals = {}
local function ScanIslandCrystals()
    -- Manual mapping berdasarkan nama folder workspace → portal name
    local islandMap = {
        -- Island name di workspace → portal key yang dikirim ke server
        ...
        -- Islands dari screenshot workspace
        ["BizarreIsland"]      = "BizarreIsland",
        ["BluePlanetIsland"]   = "BluePlanetIsland",
        ["BLUE PLANET"]        = "BluePlanetIsland",
        ["PunchIsland"]        = "PunchIsland",
        ["SlayerIsland"]       = "SlayerIsland",
        ["Slayer Island"]      = "SlayerIsland",
        ["WorldIsland"]        = "WorldIsland",
        ["StarterIsland_Sea2"] = "StarterIsland_Sea2",
        ["Lb"]                 = "Lb",
    }
    ...
    for wsName, portalKey in pairs(islandMap) do
        ...
        if folder then
            -- Cari crystal spawn point di dalam folder
            local crystal = folder:FindFirstChild("SpawnPointCrystal_" .. portalKey)
                or folder:FindFirstChildWhichIsA("BasePart")
                or folder:FindFirstChildWhichIsA("Model")
                or folder:FindFirstChildWhichIsA("Part")
            -- Kalau tidak ada crystal, pakai folder itu sendiri sebagai referensi posisi
            IslandCrystals[portalKey] = crystal or folder
        end
    end

    -- Scan otomatis folder workspace yang belum ada di map
    for _, child in ipairs(workspace:GetChildren()) do
        local name = child.Name
        -- Skip non-island folders
        if name:find("Island") or name:find("island") or name:find("Sea") then
            -- Cari key yang belum ada
            local alreadyMapped = false
            for _, key in pairs(IslandCrystals) do alreadyMapped = true; break end
            -- Tambahkan dengan nama folder sebagai key
            if not IslandCrystals[name] then
                local crystal = child:FindFirstChildWhichIsA("BasePart")
                    or child:FindFirstChildWhichIsA("Model")
                IslandCrystals[name] = crystal or child
            end
        end
    end
end

ScanIslandCrystals() -- jalankan sekali saat start
...
local function UpdateBossDropdowns()
    -- Boss list dari Modules sudah ada (TimedConfig + BossConfig)
    -- Tambahkan dari workspace untuk boss yang spawn sekarang
    ...
    if npcFolder then
        for _, obj in ipairs(npcFolder:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
                ...
                if isBoss then
                    if not table.find(Tables.BossList, cleanName) then
                        table.insert(Tables.BossList, cleanName)
                    end
                    if not table.find(Tables.AllBossList, cleanName) then
                        table.insert(Tables.AllBossList, cleanName)
                    end
                else
                    if not table.find(Tables.MobList, cleanName) then
                        table.insert(Tables.MobList, cleanName)
                    end
                end
            end
...
-- Juga fire proximity prompt langsung jika InstantPP ON
game:GetService("ProximityPromptService").PromptTriggerRequested:Connect(function(prompt, player)
    if player == Plr and Toggles.InstantPP and Toggles.InstantPP.Value then
        if Support.Proximity then
