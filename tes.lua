...
local function buildMonitor()
    ...
    ft.BackgroundTransparency=1 ft.Text="v35.0  •  Auto SK"
...
if R_Billboard then
    task.spawn(function()
        while true do
            task.wait(0.3)
            if S.AutoAnswer and not S.Busy then
                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                if pg then
                    for _, v in ipairs(pg:GetDescendants()) do
...
print("[SK] v35.0 | No Heartbeat scan | SK Manual panel")
WindUI:Notify({Title="Auto SK v35.0",Content="Loading KBBI...",Icon="zap",Duration=3})
