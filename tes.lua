...
function ESP.render(esp, player, char, cam, screenSize, screenCenter)
    ...
    if dist > Config.ESP_MaxDist then return end
    local isKillerPlayer = IsKiller(player)
    if isKillerPlayer and Config.ESP_Killer and Config.ESP_PlayerChams then
        local col = Cache.Visibility[player] and ChamsColors.KillerVis or ChamsColors.Killer
        Chams.Create(char, col, "killer")
    elseif not isKillerPlayer and Config.ESP_Survivor and Config.ESP_PlayerChams then
        local col = Cache.Visibility[player] and ChamsColors.SurvivorVis or ChamsColors.Survivor
        Chams.Create(char, col, "survivor")
    end
end
