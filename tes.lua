...
-- Intercept Spearthrow:FireServer lewat __namecall hook
-- Redirect arah spear ke target dengan kompensasi gravity
-- ================================================================
local Config_SpearSilent   = false  -- toggle silent aim
local Config_SpearSnapline = false  -- toggle snapline visual
local Config_SpearThrough  = false  -- through wall

-- Snapline drawing
local SpearSnapLine = Drawing.new("Line")
SpearSnapLine.Thickness    = 1
SpearSnapLine.Color        = Color3.fromRGB(255, 50, 50)
SpearSnapLine.Transparency = 1
SpearSnapLine.Visible      = false

local SpearRemote = nil  -- cached Spearthrow remote
...
local function SpearSilent_CalcAimPos(targetPos)
    local root = GetCharacterRoot()
...
local function SpearSilent_Hook()
    ...
    if not mt or not setreadonly then
        SpearHookActive = false; return
    end
    setreadonly(mt, false)
    local oldNC = mt.__namecall
    mt.__namecall = newcclosure(function(self, ...)
        if Config_SpearSilent and self == SpearSilent_GetRemote() then
            local args = {...}
            -- Spearthrow FireServer args biasanya: (direction: Vector3, ...)
            -- Kita redirect direction ke target
            local target = SpearSilent_GetTarget()
            if target and target.Character then
                local pr = target.Character:FindFirstChild("HumanoidRootPart")
                if pr then
                    local aimPos  = SpearSilent_CalcAimPos(pr.Position)
                    local root    = GetCharacterRoot()
                    if root then
                        local dir = (aimPos - (root.Position + Vector3.new(0,2,0))).Unit
                        -- Replace arg pertama (direction) dengan arah ke target
...
local function UpdateSpearSnapline(cam, screenCenter)
    if not Config_SpearSnapline or not Config_SpearSilent then
    ...
    local aimPos      = SpearSilent_CalcAimPos(pr.Position)
    local screenAim, onScreen = WorldToScreen(aimPos)
    if onScreen then
        SpearSnapLine.From    = screenCenter
        SpearSnapLine.To      = screenAim
