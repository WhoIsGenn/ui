    ...
    local function watchPlayer(p)
        ...
        local function hookChar(char)
            ...
            local conn = animator.AnimationPlayed:Connect(function(track)
                ...
                -- Hanya detect dari killer
                if not IsKiller(p) then return end
                local id = track.Animation.AnimationId
                local numId = id:match("%d+$") or id:match("id=(%d+)") or ""
                -- Skip idle/walk animations yang diketahui
                if IdleWalkAnimIDs[numId] then return end
                -- Cek jarak
            ...
            local visConn = Check:GetPropertyChangedSignal("Visible"):Connect(function()
                ...
                if Check.Visible then
                    ...
                    hbConn = RunService.Heartbeat:Connect(function()
                        ...
                        if inGoal then
                            pcall(function()
                                -- Mobile: tap "action" button di Survivor-mob Controls
                                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                                local actionBtn = nil
                                if pg then
                                    for _, gui in ipairs(pg:GetChildren()) do
                                        ...
                                        if controls then
                                            actionBtn = controls:FindFirstChild("action")
                                            if actionBtn then break end
                                        end
                                    end
                                end
                                if actionBtn then
