...
local Config = {
    ...
    AIM_FOV = 120,
    AIM_Smooth = 0.3,
    AIM_TargetPart = "HumanoidRootPart",
    AIM_VisCheck = true,
    AIM_ShowFOV = false,
    AIM_Crosshair = false,
    AIM_Predict = true,
...
local function UpdateGeneratorProgress(gen)
    ...
    local adornee = gen:FindFirstChild("defaultMaterial", true)
        or gen:FindFirstChildWhichIsA("BasePart") or gen
    ...
    local barFill = tag:FindFirstChild("_VHGenBarBg") and tag._VHGenBarBg:FindFirstChild("_VHGenBarFill")
    if barFill then
...
local function Aimbot_GetTargetPart(char)
    ...
    if targetPartName == "Left Arm" then
        part = char:FindFirstChild("LeftArm") or char:FindFirstChild("Left Arm")
        if part then return part end
    end
    if targetPartName == "Torso" then
        part = char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso")
        if part then return part end
    end
    if targetPartName == "UpperTorso" then
        -- UpperTorso tidak selalu ada di workspace, fallback ke Torso/HRP
        part = char:FindFirstChild("UpperTorso")
            or char:FindFirstChild("Torso")
            or char:FindFirstChild("HumanoidRootPart")
        if part then return part end
    end
    -- Final fallback: HumanoidRootPart selalu ada
...
-- PARRY FOV: pure Drawing lines, world-space circle di lantai
-- Sama persis tesvide.lua — proven works
local PFOV_N    = 24
local PFOVLines = {}
for i = 1, PFOV_N do
    ...
    ln.Transparency = 1   -- 1 = OPAQUE di Drawing API
    ln.Visible      = false
    PFOVLines[i]    = ln
end
...
local function ParryDisk_Remove() ParrySphere_Remove() end
...
local function UpdateParryFOV()
    ...
    local ox    = myRoot.Position.X
    local oz    = myRoot.Position.Z
    local col   = ParryOnCooldown and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(0, 170, 255)
    for i = 1, PFOV_N do
...
-- ================================================================
-- SPEAR SNAPLINE (aimbot camera only, no silent aim)
-- ================================================================
        ...
        if p ~= LocalPlayer and IsSurvivor(p) and p.Character then
            ...
            if pr then
                ...
                if Config_SpearThrough or vis then
                    ...
                    -- Max range 120 studs
                    if d < bd and d < 120 then bd = d; best = p end
                end
            end
        end
    end
...
-- Snapline: dari HRP kita ke HRP survivor terdekat (simple, accurate)
local function UpdateSpearSnapline(cam, screenCenter)
    ...
    -- From: HRP kita (setinggi torso)
    local fromSP, fromOn = WorldToScreen(myRoot.Position + Vector3.new(0, 1, 0))
    if not fromOn then SpearSnapLine.Visible = false; return end
    -- To: HRP survivor terdekat
    local toSP, toOn = WorldToScreen(pr.Position + Vector3.new(0, 1, 0))
    if toOn then
        SpearSnapLine.From = fromSP; SpearSnapLine.To = toSP
...
local function AutoParry_HookKillerChar(char, player)
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and kr then
                ...
                if dist <= Config.PARRY_Dist then
                    -- Direction check: killer harus facing ke kita (dot product)
                    local killerLook = kr.CFrame.LookVector
                    local toMe = (myRoot.Position - kr.Position).Unit
                    local dot = killerLook:Dot(toMe)
                    -- dot > 0.3 artinya killer menghadap kita (dalam ~72 derajat)
                    if dot > 0.3 then
                        AutoParry_TryFire()
                    end
                end
        ...
        local conn2 = RunService.Heartbeat:Connect(function()
            ...
            -- Direction check: killer harus facing ke kita
            local killerLook = kr.CFrame.LookVector
            local toMe = (myRoot.Position - kr.Position).Unit
            local dot = killerLook:Dot(toMe)
            if dot <= 0.3 then return end  -- killer tidak menghadap kita, skip
...
-- Speed di RenderStepped supaya tidak di-override server tiap frame
local function Speed_StartLoop()
    if SpeedRenderConn then SpeedRenderConn:Disconnect() end
    SpeedRenderConn = RunService.RenderStepped:Connect(function()
        if State.Unloaded then SpeedRenderConn:Disconnect(); return end
        if not Config.SPEED_Enabled then return end
        local char = LocalPlayer.Character
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then hum.WalkSpeed = Config.SPEED_Value end
    end)
end
Speed_StartLoop()
...
local function UpdateFly()
    ...
    if Config.FLY_Enabled then
        ...
        -- Mobile: thumbstick via MoveDirection
        if moveDir.Magnitude < 0.1 then
            local md = hum.MoveDirection
            if md.Magnitude > 0.1 then moveDir = md end
        end
...
-- Fly di RenderStepped agar smooth dan tidak di-reset server
local function Fly_StartLoop()
    if FlyRenderConn then FlyRenderConn:Disconnect() end
    FlyRenderConn = RunService.RenderStepped:Connect(function()
        if State.Unloaded then FlyRenderConn:Disconnect(); return end
        if not Config.FLY_Enabled then return end
        local char = LocalPlayer.Character
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        -- Paksa PlatformStand tiap frame supaya tidak di-reset
...
-- ================================================================
-- KILLER FEATURES IMPLEMENTATIONS
-- ================================================================

-- HITBOX EXPANDER: Perbesar HumanoidRootPart survivor
local HitboxConns = {}
local function UpdateHitboxes()
    if not Config.HITBOX_Enabled or GetRole() ~= "Killer" then
        -- Restore semua hitbox
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                local hrp = p.Character:FindFirstChild("HumanoidRootPart")
                if hrp and hrp:GetAttribute("_VHOrigSize") then
                    local orig = hrp:GetAttribute("_VHOrigSize")
                    if orig then hrp.Size = Vector3.new(orig.X, orig.Y, orig.Z) end
                    hrp:SetAttribute("_VHOrigSize", nil)
                end
            end
        end
        return
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and IsSurvivor(p) and p.Character then
            local hrp = p.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                if not hrp:GetAttribute("_VHOrigSize") then
                    hrp:SetAttribute("_VHOrigSize", hrp.Size)
                end
...
-- AUTO HOOK: Teleport survivor terdekat ke hook terdekat
local lastAutoHookTime = 0
local function AutoHook()
    if not Config.KILLER_AutoHook or GetRole() ~= "Killer" then return end
    if tick() - lastAutoHookTime < 3 then return end
    if not Cache.ClosestHook or not Cache.ClosestHook.part then return end
    local hookPos = Cache.ClosestHook.part.Position
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and IsSurvivor(p) and p.Character then
            local hrp = p.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                ...
                if dist < 30 then
                    hrp.CFrame = CFrame.new(hookPos + Vector3.new(0, 3, 0))
...
-- NO PALLET STUN: Set WalkSpeed kembali tiap frame saat kena stun
local NoPalletStunConn = nil
local function NoPalletStun_Setup()
    if NoPalletStunConn then NoPalletStunConn:Disconnect(); NoPalletStunConn = nil end
    if not Config.KILLER_NoPalletStun then return end
    NoPalletStunConn = RunService.Heartbeat:Connect(function()
        if not Config.KILLER_NoPalletStun or GetRole() ~= "Killer" then
            NoPalletStunConn:Disconnect(); NoPalletStunConn = nil; return
        end
        local char = LocalPlayer.Character
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        -- Kalau WalkSpeed tiba-tiba jadi 0 atau sangat kecil, restore
        if hum and hum.WalkSpeed < 5 and hum.WalkSpeed >= 0 then
            hum.WalkSpeed = OriginalSpeedStore > 5 and OriginalSpeedStore or 16
        end
    end)
end

-- ANTI BLIND: Disable BlurEffect dari lighting saat aktif
local AntiBlindConn = nil
local function AntiBlind_Setup()
    if AntiBlindConn then AntiBlindConn:Disconnect(); AntiBlindConn = nil end
    if not Config.KILLER_AntiBlind then
        -- Re-enable semua blur
        for _, v in ipairs(Lighting:GetChildren()) do
            if v:IsA("BlurEffect") then pcall(function() v.Enabled = true end) end
        end
        return
    end
    local function disableBlur()
        for _, v in ipairs(Lighting:GetChildren()) do
            if v:IsA("BlurEffect") then pcall(function() v.Enabled = false end) end
        end
    end
    disableBlur()
    AntiBlindConn = Lighting.ChildAdded:Connect(function(v)
        if v:IsA("BlurEffect") and Config.KILLER_AntiBlind then
            task.defer(function() pcall(function() v.Enabled = false end) end)
        end
    end)
end

-- NO SLOWDOWN: Prevent speed reduction (sama seperti NoPalletStun tapi untuk semua slow)
local NoSlowdownConn = nil
local function NoSlowdown_Setup()
    if NoSlowdownConn then NoSlowdownConn:Disconnect(); NoSlowdownConn = nil end
    if not Config.KILLER_NoSlowdown then return end
    NoSlowdownConn = RunService.Heartbeat:Connect(function()
        if not Config.KILLER_NoSlowdown or GetRole() ~= "Killer" then
            NoSlowdownConn:Disconnect(); NoSlowdownConn = nil; return
        end
        local char = LocalPlayer.Character
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            local baseSpeed = Config.SPEED_Enabled and Config.SPEED_Value or OriginalSpeedStore
            if baseSpeed < 5 then baseSpeed = 16 end
            if hum.WalkSpeed < baseSpeed then hum.WalkSpeed = baseSpeed end
        end
    end)
end

-- DOUBLE TAP: Auto attack 2x berturut-turut dengan delay minimal
...
local function DoubleTap()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and IsSurvivor(p) and p.Character then
            local hrp = p.Character:FindFirstChild("HumanoidRootPart")
            if hrp and (hrp.Position - root.Position).Magnitude <= Config.AUTO_AttackRange then
                task.spawn(function()
                    TriggerAttack()
                    task.wait(0.1)
                    TriggerAttack()
...
-- INFINITE LUNGE: Hold lunge state terus
local InfiniteLungeConn = nil
local function InfiniteLunge_Setup()
    if InfiniteLungeConn then InfiniteLungeConn:Disconnect(); InfiniteLungeConn = nil end
    if not Config.KILLER_InfiniteLunge then return end
    InfiniteLungeConn = RunService.Heartbeat:Connect(function()
        if not Config.KILLER_InfiniteLunge or GetRole() ~= "Killer" then
            InfiniteLungeConn:Disconnect(); InfiniteLungeConn = nil; return
        end
        local char = LocalPlayer.Character
        if not char then return end
        -- Set attribute lunge agar tidak habis
        pcall(function() char:SetAttribute("Lunging", true) end)
        pcall(function() char:SetAttribute("LungeTime", math.huge) end)
    end)
end

-- FULL GEN BREAK: Trigger break event pada semua generator
local lastGenBreakTime = 0
local function FullGenBreak()
    if not Config.KILLER_FullGenBreak or GetRole() ~= "Killer" then return end
    if tick() - lastGenBreakTime < 5 then return end
    lastGenBreakTime = tick()
    -- Cari remote BreakGenerator
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if not remotes then return end
    for _, remote in ipairs(remotes:GetDescendants()) do
        if remote:IsA("RemoteEvent") and (
            remote.Name:lower():find("break") or
            remote.Name:lower():find("gen") or
            remote.Name:lower():find("sabotage")
        ) then
            for _, g in ipairs(Cache.Generators) do
                if g.model then
                    pcall(function() remote:FireServer(g.model) end)
                end
            end
        end
    end
...
-- DESTROY PALLETS: Hapus semua pallet dari map
local function DestroyAllPallets()
    if not Config.KILLER_DestroyPallets or GetRole() ~= "Killer" then return end
    for _, p in ipairs(Cache.Pallets) do
        if p.model and p.model.Parent then
            pcall(function()
                -- Cari pallet drop remote
                local remotes = ReplicatedStorage:FindFirstChild("Remotes")
                if remotes then
                    for _, r in ipairs(remotes:GetDescendants()) do
                        if r:IsA("RemoteEvent") and r.Name:lower():find("pallet") then
                            pcall(function() r:FireServer(p.model) end)
                        end
                    end
                end
                -- Fallback: destroy langsung (local only)
                p.model:Destroy()
            end)
        end
    end
...
-- ================================================================
-- AUTO PARRY — TAMBAH DIRECTION CHECK (SixSense style)
-- Killer harus facing ke kita agar parry trigger
-- ================================================================
...
local function Unload()
    ...
    pcall(RestoreFog)
    pcall(GodMode_Stop)
    if NoPalletStunConn then pcall(function() NoPalletStunConn:Disconnect() end) end
    if AntiBlindConn     then pcall(function() AntiBlindConn:Disconnect()     end) end
    if NoSlowdownConn    then pcall(function() NoSlowdownConn:Disconnect()     end) end
    if InfiniteLungeConn then pcall(function() InfiniteLungeConn:Disconnect() end) end
    if SpeedRenderConn   then pcall(function() SpeedRenderConn:Disconnect()   end) end
    if FlyRenderConn     then pcall(function() FlyRenderConn:Disconnect()     end) end
...
secAim:AddDropdown({ Title = "Target Part", Options = { "HumanoidRootPart", "UpperTorso", "Torso", "Left Arm" }, Default = Config.AIM_TargetPart, Callback = function(v) Config.AIM_TargetPart = v end })
...
-- secSpear:AddToggle({ Title = "Spear Silent Aim", Default = false, Callback = function(v)
    
end })
secSpear:AddSlider({ Title = "Spear Gravity", Min = 10, Max = 500, Default = Config.SPEAR_Gravity, Callback = function(v) Config.SPEAR_Gravity = v end })
secSpear:AddSlider({ Title = "Spear Speed", Min = 50, Max = 500, Default = Config.SPEAR_Speed, Callback = function(v) Config.SPEAR_Speed = v end })
secSpear:AddToggle({ Title = "Snapline", Default = false, Callback = function(v) Config_SpearSnapline = v end })
secSpear:AddToggle({ Title = "Through the Veil", Default = false, Callback = function(v) Config_SpearThrough = v end })
...
secCombat:AddToggle({ Title = "Infinite Lunge", Default = Config.KILLER_InfiniteLunge, Callback = function(v) Config.KILLER_InfiniteLunge = v; InfiniteLunge_Setup() end })
...
secProtection:AddToggle({ Title = "No Pallet Stun", Default = Config.KILLER_NoPalletStun, Callback = function(v) Config.KILLER_NoPalletStun = v; NoPalletStun_Setup() end })
secProtection:AddToggle({ Title = "Anti Blind", Default = Config.KILLER_AntiBlind, Callback = function(v) Config.KILLER_AntiBlind = v; AntiBlind_Setup() end })
secProtection:AddToggle({ Title = "No Slowdown", Default = Config.KILLER_NoSlowdown, Callback = function(v) Config.KILLER_NoSlowdown = v; NoSlowdown_Setup() end })

local secDestruction = Tabs.KILLER:AddSection({ Title = "Destruction", Open = false })
secDestruction:AddToggle({ Title = "Full Gen Break", Default = Config.KILLER_FullGenBreak, Callback = function(v) Config.KILLER_FullGenBreak = v; if v then FullGenBreak() end end })
secDestruction:AddToggle({ Title = "Destroy All Pallets", Default = Config.KILLER_DestroyPallets, Callback = function(v) Config.KILLER_DestroyPallets = v; if v then DestroyAllPallets() end end })
