...
function LoadConfigElements()
    _G.LoadingConfig = true  -- Flag loading ON
    ...
    task.wait(0.2)
    _G.LoadingConfig = false  -- Flag loading OFF
end
-- ==================== CREATE MAIN WINDOW ====================
...
-- ==================== CONFIG MANAGER ====================
local configSection = Tab9:AddSection("Config Manager")

-- Variables untuk menyimpan state
local currentConfigName = "None"
local autoloadConfigName = "None"
local selectedConfigFromList = nil
local configList = {}

-- Function untuk update config list
local function updateConfigList()
    configList = {}
    if listfiles then
        local files = listfiles("VictoriaHub/Config")
        for _, file in pairs(files) do
            local name = file:gsub("VictoriaHub/Config/", ""):gsub(".json", "")
...
local function updateConfigDisplay()
    ...
    if ConfigDisplay then
        ConfigDisplay:Set("Current: " .. currentConfigName .. " | Autoload: " .. autoloadConfigName)
    end
end

-- Display Status
ConfigDisplay = configSection:AddParagraph({
    Title = "Config Manager",
    Content = "Current: " .. currentConfigName .. " | Autoload: " .. autoloadConfigName
})
...
-- Dropdown untuk select config dari list
configSection:AddDropdown({
    Title = "Select Config",
    Values = updateConfigList(),
    Default = 1,
    Callback = function(v)
        selectedConfigFromList = v
    end
})

-- Dropdown untuk select option (opsional, tergantung kebutuhan)
configSection:AddDropdown({
    Title = "Select Option",
    Values = {"Option 1", "Option 2", "Option 3"},
    Default = 1,
    Callback = function(v)
        -- Handler untuk option
    end
})
...
configSection:AddButton({
    Title = "Save Config",
    Description = "Simpan config saat ini",
    Callback = function()
        local saveName = configNameInput ~= "" and configNameInput or selectedConfigFromList
        if saveName and saveName ~= "" then
            SaveConfig(saveName)
            currentConfigName = saveName
            updateConfigDisplay()
            updateConfigList()
            -- notif("Config saved: " .. saveName, 3, Color3.fromRGB(0, 255, 0))
        else
...
configSection:AddButton({
    Title = "Load Config",
    Description = "Load config yang dipilih",
    Callback = function()
        local loadName = selectedConfigFromList
        if loadName and loadName ~= "" then
            local loaded = LoadConfigFromFile(loadName)
            if loaded then
                currentConfigName = loadName
                updateConfigDisplay()
                -- notif("Config loaded: " .. loadName, 3, Color3.fromRGB(0, 255, 0))
            else
                -- notif("Failed to load config", 3, Color3.fromRGB(255, 0, 0))
            end
        else
            -- notif("Please select a config", 3, Color3.fromRGB(255, 0, 0))
        end
    end
})

-- Row 2: Delete Config & Set Autoload
configSection:AddButton({
    Title = "Delete Config",
    Description = "Hapus config yang dipilih",
    Callback = function()
        local deleteName = selectedConfigFromList
        if deleteName and deleteName ~= "" then
            local fileName = "VictoriaHub/Config/" .. deleteName .. ".json"
            
            if delfile and isfile(fileName) then
                delfile(fileName)
                if currentConfigName == deleteName then
                    currentConfigName = "None"
                end
                if autoloadConfigName == deleteName then
                    autoloadConfigName = "None"
                end
                updateConfigDisplay()
                updateConfigList()
                -- notif("Config deleted: " .. deleteName, 3, Color3.fromRGB(0, 255, 0))
            else
                -- notif("Config not found", 3, Color3.fromRGB(255, 0, 0))
            end
        else
            -- notif("Please select a config", 3, Color3.fromRGB(255, 0, 0))
        end
...
configSection:AddButton({
    Title = "Set Autoload",
    Description = "Set config untuk autoload",
    Callback = function()
        local autoloadName = selectedConfigFromList
        if autoloadName and autoloadName ~= "" then
            autoloadConfigName = autoloadName
            -- Save autoload setting
            if writefile then
                writefile("VictoriaHub/Config/autoload.txt", autoloadName)
            end
            updateConfigDisplay()
            -- notif("Autoload set: " .. autoloadName, 3, Color3.fromRGB(0, 255, 0))
        else
            -- notif("Please select a config", 3, Color3.fromRGB(255, 0, 0))
        end
    end
...
-- Row 3: Refresh List & Clear Autoload
configSection:AddButton({
    Title = "Refresh List",
    Description = "Refresh daftar config",
    Callback = function()
        local updatedList = updateConfigList()
        -- Update dropdown jika perlu
        -- notif("Config list refreshed", 3, Color3.fromRGB(0, 255, 0))
    end
})

configSection:AddButton({
    Title = "Clear Autoload",
    Description = "Hapus pengaturan autoload",
    Callback = function()
        autoloadConfigName = "None"
        if delfile and isfile("VictoriaHub/Config/autoload.txt") then
            delfile("VictoriaHub/Config/autoload.txt")
        end
        updateConfigDisplay()
...
configSection:AddButton({
    ...
    Description = "Reset semua pengaturan ke default",
    Callback = function()
        ...
        ConfigData = {}
        updateConfigDisplay()
        -- notif("All elements reset to default", 3, Color3.fromRGB(0, 255, 0))
    end
})

-- Export Config (jika diperlukan)
configSection:AddButton({
    Title = "Export Config",
    Description = "Export config ke clipboard",
    Callback = function()
        if selectedConfigFromList and selectedConfigFromList ~= "" then
            local fileName = "VictoriaHub/Config/" .. selectedConfigFromList .. ".json"
...
-- Load autoload config on start
spawn(function()
    wait(1)
    if isfile and isfile("VictoriaHub/Config/autoload.txt") then
        local autoloadName = readfile("VictoriaHub/Config/autoload.txt")
        if autoloadName and autoloadName ~= "" then
            local loaded = LoadConfigFromFile(autoloadName)
            if loaded then
                currentConfigName = autoloadName
                autoloadConfigName = autoloadName
                updateConfigDisplay()
            end
        end
...
-- Update config list saat pertama kali
updateConfigList()
