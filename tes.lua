...
local Toggles = setmetatable({}, {
    __index = function(_, k)
        if not _flags[k] then
            _flags[k] = { Value = false, _cb = {},
                OnChanged = function(self, fn) table.insert(self._cb, fn) end,
                SetValue  = function(self, v) self.Value = v; for _, f in ipairs(self._cb) do pcall(f, v) end end,
            }
        end
        return _flags[k]
    end
...
local Options = setmetatable({}, {
    __index = function(_, k)
        local fk = "o_"..k
        if not _flags[fk] then
            _flags[fk] = { Value = nil, _cb = {}, _ref = nil,
                OnChanged  = function(self, fn) table.insert(self._cb, fn) end,
                SetValue   = function(self, v) self.Value = v; for _, f in ipairs(self._cb) do pcall(f, v) end end,
                SetValues  = function(self, v) self._vals = v; if self._ref and self._ref.SetValues then pcall(self._ref.SetValues, self._ref, v) end end,
                SetText    = function(self, t) if self._ref and self._ref.SetContent then pcall(self._ref.SetContent, self._ref, t) end end,
                SetVisible = function(self, v) if self._ref and self._ref.SetVisible then pcall(self._ref.SetVisible, self._ref, v) end end,
            }
        end
        return _flags["o_"..k]
    end
...
local Library = {
    Notify     = function(self, msg, dur)
        if VelarisUI then pcall(function() VelarisUI:MakeNotify({ Title = "Victoria Hub", Description = msg, Time = dur or 3 }) end) end
    end,
    Unload     = function(self) if Window and Window.Destroy then pcall(function() Window:Destroy() end) end end,
    Toggle     = function(self) if Window and Window.Toggle then pcall(function() Window:Toggle() end) end end,
...
local omg = {
    13820188365, 13413231458, 5638697306, 16660143487, 12669880433,
    121554255694758, 14287111618, 16119081646, 15868464144, 13699729039,
    10502160439, 76020690430974, 92696084646822,
}
...
-- IslandCrystals: posisi referensi tiap island untuk GetIslandForTarget
local IslandCrystals = {}

local function GetFolderPos(folder)
    -- Cari BasePart pertama di dalam folder untuk dapat posisinya
...
local function ScanIslandCrystals()
    IslandCrystals = {}
    -- Mapping: nama folder workspace → nama portal yang dikirim ke server
    local islandMap = {
        ["StarterIsland"]      = "Starter",
        ["JungleIsland"]       = "Jungle",
        ["DesertIsland"]       = "Desert",
        ["SnowIsland"]         = "Snow",
        ["SailorIsland"]       = "Sailor",
        ["ShibuyaStation"]     = "Shibuya",
        ["HollowIsland"]       = "HollowIsland",
        ["BossIsland"]         = "Boss",
        ["DungeonIsland"]      = "Dungeon",
        ["ShinjukuIsland"]     = "Shinjuku",
        ["SlimeIsland"]        = "Slime",
        ["AcademyIsland"]      = "Academy",
        ["JudgementIsland"]    = "Judgement",
        ["SoulDominionIsland"] = "SoulDominion",
        ["NinjaIsland"]        = "NinjaIsland",
        ["LawlessIsland"]      = "LawlessIsland",
        ["TowerIsland"]        = "TowerIsland",
        ["WorldIsland"]        = "WorldIsland",
        ["Lb"]                 = "Lb",
        ["Alx"]                = "Alx",
        -- Sea 2 islands dari screenshot
        ["HuecoMundo"]         = "HuecoMundo",
        ["ValentineIsland"]    = "Valentine",
        -- Note: Sea2MapQuest, EasterEggs, _Development, Effects, Decorations = bukan island yang bisa diTP
    }

    for wsName, portalKey in pairs(islandMap) do
        local folder = workspace:FindFirstChild(wsName)
        if folder then
            -- Simpan posisi, bukan folder object, untuk hindari GetPivot error
            local pos = GetFolderPos(folder)
            if pos then
                IslandCrystals[portalKey] = pos
            end
        end
    end
end
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("VictoriaHub • Session: %s • %s", GetSessionTime(), os.date("%x %X")) },
...
local function Func_NoGameplayPaused()
    while Toggles.NoGameplayPaused.Value do
        pcall(function()
            -- Method 1: destroy pause GUI
            local coreGui = game:GetService("CoreGui")
            local robloxGui = coreGui:FindFirstChild("RobloxGui")
            if robloxGui then
                for _, name in ipairs({"CoreScripts/NetworkPause","NetworkPauseScreen","GamepadMenu"}) do
                    local g = robloxGui:FindFirstChild(name, true)
                    if g then g:Destroy() end
                end
            end
            -- Method 2: disable via GuiService
            local ok = pcall(function()
                game:GetService("GuiService").MenuIsOpen = false
            end)
            -- Method 3: click resume button jika ada
            for _, gui in ipairs(coreGui:GetDescendants()) do
                if gui:IsA("TextButton") or gui:IsA("ImageButton") then
                    local t = gui.Text or ""
                    if t:find("Resume") or t:find("Continue") or t:find("OK") then
                        pcall(function() gui:Fire("Activated") end)
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "VictoriaHub • " .. os.date("%x %X") }
        }}
    }
...
local function GetNearestIsland(targetPos, npcName)
    ...
    for islandName, posOrObj in pairs(IslandCrystals) do
        local pos
        if typeof(posOrObj) == "Vector3" then
            pos = posOrObj
        elseif typeof(posOrObj) == "Instance" then
            if posOrObj:IsA("BasePart") then pos = posOrObj.Position
            else
                local p = posOrObj:FindFirstChildWhichIsA("BasePart")
...
local function UpdateNPCLists()
    ...
    for _, v in pairs(PATH.Mobs:GetChildren()) do
        ...
        if (isSpecial or not cleanName:find("Boss")) and not currentList[cleanName] then
            ...
            for islandName, posOrObj in pairs(IslandCrystals) do
                local pos
                if typeof(posOrObj) == "Vector3" then pos = posOrObj
                elseif typeof(posOrObj) == "Instance" then
                    if posOrObj:IsA("BasePart") then pos = posOrObj.Position
                    else local p = posOrObj:FindFirstChildWhichIsA("BasePart"); if p then pos = p.Position end end
                end
                if pos then
...
-- Fungsi yang dibutuhkan UI
local function GetData()
    return "<font color='#FFD700'>Premium User</font>", "Lifetime"
end

local ShortMap = {
    ["DamagePercent"]="DMG%", ["CritChance"]="CC%", ["CritDamage"]="CD%",
    ["BonusDropChance"]="Drop%", ["ExecuteBonus"]="BonusDMG%", ["ExecuteThreshold"]="Exec%",
    ["Damage"]="DMG", ["Crit Chance"]="CC", ["Crit Damage"]="CD", ["Luck"]="Luck"
}
...
local function Shorten(text)
    for _,long in ipairs(SortedShortKeys) do text=text:gsub(long,ShortMap[long]:gsub("%%","%%%%")) end
    return text
end

local sfEN_Cfg_ref = nil
local function UpdatePassiveSliders()
    local sel = Options.SelectedSpec.Value or {}
    for _,s in pairs(Shared.SpecStatsSlider or {}) do pcall(function() s:SetVisible(false) end) end
    if not sfEN_Cfg_ref then return end
    for pName, isSel in pairs(sel) do
        if isSel then
            local data = Modules.SpecPassive.Passives[pName]
            if data and data.Buffs then
                for statKey, range in pairs(data.Buffs) do
                    local sid = "Min_"..pName:gsub("%s+","").."_"..statKey
                    if not (Shared.SpecStatsSlider and Shared.SpecStatsSlider[sid]) then
                        S(sfEN_Cfg_ref, sid, {Text=Shorten(pName).." ["..Shorten(statKey).."]", Default=range[1], Min=range[1], Max=range[2], Rounding=1})
                        if Shared.SpecStatsSlider then Shared.SpecStatsSlider[sid]=Options[sid] end
                    end
                    if Shared.SpecStatsSlider and Shared.SpecStatsSlider[sid] then
                        pcall(function() Shared.SpecStatsSlider[sid]:SetVisible(true) end)
                    end
                end
            end
        end
...
local Window = VelarisUI:Window({
    Title = "Victoria Hub",
    Footer = "" .. assetName .. " | Gennskie Development",
    Content = "Sailor Piece",
    Color = "Default",
    Version = 1.0,
    ["Tab Width"] = 120,
    Image = tostring(theChosenOne),
    Configname = "VictoriaHub_SailorPiece",
    Uitransparent = 0.15,
    ShowUser = true,
    Search = true,
    Animation = true,
...
local Tabs = {
    ...
    Artifact    = Window:AddTab({ Name = "Artifact",    Icon = "lucide:star"          }),
    ...
    Webhook     = Window:AddTab({ Name = "Webhook",     Icon = "lucide:send"          }),
    Misc        = Window:AddTab({ Name = "Misc",        Icon = "lucide:layers"        }),
    Config      = Window:AddTab({ Name = "Config",      Icon = "lucide:cog"           }),
}

-- Helpers
local function T(sec, id, cfg)
    local flag = Toggles[id]; flag.Value = cfg.Default or false
    sec:AddToggle({
        Title = cfg.Text or id, Default = cfg.Default or false, Locked = cfg.Disabled or false,
        Callback = function(v)
            flag.Value = v
            for _, f in ipairs(flag._cb) do pcall(f, v) end
...
local function D(sec, id, cfg)
    local flag = Options[id]; flag.Value = cfg.Default
    local isMulti = cfg.Multi or false

    -- Convert Brigida array → Obsidian dict format for Multi dropdowns
    local function convertValue(v)
        if isMulti and type(v) == "table" then
            -- Cek apakah sudah dict format {["x"]=true}
            local isArray = (#v > 0)
            if isArray then
                local dict = {}
                for _, item in ipairs(v) do dict[item] = true end
                return dict
            end
        end
        return v
    ...
    -- Convert default value juga
    if isMulti and type(cfg.Default) == "table" then
        local isArray = (#cfg.Default > 0)
        if isArray then
            local dict = {}
            for _, item in ipairs(cfg.Default) do dict[item] = true end
            flag.Value = dict
        end
    end

    local el = sec:AddDropdown({
        Title = cfg.Text or id, Options = cfg.Values or cfg.Options or {},
        Multi = isMulti, Default = cfg.Default, Locked = cfg.Disabled or false,
        Callback = function(v)
            local converted = convertValue(v)
            flag.Value = converted
            for _, f in ipairs(flag._cb) do pcall(f, converted) end
            if cfg.Callback then pcall(cfg.Callback, converted) end
        end,
    })
    flag._ref = el
    flag.SetValues = function(self, vals)
        self._vals = vals
        if el and el.SetValues then pcall(function() el:SetValues(vals) end) end
    end
    return flag
end

local function S(sec, id, cfg)
    local flag = Options[id]; flag.Value = cfg.Default or 0
    sec:AddSlider({
        Title = cfg.Text or id, Content = cfg.Content or "",
        Min = cfg.Min or 0, Max = cfg.Max or 100, Default = cfg.Default or 0,
        Increment = cfg.Increment or (cfg.Rounding ~= nil and (10^-(cfg.Rounding)) or 1),
...
local function I(sec, id, cfg)
    local flag = Options[id]; flag.Value = cfg.Default or ""
    sec:AddInput({
        Title = cfg.Text or id, Content = cfg.Placeholder or cfg.Content or "",
...
local function B(sec, titleOrCfg, fn)
    local title, callback, locked
    if type(titleOrCfg) == "string" then
        title = titleOrCfg; callback = fn; locked = false
    else
        title = titleOrCfg.Text or titleOrCfg.Title or "Button"
        callback = titleOrCfg.Func or titleOrCfg.Callback or fn
        locked = titleOrCfg.Disabled or false
    end
    sec:AddButton({ Title = title, Locked = locked, Callback = callback or function() end })
end

local function L(sec, text)
    if type(text) == "table" then text = text.Text or "" end
    local el = sec:AddParagraph({ Title = "", Content = tostring(text or "") })
    return {
        SetText    = function(self, t) if el and el.SetContent then pcall(function() el:SetContent(tostring(t)) end) end end,
        SetVisible = function(self, v) if el and el.SetVisible then pcall(function() el:SetVisible(v) end) end end,
        AddKeyPicker = function(self, id, cfg) Options[id].Value = cfg.Default or ""; return self end,
    }
end

-- ============================================================
-- Override AddSliderToggle dan CreateSwitchGroup ke Brigida syntax
AddSliderToggle = function(cfg)
    local tFlag = T(cfg.Group, cfg.Id, { Text=cfg.Text, Default=cfg.DefaultToggle or false, Disabled=cfg.Disabled or false })
    local sFlag = S(cfg.Group, cfg.Id.."Value", { Text=cfg.Text, Default=cfg.Default, Min=cfg.Min, Max=cfg.Max, Rounding=cfg.Rounding or 0, Disabled=cfg.Disabled or false })
    return tFlag, sFlag
end

CreateSwitchGroup = function(sec, id, displayName, tableSource)
    local listToUse = (id == "Title") and CombinedTitleList or tableSource
    T(sec, "Auto"..id, { Text="Auto Switch "..displayName, Default=false,
        Callback = function(state) if not state then Shared.LastSwitch[id]="" end end })
    D(sec, "Default"..id,    { Text="Select Default "..displayName,  Values=listToUse })
    D(sec, id.."_Mob",       { Text=displayName.." [Mob]",            Values=listToUse })
    D(sec, id.."_Boss",      { Text=displayName.." [Boss]",           Values=listToUse })
    D(sec, id.."_Combo",     { Text=displayName.." [Combo F Move]",   Values=listToUse })
    D(sec, id.."_BossHP",    { Text=displayName.." [Boss HP%]",       Values=listToUse })
    S(sec, id.."_BossHPAmt", { Text="Change Until Boss HP%", Default=15, Min=0, Max=100, Rounding=0 })
end

-- INFORMATION TAB
-- ============================================================
local secInfoUser   = Tabs.Information:AddSection({ Title = "User",   Open = false })
local secInfoGame   = Tabs.Information:AddSection({ Title = "Game",   Open = false })
local secInfoOthers = Tabs.Information:AddSection({ Title = "Others", Open = false })

local executorDisplayName = (identifyexecutor and identifyexecutor() or "Unknown")
local statusText = isLimitedExecutor and "<font color='#FFA500'>Semi-Working</font>" or "<font color='#00FF00'>Working</font>"
local extraNote  = isLimitedExecutor and "NOTE: May experiencing bugs!" or "All features should works properly!"
local initTier, initTime = GetData()

TierLabel = L(secInfoUser, "Type: " .. initTier)
TimeLabel = L(secInfoUser, "Time Left: " .. initTime)
task.spawn(function()
    while task.wait(5) do
        if not getgenv().celina_Running then break end
        local tier, timeLeft = GetData()
        if TierLabel then TierLabel:SetText("Type: " .. tier) end
        if TimeLabel then TimeLabel:SetText("Time Left: " .. timeLeft) end
        if timeLeft ~= "Loading..." then task.wait(55) end
    end
end)
L(secInfoUser, "Executor: " .. executorDisplayName .. " | Status: " .. statusText .. "\n" .. extraNote)

B(secInfoGame, "Redeem All Codes", function()
    ...
    for codeName, data in pairs(allCodes) do
        ...
        if playerLevel >= levelReq then
            local rewards = data.Rewards; local rewardText = ""
            if rewards then
                rewardText = rewardText .. string.format("\nCurrency: %s Coins, %s Gems", rewards.Money and Abbreviate(rewards.Money) or "0", rewards.Gems or 0)
            ...
            Library:Notify("Attempt to redeem: " .. codeName .. rewardText, 8)
            Remotes.UseCode:InvokeServer(codeName); task.wait(2)
        else
            Library:Notify(string.format("Not enough level for: %s (Req. %d)", codeName, levelReq), 4)
        end
    end
end)

L(secInfoOthers, "If some features are disabled, your executor lacks required functions.")
B(secInfoOthers, { Text = "Join Discord Server", Func = function()
    if request then pcall(function()
        request({ Url = "http://127.0.0.1:6463/rpc?v=1", Method = "POST",
            Headers = { ["Content-Type"] = "application/json", ["Origin"] = "https://discord.com" },
            Body = HttpService:JSONEncode({ cmd = "INVITE_BROWSER", args = { code = "qDccUkch9B" }, nonce = HttpService:GenerateGUID(false) }) })
    end) end
end })
...
-- PRIORITY TAB
-- ============================================================
local secPriority = Tabs.Priority:AddSection({ Title = "Config", Open = false })
for i = 1, #PriorityTasks do
    D(secPriority, "SelectedPriority_" .. i, { Text = "Priority " .. i, Values = PriorityTasks, Default = DefaultPriority[i] })
end

-- ============================================================
-- MAIN TAB
-- ============================================================
local secFarm = Tabs.Main:AddSection({ Title = "Farm", Open = false })
D(secFarm, "SelectedMob", { Text = "Select Mob (s)", Values = Tables.MobList, Multi = true })
B(secFarm, "Refresh", UpdateNPCLists)
T(secFarm, "MobFarm",    { Text = "Autofarm Selected Mob", Default = false })
T(secFarm, "AllMobFarm", { Text = "Autofarm All Mobs",     Default = false })
D(secFarm, "AllMobType", { Text = "Select Type [All Mob]", Values = { "Normal", "Fast" }, Default = "Normal" })
T(secFarm, "LevelFarm",  { Text = "Autofarm Level",        Default = false })

local secBoss = Tabs.Main:AddSection({ Title = "Boss", Open = false })
D(secBoss, "SelectedBosses",     { Text = "Select Bosses",       Values = Tables.AllBossList,        Multi = true  })
T(secBoss, "BossesFarm",         { Text = "Autofarm Selected Boss", Default = false })
T(secBoss, "AllBossesFarm",      { Text = "Autofarm All Bosses",    Default = false })
D(secBoss, "SelectedSummon",     { Text = "Select Summon Boss",  Values = Tables.SummonList,      Multi = false })
D(secBoss, "SelectedSummonDiff", { Text = "Select Difficulty",   Values = Tables.DiffList,        Multi = false })
T(secBoss, "AutoSummon",         { Text = "Auto Summon",          Default = false })
T(secBoss, "SummonBossFarm",     { Text = "Autofarm Summon Boss", Default = false })
PityLabel = L(secBoss, "Pity: 0/25")
D(secBoss, "SelectedBuildPity",      { Text = "Select Boss [Build Pity]",    Values = Tables.AllBossList, Multi = true  })
D(secBoss, "SelectedUsePity",        { Text = "Select Boss [Use Pity]",      Values = Tables.AllBossList, Multi = false })
D(secBoss, "SelectedPityDiff",       { Text = "Select Difficulty [Use Pity]",Values = Tables.DiffList,    Multi = false })
T(secBoss, "PityBossFarm",           { Text = "Autofarm Pity Boss",          Default = false })
D(secBoss, "SelectedOtherSummon",    { Text = "Select Summon Boss (Other)",  Values = Tables.OtherSummonList, Multi = false })
D(secBoss, "SelectedOtherSummonDiff",{ Text = "Select Difficulty",           Values = Tables.DiffList,        Multi = false })
T(secBoss, "AutoOtherSummon",        { Text = "Auto Summon",          Default = false })
T(secBoss, "OtherSummonFarm",        { Text = "Autofarm Summon Boss", Default = false })

local secAltHelp = Tabs.Main:AddSection({ Title = "Alt Help", Open = false })
D(secAltHelp, "SelectedAltBoss", { Text = "Select Boss",       Values = Tables.AllBossList, Multi = false })
D(secAltHelp, "SelectedAltDiff", { Text = "Select Difficulty", Values = Tables.DiffList,    Multi = false })
for i = 1, 5 do D(secAltHelp, "SelectedAlt_"..i, { Text = "Select Alt #"..i, Values = {}, Multi = false }) end
T(secAltHelp, "AltBossFarm", { Text = "Auto Help Alt", Default = false })

local secFarmCfg = Tabs.Main:AddSection({ Title = "Farm Config", Open = false })
D(secFarmCfg, "SelectedWeaponType",   { Text = "Select Weapon Type",   Values = Tables.Weapon, Multi = true })
S(secFarmCfg, "SwitchWeaponCD",       { Text = "Switch Weapon Delay",  Default = 4,    Min = 1, Max = 20,   Rounding = 0 })
T(secFarmCfg, "SwitchWeapon",         { Text = "Auto Switch Weapon",   Default = true })
T(secFarmCfg, "IslandTP",             { Text = "Island TP [Autofarm]", Default = true })
S(secFarmCfg, "IslandTPCD",           { Text = "Island TP CD",         Default = 0.67, Min = 0, Max = 2.5,  Rounding = 2 })
S(secFarmCfg, "TargetTPCD",           { Text = "Target TP CD",         Default = 0,    Min = 0, Max = 5,    Rounding = 2 })
S(secFarmCfg, "TargetDistTP",         { Text = "Target Distance TP",   Default = 0,    Min = 0, Max = 100,  Rounding = 0 })
S(secFarmCfg, "M1Speed",              { Text = "M1 Attack Cooldown",   Default = 0,    Min = 0, Max = 1,    Rounding = 2 })
D(secFarmCfg, "SelectedMovementType", { Text = "Select Movement Type", Values = { "Teleport", "Tween" }, Default = "Teleport" })
D(secFarmCfg, "SelectedFarmType",     { Text = "Select Farm Type",     Values = { "Behind", "Above", "Below" }, Default = "Above" })
S(secFarmCfg, "Distance",             { Text = "Farm Distance",        Default = 12, Min = 0, Max = 30,  Rounding = 0 })
S(secFarmCfg, "TweenSpeed",           { Text = "Tween Speed",          Default = 160, Min = 0, Max = 500, Rounding = 0 })
T(secFarmCfg, "InstaKill",            { Text = "Instant Kill",         Default = false })
D(secFarmCfg, "InstaKillType",        { Text = "Select Type",          Values = { "V1", "V2" }, Default = "V1" })
S(secFarmCfg, "InstaKillHP",          { Text = "HP% For Insta-Kill",   Default = 90, Min = 1, Max = 100, Rounding = 0 })
I(secFarmCfg, "InstaKillMinHP",       { Text = "Min MaxHP for Insta-Kill", Placeholder = "Number.." })

local secHaki = Tabs.Main:AddSection({ Title = "Haki", Open = false })
T(secHaki, "ObserHaki",    { Text = "Auto Observation Haki", Default = false })
T(secHaki, "ArmHaki",      { Text = "Auto Armament Haki",    Default = false })
T(secHaki, "ConquerorHaki",{ Text = "Auto Conqueror Haki",   Default = false })

local secSkill = Tabs.Main:AddSection({ Title = "Kill Aura & Skill", Open = false })
L(secSkill, "Kill Aura = Auto Attack + serang semua NPC dalam range.\nFarm toggle sudah include auto-attack ke target.")
T(secSkill, "KillAura", { Text = "Kill Aura (Auto Attack)", Default = false })
S(secSkill, "KillAuraCD",    { Text = "Kill Aura CD",    Default = 0.1, Min = 0.05, Max = 2,   Rounding = 2 })
S(secSkill, "KillAuraRange", { Text = "Kill Aura Range", Default = 200, Min = 0,    Max = 200, Rounding = 0 })
D(secSkill, "SelectedSkills",     { Text = "Select Skills", Values = { "Z", "X", "C", "V", "F" }, Multi = true })
D(secSkill, "AutoSkillType",      { Text = "Select Mode",   Values = { "Normal", "Instant" }, Default = "Normal" })
T(secSkill, "AutoSkill_BossOnly", { Text = "Skills On Boss Only", Default = false })
S(secSkill, "AutoSkill_BossHP",   { Text = "Boss HP%",            Default = 100, Min = 1, Max = 100, Rounding = 0 })
T(secSkill, "AutoSkill",          { Text = "Auto Use Skills",     Default = false })

local secCombo = Tabs.Main:AddSection({ Title = "Combo", Open = false })
L(secCombo, "Example: Z > X > C > 0.5 > V | Mode: Normal/Instant")
I(secCombo, "ComboPattern", { Text = "Combo Pattern", Default = "Z > X > C > V > F", Placeholder = "combo.." })
D(secCombo, "ComboMode",    { Text = "Select Mode", Values = { "Normal", "Instant" }, Default = "Normal" })
T(secCombo, "ComboBossOnly",{ Text = "Boss Only", Default = false })
T(secCombo, "AutoCombo",    { Text = "Auto Skill Combo", Default = false,
    Callback = function(state)
        if state and Toggles.AutoSkill.Value then
            Toggles.AutoSkill:SetValue(false)
            Library:Notify("Auto Skill disabled for combo to work.", 3)
        end
    end
})

local secSwitch = Tabs.Main:AddSection({ Title = "Switch - Title", Open = false })
local secSwitchRune  = Tabs.Main:AddSection({ Title = "Switch - Rune",  Open = false })
local secSwitchBuild = Tabs.Main:AddSection({ Title = "Switch - Build", Open = false })
CreateSwitchGroup(secSwitch,      "Title", "Title", CombinedTitleList)
CreateSwitchGroup(secSwitchRune,  "Rune",  "Rune",  Tables.RuneList)
CreateSwitchGroup(secSwitchBuild, "Build", "Build", Tables.BuildList)

-- ============================================================
-- AUTOMATION TAB
-- ============================================================
local secAscend = Tabs.Automation:AddSection({ Title = "Ascend", Open = false })
T(secAscend, "AutoAscend", { Text = "Auto Ascend", Default = false })
for i = 1, 10 do
    Tables.AscendLabels[i] = L(secAscend, "")
    Tables.AscendLabels[i]:SetVisible(false)
end

local secRolls = Tabs.Automation:AddSection({ Title = "Rolls", Open = false })
L(secRolls, "Increase delay based on ping. Low delay not recommended.")
S(secRolls, "RollCD", { Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2 })
D(secRolls, "SelectedTrait", { Text = "Select Trait (s)", Values = Tables.TraitList, Multi = true })
T(secRolls, "AutoTrait",     { Text = "Auto Roll Trait",  Default = false })
D(secRolls, "SelectedRace",  { Text = "Select Race (s)",  Values = Tables.RaceList,  Multi = true })
T(secRolls, "AutoRace",      { Text = "Auto Roll Race",   Default = false })
D(secRolls, "SelectedClan",  { Text = "Select Clan (s)",  Values = Tables.ClanList,  Multi = true })
T(secRolls, "AutoClan",      { Text = "Auto Roll Clan",   Default = false })

local secTrade = Tabs.Automation:AddSection({ Title = "Trade", Open = false })
D(secTrade, "SelectedTradePlr",   { Text = "Select Player",   Values = {}, Multi = false })
D(secTrade, "SelectedTradeItems", { Text = "Select Item (s)", Values = Tables.OwnedItem, Multi = true })
T(secTrade, "ReqTrade",       { Text = "Auto Send Request",   Default = false })
T(secTrade, "ReqTradeAccept", { Text = "Auto Accept Request", Default = false })
T(secTrade, "AutoAccept",     { Text = "Auto Accept Trade",   Default = false })

local secStatLevel = Tabs.Automation:AddSection({ Title = "Stats - Level", Open = false })
D(secStatLevel, "SelectedStats", { Text = "Select Stat (s)", Values = { "Melee", "Defense", "Sword", "Power" }, Multi = true })
T(secStatLevel, "AutoStats",     { Text = "Auto UP Stats",   Default = false })

local secStatGem = Tabs.Automation:AddSection({ Title = "Stats - Gem", Open = false })
L(secStatGem, "Increase delay based on ping. Reroll once for this to work.")
StatsLabel = L(secStatGem, "N/A")
D(secStatGem, "SelectedGemStats", { Text = "Select Stat (s)", Values = Tables.GemStat, Multi = true })
D(secStatGem, "SelectedRank",     { Text = "Select Rank (s)", Values = Tables.GemRank, Multi = true })
S(secStatGem, "StatsRollCD",      { Text = "Roll Delay",      Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(secStatGem, "AutoRollStats",    { Text = "Auto Roll Stats", Default = false })

local secStatMisc = Tabs.Automation:AddSection({ Title = "Stats - Misc", Open = false })
T(secStatMisc, "AutoSkillTree",     { Text = "Auto Skill Tree",         Default = false })
T(secStatMisc, "ArtifactMilestone", { Text = "Auto Artifact Milestone", Default = false })

local secEnchant = Tabs.Automation:AddSection({ Title = "Enchant", Open = false })
D(secEnchant, "SelectedEnchant",  { Text = "Select Enchant",  Values = Tables.OwnedAccessory, Multi = true })
T(secEnchant, "AutoEnchant",      { Text = "Auto Enchant",     Default = false })
T(secEnchant, "AutoEnchantAll",   { Text = "Auto Enchant All", Default = false })
D(secEnchant, "SelectedBlessing", { Text = "Select Blessing",  Values = Tables.OwnedWeapon,    Multi = true })
T(secEnchant, "AutoBlessing",     { Text = "Auto Blessing",     Default = false })
T(secEnchant, "AutoBlessingAll",  { Text = "Auto Blessing All", Default = false })

local secPassive = Tabs.Automation:AddSection({ Title = "Passive", Open = false })
SpecPassiveLabel = L(secPassive, "N/A")
D(secPassive, "SelectedPassive", { Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons, Multi = true })
D(secPassive, "SelectedSpec",    { Text = "Target Passives",   Values = Tables.SpecPassive,     Multi = true })
S(secPassive, "SpecRollCD",      { Text = "Roll Delay",        Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(secPassive, "AutoSpec",        { Text = "Auto Reroll Passive", Default = false })

sfEN_Cfg_ref = Tabs.Automation:AddSection({ Title = "Passive Config", Open = false })
L(sfEN_Cfg_ref, "Advanced users only. Will stop once passive reaches min value.")

-- ============================================================
-- ARTIFACT TAB
-- ============================================================
local secArtStatus  = Tabs.Artifact:AddSection({ Title = "Status",     Open = false })
local secArtEquip   = Tabs.Artifact:AddSection({ Title = "Auto-Equip", Open = false })
local secArtUpgrade = Tabs.Artifact:AddSection({ Title = "Upgrade",    Open = false })
local secArtLock    = Tabs.Artifact:AddSection({ Title = "Lock",       Open = false })
local secArtDelete  = Tabs.Artifact:AddSection({ Title = "Delete",     Open = false })

ArtifactLabel   = L(secArtStatus, "Status: N/A")
DustLabel       = L(secArtStatus, "Dust: N/A")
InvLabel_Helmet = L(secArtStatus, "Helmet: 0/500")
InvLabel_Gloves = L(secArtStatus, "Gloves: 0/500")
InvLabel_Body   = L(secArtStatus, "Body: 0/500")
InvLabel_Boots  = L(secArtStatus, "Boots: 0/500")

D(secArtEquip, "Eq_Type", { Text = "Artifact Type",    Values = {}, Multi = true })
D(secArtEquip, "Eq_MS",   { Text = "Main Stat Filter", Values = {}, Multi = true })
D(secArtEquip, "Eq_SS",   { Text = "Sub Stat Filter",  Values = {}, Multi = true })
T(secArtEquip, "ArtifactEquip", { Text = "Auto Equip", Default = false })

S(secArtUpgrade, "UpgradeLimit",    { Text = "Upgrade Limit",    Default = 0, Min = 0, Max = 15, Rounding = 0 })
D(secArtUpgrade, "Up_MS",           { Text = "Main Stat Filter", Values = {}, Multi = true })
T(secArtUpgrade, "ArtifactUpgrade", { Text = "Auto Upgrade",      Default = false })
T(secArtUpgrade, "UpgradeStage",    { Text = "Upgrade in Stages", Default = false })

D(secArtLock, "Lock_Type",    { Text = "Artifact Type",    Values = {}, Multi = true })
D(secArtLock, "Lock_Set",     { Text = "Artifact Set",     Values = {}, Multi = true })
D(secArtLock, "Lock_MS",      { Text = "Main Stat Filter", Values = {}, Multi = true })
D(secArtLock, "Lock_SS",      { Text = "Sub Stat Filter",  Values = {}, Multi = true })
S(secArtLock, "Lock_MinSS",   { Text = "Min Sub-Stats",    Default = 0, Min = 0, Max = 4, Rounding = 0 })
T(secArtLock, "ArtifactLock", { Text = "Auto Lock",        Default = false })

D(secArtDelete, "Del_Type",       { Text = "Artifact Type",       Values = {},                           Multi = true })
D(secArtDelete, "Del_Set",        { Text = "Artifact Set",         Values = {},                           Multi = true })
D(secArtDelete, "Del_MS_Helmet",  { Text = "Main Stat [Helmet]",   Values = { "FlatDefense", "Defense" }, Multi = true })
D(secArtDelete, "Del_MS_Gloves",  { Text = "Main Stat [Gloves]",   Values = { "Damage" },                 Multi = true })
D(secArtDelete, "Del_MS_Body",    { Text = "Main Stat [Body]",     Values = {},                           Multi = true })
D(secArtDelete, "Del_MS_Boots",   { Text = "Main Stat [Boots]",    Values = {},                           Multi = true })
D(secArtDelete, "Del_SS",         { Text = "Sub Stat Filter",       Values = {},                           Multi = true })
S(secArtDelete, "Del_MinSS",      { Text = "Min Sub-Stats",         Default = 0, Min = 0, Max = 4, Rounding = 0 })
T(secArtDelete, "ArtifactDelete", { Text = "Auto Delete",            Default = false })
T(secArtDelete, "DeleteUnlock",   { Text = "Auto Delete Unlocked",   Default = false })

-- ============================================================
-- DUNGEON TAB
-- ============================================================
local secDungeonJoin = Tabs.Dungeon:AddSection({ Title = "Auto Join", Open = false })
L(secDungeonJoin, "BossRush & Infinite Tower Supported.")
D(secDungeonJoin, "SelectedDungeon",   { Text = "Select Dungeon",     Values = Tables.DungeonList, Multi = false })
T(secDungeonJoin, "AutoDungeon",       { Text = "Auto Join Dungeon",   Default = false })
T(secDungeonJoin, "AutoInfiniteTower", { Text = "Auto Infinite Tower", Default = false })

local secDungeonCfg = Tabs.Dungeon:AddSection({ Title = "Config", Open = false })
DungeonCount = L(secDungeonCfg, "Dungeon Completed: N/A")
D(secDungeonCfg, "SelectedDiff",    { Text = "Select Difficulty", Values = { "Easy", "Medium", "Hard", "Extreme" }, Multi = false })
T(secDungeonCfg, "AutoDiff",        { Text = "Auto Select Difficulty", Default = false })
T(secDungeonCfg, "AutoReplay",      { Text = "Auto Replay",            Default = false })
T(secDungeonCfg, "DungeonAutofarm", { Text = "Autofarm Dungeon",        Default = false })

-- ============================================================
-- PLAYER TAB
-- ============================================================
local secGeneral = Tabs.Player:AddSection({ Title = "General", Open = false })
local secServer  = Tabs.Player:AddSection({ Title = "Server",  Open = false })
local secGame    = Tabs.Player:AddSection({ Title = "Game",    Open = false })
local secSafety  = Tabs.Player:AddSection({ Title = "Safety",  Open = false })

AddSliderToggle({ Group = secGeneral, Id = "WS",       Text = "WalkSpeed",       Default = 16,  Min = 16,  Max = 250  })
AddSliderToggle({ Group = secGeneral, Id = "TPW",      Text = "TPWalk",          Default = 1,   Min = 1,   Max = 10,  Rounding = 1 })
AddSliderToggle({ Group = secGeneral, Id = "JP",       Text = "JumpPower",       Default = 50,  Min = 0,   Max = 500  })
AddSliderToggle({ Group = secGeneral, Id = "HH",       Text = "HipHeight",       Default = 2,   Min = 0,   Max = 10,  Rounding = 1 })
T(secGeneral, "Noclip",          { Text = "Noclip"                })
T(secGeneral, "AntiKnockback",   { Text = "Anti Knockback",   Default = false })
T(secGeneral, "Disable3DRender", { Text = "Disable 3D Rendering"  })
AddSliderToggle({ Group = secGeneral, Id = "Grav",     Text = "Gravity",         Default = 196, Min = 0,   Max = 500, Rounding = 1 })
AddSliderToggle({ Group = secGeneral, Id = "Zoom",     Text = "Camera Zoom",     Default = 128, Min = 128, Max = 10000 })
AddSliderToggle({ Group = secGeneral, Id = "FOV",      Text = "Field of View",   Default = 70,  Min = 30,  Max = 120  })
AddSliderToggle({ Group = secGeneral, Id = "LimitFPS", Text = "Set Max FPS",     Default = 60,  Min = 5,   Max = 360, Disabled = not Support.FPS })
T(secGeneral, "FPSBoost",    { Text = "FPS Boost"            })
T(secGeneral, "FPSBoost_AF", { Text = "FPS Boost [Autofarm]" })

T(secServer, "AntiAFK",          { Text = "Anti AFK",         Default = true, Disabled = not Support.Connections })
T(secServer, "AntiKick",         { Text = "Anti Kick (Client)" })
T(secServer, "AutoReconnect",    { Text = "Auto Reconnect"     })
T(secServer, "NoGameplayPaused", { Text = "No Gameplay Paused" })
B(secServer, "Serverhop", function()
    game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")
end)
B(secServer, "Rejoin", function() Services.TeleportService:Teleport(game.PlaceId, Plr) end)
T(secServer, "AutoServerhop", { Text = "Auto Serverhop" })
S(secServer, "AutoHopMins",   { Text = "Minutes", Default = 30, Min = 0, Max = 300 })

T(secGame, "InstantPP",  { Text = "Instant Prompt" })
T(secGame, "Fullbright", { Text = "Fullbright"     })
T(secGame, "NoFog",      { Text = "No Fog"         })
AddSliderToggle({ Group = secGame, Id = "OverrideTime", Text = "Time Of Day", Default = 12, Min = 0, Max = 24, Rounding = 1 })

L(secSafety, "Panic"):AddKeyPicker("PanicKeybind", { Default = "P" })
T(secSafety, "AutoKick", { Text = "Auto Kick", Default = true })
D(secSafety, "SelectedKickType", {
    Text = "Select Type", Values = { "Mod", "Player Join", "Public Server" }, Default = { "Mod" }, Multi = true,
})

-- ============================================================
-- TELEPORT TAB
-- ============================================================
local secIsland = Tabs.Teleport:AddSection({ Title = "Island", Open = false })
D(secIsland, "SelectedIsland", {
    Text = "Select Island", Values = Tables.IslandList, Multi = false,
    Callback = function(a)
        if a then Remotes.TP_Portal:FireServer(a)
        else Library:Notify("Please select an island.", 2) end
    end,
})

local secQuestNPC = Tabs.Teleport:AddSection({ Title = "Quest NPC", Open = false })
D(secQuestNPC, "SelectedQuestNPC", {
    Text = "Select NPC", Values = Tables.NPC_QuestList, Multi = false,
    Callback = function(a)
        SafeTeleportToNPC(a, { ["DungeonUnlock"] = "DungeonPortalsNPC", ["SlimeKeyUnlock"] = "SlimeCraftNPC" })
    end,
})
B(secQuestNPC, "Teleport to Level Based Quest", function()
    local distance = tonumber(pingUI.PingMarker:WaitForChild("DistanceLabel").Text:match("%d+"))
    if not distance then Library:Notify("Something wrong..", 2); return end
...
local secMiscNPC = Tabs.Teleport:AddSection({ Title = "Misc NPC", Open = false })
D(secMiscNPC, "SelectedMiscNPC", {
    Text = "Select NPC [Misc]", Values = Tables.NPC_MiscList, Multi = false,
    Callback = function(a)
        SafeTeleportToNPC(tostring(a), { ["ArmHaki"] = "HakiQuest", ["Observation"] = "ObservationBuyer" })
    end,
})
D(secMiscNPC, "SelectedMiscAllNPC", {
    Text = "Select NPC [All]", Values = Tables.AllNPCList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

local secMovesetNPC = Tabs.Teleport:AddSection({ Title = "Moveset NPC", Open = false })
D(secMovesetNPC, "SelectedMovesetNPC", {
    Text = "Select NPC", Values = Tables.NPC_MovesetList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

local secMasteryNPC = Tabs.Teleport:AddSection({ Title = "Mastery NPC", Open = false })
D(secMasteryNPC, "SelectedMasteryNPC", {
    Text = "Select NPC", Values = Tables.NPC_MasteryList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})
...
-- MISC TAB
-- ============================================================
local secMerchant = Tabs.Misc:AddSection({ Title = "Regular Merchant", Open = false })
MerchantTimerLabel = L(secMerchant, "Refresh: N/A")
D(secMerchant, "SelectedMerchantItems", { Text = "Select Item (s)", Values = Tables.MerchantList, Multi = true })
T(secMerchant, "AutoMerchant",          { Text = "Auto Buy Selected Items", Default = false })

local secDungeonMerchant = Tabs.Misc:AddSection({ Title = "Dungeon Merchant", Open = false })
D(secDungeonMerchant, "SelectedDungeonMerchantItems", { Text = "Select Item (s)", Values = Tables.DungeonMerchantList or {}, Multi = true })
T(secDungeonMerchant, "AutoDungeonMerchant",          { Text = "Auto Buy Dungeon Items", Default = false })

local secValentine = Tabs.Misc:AddSection({ Title = "Valentine Merchant", Open = false })
L(secValentine, "Able to cause performance issues.")
ValentineLabel = L(secValentine, "Hearts: N/A")
D(secValentine, "SelectedValentineMerchantItems", { Text = "Select Item (s)", Values = Tables.ValentineMerchantList, Multi = true })
ValentinePriceLabel = L(secValentine, "Price: N/A")
T(secValentine, "AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

local secTowerMerchant = Tabs.Misc:AddSection({ Title = "Tower Merchant", Open = false })
D(secTowerMerchant, "SelectedTowerMerchantItems", { Text = "Select Item (s)", Values = Tables.InfiniteTowerMerchantList or {}, Multi = true })
T(secTowerMerchant, "AutoTowerMerchant",          { Text = "Auto Buy Tower Items", Default = false })

local secBossRushMerchant = Tabs.Misc:AddSection({ Title = "Boss Rush Merchant", Open = false })
D(secBossRushMerchant, "SelectedBossRushMerchantItems", { Text = "Select Item (s)", Values = Tables.BossRushMerchantList or {}, Multi = true })
T(secBossRushMerchant, "AutoBossRushMerchant",          { Text = "Auto Buy Boss Rush Items", Default = false })

local secChests = Tabs.Misc:AddSection({ Title = "Chests", Open = false })
D(secChests, "SelectedChests", { Text = "Select Chest (s)", Values = Tables.Rarities, Multi = true })
T(secChests, "AutoChest",      { Text = "Auto Open Chest",  Default = false })

local secCraft = Tabs.Misc:AddSection({ Title = "Craft", Open = false })
D(secCraft, "SelectedCraftItems", { Text = "Select Item (s) To Craft", Values = Tables.CraftItemList, Multi = true })
T(secCraft, "AutoCraftItem",      { Text = "Auto Craft Item",          Default = false })

local secNotify = Tabs.Misc:AddSection({ Title = "Notify", Open = false })
T(secNotify, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

local secPuzzles = Tabs.Misc:AddSection({ Title = "Puzzles", Open = false })
B(secPuzzles, { Text = "Complete Dungeon Puzzle",   Disabled = not Support.Proximity, Func = function()
    local lv = Plr.Data.Level.Value
    if lv >= 5000 then UniversalPuzzleSolver("Dungeon")
    else Library:Notify("Level 5000 required! Current: "..lv, 3) end
end })
B(secPuzzles, { Text = "Complete Slime Key Puzzle", Disabled = not Support.Proximity, Func = function() UniversalPuzzleSolver("Slime") end })
B(secPuzzles, { Text = "Complete Valentine Puzzle", Disabled = not Support.Proximity, Func = function()
    local Char = GetCharacter(); Remotes.TP_Portal:FireServer("Valentine"); task.wait(2.5)
    for i = 1, 3 do
        ...
        if item then
            ...
            if prompt then Char.HumanoidRootPart.CFrame = item.CFrame * CFrame.new(0,3,0); task.wait(0.2); fireproximityprompt(prompt); task.wait(0.5)
            else Library:Notify("No prompt in Heart"..i, 2) end
        else Library:Notify("Heart"..i.." not found!", 2) end
    end
end })
B(secPuzzles, { Text = "Complete Demonite Puzzle",  Disabled = not Support.Proximity, Func = function() UniversalPuzzleSolver("Demonite") end })
B(secPuzzles, { Text = "Complete Hogyoku Puzzle",   Disabled = not Support.Proximity, Func = function()
    local lv = Plr.Data.Level.Value
    if lv >= 8500 then UniversalPuzzleSolver("Hogyoku")
    else Library:Notify("Level 8500 required! Current: "..lv, 4) end
end })

local secQuestlines = Tabs.Misc:AddSection({ Title = "Questlines", Open = false })
L(secQuestlines, "Experimental. Store race & clan before using. Dungeon tasks only make you join dungeon.")
D(secQuestlines, "SelectedQuestline",          { Text = "Select Questline",       Values = Tables.QuestlineList,   Multi = false })
D(secQuestlines, "SelectedQuestline_Player",   { Text = "Select Player",          Values = {},                     Multi = false })
D(secQuestlines, "SelectedQuestline_DMGTaken", { Text = "Select Mob [Take Damage]",Values = Tables.AllEntitiesList, Multi = false })
B(secQuestlines, "Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)
T(secQuestlines, "AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })

-- ============================================================
-- WEBHOOK TAB
-- ============================================================
local secWebhook = Tabs.Webhook:AddSection({ Title = "Config", Open = false })
I(secWebhook, "WebhookURL",  { Text = "Webhook URL",  Placeholder = "Enter Webhook URL..." })
I(secWebhook, "UID",         { Text = "User ID",       Placeholder = "Enter UID..." })
D(secWebhook, "SelectedData", { Text = "Select Data (s)", Values = { "Name", "Stats", "New Items", "All Items" }, Multi = true })
D(secWebhook, "SelectedItemRarity", {
    Text = "Select Rarity To Send",
    Values = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythical", "Secret" },
    Default = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythical", "Secret" }, Multi = true,
})
T(secWebhook, "PingUser",    { Text = "Ping User",    Default = false })
T(secWebhook, "SendWebhook", { Text = "Send Webhook", Default = false, Disabled = not Support.Webhook })
S(secWebhook, "WebhookDelay",{ Text = "Send every [x] minutes", Default = 5, Min = 1, Max = 30, Rounding = 0 })
B(secWebhook, "Test Webhook", function() PostToWebhook() end)

-- ============================================================
-- CONFIG TAB
-- ============================================================
local secMenu = Tabs.Config:AddSection({ Title = "Menu", Open = false })
T(secMenu, "AutoShowUI",       { Text = "Auto Show UI", Default = true })
T(secMenu, "ShowCustomCursor", { Text = "Custom Cursor", Default = false, Callback = function(v) Library.ShowCustomCursor = v end })
D(secMenu, "NotificationSide", { Text = "Notification Side", Values = { "Left", "Right" }, Default = "Right",
    Callback = function(v) Library:SetNotifySide(v) end })
D(secMenu, "DPIDropdown", { Text = "DPI Scale",
    Values = { "50%", "75%", "100%", "125%", "150%", "175%", "200%" }, Default = "100%",
    Callback = function(v) Library:SetDPIScale(tonumber(v:gsub("%%", ""))) end })
secMenu:AddKeybind({ Title = "Menu Keybind", Value = "U", Callback = function() end })
Options["MenuKeybind"].Value = "U"
B(secMenu, "Unload", function()
...
-- ============================================================
-- ============================================================
-- FARM & KILL AURA LOGIC
-- ============================================================

-- Update boss dropdown dari Modules (dipanggil sekali di awal)
local function UpdateBossDropdowns()
    -- 1. Dari Modules.TimedConfig (boss spawn)
    if Modules and Modules.TimedConfig and Modules.TimedConfig.Bosses then
        for _, data in pairs(Modules.TimedConfig.Bosses) do
            local name = data.displayName or data.name
            if name and not table.find(Tables.BossList, name) then
                table.insert(Tables.BossList, name)
            end
        end
    end
    -- 2. Dari Modules.BossConfig (semua boss)
    if Modules and Modules.BossConfig and Modules.BossConfig.Bosses then
        for internalId, _ in pairs(Modules.BossConfig.Bosses) do
            local clean = internalId:gsub("Boss$","")
            if clean and not table.find(Tables.AllBossList, clean) then
                table.insert(Tables.AllBossList, clean)
            end
        end
    end
    -- 3. Scan workspace NPCs
    local npcFolder = workspace:FindFirstChild("NPCs")
    if npcFolder then
        for _, obj in ipairs(npcFolder:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
                local cleanName = obj.Name:gsub("%d+$",""):gsub("%s*%[.*%]%s*","")
                local isBoss = cleanName:find("Boss") or table.find(Tables.MiniBossList or {}, cleanName)
                if isBoss then
                    if cleanName ~= "" and not table.find(Tables.BossList, cleanName) then table.insert(Tables.BossList, cleanName) end
                    if cleanName ~= "" and not table.find(Tables.AllBossList, cleanName) then table.insert(Tables.AllBossList, cleanName) end
                else
                    if cleanName ~= "" and not table.find(Tables.MobList, cleanName) then table.insert(Tables.MobList, cleanName) end
                end
            end
        end
    end
    table.sort(Tables.BossList); table.sort(Tables.MobList); table.sort(Tables.AllBossList)
    pcall(function() Options.SelectedBosses:SetValues(Tables.BossList) end)
    pcall(function() Options.SelectedMob:SetValues(Tables.MobList) end)
end

task.spawn(function()
    task.wait(2)
    UpdateBossDropdowns()
    while getgenv().celina_Running do task.wait(10); UpdateBossDropdowns() end
end)


-- ============================================================
-- HELPER: Convert Brigida dropdown value ke dict {[name]=true}
-- ============================================================
local function ToDict(val)
    if type(val) ~= "table" then return {} end
    for k, v in pairs(val) do
        if type(k) == "string" and type(v) == "boolean" then return val end
        break
    end
    local dict = {}
    for _, v in ipairs(val) do
        if type(v) == "string" then dict[v] = true end
    end
    return dict
end
...
-- ISLAND / PORTAL
-- ============================================================
local function GetIslandForTarget(targetPos, targetName)
    local cleanName = targetName and targetName:gsub("%d+$","") or ""
    if Shared.BossTIMap[targetName]  then return Shared.BossTIMap[targetName]  end
    if Shared.BossTIMap[cleanName]   then return Shared.BossTIMap[cleanName]   end
    if Tables.MobToIsland[cleanName] then return Tables.MobToIsland[cleanName] end
    -- IslandCrystals sekarang menyimpan Vector3 position langsung
    local nearest, minDist = "Starter", math.huge
    for islandName, pos in pairs(IslandCrystals) do
        if typeof(pos) == "Vector3" then
            local d = (targetPos - pos).Magnitude
            if d < minDist then minDist = d; nearest = islandName end
        end
    end
    return nearest
end
...
local function TeleportToIsland(islandName)
    if not islandName or islandName == Shared.Island then return end
    Shared.MovingIsland = true
    pcall(function() Remotes.TP_Portal:FireServer(islandName) end)
    task.wait(math.max(tonumber(Options.IslandTPCD and Options.IslandTPCD.Value) or 1.2, 0.5))
    Shared.Island = islandName
    Shared.MovingIsland = false
end
...
-- WEAPON EQUIP
-- ============================================================
local function EquipCorrectWeapon()
    local char = GetCharacter()
    local hum  = char and char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    local enabledTypes = ToDict(Options.SelectedWeaponType and Options.SelectedWeaponType.Value or {})
    local hasFilter = false
    for _ in pairs(enabledTypes) do hasFilter = true; break end
    local currentTool = char:FindFirstChildOfClass("Tool")
    if currentTool then
        local cType = (GetToolTypeFromModule and GetToolTypeFromModule(currentTool.Name)) or "Melee"
        if not hasFilter or enabledTypes[cType] then return end
    end
    for _, container in ipairs({Plr.Backpack, char}) do
        for _, tool in ipairs(container:GetChildren()) do
            if tool:IsA("Tool") then
                local tType = (GetToolTypeFromModule and GetToolTypeFromModule(tool.Name)) or "Melee"
                if not hasFilter or enabledTypes[tType] then
...
local function MoveToTarget(target)
    ...
    local dist  = tonumber(Options.Distance and Options.Distance.Value) or 12
    local fType = Options.SelectedFarmType and Options.SelectedFarmType.Value or "Behind"
    local tPos  = tRoot.Position

    -- Hitung posisi tujuan HANYA berdasarkan posisi NPC, jangan sentuh NPC CFrame
    local finalPos
    local look = (root.Position - tPos).Unit -- arah dari NPC ke char
    if fType == "Above" then
        finalPos = tPos + Vector3.new(0, dist, 0)
    elseif fType == "Below" then
        finalPos = tPos + Vector3.new(0, -dist, 0)
    else -- Behind: char di belakang NPC
        finalPos = tPos + look * dist
    end
    finalPos = Vector3.new(finalPos.X, tPos.Y, finalPos.Z) -- samakan Y dengan NPC

    if (root.Position - finalPos).Magnitude < 3 then return end

    -- Arah char menghadap NPC (bukan sebaliknya)
    local dest = CFrame.lookAt(finalPos, tPos)
    StopMovement()

    local mType = Options.SelectedMovementType and Options.SelectedMovementType.Value or "Tween"
    if mType == "Teleport" then
        root.CFrame = dest
        root.AssemblyLinearVelocity  = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
    else
        local speed    = math.max(tonumber(Options.TweenSpeed and Options.TweenSpeed.Value) or 160, 10)
        local duration = (root.Position - finalPos).Magnitude / speed
        local tween    = game:GetService("TweenService"):Create(
            root, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = dest}
        )
        _activeTween = tween
        tween:Play()

        local t = 0
        while t < duration + 0.5 do
            task.wait(0.05); t = t + 0.05
            local farmOn = Toggles.MobFarm.Value or Toggles.AllMobFarm.Value
                        or Toggles.BossesFarm.Value or Toggles.AllBossesFarm.Value
            if not farmOn then StopMovement(); return end
            local hum = target:FindFirstChildOfClass("Humanoid")
            if not hum or hum.Health <= 0 or not target.Parent then StopMovement(); return end
        end
        root.AssemblyLinearVelocity  = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
        _activeTween = nil
...
local function AttackTarget(target)
    ...
    -- Equip weapon dulu sesuai dropdown (farm loop yang call ini)
    EquipCorrectWeapon()
    local char = GetCharacter()
    local tool = char and char:FindFirstChildOfClass("Tool")
    local toolType = (tool and GetToolTypeFromModule and GetToolTypeFromModule(tool.Name)) or "Melee"

    local m1CD = tonumber(Options.M1Speed and Options.M1Speed.Value) or 0.05
    if tick() - _lastM1 >= m1CD then
        pcall(function() Remotes.M1:FireServer() end)
        _lastM1 = tick()
    end

    if Toggles.AutoSkill and Toggles.AutoSkill.Value and tool then
        local selected = ToDict(Options.SelectedSkills and Options.SelectedSkills.Value or {})
        local useMode  = Options.AutoSkillType and Options.AutoSkillType.Value or "Normal"
        local bossOnly = Toggles.AutoSkill_BossOnly and Toggles.AutoSkill_BossOnly.Value
        ...
        if not bossOnly or isBoss then
            ...
            for _, key in ipairs({"Z","X","C","V","F"}) do
                if selected[key] then
                    if useMode == "Normal" and IsSkillReady and not IsSkillReady(key) then continue end
                    pcall(function()
                        if toolType == "Power" then
                            Remotes.UseFruit:FireServer("UseAbility", {
...
-- ============================================================
-- FIND TARGET - bangun list semua target valid, loop satu per satu
-- ============================================================

-- State untuk cycling mob list
local _mobQueue    = {}  -- antrian semua mob yang harus dikunjungi
local _mobQueueIdx = 1   -- index saat ini di antrian

local function BuildMobQueue()
    -- Bangun list SEMUA mob yang cocok dengan selection
    local char = GetCharacter()
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return end

    local selMobs = ToDict(Options.SelectedMob and Options.SelectedMob.Value or {})
    local hasSelection = false
    for _ in pairs(selMobs) do hasSelection = true; break end

    -- Kalau AllMobFarm ON, tidak perlu selection
    if not Toggles.AllMobFarm.Value and not hasSelection then
        _mobQueue = {}
        return
    end
    ...
    for _, obj in ipairs(npcFolder:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if hum and hum.Health > 0 and oRoot then
                local cleanName = obj.Name:gsub("%d+$","")
                local isBoss = cleanName:find("Boss") or table.find(Tables.MiniBossList or {}, cleanName)
                if not isBoss then -- hanya mob
                    local sel = Toggles.AllMobFarm.Value or selMobs[obj.Name] or selMobs[cleanName]
                    if sel then
                        table.insert(newQueue, obj)
                    end
                end
            end
        end
    ...
    -- Sort berdasarkan jarak dari char
   --[[ table.sort(newQueue, function(a, b)
        ...
        return (root.Position - pa.Position).Magnitude < (root.Position - pb.Position).Magnitude
    end) --]]
...
local function GetNextMobInQueue()
    -- Bersihkan mob yang sudah mati dari queue
    local alive = {}
    for _, obj in ipairs(_mobQueue) do
        local hum = obj:FindFirstChildOfClass("Humanoid")
        if obj.Parent and hum and hum.Health > 0 then
            table.insert(alive, obj)
        end
    end
    _mobQueue = alive

    if #_mobQueue == 0 then return nil end

    -- Clamp index
    if _mobQueueIdx > #_mobQueue then _mobQueueIdx = 1 end

    local target = _mobQueue[_mobQueueIdx]
    _mobQueueIdx = _mobQueueIdx + 1
    if _mobQueueIdx > #_mobQueue then _mobQueueIdx = 1 end
...
local function FindBossTarget()
    ...
    local selBosses = ToDict(Options.SelectedBosses and Options.SelectedBosses.Value or {})
    local npcFolder = workspace:FindFirstChild("NPCs")
    ...
    for _, obj in ipairs(npcFolder:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if hum and hum.Health > 0 and oRoot then
                ...
                local isBoss = cleanName:find("Boss") or table.find(Tables.MiniBossList or {}, cleanName)
                if isBoss then
                    local sel = Toggles.AllBossesFarm.Value
                        or selBosses[obj.Name] or selBosses[cleanName]
                    if sel then
...
-- ============================================================
-- MAIN LOOP: FARM + KILL AURA
-- ============================================================

-- Rebuild mob queue setiap kali selection berubah
Options.SelectedMob:OnChanged(function() BuildMobQueue() end)
Toggles.AllMobFarm:OnChanged(function()  BuildMobQueue() end)
Toggles.MobFarm:OnChanged(function()     BuildMobQueue() end)

task.spawn(function()
    -- Rebuild queue tiap 5 detik untuk handle respawn
    while getgenv().celina_Running do
        task.wait(5)
        local farmOn = Toggles.MobFarm.Value or Toggles.AllMobFarm.Value
        if farmOn then BuildMobQueue() end
    end
end)
...
task.spawn(function()
    while getgenv().celina_Running do
        ...
        -- KILL AURA (tidak equip weapon, biarkan char pakai weapon yang ada)
        if Toggles.KillAura and Toggles.KillAura.Value then
            local char = GetCharacter()
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
                local range = tonumber(Options.KillAuraRange and Options.KillAuraRange.Value) or 200
                local npcFolder = workspace:FindFirstChild("NPCs")
                if npcFolder then
                    for _, obj in ipairs(npcFolder:GetDescendants()) do
                        if obj:IsA("Model") and obj ~= char then
                            local hum  = obj:FindFirstChildOfClass("Humanoid")
                            local oRoot= obj:FindFirstChild("HumanoidRootPart")
                            if hum and hum.Health > 0 and oRoot
                               and (root.Position - oRoot.Position).Magnitude <= range then
                                -- Jangan move char, hanya fire M1
                                pcall(function() Remotes.M1:FireServer() end)
                                task.wait(tonumber(Options.KillAuraCD and Options.KillAuraCD.Value) or 0.1)
                                break
                            end
                        end
                    end
        ...
        -- FARM
        local isBossFarm = Toggles.BossesFarm.Value or Toggles.AllBossesFarm.Value
        local isMobFarm  = Toggles.MobFarm.Value    or Toggles.AllMobFarm.Value

        if not isBossFarm and not isMobFarm then
            Shared.Target = nil
            StopMovement()
            continue
        end
        ...
        local char = GetCharacter()
        if not char or Shared.Recovering or Shared.MovingIsland then continue end

        -- Cari target: Boss prioritas, lalu mob dari queue
        local target = nil
        if isBossFarm then target = FindBossTarget() end
        if not target and isMobFarm then
            target = GetNextMobInQueue()
        end
        ...
        if Toggles.IslandTP and Toggles.IslandTP.Value then
            local neededIsland = GetIslandForTarget(target:GetPivot().Position, target.Name)
            if neededIsland ~= Shared.Island then
        ...
        if not hum or hum.Health <= 0 or not target.Parent then
            Shared.Target = nil; continue
        end

        local root  = char:FindFirstChild("HumanoidRootPart")
        local tRoot = target:FindFirstChild("HumanoidRootPart")
        if root and tRoot then
            local attackRange = (tonumber(Options.Distance and Options.Distance.Value) or 12) + 3
            local curDist     = (root.Position - tRoot.Position).Magnitude
            if curDist > attackRange then
                MoveToTarget(target)
            else
                StopMovement()
                AttackTarget(target)
...
local function ToDict(val)
    if type(val) ~= "table" then return {} end
    -- Cek apakah sudah dict (key = string, value = bool)
    for k, v in pairs(val) do
        if type(k) == "string" and type(v) == "boolean" then return val end
        break
    end
...
local function MoveToTarget(target)
    ...
    local finalPos
    if     fType == "Above" then finalPos = tPos + Vector3.new(0,  dist, 0)
    elseif fType == "Below" then finalPos = tPos + Vector3.new(0, -dist, 0)
    else                         finalPos = (pivot * CFrame.new(0, 0, dist)).Position end
    ...
    if mType == "Teleport" then
        ...
        -- Tunggu selesai ATAU target mati ATAU farm dimatikan
        local t = 0
        while t < duration + 0.5 do
            task.wait(0.05); t = t + 0.05
            -- Stop jika farm dimatikan
            local farmActive = Toggles.MobFarm.Value or Toggles.AllMobFarm.Value
                            or Toggles.BossesFarm.Value or Toggles.AllBossesFarm.Value
            if not farmActive then StopMovement(); return end
            -- Stop jika target sudah mati
            local hum = target:FindFirstChildOfClass("Humanoid")
            if not hum or hum.Health <= 0 or not target.Parent then StopMovement(); return end
        end
        -- Pastikan berhenti setelah tween
        root.AssemblyLinearVelocity  = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
        _activeTween = nil
    end
...
local function AttackTarget(target)
    ...
    -- Skills jika AutoSkill ON dan ada target
    if Toggles.AutoSkill and Toggles.AutoSkill.Value and tool then
        local selected  = ToDict(Options.SelectedSkills and Options.SelectedSkills.Value or {})
        local useMode   = Options.AutoSkillType and Options.AutoSkillType.Value or "Normal"
        local bossOnly  = Toggles.AutoSkill_BossOnly and Toggles.AutoSkill_BossOnly.Value
...
-- ============================================================
-- FIND TARGET (Boss diutamakan atas Mob)
-- ============================================================
local function FindFarmTarget()
    ...
    local isBossFarm = Toggles.BossesFarm.Value or Toggles.AllBossesFarm.Value
    local isMobFarm  = Toggles.MobFarm.Value    or Toggles.AllMobFarm.Value
    if not isBossFarm and not isMobFarm then return nil end
    ...
    local bestMob,  bestMobDist  = nil, math.huge
    ...
    for _, obj in ipairs(npcFolder:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if hum and hum.Health > 0 and oRoot then
                ...
                if isBoss and isBossFarm then
                    local sel = Toggles.AllBossesFarm.Value
                        or selBosses[obj.Name] or selBosses[cleanName]
                    if sel and d < bestBossDist then
                        bestBoss, bestBossDist = obj, d
                    end
                elseif not isBoss and isMobFarm then
                    local sel = Toggles.AllMobFarm.Value
                        or selMobs[obj.Name] or selMobs[cleanName]
                    if sel and d < bestMobDist then
                        bestMob, bestMobDist = obj, d
                    end
                end
            end
        end
...
Toggles.AutoTitle:OnChanged(function(s)
    if s and #Tables.UnlockedTitle == 0 then Remotes.TitleUnequip:FireServer() end
end)
...
Toggles.AutoCombo:OnChanged(function(s)
...
Toggles.AutoInfiniteTower:OnChanged(function(s)  Thread("AutoInfiniteTower", Func_AutoInfiniteTower, s) end)
...
-- NoGameplayPaused
Toggles.NoGameplayPaused:OnChanged(function(s)
    Thread("NoGameplayPaused", SafeLoop("Anti-Pause", Func_NoGameplayPaused), s)
end)

-- InstantPP: set HoldDuration = 0 saat prompt dimulai
...
Options.Lock_Set:SetValues(allSets);   Options.Lock_MS:SetValues(allStats);  Options.Lock_SS:SetValues(allStats)
Options.Del_Type:SetValues(Modules.ArtifactConfig.Categories)
Options.Del_Set:SetValues(allSets);    Options.Del_MS_Body:SetValues(allStats)
Options.Del_MS_Boots:SetValues(allStats); Options.Del_SS:SetValues(allStats)
Options.Up_MS:SetValues(allStats)
Options.Eq_Type:SetValues(Modules.ArtifactConfig.Categories)
Options.Eq_MS:SetValues(allStats);     Options.Eq_SS:SetValues(allStats)

UpdateNPCLists(); UpdateAllEntities(); InitAutoKick(); ACThing(true)
...
task.spawn(function()
    ...
    task.wait(0.25)
    if Toggles.AutoRune and Toggles.AutoRune.Value then
        Shared.LastSwitch.Rune = "REFRESHING"
        Toggles.AutoRune:SetValue(false); task.wait(.01); Toggles.AutoRune:SetValue(true)
    end
...
task.spawn(function()
    task.wait(0.1)
    if Toggles.AutoTitle and Toggles.AutoTitle.Value then Remotes.TitleUnequip:FireServer() end
end)

if UIS.TouchEnabled and not UIS.KeyboardEnabled then
    Library:SetDPIScale(75)
    Library:Notify("Device: Phone - DPI resized.", 3)
elseif UIS.KeyboardEnabled then
    Library:SetDPIScale(100)
    Library:Notify("Device: PC - Press " .. (Options["MenuKeybind"].Value or "U") .. " to show/hide UI.", 10)
end

task.spawn(function()
    task.wait(0.1)
    if Toggles.AutoShowUI and Toggles.AutoShowUI.Value == false then Library:Toggle() end
end)

VelarisUI:MakeNotify({ Title = "Victoria Hub", Description = "Script loaded!", Content = "Report bugs in Discord!", Color = "Success", Time = 5 })

-- Sync semua flags ke OnChanged callbacks setelah config autoload
task.spawn(function()
    task.wait(1) -- tunggu autoload selesai
    for k, flag in pairs(_flags) do
        if flag._cb and #flag._cb > 0 and flag.Value ~= nil then
            for _, f in ipairs(flag._cb) do
                pcall(f, flag.Value)
            end
        end
    end
...
if not eh_success then
    warn("[Victoria Hub] ERROR: " .. tostring(err))
    if VelarisUI then
        pcall(function()
            VelarisUI:MakeNotify({ Title = "VictoriaHub", Description = "ERROR: " .. tostring(err), Color = "Danger", Time = 6 })
        end)
    end
end
