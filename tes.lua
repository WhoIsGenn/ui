...
-- ============================================================
-- FUNGSI YANG DIBUTUHKAN UI (dari bagian UI original)
-- ============================================================
...
-- UpdatePassiveSliders dipanggil setelah sfEN_Cfg dibuat - forward declare
local sfEN_Cfg_ref -- akan diisi setelah Window dibuat
        ...
        if isSelected then
            ...
            if data and data.Buffs then
                for statKey, range in pairs(data.Buffs) do
                    ...
                    if not Shared.SpecStatsSlider[sliderId] and sfEN_Cfg_ref then
                        local minVal, maxVal = range[1], range[2]
                        S(sfEN_Cfg_ref, sliderId, { Text = label, Default = minVal, Min = minVal, Max = maxVal, Rounding = 1 })
                        Shared.SpecStatsSlider[sliderId] = Options[sliderId]
                        Options[sliderId]:SetVisible(true)
                    elseif Shared.SpecStatsSlider[sliderId] then
...
local function B(sec, titleOrCfg, fn)
    ...
    if type(titleOrCfg) == "string" then title = titleOrCfg; callback = fn
    else title = titleOrCfg.Text or titleOrCfg.Title or "Button"; callback = titleOrCfg.Func or titleOrCfg.Callback or fn end
...
-- Helper: buat SubPage + Section default di dalamnya
-- Brigida: Tab:AddSubPage({Title="x"}) → sub:AddSection({Title=""}) → section bisa AddToggle dll
local function SP(tab, title)
    local sub = tab:AddSubPage({ Title = title })
    local sec = sub:AddSection({ Title = "" })
    return sec
end
    ...
    for codeName, data in pairs(allCodes) do
        ...
        if playerLevel >= levelReq then
            ...
            if rewards then
                ...
                rewardText = rewardText .. string.format("\nCurrency: %s Coins, %s Gems", moneyStr, rewards.Gems or 0)
...
-- ============================================================
-- Farm SubPage
local sfAF_Farm = SP(Tabs.Main, "Farm")
D(sfAF_Farm, "SelectedMob", { Text = "Select Mob (s)", Values = Tables.MobList, Multi = true })
B(sfAF_Farm, "Refresh", UpdateNPCLists)
T(sfAF_Farm, "MobFarm",    { Text = "Autofarm Selected Mob", Default = false })
T(sfAF_Farm, "AllMobFarm", { Text = "Autofarm All Mobs",     Default = false })
D(sfAF_Farm, "AllMobType", { Text = "Select Type [All Mob]", Values = { "Normal", "Fast" }, Default = "Normal" })
T(sfAF_Farm, "LevelFarm",  { Text = "Autofarm Level",        Default = false })

-- Boss SubPage
local sfAF_Boss = SP(Tabs.Main, "Boss")
D(sfAF_Boss, "SelectedBosses",    { Text = "Select Bosses",       Values = Tables.BossList,        Multi = true  })
T(sfAF_Boss, "BossesFarm",        { Text = "Autofarm Selected Boss", Default = false })
T(sfAF_Boss, "AllBossesFarm",     { Text = "Autofarm All Bosses",    Default = false })
D(sfAF_Boss, "SelectedSummon",    { Text = "Select Summon Boss",  Values = Tables.SummonList,      Multi = false })
D(sfAF_Boss, "SelectedSummonDiff",{ Text = "Select Difficulty",   Values = Tables.DiffList,        Multi = false })
T(sfAF_Boss, "AutoSummon",        { Text = "Auto Summon",          Default = false })
T(sfAF_Boss, "SummonBossFarm",    { Text = "Autofarm Summon Boss", Default = false })
PityLabel = L(sfAF_Boss, "<b>Pity:</b> 0/25")
D(sfAF_Boss, "SelectedBuildPity", { Text = "Select Boss [Build Pity]",    Values = Tables.AllBossList, Multi = true  })
D(sfAF_Boss, "SelectedUsePity",   { Text = "Select Boss [Use Pity]",      Values = Tables.AllBossList, Multi = false })
D(sfAF_Boss, "SelectedPityDiff",  { Text = "Select Difficulty [Use Pity]",Values = Tables.DiffList,    Multi = false })
T(sfAF_Boss, "PityBossFarm",      { Text = "Autofarm Pity Boss",          Default = false })
D(sfAF_Boss, "SelectedOtherSummon",    { Text = "Select Summon Boss (Other)", Values = Tables.OtherSummonList, Multi = false })
D(sfAF_Boss, "SelectedOtherSummonDiff",{ Text = "Select Difficulty",          Values = Tables.DiffList,        Multi = false })
T(sfAF_Boss, "AutoOtherSummon",   { Text = "Auto Summon",          Default = false })
T(sfAF_Boss, "OtherSummonFarm",   { Text = "Autofarm Summon Boss", Default = false })

-- Alt SubPage
local sfAF_Alt = SP(Tabs.Main, "Alt Help")
D(sfAF_Alt, "SelectedAltBoss", { Text = "Select Boss",       Values = Tables.AllBossList, Multi = false })
D(sfAF_Alt, "SelectedAltDiff", { Text = "Select Difficulty", Values = Tables.DiffList,    Multi = false })
for i = 1, 5 do D(sfAF_Alt, "SelectedAlt_" .. i, { Text = "Select Alt #" .. i, Values = {}, Multi = false }) end
T(sfAF_Alt, "AltBossFarm", { Text = "Auto Help Alt", Default = false })

-- Config SubPage
local sfAF_Cfg = SP(Tabs.Main, "Config")
D(sfAF_Cfg, "SelectedWeaponType",   { Text = "Select Weapon Type",   Values = Tables.Weapon, Multi = true })
S(sfAF_Cfg, "SwitchWeaponCD",       { Text = "Switch Weapon Delay",  Default = 4,    Min = 1, Max = 20,   Rounding = 0 })
T(sfAF_Cfg, "SwitchWeapon",         { Text = "Auto Switch Weapon",   Default = true })
T(sfAF_Cfg, "IslandTP",             { Text = "Island TP [Autofarm]", Default = true })
S(sfAF_Cfg, "IslandTPCD",           { Text = "Island TP CD",         Default = 0.67, Min = 0, Max = 2.5,  Rounding = 2 })
S(sfAF_Cfg, "TargetTPCD",           { Text = "Target TP CD",         Default = 0,    Min = 0, Max = 5,    Rounding = 2 })
S(sfAF_Cfg, "TargetDistTP",         { Text = "Target Distance TP",   Default = 0,    Min = 0, Max = 100,  Rounding = 0 })
S(sfAF_Cfg, "M1Speed",              { Text = "M1 Attack Cooldown",   Default = 0,    Min = 0, Max = 1,    Rounding = 2 })
D(sfAF_Cfg, "SelectedMovementType", { Text = "Select Movement Type", Values = { "Teleport", "Tween" }, Default = "Tween" })
D(sfAF_Cfg, "SelectedFarmType",     { Text = "Select Farm Type",     Values = { "Behind", "Above", "Below" }, Default = "Behind" })
S(sfAF_Cfg, "Distance",             { Text = "Farm Distance",        Default = 12,   Min = 0, Max = 30,   Rounding = 0 })
S(sfAF_Cfg, "TweenSpeed",           { Text = "Tween Speed",          Default = 160,  Min = 0, Max = 500,  Rounding = 0 })
T(sfAF_Cfg, "InstaKill",            { Text = "Instant Kill",         Default = false })
D(sfAF_Cfg, "InstaKillType",        { Text = "Select Type",          Values = { "V1", "V2" }, Default = "V1" })
S(sfAF_Cfg, "InstaKillHP",          { Text = "HP% For Insta-Kill",   Default = 90,   Min = 1, Max = 100,  Rounding = 0 })
I(sfAF_Cfg, "InstaKillMinHP",       { Text = "Min MaxHP for Insta-Kill", Placeholder = "Number.." })

-- Haki SubPage
local sfMA_Haki = SP(Tabs.Main, "Haki")
T(sfMA_Haki, "ObserHaki",    { Text = "Auto Observation Haki", Default = false })
T(sfMA_Haki, "ArmHaki",      { Text = "Auto Armament Haki",    Default = false })
T(sfMA_Haki, "ConquerorHaki",{ Text = "Auto Conqueror Haki",   Default = false })

-- Skill SubPage
local sfMA_Skill = SP(Tabs.Main, "Skill")
L(sfMA_Skill, "Autofarm already has <b>auto-M1 built in</b>.\nEnable this only if you have issues with autofarm M1.")
T(sfMA_Skill, "AutoM1",   { Text = "Auto Attack", Default = false })
T(sfMA_Skill, "KillAura", { Text = "Kill Aura",   Default = false })
S(sfMA_Skill, "KillAuraCD",    { Text = "Kill Aura CD",    Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
S(sfMA_Skill, "KillAuraRange", { Text = "Kill Aura Range", Default = 200, Min = 0,   Max = 200, Rounding = 0 })
L(sfMA_Skill, "Mode:\n- Normal: Check skill cooldowns\n- Instant: No check (may affect performance)")
D(sfMA_Skill, "SelectedSkills", { Text = "Select Skills", Values = { "Z", "X", "C", "V", "F" }, Multi = true })
D(sfMA_Skill, "AutoSkillType",  { Text = "Select Mode",   Values = { "Normal", "Instant" }, Default = "Normal" })
T(sfMA_Skill, "OnlyTarget",         { Text = "Target Only",      Default = false })
T(sfMA_Skill, "AutoSkill_BossOnly", { Text = "Use On Boss Only", Default = false })
S(sfMA_Skill, "AutoSkill_BossHP",   { Text = "Boss HP%",         Default = 100, Min = 1, Max = 100, Rounding = 0 })
T(sfMA_Skill, "AutoSkill",          { Text = "Auto Use Skills",  Default = false })

-- Combo SubPage
local sfMA_Combo = SP(Tabs.Main, "Combo")
L(sfMA_Combo, "Example: Z > X > C > 0.5 > V\nMode:\n- Normal: Wait input CD\n- Instant: Ignore input CD")
I(sfMA_Combo, "ComboPattern", { Text = "Combo Pattern", Default = "Z > X > C > V > F", Placeholder = "combo.." })
D(sfMA_Combo, "ComboMode",    { Text = "Select Mode", Values = { "Normal", "Instant" }, Default = "Normal" })
T(sfMA_Combo, "ComboBossOnly",{ Text = "Boss Only", Default = false })
T(sfMA_Combo, "AutoCombo",    { Text = "Auto Skill Combo", Default = false,
...
-- Switch SubPages
local sfSW_Title = SP(Tabs.Main, "Title")
local sfSW_Rune  = SP(Tabs.Main, "Rune")
local sfSW_Build = SP(Tabs.Main, "Build")
CreateSwitchGroup(sfSW_Title, "Title", "Title", CombinedTitleList)
CreateSwitchGroup(sfSW_Rune,  "Rune",  "Rune",  Tables.RuneList)
CreateSwitchGroup(sfSW_Build, "Build", "Build", Tables.BuildList)

-- ============================================================
-- AUTOMATION TAB
-- ============================================================
local sfMI_Asc = SP(Tabs.Automation, "Ascend")
T(sfMI_Asc, "AutoAscend", { Text = "Auto Ascend", Default = false })
for i = 1, 10 do
    Tables.AscendLabels[i] = L(sfMI_Asc, "")
    Tables.AscendLabels[i]:SetVisible(false)
end

local sfMI_Rll = SP(Tabs.Automation, "Rolls")
L(sfMI_Rll, "⚠️ Increase delay based on your ping.\n⚠️ Low delay settings are not recommended.")
S(sfMI_Rll, "RollCD", { Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2 })
D(sfMI_Rll, "SelectedTrait", { Text = "Select Trait (s)", Values = Tables.TraitList, Multi = true })
T(sfMI_Rll, "AutoTrait",     { Text = "Auto Roll Trait",  Default = false })
D(sfMI_Rll, "SelectedRace",  { Text = "Select Race (s)",  Values = Tables.RaceList,  Multi = true })
T(sfMI_Rll, "AutoRace",      { Text = "Auto Roll Race",   Default = false })
D(sfMI_Rll, "SelectedClan",  { Text = "Select Clan (s)",  Values = Tables.ClanList,  Multi = true })
T(sfMI_Rll, "AutoClan",      { Text = "Auto Roll Clan",   Default = false })

local sfMI_Trd = SP(Tabs.Automation, "Trade")
D(sfMI_Trd, "SelectedTradePlr",   { Text = "Select Player",   Values = {}, Multi = false })
D(sfMI_Trd, "SelectedTradeItems", { Text = "Select Item (s)", Values = Tables.OwnedItem, Multi = true })
T(sfMI_Trd, "ReqTrade",       { Text = "Auto Send Request",   Default = false })
T(sfMI_Trd, "ReqTradeAccept", { Text = "Auto Accept Request", Default = false })
T(sfMI_Trd, "AutoAccept",     { Text = "Auto Accept Trade",   Default = false })

local sfST_Lv = SP(Tabs.Automation, "Level")
D(sfST_Lv, "SelectedStats", { Text = "Select Stat (s)", Values = { "Melee", "Defense", "Sword", "Power" }, Multi = true })
T(sfST_Lv, "AutoStats",     { Text = "Auto UP Stats",   Default = false })

local sfST_Gem = SP(Tabs.Automation, "Gem")
L(sfST_Gem, "⚠️ Increase delay based on your ping.\n- Reroll once for this to work.")
StatsLabel = L(sfST_Gem, "N/A")
D(sfST_Gem, "SelectedGemStats", { Text = "Select Stat (s)", Values = Tables.GemStat, Multi = true })
D(sfST_Gem, "SelectedRank",     { Text = "Select Rank (s)", Values = Tables.GemRank, Multi = true })
S(sfST_Gem, "StatsRollCD",      { Text = "Roll Delay",      Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(sfST_Gem, "AutoRollStats",    { Text = "Auto Roll Stats", Default = false })

local sfST_Mi = SP(Tabs.Automation, "Misc")
T(sfST_Mi, "AutoSkillTree",     { Text = "Auto Skill Tree",         Default = false })
T(sfST_Mi, "ArtifactMilestone", { Text = "Auto Artifact Milestone", Default = false })

local sfEN_Enc = SP(Tabs.Automation, "Enchant")
D(sfEN_Enc, "SelectedEnchant",  { Text = "Select Enchant",  Values = Tables.OwnedAccessory, Multi = true })
T(sfEN_Enc, "AutoEnchant",      { Text = "Auto Enchant",     Default = false })
T(sfEN_Enc, "AutoEnchantAll",   { Text = "Auto Enchant All", Default = false })
D(sfEN_Enc, "SelectedBlessing", { Text = "Select Blessing",  Values = Tables.OwnedWeapon,    Multi = true })
T(sfEN_Enc, "AutoBlessing",     { Text = "Auto Blessing",     Default = false })
T(sfEN_Enc, "AutoBlessingAll",  { Text = "Auto Blessing All", Default = false })

local sfEN_Pas = SP(Tabs.Automation, "Passive")
SpecPassiveLabel = L(sfEN_Pas, "N/A")
D(sfEN_Pas, "SelectedPassive", { Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons, Multi = true })
D(sfEN_Pas, "SelectedSpec",    { Text = "Target Passives",   Values = Tables.SpecPassive,     Multi = true })
S(sfEN_Pas, "SpecRollCD",      { Text = "Roll Delay",        Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(sfEN_Pas, "AutoSpec",        { Text = "Auto Reroll Passive", Default = false })

local sfEN_Cfg = SP(Tabs.Automation, "Enc.Config")
sfEN_Cfg_ref = sfEN_Cfg  -- untuk UpdatePassiveSliders
L(sfEN_Cfg, "⚠️ Advanced users only.\nWill stop once passive reaches min value.")
...
-- DUNGEON TAB
-- ============================================================
local sfDG_Join = SP(Tabs.Dungeon, "Auto Join")
L(sfDG_Join, "BossRush & Infinite Tower Supported.")
D(sfDG_Join, "SelectedDungeon",   { Text = "Select Dungeon",     Values = Tables.DungeonList, Multi = false })
T(sfDG_Join, "AutoDungeon",       { Text = "Auto Join Dungeon",   Default = false })
T(sfDG_Join, "AutoInfiniteTower", { Text = "Auto Infinite Tower", Default = false })

local sfDG_Cfg = SP(Tabs.Dungeon, "Config")
DungeonCount = L(sfDG_Cfg, "Dungeon Completed: N/A")
D(sfDG_Cfg, "SelectedDiff",    { Text = "Select Difficulty", Values = { "Easy", "Medium", "Hard", "Extreme" }, Multi = false })
T(sfDG_Cfg, "AutoDiff",        { Text = "Auto Select Difficulty", Default = false })
T(sfDG_Cfg, "AutoReplay",      { Text = "Auto Replay",            Default = false })
T(sfDG_Cfg, "DungeonAutofarm", { Text = "Autofarm Dungeon",        Default = false })
...
-- TELEPORT TAB
-- ============================================================
local sfWP_Isl = SP(Tabs.Teleport, "Island")
D(sfWP_Isl, "SelectedIsland", {
...
local sfWP_Qst = SP(Tabs.Teleport, "Quest NPC")
D(sfWP_Qst, "SelectedQuestNPC", {
...
B(sfWP_Qst, "Teleport to Level Based Quest", function()
...
local sfWP_Msc = SP(Tabs.Teleport, "Misc NPC")
D(sfWP_Msc, "SelectedMiscNPC", {
...
local sfNPC_MV = SP(Tabs.Teleport, "Moveset")
D(sfNPC_MV, "SelectedMovesetNPC", {
    Text = "Select NPC", Values = Tables.NPC_MovesetList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

local sfNPC_MS = SP(Tabs.Teleport, "Mastery")
D(sfNPC_MS, "SelectedMasteryNPC", {
...
-- MISC TAB
-- ============================================================
local sfMC_Reg = SP(Tabs.Misc, "Regular")
MerchantTimerLabel = L(sfMC_Reg, "Refresh: N/A")
D(sfMC_Reg, "SelectedMerchantItems", { Text = "Select Item (s)", Values = Tables.MerchantList, Multi = true })
T(sfMC_Reg, "AutoMerchant",          { Text = "Auto Buy Selected Items", Default = false })

local sfMC_Dun = SP(Tabs.Misc, "Dungeon Shop")
D(sfMC_Dun, "SelectedDungeonMerchantItems", { Text = "Select Item (s)", Values = Tables.DungeonMerchantList or {}, Multi = true })
T(sfMC_Dun, "AutoDungeonMerchant",          { Text = "Auto Buy Dungeon Items", Default = false })

local sfMC_Val = SP(Tabs.Misc, "Valentine")
L(sfMC_Val, "Able to cause performance issues.")
ValentineLabel = L(sfMC_Val, "Hearts: N/A")
D(sfMC_Val, "SelectedValentineMerchantItems", { Text = "Select Item (s)", Values = Tables.ValentineMerchantList, Multi = true })
ValentinePriceLabel = L(sfMC_Val, "Price: N/A")
T(sfMC_Val, "AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

local sfMC2_Twr = SP(Tabs.Misc, "Tower Shop")
D(sfMC2_Twr, "SelectedTowerMerchantItems", { Text = "Select Item (s)", Values = Tables.InfiniteTowerMerchantList or {}, Multi = true })
T(sfMC2_Twr, "AutoTowerMerchant",          { Text = "Auto Buy Tower Items", Default = false })

local sfMC2_BR = SP(Tabs.Misc, "Boss Rush Shop")
D(sfMC2_BR, "SelectedBossRushMerchantItems", { Text = "Select Item (s)", Values = Tables.BossRushMerchantList or {}, Multi = true })
T(sfMC2_BR, "AutoBossRushMerchant",          { Text = "Auto Buy Boss Rush Items", Default = false })

local sfMI2_Ch = SP(Tabs.Misc, "Chests")
D(sfMI2_Ch, "SelectedChests", { Text = "Select Chest (s)", Values = Tables.Rarities,     Multi = true })
T(sfMI2_Ch, "AutoChest",      { Text = "Auto Open Chest",  Default = false })

local sfMI2_Cr = SP(Tabs.Misc, "Craft")
D(sfMI2_Cr, "SelectedCraftItems", { Text = "Select Item (s) To Craft", Values = Tables.CraftItemList, Multi = true })
T(sfMI2_Cr, "AutoCraftItem",      { Text = "Auto Craft Item",          Default = false })

local sfMI2_Nt = SP(Tabs.Misc, "Notify")
T(sfMI2_Nt, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

local sfQS_Puz = SP(Tabs.Misc, "Puzzles")
B(sfQS_Puz, { Text = "Complete Dungeon Puzzle",   Disabled = not Support.Proximity, Func = function()
...
B(sfQS_Puz, { Text = "Complete Slime Key Puzzle", Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Slime") end })
B(sfQS_Puz, { Text = "Complete Valentine Puzzle", Disabled = not Support.Proximity, Func = function()
    local Char = GetCharacter()
    Remotes.TP_Portal:FireServer("Valentine"); task.wait(2.5)
    for i = 1, 3 do
...
B(sfQS_Puz, { Text = "Complete Demonite Puzzle",  Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Demonite") end })
B(sfQS_Puz, { Text = "Complete Hogyoku Puzzle",   Disabled = not Support.Proximity, Func = function()
    local lv = Plr.Data.Level.Value
    if lv >= 8500 then UniversalPuzzleSolver("Hogyoku")
    else Library:Notify("Level 8500 required! Current: " .. lv, 4) end
end })

local sfQS_Qln = SP(Tabs.Misc, "Questlines")
L(sfQS_Qln, "⚠️ Experimental feature.\n⚠️ Store your race & clan before using.\n- Dungeon tasks only make you join dungeon.")
D(sfQS_Qln, "SelectedQuestline",          { Text = "Select Questline",       Values = Tables.QuestlineList,   Multi = false })
D(sfQS_Qln, "SelectedQuestline_Player",   { Text = "Select Player",          Values = {},                     Multi = false })
D(sfQS_Qln, "SelectedQuestline_DMGTaken", { Text = "Select Mob [Take Damage]",Values = Tables.AllEntitiesList, Multi = false })
B(sfQS_Qln, "Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)
T(sfQS_Qln, "AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })
