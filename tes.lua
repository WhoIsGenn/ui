...
if getgenv().VictoriaHub_Running then
...
getgenv().VictoriaHub_Running = true
...
local Remotes = {
    ...
    -- FIX: Path M1 yang benar
    M1 = (RS:FindFirstChild("CombatSystem") and RS.CombatSystem:FindFirstChild("Remotes") and RS.CombatSystem.Remotes:FindFirstChild("RequestHit")) 
        or (RS:FindFirstChild("RemoteEvents") and RS.RemoteEvents:FindFirstChild("RequestHit"))
        or (RS:FindFirstChild("Remotes") and RS.Remotes:FindFirstChild("RequestHit")),
    
    EquipWeapon = GetRemote(RS, "Remotes.EquipWeapon") or (RS:FindFirstChild("Remotes") and RS.Remotes:FindFirstChild("EquipWeapon")),
    
    UseSkill = (RS:FindFirstChild("AbilitySystem") and RS.AbilitySystem:FindFirstChild("Remotes") and RS.AbilitySystem.Remotes:FindFirstChild("RequestAbility"))
        or (RS:FindFirstChild("RemoteEvents") and RS.RemoteEvents:FindFirstChild("RequestAbility")),
    ...
    TP_Portal = GetRemote(RS, "Remotes.TeleportToPortal") or (RS:FindFirstChild("Remotes") and RS.Remotes:FindFirstChild("TeleportToPortal")),
...
-- DEBUG: Cek apakah remote M1 ditemukan
task.spawn(function()
    task.wait(3)
    if Remotes.M1 then
        print("✅ M1 Remote FOUND!")
    else
        warn("❌ M1 Remote NOT FOUND! Coba cari manual:")
        -- Cari manual
        local function search(obj, depth)
            if depth > 3 then return end
            for _, child in ipairs(obj:GetChildren()) do
                if child.Name:lower():find("requesthit") or child.Name:lower():find("m1") or child.Name:lower():find("attack") then
...
-- FUNGSI KILL AURA YANG LEBIH BAGUS
local function Func_KillAura()
    local lastAttack = 0
    local killRange = Options.KillAuraRange and Options.KillAuraRange.Value or 200
    local attackDelay = Options.KillAuraCD and Options.KillAuraCD.Value or 0.12
    
    while getgenv().VictoriaHub_Running do
        task.wait(0.05)
        ...
        if nearestTarget and tick() - lastAttack >= attackDelay then
            local targetPos = nearestTarget:GetPivot().Position
            ...
            if Remotes.M1 then
                ...
                if combat and combat:FindFirstChild("Remotes") then
                    local m1 = combat.Remotes:FindFirstChild("RequestHit")
                    if m1 then pcall(function() m1:FireServer(targetPos) end) end
                end
            end
            
            lastAttack = tick()
            Shared.Target = nearestTarget
        end
    end
end
...
-- FUNGSI AUTO M1 YANG LEBIH BAGUS
local function Func_AutoM1()
    local lastAttack = 0
    local attackDelay = Options.M1Speed and Options.M1Speed.Value or 0.15
    
    while getgenv().VictoriaHub_Running do
        task.wait(0.05) -- Loop lebih cepat untuk respons lebih baik
        
        if not (Toggles.AutoM1 and Toggles.AutoM1.Value) then
            task.wait(0.5)
            continue
        end
        ...
        if not target or not target.Parent then
            target = GetNearestAuraTarget() -- Cari target terdekat
        end
        
        if target and IsValidTarget(target) and tick() - lastAttack >= attackDelay then
            ...
            -- Coba M1 lewat berbagai metode
            local success = false
            
            if Remotes.M1 then
                pcall(function()
                    -- Method 1: FireServer dengan position
                    Remotes.M1:FireServer(targetPos)
                    success = true
                end)
                if not success then
                    pcall(function()
                        -- Method 2: FireServer tanpa parameter
                        Remotes.M1:FireServer()
                        success = true
                    end)
                end
            ...
            -- Fallback: cari remote M1 manual
            if not success then
                local combat = RS:FindFirstChild("CombatSystem")
                if combat then
                    local remotes = combat:FindFirstChild("Remotes")
                    if remotes then
                        local m1 = remotes:FindFirstChild("RequestHit")
                        if m1 then
                            pcall(function() m1:FireServer(targetPos) end)
                            Remotes.M1 = m1
                        end
                    end
                end
...
local Window = Library:CreateWindow({
	Title = "VictoriaHub",
	Footer = "" .. assetName .. " | by VictoriaHub",
	NotifySide = "Right",
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().VictoriaHub_Running then break end
...
MenuGroup:AddButton("Unload", function()
    getgenv().VictoriaHub_Running = false
    Shared.Farm = false
    Cleanup(Connections)
    Cleanup(Flags)
	Library:Unload()
...
ThemeManager:SetFolder("VictoriaHub")
SaveManager:SetFolder("VictoriaHub/SailorPiece")
