...
-- Intercept workspace:Raycast - redirect ke killer terdekat
-- ================================================================
...
local function GunSilentAim_Setup()
    ...
    if not hookmetamethod then
        warn("[SilentAim] hookmetamethod not available")
        return
    end
    ...
    OriginalNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
        ...
        if Config.GUN_SilentAim and self == workspace and method == "Raycast" and not checkcaller() then
            local origin = args[2]
            local direction = args[3]
            if typeof(origin) == "Vector3" and typeof(direction) == "Vector3" then
                local cam = workspace.CurrentCamera
                -- Skip kalau origin dekat camera (camera raycast) atau direction pendek
                local camDist = cam and (origin - cam.CFrame.Position).Magnitude or 999
                local dirMag = direction.Magnitude
                if dirMag > 50 and camDist > 5 then
                    local target = GunSilentAim_GetTarget()
                    if target then
                        args[3] = (target.Position - origin).Unit * dirMag
    ...
    print("[SilentAim] Hooked workspace raycast")
end
