    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if kr and (kr.Position - root.Position).Magnitude <= Config.PARRY_Dist then
                ...
                return  -- cukup 1 killer per loop
            end
        end
    end
end
...
local function SetupSkillCheckMonitor()
    ...
    pcall(function()
        ...
        -- Generator QTE
        local genFolder = remotes:WaitForChild("Generator", 5)
        if genFolder then
            local genEvent    = genFolder:FindFirstChild("SkillCheckEvent")
            local genResult   = genFolder:FindFirstChild("SkillCheckResultEvent")
            if genEvent and genResult then
                local c = genEvent.OnClientEvent:Connect(function()
                    if not Config.AUTO_SkillCheck then return end
                    -- Tunggu sebentar agar QTE frame muncul dulu
                    task.wait(0.05)
                    pcall(function() genResult:FireServer("success", 1) end)
                end)
        ...
        -- Healing QTE
        local healFolder = remotes:WaitForChild("Healing", 5)
        if healFolder then
            local healEvent  = healFolder:FindFirstChild("SkillCheckEvent")
            local healResult = healFolder:FindFirstChild("SkillCheckResultEvent")
            if healEvent and healResult then
                local c = healEvent.OnClientEvent:Connect(function()
                    if not Config.AUTO_SkillCheck then return end
                    task.wait(0.05)
                    pcall(function() healResult:FireServer("success", 1) end)
                end)
                table.insert(QTE_Conns, c)
            end
        end
