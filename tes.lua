...
local secSkill = Tabs.Main:AddSection({ Title = "Skill", Open = false })
L(secSkill, "Autofarm has auto-M1 built in. Only enable if you have issues.")
T(secSkill, "AutoM1",   { Text = "Auto Attack", Default = false })
T(secSkill, "KillAura", { Text = "Kill Aura",   Default = false })
S(secSkill, "KillAuraCD",    { Text = "Kill Aura CD",    Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
...
T(secSkill, "OnlyTarget",         { Text = "Target Only",      Default = false })
T(secSkill, "AutoSkill_BossOnly", { Text = "Use On Boss Only", Default = false })
...
-- ============================================================
-- ============================================================
-- FARM & KILL LOGIC (CLEAN REWRITE)
-- ============================================================

-- Scan NPCs dari workspace secara dinamis
local function ScanNPCs()
    local mobs, bosses = {}, {}
    local npcFolder = workspace:FindFirstChild("NPCs")
    if not npcFolder then return mobs, bosses end

    local seenMob, seenBoss = {}, {}
    for _, v in ipairs(npcFolder:GetDescendants()) do
        if v:IsA("Model") and v:FindFirstChildOfClass("Humanoid") then
            local cleanName = v.Name:gsub("%d+$","")
            local isBoss = cleanName:find("Boss") or cleanName:find("boss")
            if isBoss then
                if not seenBoss[cleanName] then seenBoss[cleanName]=true; table.insert(bosses, cleanName) end
            else
                if not seenMob[cleanName] then seenMob[cleanName]=true; table.insert(mobs, cleanName) end
            end
        end
    end
    return mobs, bosses
...
-- Refresh dropdown NPC/Boss lists dari workspace
local function RefreshLists()
    local mobs, bosses = ScanNPCs()
    -- Merge dengan existing list tanpa duplikat
    local existMob = {}
    for _, v in ipairs(Tables.MobList) do existMob[v]=true end
    for _, v in ipairs(mobs) do
        if not existMob[v] then table.insert(Tables.MobList, v); existMob[v]=true end
    end
    local existBoss = {}
    for _, v in ipairs(Tables.BossList) do existBoss[v]=true end
    for _, v in ipairs(bosses) do
        if not existBoss[v] then table.insert(Tables.BossList, v); existBoss[v]=true end
        if not existBoss[v.."_all"] then table.insert(Tables.AllBossList, v) end
    end
...
-- Auto-scan sekali saat start
task.spawn(function()
    task.wait(3)
    RefreshLists()
    -- Re-scan setiap 30 detik untuk update jika boss/mob baru spawn
...
-- ============================================================
-- ISLAND/PORTAL NAVIGATION
-- ============================================================
-- Cari island terdekat dari posisi target menggunakan IslandCrystals
local function GetIslandForTarget(targetPos, targetName)
    -- Cek BossTIMap dulu (explicit mapping dari game)
    if targetName and Shared.BossTIMap[targetName] then
        return Shared.BossTIMap[targetName]
    end
    -- Cek MobToIsland
    if targetName and Tables.MobToIsland[targetName] then
...
-- Teleport ke island via portal jika beda island
local function TeleportToIsland(islandName)
    if not islandName or islandName == "" or islandName == Shared.Island then return end
    Shared.MovingIsland = true
    pcall(function() Remotes.TP_Portal:FireServer(islandName) end)
    task.wait(tonumber(Options.IslandTPCD and Options.IslandTPCD.Value) or 1.2)
...
local function EquipCorrectWeapon()
    ...
    for _, v in pairs(enabledTypes) do if v then hasFilter = true; break end end

    -- Kalau sudah pegang tool yang sesuai, skip
    ...
    -- Equip tool yang sesuai dari backpack
    local containers = {Plr.Backpack, char}
    for _, container in ipairs(containers) do
        for _, tool in ipairs(container:GetChildren()) do
            if tool:IsA("Tool") then
...
local function MoveToTarget(target)
    ...
    elseif fType == "Below" then
        ...
        finalPos = (pivot * CFrame.new(0, 0, dist)).Position
    end
    ...
    local moveType = Options.SelectedMovementType and Options.SelectedMovementType.Value or "Tween"
    if moveType == "Teleport" then
        ...
        _activeTween = tween
        tween:Play()
        -- Tunggu tween selesai atau target mati
        local elapsed = 0
        ...
        -- Setelah tween, stop velocity agar tidak melayang
        root.AssemblyLinearVelocity = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
        _activeTween = nil
    end
...
-- ATTACK & SKILL
-- ============================================================
local function DoAttackAndSkill(target)
    if not target or not target.Parent then return end
    local hum = target:FindFirstChildOfClass("Humanoid")
    if not hum or hum.Health <= 0 then return end

    -- Equip senjata yang benar dulu
    EquipCorrectWeapon()
    ...
    -- M1 Attack
    local m1Delay = tonumber(Options.M1Speed and Options.M1Speed.Value) or 0.05
    if tick() - (Shared.LastM1 or 0) >= m1Delay then
        pcall(function() Remotes.M1:FireServer() end)
        Shared.LastM1 = tick()
    end

    -- Skills jika AutoSkill ON
    if Toggles.AutoSkill and Toggles.AutoSkill.Value then
        ...
        for _, key in ipairs({"Z","X","C","V","F"}) do
            if selected[key] then
                -- Cek boss-only filter
                if Toggles.AutoSkill_BossOnly and Toggles.AutoSkill_BossOnly.Value then
                    if not target.Name:find("Boss") then break end
                end
                -- Cek cooldown jika Normal mode
                if useMode == "Normal" and not (IsSkillReady and IsSkillReady(key)) then continue end
...
local function FindTarget()
    ...
    if not isBossFarm and not isMobFarm then return nil, nil end

    local selBosses = Options.SelectedBosses and Options.SelectedBosses.Value or {}
    local selMobs   = Options.SelectedMob and Options.SelectedMob.Value or {}

    local bestTarget, bestDist, bestType = nil, math.huge, nil
        ...
        if obj:IsA("Model") then
            ...
            if hum and hum.Health > 0 and objRoot then
                ...
                if isBoss and isBossFarm then
                    isSelected = Toggles.AllBossesFarm.Value or selBosses[obj.Name] or selBosses[cleanName]
                    tType = "Boss"
                elseif not isBoss and isMobFarm then
                    isSelected = Toggles.AllMobFarm.Value or selMobs[obj.Name] or selMobs[cleanName]
                    tType = "Mob"
                end

                if isSelected then
                    local d = (root.Position - objRoot.Position).Magnitude
                    -- Prioritaskan Boss > Mob
                    if tType == "Boss" and bestType ~= "Boss" then
                        bestTarget, bestDist, bestType = obj, d, tType
                    elseif tType == bestType and d < bestDist then
                        bestTarget, bestDist, bestType = obj, d, tType
...
-- ============================================================
-- HANDLE GAME PAUSE (auto-dismiss)
-- ============================================================
local function HandleGamePause()
    pcall(function()
        local pauseGui = game:GetService("CoreGui").RobloxGui
        local pauseFrame = pauseGui:FindFirstChild("CoreScripts/NetworkPause")
            or pauseGui:FindFirstChild("NetworkPauseScreen")
        if pauseFrame then pauseFrame:Destroy() end
    end)
    -- Coba klik tombol OK/resume jika ada
    pcall(function()
        for _, gui in ipairs(game:GetService("CoreGui"):GetDescendants()) do
            if gui:IsA("TextButton") and (gui.Text:find("OK") or gui.Text:find("Resume") or gui.Text:find("Continue")) then
                gui:Fire("Activated")
            end
        end
    end)
...
-- ============================================================
-- MAIN FARM + KILL AURA LOOP (GABUNGAN)
-- ============================================================
task.spawn(function()
    Shared.LastM1 = tick()
    ...
    while getgenv().celina_Running do
        ...
        -- Handle game pause
        if Toggles.NoGameplayPaused and Toggles.NoGameplayPaused.Value then
            HandleGamePause()
        end

        -- Kill Aura (jika ON, cari target terdekat dalam range dan serang)
                ...
                if npcFolder then
                    ...
                    for _, obj in ipairs(npcFolder:GetDescendants()) do
                        if obj:IsA("Model") and obj ~= char then
                            ...
                            local objRoot = obj:FindFirstChild("HumanoidRootPart")
                            if hum and hum.Health > 0 and objRoot then
                                if (root.Position - objRoot.Position).Magnitude <= range then
                                    pcall(function() Remotes.M1:FireServer() end)
        ...
        -- Farm loop: hanya jalan jika ada mode farm aktif
        local isFarmActive = Toggles.MobFarm.Value or Toggles.AllMobFarm.Value
        ...
        -- Cek apakah perlu ganti island via portal
        if Toggles.IslandTP and Toggles.IslandTP.Value then
            local targetPos = target:GetPivot().Position
            local cleanName = target.Name:gsub("%d+$","")
            local neededIsland = GetIslandForTarget(targetPos, cleanName)
            if neededIsland ~= Shared.Island then
                TeleportToIsland(neededIsland)
                task.wait(0.5)
                continue -- loop lagi setelah TP
            end
        end

        -- Pastikan target masih valid setelah mungkin ada island TP
        ...
        if root and targetRoot then
            ...
            local attackRange = tonumber(Options.Distance and Options.Distance.Value) or 12

            if currentDist > attackRange + 3 then
                -- Masih jauh, gerak dulu
                MoveToTarget(target)
            else
                -- Sudah dalam range, stop tween & serang
                StopTween()
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
                DoAttackAndSkill(target)
            end
        end
    end
end)
