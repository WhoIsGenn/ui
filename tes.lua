...
-- ================================================================
-- PARRY FOV CIRCLE (3D - Beam outline + fill)
-- Struktur persis dari contoh yang sudah terbukti work
-- ================================================================
local parryFOVHolder    = nil
local parryFOVFillPart  = nil
local parryFOVBeams     = {}
...
local function ParryFOV_Create()
    ...
    -- Holder: invisible Part langsung di workspace (persis contoh)
    ...
    -- Fill Part: flat cylinder (persis contoh, parent ke holder)
    local fillPart = Instance.new("Part")
    fillPart.Size         = Vector3.new(r * 2, 0.1, r * 2)
    ...
    for i = 1, beamCount do
        ...
        att1.Parent   = holder
        table.insert(parryFOVObjects, att1)

        local att2 = Instance.new("Attachment")
        att2.Position = Vector3.new(math.cos(a2)*r, 0, math.sin(a2)*r)
        att2.Parent   = holder
        table.insert(parryFOVObjects, att2)
        ...
        table.insert(parryFOVBeams, {beam=beam, decal=nil})
    end

    -- RenderStepped: update posisi holder (Y-2.9 persis contoh)
    parryFOVConn = RunService.RenderStepped:Connect(function()
        ...
        -- Holder di lantai (Y-2.9 dari HRP, persis contoh)
        holder.CFrame   = CFrame.new(root.Position.X, root.Position.Y - 2.9, root.Position.Z)
        -- fillPart diputar 90 derajat agar top face menghadap atas
        fillPart.CFrame = CFrame.new(root.Position.X, root.Position.Y - 2.9, root.Position.Z)
                        * CFrame.Angles(0, 0, math.rad(90))

        -- Update warna sesuai cooldown state
        local newCol = ParryOnCooldown
            and Color3.fromRGB(255, 60, 60)
            or  Color3.fromRGB(0, 170, 255)
        local colSeq = ColorSequence.new(newCol)
        decal.Color3 = newCol
        for _, bd in ipairs(parryFOVBeams) do
            bd.beam.Color = colSeq
        end
    end)
end
...
local function UpdateParryFOV()
    ...
    if not parryFOVHolder or not parryFOVHolder.Parent then
        ParryFOV_Create()
    end
end
