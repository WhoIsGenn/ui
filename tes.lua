...
local Config = {
    ...
    AUTO_Parry = false,
    PARRY_Dist      = 15,
    PARRY_Visual    = true,   -- tampilkan lingkaran range parry
    GODMODE         = false,
    ANTI_FailGen    = false,  -- Anti Fail Gen (disable Skillcheck-gen script)
    ANTI_SkillCheck = false,  -- Anti Skill Check (disable Skillcheck-player script)
    PERFECT_SkillCheck = false, -- Perfect Skill Check (VirtualInputManager)
    SURV_NoFall = false,
...
local function GetRole()
    if not LocalPlayer.Team then return "Spectator" end
    local name = LocalPlayer.Team.Name
    if name == "Killer" then return "Killer" end
    if name == "Survivors" then return "Survivor" end
    if name:lower():find("spectator") or name:lower():find("lobby") then return "Spectator" end
    return "Spectator"
end
...
-- ================================================================
-- ROLE MANAGER
-- Detect team change → suspend fitur saat Spectator (lobby)
-- Re-apply saat masuk match sebagai Killer / Survivor
-- ================================================================
local RoleManagerConn   = nil
local LastRole          = "Spectator"

-- ================================================================
-- SUSPEND: matikan semua fitur aktif, tanpa mengubah Config
-- Dipanggil saat masuk Spectator/lobby
-- ================================================================
local function SuspendAllFeatures()
    pcall(Godmode_Remove)
    ...
    pcall(function()
        ...
        if hum then
            if State and State.OriginalSpeed then hum.WalkSpeed = State.OriginalSpeed end
            hum.JumpPower = 50
        end
    end)
    SpeedWasOn       = false
    NoSlowdownReady  = false
    -- Reset remote caches supaya nanti dicari ulang
    ParryRemote      = nil
    ParrySlowRemote  = nil
...
-- ================================================================
-- REHOOK: dipanggil setiap kali character baru siap DAN role bukan Spectator
-- Ini yang handle: role change Spectator→Match, ATAU respawn dalam match
-- ================================================================
local function RehookForRole(char, role)
    if State.Unloaded then return end
    if role == "Spectator" then return end

    -- Reset state
    State.KillerBaseSpeed = nil
    SpeedWasOn            = false
    NoSlowdownReady       = false
    ParryRemote           = nil
    ParrySlowRemote       = nil
    ...
    task.delay(1.5, function()
        if not State.Unloaded and GetRole() == role then
            NoSlowdownReady = true
        end
    end)

    -- === Semua role ===
    if Config.GODMODE then
        task.spawn(Godmode_Apply)
    end

    -- Perfect Skill Check: hook ulang GUI karena di-recreate tiap match
    if Config.PERFECT_SkillCheck then
        task.delay(1, function()
            if not State.Unloaded and GetRole() ~= "Spectator" then
                pcall(PerfectSC_Stop)   -- cleanup hook lama dulu
                pcall(PerfectSC_Setup)  -- hook ke GUI baru
            end
        end)
    end
    ...
    if role == "Survivor" then
        -- Anti Fail Gen / Anti Skill Check: hook ulang ke char baru
        -- Delay karena Skillcheck-gen/player spawn beberapa detik setelah char
        if Config.ANTI_FailGen or Config.ANTI_SkillCheck then
            task.delay(2, function()
                if not State.Unloaded and GetRole() == "Survivor" then
                    local c = LocalPlayer.Character
                    if c then pcall(AntiScript_Apply, c) end
                end
            end)
        end

        -- Auto Parry: cleanup hook lama, hook ulang ke remote baru
        if Config.AUTO_Parry then
            for _, c in ipairs(KillerHitConns) do pcall(function() c:Disconnect() end) end
            KillerHitConns = {}
            task.delay(1, function()
                if not State.Unloaded and GetRole() == "Survivor" then
                    pcall(AutoParry_HookAttackRemote)
                end
            end)
        end
    end
    ...
    -- === Killer only ===
    if role == "Killer" then
        -- NoSlowdown sudah di-handle via NoSlowdownReady di atas
        -- Setup ulang NoPalletStun & AntiBlind (hook ke remote baru)
        task.delay(1, function()
            if not State.Unloaded and GetRole() == "Killer" then
...
-- ================================================================
-- ROLE MANAGER
-- Listen: Team change → suspend atau rehook
-- Listen: CharacterAdded → rehook (respawn dalam match)
-- ================================================================
local RoleCharConn = nil  -- track CharacterAdded connection
...
local function RoleManager_Start()
    ...
    -- CharacterAdded: selalu rehook jika sedang di match (bukan Spectator)
    RoleCharConn = LocalPlayer.CharacterAdded:Connect(function(char)
        local role = GetRole()
        if role == "Spectator" then return end  -- di lobby, skip
        -- Tunggu character fully loaded
        task.delay(2, function()
            if not State.Unloaded then
                RehookForRole(char, GetRole())
            end
        end)
    end)

    -- Team change: Spectator↔Match
    RoleManagerConn = LocalPlayer:GetPropertyChangedSignal("Team"):Connect(function()
        ...
        local prevRole = LastRole
        LastRole = newRole

        if newRole == "Spectator" then
            -- Masuk lobby → suspend semua fitur
            task.spawn(SuspendAllFeatures)
        else
            -- Keluar lobby → masuk match sebagai Killer/Survivor
            -- Character juga akan di-spawn (CharacterAdded akan fire),
            -- tapi kita juga handle di sini untuk jaga-jaga timing race
            task.delay(3, function()
                if not State.Unloaded and GetRole() == newRole then
                    local char = LocalPlayer.Character
                    RehookForRole(char, newRole)
                end
            end)
        end
    end)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if not char or not char:FindFirstChild("HumanoidRootPart") then
                ...
                local isKillerPlayer = IsKiller(player)
                local isSurvivorPlayer = IsSurvivor(player)
                -- Filter berdasarkan role target (non-team player)
                local shouldShow = false
                if isKillerPlayer and Config.ESP_Killer then
                    -- Killer ESP: tampil ke semua (Survivor bisa lihat Killer, Killer bisa lihat sesama)
                    shouldShow = true
                elseif isSurvivorPlayer and Config.ESP_Survivor then
                    -- Survivor ESP: tampil ke semua
                    shouldShow = true
                end
...
local function UpdateObjectESP(cam)
    -- Spectator: skip semua object ESP
...
-- Attack anim IDs: confirmed dari source (Animations-new) + live testing
-- Attack anims = trigger parry
local AttackAnimIDs = {
    -- Confirmed dari source decompile (killer Animations script)
    ["129784271201071"] = true,  -- lungehold
    ["132817836308238"] = true,  -- attack
    ["82666958311998"]  = true,  -- attackfr (frenzy)
    -- Live testing confirmed pre-hit
    ["110355011987939"] = true,
    ["139369275981139"] = true,
    ["121571390309073"] = true,
    ["119596435929738"] = true,
    ["100492499356103"] = true,
    ["119030770442343"] = true,
...
-- Non-attack anims = skip (carry, vault, stun, idle, survivor anims, dll)
local NonAttackAnimIDs = {
    -- Killer non-attack (dari source decompile)
    ...
    ["117224999672195"] = true,  -- parriedvm
    ...
    -- Survivor anims (dari Animations-new decompile)
    ["121364777933025"] = true,  -- walk
    ["88089545021831"]  = true,  -- run
    ["73650663675588"]  = true,  -- crouchIdle
    ["137834747905828"] = true,  -- crouchWalk
    ["137828867671413"] = true,  -- injuredIdle
    ["136695692860739"] = true,  -- injuredWalk
    ["79999409695920"]  = true,  -- injuredSprint
    ["84095653804164"]  = true,  -- injuredCrouchIdle
    ["102450923773041"] = true,  -- injuredCrouchWalk
    ["104682704142865"] = true,  -- hit1
    ["139830743437188"] = true,  -- hit2/hitFront
    ["74118390445259"]  = true,  -- knockedIdle
    ["106618106536124"] = true,  -- knockedWalk
    ["121845674088602"] = true,  -- hitBack
    ["110392490296814"] = true,  -- heal
    ["87055506624885"]  = true,  -- failheal
    ["95836365038528"]  = true,  -- getheal
    ["123959675151191"] = true,  -- leveropen
    ["134838390519433"] = true,  -- leveropeninjured
    -- High-count idle/walk from testing
    ["96355067685976"]  = true,
    ["134874447821603"] = true,
    ["84678759985652"]  = true,
    ["180435571"]       = true,
    ["180435792"]       = true,
}
...
local ParryTimer = 0
-- Track killer attack via velocity approach detection + HealthChanged
local KillerAttackDetected = false
local KillerHitConns       = {}
local LastHealth           = 100
local KillerLastPos        = {}  -- track killer position tiap frame
...
local function AutoParry_HookAttackRemote()
    ...
    -- Watch semua player untuk attack animations
    local function watchPlayer(p)
        ...
        local function hookChar(char)
            ...
            local conn = animator.AnimationPlayed:Connect(function(track)
                ...
                -- Skip semua non-attack anims (carry, vault, stun, idle, survivor, dll)
                if NonAttackAnimIDs[numId] then return end
                -- Confirmed attack OR unknown anim
                ...
                if myRoot and killerRoot then
                    ...
                    -- Confirmed attack: trigger sampai PARRY_Dist
                    -- Unknown anim: hanya trigger kalau sangat dekat (5 studs) untuk safety
...
local function CheckKillerApproach(killerRoot, myRoot)
    ...
    -- Cek apakah killer bergerak cepat ke arah kita
    local velocity = killerRoot:IsA("BasePart") and killerRoot.AssemblyLinearVelocity or Vector3.zero
    local speed    = velocity.Magnitude
    local dot      = velocity.Unit:Dot(toMe.Unit)  -- 1 = langsung ke arah kita

    -- Speed > 25 studs/s DAN mengarah ke kita (dot > 0.5) = lunge/dash attack
...
local function AutoParry_Tick(dt)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            -- Trigger parry kalau:
            -- 1. HealthChanged detected (kena hit)
            -- 2. Killer lagi dash/lunge cepat ke arah kita
...
local function Godmode_Apply()
    ...
    -- Server punya authority atas health, tidak bisa override dari client
    -- Block mekanisme mati via attribute reset setiap frame
    GodmodeLoop = RunService.Heartbeat:Connect(function()
        if not Config.GODMODE then return end
        local c = LocalPlayer.Character
        if not c then return end
        pcall(function() c:SetAttribute("IsDown", false) end)
        pcall(function() c:SetAttribute("IsCarried", false) end)
        pcall(function() c:SetAttribute("Knocked", false) end)
    end)

    -- HealthChanged: coba reset tapi mungkin tidak work (server auth)
...
-- Semua auto-apply saat CharacterAdded / map swap
-- ================================================================
local VirtualInputManager  = game:GetService("VirtualInputManager")

-- ── Anti Fail Gen + Anti Skill Check (disable script approach) ──
...
local function AntiScript_Apply(char)
    if not char then return end
    if GetRole() ~= "Survivor" then return end  -- hanya untuk Survivor
    ...
    -- Watch DescendantAdded: script Skillcheck bisa spawn telat
...
-- ── Perfect Skill Check (VirtualInputManager) ──
local PerfectSCConns  = {}
local PerfectSCActive = false
local PerfectSCGuiWatchConn = nil  -- top-level PlayerGui watcher

local function PerfectSC_ConnectGui()
    -- Cleanup koneksi lama dulu (kecuali GuiWatcher, itu dikelola PerfectSC_Setup)
    ...
    task.spawn(function()
        pcall(function()
            ...
            -- WaitForChild dengan timeout pendek, lalu fallback ke watch
            local CheckGui = PlayerGui:FindFirstChild("SkillCheckPromptGui")
            if not CheckGui then
                -- Tunggu sampai muncul (max 30 detik, karena saat match baru GUI bisa muncul telat)
                local waited = 0
                repeat
                    task.wait(0.5)
                    waited = waited + 0.5
            ...
            if not Check then
                Check = CheckGui:WaitForChild("Check", 10)
            end
            ...
            if not Line or not Goal or not PerfectSCActive then return end
...
local function PerfectSC_Setup()
    -- Cleanup semua koneksi lama
    for _, c in ipairs(PerfectSCConns) do pcall(function() c:Disconnect() end) end
    PerfectSCConns = {}
    if PerfectSCGuiWatchConn then
        pcall(function() PerfectSCGuiWatchConn:Disconnect() end)
        PerfectSCGuiWatchConn = nil
    end

    PerfectSCActive = true

    -- Connect ke GUI sekarang
    PerfectSC_ConnectGui()

    -- Watch PlayerGui: kalau SkillCheckPromptGui di-recreate (ganti map/role), hook ulang
    local PlayerGui = LocalPlayer:FindFirstChild("PlayerGui")
    if PlayerGui then
...
local function ThirdPerson_Apply()
    ...
    -- Re-apply offset saat karakter respawn (bukan tiap frame!)
...
local function ThirdPerson_Remove()
    ...
    if ThirdPersonCharConn then ThirdPersonCharConn:Disconnect(); ThirdPersonCharConn = nil end
    ThirdPersonWasActive = false
end

-- UpdateThirdPerson: hanya edge-trigger, TIDAK set CameraOffset tiap tick
...
-- GUN AIMBOT
-- ================================================================
-- GUN SILENT AIM (Twist of Fate)
-- Hook FireServer langsung di Fire remote — tidak affect hal lain
-- ================================================================
local secSpear = TabAIM:AddSection("Spear Aimbot (Veil)")
secSpear:AddToggle({
    Title = "Spear Aimbot",
...
secSurvival:AddToggle({
    ...
    Callback = function(v)
        ...
        local char = LocalPlayer.Character
        if v and char then AntiScript_Apply(char)
        elseif not v then AntiScript_Restore() end
...
secSurvival:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then task.spawn(PerfectSC_Setup) else PerfectSC_Stop() end
...
secSurvival:AddToggle({
    ...
    Callback = function(v)
        Config.AUTO_Parry = v
        Config.PARRY_Visual = v  -- visualizer ikut toggle parry
        if v then AutoParry_Setup() end
        if not v and GUI.Drawings.ParryPart then
            GUI.Drawings.ParryPart.CFrame = CFrame.new(0, -1000, 0)
        end
...
local function AutoHook() end  -- stub
...
local function UpdateFog()
    if Config.NO_Fog then RemoveFog() end
end
...
Unload = function()
    ...
    if RoleManagerConn then pcall(function() RoleManagerConn:Disconnect() end) end
    if RoleCharConn    then pcall(function() RoleCharConn:Disconnect() end) end
    if ThirdPersonCharConn then pcall(function() ThirdPersonCharConn:Disconnect() end) end
    pcall(function()
        if GUI.Drawings.ParryPart then GUI.Drawings.ParryPart:Destroy() end
        if GUI.Drawings.ParryHighlight then GUI.Drawings.ParryHighlight:Destroy() end
    end)
    Config.AUTO_Generator = false
    ...
    Config.ANTI_FailGen      = false
    Config.ANTI_SkillCheck   = false
    Config.PERFECT_SkillCheck = false
    pcall(AntiScript_Restore)
    pcall(PerfectSC_Stop)
    pcall(AutoParry_Cleanup)
...
local function Init()
    -- Setup Drawing-based overlays (FOV circle, AutoGen hint) - tetap pakai Drawing
    GUI.Drawings = {}

    -- FOV Circle + Crosshair: dedicated Heartbeat loop, independent dari MainLoop
    GUI.Drawings.FOVCircle = nil
    GUI.Drawings.CrosshairDot = nil
    GUI.Drawings.CrossH = nil
    GUI.Drawings.CrossV = nil

    task.spawn(function()
        -- Buat semua drawing objects dulu
        ...
        GUI.Drawings.FOVCircle    = circle
        GUI.Drawings.CrosshairDot = dot
        GUI.Drawings.CrossH       = lineH
        GUI.Drawings.CrossV       = lineV

        -- Parry visualizer: CylinderPart di bawah karakter (work di semua executor)
        local parryPart = Instance.new("Part")
        parryPart.Name          = "ParryVisualizer"
        parryPart.Anchored      = true
        parryPart.CanCollide    = false
        parryPart.CanTouch      = false
        parryPart.CastShadow    = false
        parryPart.Shape         = Enum.PartType.Cylinder
        parryPart.Size          = Vector3.new(0.2, Config.PARRY_Dist * 2, Config.PARRY_Dist * 2)
        parryPart.CFrame        = CFrame.new(0, -1000, 0)
        parryPart.Transparency  = 0.4
        parryPart.Color         = Color3.fromRGB(0, 170, 255)
        parryPart.Material      = Enum.Material.Neon
        parryPart.Parent        = workspace.CurrentCamera

        local parryHighlight = Instance.new("SelectionBox")
        parryHighlight.LineThickness  = 0.05
        parryHighlight.Color3         = Color3.fromRGB(0, 170, 255)
        parryHighlight.SurfaceColor3  = Color3.fromRGB(0, 170, 255)
        parryHighlight.SurfaceTransparency = 0.85
        parryHighlight.Adornee        = parryPart
        parryHighlight.Parent         = workspace.CurrentCamera

        GUI.Drawings.ParryPart      = parryPart
        GUI.Drawings.ParryHighlight = parryHighlight
        ...
        RunService.Heartbeat:Connect(function()
            ...
            local showFOV = Config.AIM_ShowFOV
            local hasTarget = State.AimTarget ~= nil
            local fovColor = hasTarget and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(0, 170, 255)
            ...
            -- Crosshair dot + lines (selalu tampil saat AIM_ShowFOV)
            dot.Position = center
            dot.Color    = crossColor
            dot.Visible  = showFOV

            local s = 8  -- crosshair arm length
            lineH.From  = Vector2.new(cx - s, cy)
            lineH.To    = Vector2.new(cx + s, cy)
            lineH.Color = crossColor
            lineH.Visible = showFOV

            lineV.From  = Vector2.new(cx, cy - s)
            lineV.To    = Vector2.new(cx, cy + s)
            lineV.Color = crossColor
            lineV.Visible = showFOV

            -- Parry Visualizer: cylinder part di bawah karakter
            local parryPart = GUI.Drawings.ParryPart
            if parryPart then
                local showParry = Config.AUTO_Parry and Config.PARRY_Visual
                if showParry then
                    local char = LocalPlayer.Character
                    local root = char and char:FindFirstChild("HumanoidRootPart")
                    if root then
                        local d = Config.PARRY_Dist * 2
                        parryPart.Size   = Vector3.new(0.2, d, d)
                        parryPart.CFrame = CFrame.new(root.Position - Vector3.new(0, 3, 0))
                            * CFrame.Angles(0, 0, math.pi / 2)
                        -- Warna berubah saat attack detected
                        parryPart.Color = KillerAttackDetected
                            and Color3.fromRGB(255, 50, 50)
                            or  Color3.fromRGB(0, 170, 255)
                        if GUI.Drawings.ParryHighlight then
                            GUI.Drawings.ParryHighlight.Color3 = parryPart.Color
                            GUI.Drawings.ParryHighlight.SurfaceColor3 = parryPart.Color
                        end
                    else
                        parryPart.CFrame = CFrame.new(0, -1000, 0)
                    end
                else
                    if parryPart then parryPart.CFrame = CFrame.new(0, -1000, 0) end
                end
            end
        end)
    end)
    ...
    GUI.Drawings.AutoGenHint.Color = Color3.fromRGB(0, 170, 255)
    ...
    -- AutoParry dikelola oleh AutoLoop (AutoParry() dipanggil tiap 0.1s)

    -- Input: Keybinds + RMB Aimbot (VictUI handles its own UI input)
    Connections.Input = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            if input.KeyCode == Config.KEY_LeaveGen and not gameProcessed then
    ...
    Connections.InputChanged = UserInputService.InputChanged:Connect(function(input)
        -- (scroll handling removed, handled by VictUI window)
        _ = input
    end)
    
    Connections.Render = RunService.RenderStepped:Connect(MainLoop)
    RoleManager_Start()
    LastRole = GetRole()  -- set initial role
    ...
    -- CharacterAdded dihandle oleh RoleManager_Start (RoleCharConn)
    -- Hanya perlu ScanMap saat ada player baru
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        -- ScanMap ulang karena map bisa berubah antar match
        task.delay(1, function()
            if not State.Unloaded then ScanMap() end
        end)
    end)
...
Init()
-- Set NoSlowdownReady setelah delay di spawn pertama
task.delay(1.5, function()
    if not State.Unloaded then
        NoSlowdownReady = true
    end
end)
-- Setup Gun Silent Aim hook di background
task.spawn(AutoParry_Setup)
-- Setup parry hooks sesuai mode default
if Config.AUTO_Parry then task.spawn(AutoParry_Setup) end
notif("Violence District Loaded!", 3, Color3.fromRGB(0, 170, 255))
