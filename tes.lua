...
local function SilentAim_GetTarget()
    -- Cari killer terdekat (survivor mode) atau survivor (killer mode)
    local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        -- Target: killer jika kita survivor, survivor jika kita killer
        local shouldTarget = false
        if myRole == "Survivor" and IsKiller(p) then
            shouldTarget = true
        elseif myRole == "Killer" and IsSurvivor(p) then
            shouldTarget = true
        end
...
local function SilentAim_Setup()
    if not getrawmetatable then return end
            ...
            if method == "FireServer" and self == fireRemote then
                if Config.SILENT_AIM then
                    ...
                    if target and target.Character and myRoot then
                        ...
                        local aimPos = (targetHead or targetHRP)
                        if aimPos then
                            -- Hitung arah dari barrel ke target
                            local cam = workspace.CurrentCamera
                            local origin = cam and cam.CFrame.Position or myRoot.Position
                            local dir = (aimPos.Position - origin).Unit
