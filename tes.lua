...
function Farmer:EquipWeapon(isBoss)
	...
	if weaponName == "None" or weaponName == "" then
		...
		if equippedTool then
			...
			-- (VictUI: no options system needed)
			return
		end
...
function Utility.EnableAutoRejoin()
	...
	local conn = GuiService.ErrorMessageChanged:Connect(function()
		...
		if lastError:find("Victoria Security") then
	...
	task.spawn(function()
		...
		while _timedRejoinRunning and task.wait(1) do
			...
			if elapsed >= target then
				...
				print("[Victoria] Timed Rejoin: Rejoining now (" .. (cfg.RejoinDelay or 10) .. " min timer)...")
				task.wait(5)
...
local function RedeemCodes()
	...
	if not ok or not CodesConfig then
		warn("[Victoria] Code Redeemer: CodesConfig module not found.")
		return
	end
	...
	if not CodeRedeem then
		warn("[Victoria] Code Redeemer: CodeRedeem remote not found.")
		return
	end

	print("[Victoria] Starting code auto-redeem...")

	for codeName, _ in pairs(CodesConfig.Codes) do
		if CodesConfig.IsValid(codeName) then
			print("[Victoria] Redeeming: " .. codeName)
			...
			if success then
				print("[Victoria] Response for " .. codeName .. ":", serverResponse)
			else
				warn("[Victoria] Error redeeming " .. codeName .. ":", serverResponse)
			end
	...
	print("[Victoria] Code auto-redeem finished.")
end
...
function QuestManager:AcceptOnce(npcName)
	if not npcName or npcName == "None" or npcName == "" then
		warn("[Victoria] AcceptOnce: No Quest NPC selected.")
		return false
	end
	...
	if not self._remote then
		warn("[Victoria] QuestAccept remote not found.")
		return false
	end
	...
	if ok then
		print("[Victoria] Quest accepted from " .. npcName)
		return true
	else
		warn("[Victoria] Quest accept failed:", err)
		return false
	end
end
...
-- ==========================================
if _G.Victoria_Spawner then _G.Victoria_Spawner:Stop()    end
if _G.Victoria_Farmer  then _G.Victoria_Farmer:Stop()     end
if _G.Victoria_Tracker then _G.Victoria_Tracker:Destroy() end
Utility.Cleanup()
...
-- notif() injected by VictUI library automatically

-- ==========================================
-- || TABS
-- ==========================================
local TabMain     = Window:AddTab({ Name = "Main",     Icon = "rbxassetid://122032243989747"     })
local TabMobs     = Window:AddTab({ Name = "Entities", Icon = "rbxassetid://82472368671405"   })
local TabBosses   = Window:AddTab({ Name = "Bosses",   Icon = "rbxassetid://13132186360"    })
local TabSpecials = Window:AddTab({ Name = "Specials", Icon = "rbxassetid://107005941750079"     })
local TabCrafting = Window:AddTab({ Name = "Crafting", Icon = "rbxassetid://8601111810"   })
local TabMisc     = Window:AddTab({ Name = "Misc",     Icon = "rbxassetid://128874923961846"     })
local TabSettings = Window:AddTab({ Name = "Settings", Icon = "rbxassetid://70386228443175" })

-- ==========================================
-- || MAIN TAB
-- ==========================================
local secFarm = TabMain:AddSection("Auto Farm")
...
secFarm:AddToggle({
    ...
    Callback = function(v)
        Config.LoopFarm = v
        notif("Auto Farm: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
    end
})
...
local secHaki = TabMain:AddSection("Haki")

secHaki:AddToggle({
    Title   = "Auto Armament Haki",
    Default = Config.AutoHaki,
    Callback = function(v) Config.AutoHaki = v; notif("Auto Armament Haki: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})

secHaki:AddToggle({
    Title   = "Auto Observation Haki",
    Default = Config.AutoObservationHaki,
    Callback = function(v) Config.AutoObservationHaki = v; notif("Auto Obs Haki: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})

local secWeapon = TabMain:AddSection("Inventory Management")

secWeapon:AddToggle({
    Title   = "Auto Equip Weapon",
    Default = Config.AutoEquip,
    Callback = function(v) Config.AutoEquip = v; notif("Auto Equip: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})

local weaponList = pcall(function() return Utility.GetWeapons() end) and Utility.GetWeapons() or {"None"}

secWeapon:AddDropdown({
...
secWeapon:AddButton({
    ...
    Callback = function()
        notif("Refreshed weapon list", 2)
        -- Fluent options refresh tidak diperlukan di VictUI
        -- Weapons akan diambil ulang saat next equip
    end
})

local secSkills = TabMain:AddSection("Auto Skills")

secSkills:AddToggle({
    Title   = "Use Skills on Bosses",
    Default = Config.AutoSkill.Bosses,
    Callback = function(v) Config.AutoSkill.Bosses = v; notif("Skills on Bosses: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})

secSkills:AddDropdown({
    Title   = "Boss Skills",
    Options = { "Z", "X", "C", "V", "F" },
    Default = "Z",
    Callback = function(v)
        Config.AutoSkill.BossSkills = {}
        Config.AutoSkill.BossSkills[v] = true
    end
})
...
secSkills:AddToggle({
    ...
    Callback = function(v) Config.AutoSkill.NPCs = v; notif("Skills on NPCs: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})
...
secSkills:AddDropdown({
    ...
    Default = "Z",
    Callback = function(v)
        Config.AutoSkill.NPCSkills = {}
        Config.AutoSkill.NPCSkills[v] = true
    end
})
...
-- || ENTITIES TAB
-- ==========================================
local secNPCSettings = TabMobs:AddSection("NPC Settings")

secNPCSettings:AddSlider({
    Title   = "Wait for NPCs",
    Min     = 1, Max = 5,
...
for _, category in ipairs(EntityCategories) do
    local sec = TabMobs:AddSection(category.Name)
    for _, entityName in ipairs(category.List) do
        sec:AddToggle({
            ...
            Callback = function(v)
                ...
                notif((v and "Farming " or "Skipping ") .. entityName, 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
            end
        })
    end
end
...
-- || BOSSES TAB
-- ==========================================
local secBoss = TabBosses:AddSection("Standard Boss Spawner")
...
secBoss:AddToggle({
    ...
    Callback = function(v)
        ...
        notif("Auto Spawn: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
    end
})
...
secBoss:AddDropdown({
    ...
    Default = "Saber",
    Callback = function(v)
        Config.Boss.Selected = {}
        Config.Boss.Selected[v] = true
    end
})
...
-- || SPECIALS TAB
-- ==========================================
local secSpecials = TabSpecials:AddSection("Special Boss Spawners")

local difficultyLevels = { "Normal", "Medium", "Hard", "Extreme" }

-- Urutan fixed agar tidak random (pairs() tidak guaranteed order)
...
for _, bossName in ipairs(specialOrder) do
    ...
    local bn = bossName  -- local capture untuk closure
    local sec = TabSpecials:AddSection(bn)
    sec:AddToggle({
        Title   = "Auto Spawn " .. bn,
        Default = bossData.Auto,
        Callback = function(v)
            Config.Specials[bn].Auto = v
            notif(bn .. " Spawn: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
        end
    })
    sec:AddDropdown({
        Title   = "Difficulty",
        ...
        Default = "Normal",
        Callback = function(v) Config.Specials[bn].Diff = v end
    })
end
...
-- || CRAFTING TAB
-- ==========================================
local secCraftCalc = TabCrafting:AddSection("Crafting Calculator")

local craftingSets = {}
if CONSTANTS and CONSTANTS.CraftingSets then
    for k, _ in pairs(CONSTANTS.CraftingSets) do
        table.insert(craftingSets, k)
    end
end
if #craftingSets == 0 then
    craftingSets = { "StrongestinHistory", "TrueAizen", "StrongestofToday", "BlessedMaiden" }
end

local selectedSet = craftingSets[1]

secCraftCalc:AddDropdown({
    Title   = "Select Set",
    Options = craftingSets,
    Default = craftingSets[1],
    Callback = function(v) selectedSet = v end
})

secCraftCalc:AddButton({
    Title = "Check Amount",
    Callback = function()
        if CheckSetAmount then
            local normalSets, skillSets, combinedSets = CheckSetAmount(selectedSet)
            local setInfo = CONSTANTS and CONSTANTS.CraftingSets[selectedSet]
            local msg = "Normal: " .. normalSets
            if setInfo and setInfo.SkillUnlock then
                msg = msg .. " | Combined: " .. combinedSets
            end
            notif(msg, 4)
        else
            notif("CheckSetAmount not available", 2)
        end
    end
})

local secCraftItems = TabCrafting:AddSection("Craft Items")

local craftAmount = 1
secCraftItems:AddSlider({
    Title   = "Amount to Craft",
    Min     = 1, Max = 99,
    Default = 1,
    Callback = function(v) craftAmount = v end
})

secCraftItems:AddButton({
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                game:GetService("ReplicatedStorage")
                    ...
                    :InvokeServer("SlimeKey", craftAmount)
            end)
            notif(ok and ("Crafted " .. craftAmount .. "x SlimeKey") or "Craft failed: " .. tostring(err), 3)
        end)
    end
})
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                game:GetService("ReplicatedStorage")
                    ...
                    :InvokeServer("DivineGrail", craftAmount)
            end)
            notif(ok and ("Crafted " .. craftAmount .. "x Divine Grail") or "Craft failed: " .. tostring(err), 3)
        end)
    end
})

local secAutoCraft = TabCrafting:AddSection("Auto Crafting")

secAutoCraft:AddToggle({
    Title   = "Auto Craft SlimeKey",
    Default = Config.AutoCraft.SlimeKey,
    Callback = function(v) Config.AutoCraft.SlimeKey = v; notif("Auto Craft SlimeKey: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})

secAutoCraft:AddToggle({
    Title   = "Auto Craft Divine Grail",
    Default = Config.AutoCraft.DivineGrail,
    Callback = function(v) Config.AutoCraft.DivineGrail = v; notif("Auto Craft Divine Grail: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})
...
-- || MISC TAB
-- ==========================================
local secCodes = TabMisc:AddSection("Code Redeemer")
...
secCodes:AddButton({
    ...
    Callback = function()
        if RedeemCodes then
            ...
            notif("Redeeming codes...", 2)
        else
            notif("RedeemCodes not available", 2)
        end
    end
})

local secQuest = TabMisc:AddSection("Get Quest")

local questNPCList = { "None" }
if QuestManager and QuestManager.GetQuestNPCs then
    local ok, list = pcall(function() return QuestManager.GetQuestNPCs() end)
    if ok and list and #list > 0 then questNPCList = list end
end

secQuest:AddDropdown({
    Title   = "Quest NPC",
...
secQuest:AddButton({
    ...
    Callback = function()
        if AutoQuest then
            task.spawn(function()
                local ok = AutoQuest:AcceptOnce(Config.AutoQuest.SelectedNPC)
                notif(ok and "Quest accepted from " .. Config.AutoQuest.SelectedNPC or "Quest accept failed", 3)
            end)
        else
            notif("QuestManager not available", 2)
        end
    end
})
...
-- || SETTINGS TAB
-- ==========================================
local secUtils = TabSettings:AddSection("Utilities")
...
secUtils:AddToggle({
    ...
    Callback = function(v)
        ...
        notif("WhiteScreen: " .. (v and "ON (save CPU)" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
    end
})
...
secUtils:AddToggle({
    ...
    Callback = function(v)
        Config.AutoRejoin = v
        if Utility then Utility.EnableAutoRejoin() end
        notif("Auto Rejoin: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145))
    end
})
        ...
        if v then
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer then
                    ...
                    if not isFriend then
                        LocalPlayer:Kick("[Victoria] Friend-Only Mode: Stranger detected.")
                    end
                end
            end
        end
...
local secRejoin = TabSettings:AddSection("Timed Auto Rejoin")

secRejoin:AddToggle({
    Title   = "Timed Auto Rejoin",
    Default = Config.TimedRejoin,
    Callback = function(v) Config.TimedRejoin = v; notif("Timed Rejoin: " .. (v and "ON" or "OFF"), 2, v and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(130, 130, 145)) end
})
...
pcall(function()
    ...
    AutoQuest = QuestManager.new()

    _G.Victoria_Tracker = Tracker
    _G.Victoria_Spawner = Spawner
    _G.Victoria_Farmer  = AutoFarm
...
-- Auto redeem on load disabled (use button manually)

notif("Sailor Piece loaded! Refresh & select your weapon.", 5, Color3.fromRGB(255, 140, 0))
print("Victoria VictUI Initialized.")
