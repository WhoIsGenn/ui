...
local function AutoParry_GetRemote()
    ...
    -- Game protect remote via metamethod, cloneref bypass protection
    ParryRemote = cloneref(raw)
    return ParryRemote
end

-- AutoParry: simulate tap GUI button Gui-mob di Survivor-mob
-- Ini cara yang sama dengan hub lain — trigger button click, bukan fire remote langsung
-- Sehingga animasi, cooldown GUI, semua jalan normal persis seperti manual tap
local ParryButton   = nil  -- cache ImageButton Gui-mob
local ParryOnCooldown = false

local function AutoParry_FindButton()
    if ParryButton and ParryButton.Parent then return ParryButton end
    local pg = LocalPlayer:FindFirstChild("PlayerGui")
    if not pg then return nil end
    -- Cari di Survivor-mob (mobile) dan Survivor/Survivor-con (PC/console)
    for _, guiName in ipairs({"Survivor-mob", "Survivor", "Survivor-con"}) do
        local gui = pg:FindFirstChild(guiName)
        if gui then
            -- Cari Controls folder lalu Gui-mob button
            local function findBtn(parent, depth)
                if depth > 5 then return nil end
                for _, child in ipairs(parent:GetChildren()) do
                    if child:IsA("ImageButton") and child.Name == "Gui-mob" then
                        return child
                    end
                    local found = findBtn(child, depth + 1)
                    if found then return found end
                end
            end
            local btn = findBtn(gui, 0)
            if btn then
                ParryButton = btn
                return btn
            end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if kr and (kr.Position - root.Position).Magnitude <= Config.PARRY_Dist then
                local btn = AutoParry_FindButton()
                if btn then
                    -- Simulate tap button parry persis seperti player manual tap
                    pcall(function()
                        btn:SimulateClick()
                    end)
                    -- Fallback: fire MouseButton1Down event langsung
                    pcall(function()
                        local vInputService = game:GetService("VirtualInputManager")
                        if vInputService then
                            -- tidak tersedia di semua executor
                        end
                    ...
                    task.delay(51, function()
                        ...
                        ParryButton = nil  -- refresh button cache
                    end)
                else
                    -- Fallback: fire remote langsung kalau button tidak ditemukan
...
local function GunSilentAim_Setup()
    ...
    pcall(function()
        ...
        -- cloneref bypass game metamethod protection
        local fireClone = cloneref(fireRE)
        fireClone.OnClientEvent:Connect(function(...)
            if not Config.GUN_SilentAim then return end
            local target = GunSilentAim_GetTarget()
            if target then
                pcall(function() fireClone:FireServer(target.CFrame) end)
            end
        end)
