...
-- Highlight disk: Anchored Part, posisi di-update tiap frame di UpdateParryFOV
-- TIDAK pakai Weld/physics — agar tidak glitch karakter
...
local function ParryDisk_GetOrCreate()
    ...
    ParryDiskPart.Anchored     = true   -- ANCHORED, tidak ada physics
...
local function UpdateParryFOV()
    ...
    -- Update posisi disk setiap frame (Anchored, tidak ada physics)
    local disk = ParryDisk_GetOrCreate()
    local d    = Config.PARRY_Dist * 2
    if disk.Size.Y ~= d then disk.Size = Vector3.new(0.05, d, d) end
    -- Posisi: tepat di Y-3 dari root, flat horizontal
    disk.CFrame = CFrame.new(root.Position + Vector3.new(0, -3, 0))
    ...
    -- World-space line segments (outline putih + glow biru)
