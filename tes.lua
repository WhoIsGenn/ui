...
local function AutoParry_HookAttackRemote()
    ...
    pcall(function()
        ...
        -- Hook semua attack remotes
        local function hookRemote(remote)
            if not remote then return end
            pcall(function()
                local conn = remote.OnClientEvent:Connect(function()
                    if not Config.AUTO_Parry then return end
                    print("[AutoParry] Attack detected via:", remote.Name)
        ...
        hookRemote(attacks:FindFirstChild("BasicAttack"))
        hookRemote(attacks:FindFirstChild("AfterAttack"))
        hookRemote(attacks:FindFirstChild("Lunge"))
        hookRemote(attacks:FindFirstChild("hit"))

        -- Mechanics AttackEvent juga
        local mech = remotes:FindFirstChild("Mechanics")
        if mech then
            hookRemote(mech:FindFirstChild("AttackEvent"))
        end
    end)
end
