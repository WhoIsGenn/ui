...
-- // ============================================================
-- // BRIGIDA UI LIBRARY (replaces Obsidian)
-- // ============================================================

getgenv().celina_Running = true

-- Flags compatibility layer (replaces Library.Options / Library.Toggles)
local Flags = {}
    ...
    __index = function(_, key)
        if not Flags[key] then
            Flags[key] = {
                Value = false,
                _callbacks = {},
                OnChanged = function(self, fn) table.insert(self._callbacks, fn) end,
                SetValue = function(self, val)
                    self.Value = val
                    for _, fn in ipairs(self._callbacks) do pcall(fn, val) end
                end
            }
        end
        return Flags[key]
        ...
        if not Flags["opt_"..key] then
            Flags["opt_"..key] = {
                ...
                SetValues = function(self, vals)
                    self._values = vals
                    if self._uiRef then
                        if self._uiRef.SetValues then self._uiRef:SetValues(vals)
                        elseif self._uiRef.SetOptions then self._uiRef:SetOptions(vals) end
                    end
                end,
                SetText = function(self, txt)
                    if self._uiRef and self._uiRef.SetContent then self._uiRef:SetContent(txt)
                    elseif self._uiRef and self._uiRef.SetTitle then self._uiRef:SetTitle(txt) end
                end,
                SetVisible = function(self, vis)
                    if self._uiRef and self._uiRef.SetVisible then self._uiRef:SetVisible(vis) end
                end,
            }
        end
        return Flags["opt_"..key]
    end
})

local function RegToggle(id, default)
    local t = Toggles[id]; t.Value = default or false; return t
end
local function RegOption(id, default)
    local o = Options[id]; o.Value = default; return o
end

-- Notify wrapper
local _Nt = function(msg, dur) end
function Nt(msg, dur) _Nt(msg, dur) end

local Library = {
    Notify = function(self, msg, dur) Nt(msg, dur) end,
    ...
    ToggleKeybind = nil,
    Toggle = function(self) if Window and Window.Toggle then Window:Toggle() end end,
    SetNotifySide = function(self, side) end,
    SetDPIScale = function(self, dpi) end,
    ForceCheckbox = true,
...
-- Setup Notify setelah window ada
_Nt = function(msg, dur)
    -- Brigida notification via Window (jika ada method Notify)
    if Window.Notify then
        Window:Notify({ Title = "celina", Content = msg, Duration = dur or 3 })
    end
end

-- Alias untuk Library:Notify agar semua referensi lama tetap jalan
local Library = {
    Notify = function(self, msg, dur) Nt(msg, dur) end,
    Unload = function(self)
        -- cleanup
...
local Tabs = {
    ...
    Artifact    = Window:AddTab({ Name = "Artifact",     Icon = "lucide:martini" }),
...
-- // ============================================================
-- // GROUPBOXES → Sections (Brigida pakai AddSection)
-- // Obsidian: AddLeftGroupbox / AddRightGroupbox → Brigida: AddSection + Tabbox Left/Right
-- // ============================================================
local GB = {
    Information = {
        Left = {
            User = Tabs.Information:AddSection({ Title = "User",   Tabbox = "Left" }),
            Game = Tabs.Information:AddSection({ Title = "Game",   Tabbox = "Left" }),
        },
        Right = {
            Others = Tabs.Information:AddSection({ Title = "Others", Tabbox = "Right" }),
        },
    },
    Priority = {
        Left = {
            Config = Tabs.Priority:AddSection({ Title = "Config", Tabbox = "Left" }),
        },
    },
    Artifact = {
        Left = {
            Status  = Tabs.Artifact:AddSection({ Title = "Status",     Tabbox = "Left" }),
            Equip   = Tabs.Artifact:AddSection({ Title = "Auto-Equip", Tabbox = "Left" }),
            Upgrade = Tabs.Artifact:AddSection({ Title = "Upgrade",    Tabbox = "Left" }),
        },
        Right = {
            Lock   = Tabs.Artifact:AddSection({ Title = "Lock",   Tabbox = "Right" }),
            Delete = Tabs.Artifact:AddSection({ Title = "Delete",  Tabbox = "Right" }),
        },
    },
    Player = {
        Left = {
            General = Tabs.Player:AddSection({ Title = "General", Tabbox = "Left" }),
            Server  = Tabs.Player:AddSection({ Title = "Server",  Tabbox = "Left" }),
        },
        Right = {
            Game   = Tabs.Player:AddSection({ Title = "Game",   Tabbox = "Right" }),
            Safety = Tabs.Player:AddSection({ Title = "Safety", Tabbox = "Right" }),
        },
    },
    Webhook = {
        Left = {
            Config = Tabs.Webhook:AddSection({ Title = "Config", Tabbox = "Left" }),
        },
    },
}

-- // ============================================================
-- // TABBOXES → Brigida pakai AddTabbox() di dalam Section
-- // Obsidian: Tab:AddLeftTabbox() → Brigida: Section:AddTabbox(...)
-- // ============================================================

-- Main Tab Sections untuk Tabboxes
local _MainLeftSec1  = Tabs.Main:AddSection({ Title = "Autofarm",  Tabbox = "Left" })
local _MainLeftSec2  = Tabs.Main:AddSection({ Title = "MiscAuto",  Tabbox = "Left" })
local _MainRightSec1 = Tabs.Main:AddSection({ Title = "Switch",    Tabbox = "Right" })

local _AutoLeftSec1  = Tabs.Automation:AddSection({ Title = "Misc",    Tabbox = "Left" })
local _AutoLeftSec2  = Tabs.Automation:AddSection({ Title = "Stats",   Tabbox = "Left" })
local _AutoRightSec1 = Tabs.Automation:AddSection({ Title = "Enchant", Tabbox = "Right" })

local _DungeonLeftSec1  = Tabs.Dungeon:AddSection({ Title = "Auto Join", Tabbox = "Left" })

local _TeleportLeftSec1  = Tabs.Teleport:AddSection({ Title = "Waypoint", Tabbox = "Left" })
local _TeleportRightSec1 = Tabs.Teleport:AddSection({ Title = "NPCs",     Tabbox = "Right" })

local _MiscLeftSec1  = Tabs.Misc:AddSection({ Title = "Merchant",   Tabbox = "Left" })
local _MiscLeftSec2  = Tabs.Misc:AddSection({ Title = "Merchant 2", Tabbox = "Left" })
local _MiscLeftSec3  = Tabs.Misc:AddSection({ Title = "Misc",       Tabbox = "Left" })
local _MiscRightSec1 = Tabs.Misc:AddSection({ Title = "Quests",     Tabbox = "Right" })

local TB = {
    Main = {
        Left  = { Autofarm = _MainLeftSec1,  MiscAuto = _MainLeftSec2 },
        Right = { Switch = _MainRightSec1 },
    },
    Automation = {
        Left  = { Misc1 = _AutoLeftSec1,  Stats1 = _AutoLeftSec2 },
        Right = { Enchant = _AutoRightSec1 },
    },
    Dungeon = {
        Left = { Autojoin = _DungeonLeftSec1 },
    },
    Teleport = {
        Left  = { Waypoint = _TeleportLeftSec1 },
        Right = { NPCs = _TeleportRightSec1 },
    },
    Misc = {
        Left  = { Merchant = _MiscLeftSec1, Merchant2 = _MiscLeftSec2, Misc1 = _MiscLeftSec3 },
        Right = { Quests = _MiscRightSec1 },
    },
}

-- Brigida AddTabbox returns a tabbox object, AddTab pada tabbox returns a section-like object
local TB_Tabs = {
    Autofarm = {
        T1 = _MainLeftSec1:AddTabbox({ Title = "Autofarm", Icon = "lucide:swords" }),
        T2 = _MainLeftSec1:AddTabbox({ Title = "Boss",     Icon = "lucide:skull" }),
        T3 = _MainLeftSec1:AddTabbox({ Title = "Misc",     Icon = "lucide:layers" }),
        T4 = _MainLeftSec1:AddTabbox({ Title = "Config",   Icon = "lucide:settings" }),
    },
    MiscAuto = {
        T1 = _MainLeftSec2:AddTabbox({ Title = "Haki",  Icon = "lucide:flame" }),
        T2 = _MainLeftSec2:AddTabbox({ Title = "Skill", Icon = "lucide:zap" }),
        T3 = _MainLeftSec2:AddTabbox({ Title = "Combo", Icon = "lucide:list" }),
    },
    Switch = {
        T1 = _MainRightSec1:AddTabbox({ Title = "Title", Icon = "lucide:tag" }),
        T2 = _MainRightSec1:AddTabbox({ Title = "Rune",  Icon = "lucide:diamond" }),
        T3 = _MainRightSec1:AddTabbox({ Title = "Build", Icon = "lucide:wrench" }),
    },
    MiscAuto_Left = {
        T1 = _AutoLeftSec1:AddTabbox({ Title = "Ascend", Icon = "lucide:arrow-up" }),
        T2 = _AutoLeftSec1:AddTabbox({ Title = "Rolls",  Icon = "lucide:dices" }),
        T3 = _AutoLeftSec1:AddTabbox({ Title = "Trade",  Icon = "lucide:repeat" }),
    },
    Stats1 = {
        T1 = _AutoLeftSec2:AddTabbox({ Title = "Level", Icon = "lucide:bar-chart-2" }),
        T2 = _AutoLeftSec2:AddTabbox({ Title = "Gem",   Icon = "lucide:gem" }),
        T3 = _AutoLeftSec2:AddTabbox({ Title = "Misc",  Icon = "lucide:more-horizontal" }),
    },
    Enchant = {
        T1 = _AutoRightSec1:AddTabbox({ Title = "Enchant", Icon = "lucide:sparkles" }),
        T2 = _AutoRightSec1:AddTabbox({ Title = "Passive", Icon = "lucide:shield" }),
        T3 = _AutoRightSec1:AddTabbox({ Title = "Config",  Icon = "lucide:settings" }),
    },
    Dungeon = {
        T1 = _DungeonLeftSec1:AddTabbox({ Title = "Autojoin", Icon = "lucide:door-open" }),
        T2 = _DungeonLeftSec1:AddTabbox({ Title = "Config",   Icon = "lucide:settings" }),
    },
    Waypoint = {
        T1 = _TeleportLeftSec1:AddTabbox({ Title = "Island", Icon = "lucide:map" }),
        T2 = _TeleportLeftSec1:AddTabbox({ Title = "Quest",  Icon = "lucide:scroll" }),
        T3 = _TeleportLeftSec1:AddTabbox({ Title = "Misc",   Icon = "lucide:more-horizontal" }),
    },
    NPCs = {
        T1 = _TeleportRightSec1:AddTabbox({ Title = "Moveset", Icon = "lucide:sword" }),
        T2 = _TeleportRightSec1:AddTabbox({ Title = "Mastery", Icon = "lucide:star" }),
    },
    Merchant = {
        T1 = _MiscLeftSec1:AddTabbox({ Title = "Regular",   Icon = "lucide:shopping-cart" }),
        T2 = _MiscLeftSec1:AddTabbox({ Title = "Dungeon",   Icon = "lucide:door-open" }),
        T3 = _MiscLeftSec1:AddTabbox({ Title = "Valentine", Icon = "lucide:heart" }),
    },
    Merchant2 = {
        T1 = _MiscLeftSec2:AddTabbox({ Title = "Tower",     Icon = "lucide:tower" }),
        T2 = _MiscLeftSec2:AddTabbox({ Title = "Boss Rush", Icon = "lucide:skull" }),
    },
    Misc1 = {
        T1 = _MiscLeftSec3:AddTabbox({ Title = "Chests", Icon = "lucide:package" }),
        T2 = _MiscLeftSec3:AddTabbox({ Title = "Craft",  Icon = "lucide:hammer" }),
        T3 = _MiscLeftSec3:AddTabbox({ Title = "Notify", Icon = "lucide:bell" }),
    },
    Puzzle = {
        T1 = _MiscRightSec1:AddTabbox({ Title = "Puzzles",    Icon = "lucide:puzzle" }),
        T2 = _MiscRightSec1:AddTabbox({ Title = "Questlines", Icon = "lucide:scroll" }),
    },
}

-- // ============================================================
-- // HELPER: AddToggle wrapper dengan flags
-- // ============================================================
local function MakeToggle(section, id, cfg)
    local flag = RegToggle(id, cfg.Default or false)
    local el = section:AddToggle({
        Title    = cfg.Text or cfg.Title or id,
        Default  = cfg.Default or false,
        Locked   = cfg.Disabled or false,
        Callback = function(val)
            flag.Value = val
            for _, fn in ipairs(flag._callbacks) do
                pcall(fn, val)
            end
...
local function MakeDropdown(section, id, cfg)
    local flag = RegOption(id, cfg.Default)
    local el = section:AddDropdown({
        Title    = cfg.Text or cfg.Title or id,
    ...
    flag._uiRef = el
    -- patch SetValues to update dropdown
    flag.SetValues = function(self, vals)
        self._values = vals
        if el.SetValues then el:SetValues(vals)
        elseif el.SetOptions then el:SetOptions(vals) end
    end
    return el
end

local function MakeSlider(section, id, cfg)
    local flag = RegOption(id, cfg.Default)
    local el = section:AddSlider({
        Title     = cfg.Text or cfg.Title or id,
        Content   = cfg.Content or "",
        Min       = cfg.Min or 0,
        Max       = cfg.Max or 100,
        Default   = cfg.Default or 0,
        Increment = cfg.Increment or (cfg.Rounding and (10^-cfg.Rounding) or 1),
        Locked    = cfg.Disabled or false,
        Callback  = function(val)
            flag.Value = val
            for _, fn in ipairs(flag._callbacks) do
...
local function MakeInput(section, id, cfg)
    local flag = RegOption(id, cfg.Default or "")
    local el = section:AddInput({
        Title    = cfg.Text or cfg.Title or id,
...
local function MakeButton(section, cfg, callback)
    -- Support both: AddButton("Text", func) dan AddButton({ Text=..., Func=... })
    local title, func
    if type(cfg) == "string" then
        title = cfg
        func = callback
    else
        title = cfg.Text or cfg.Title or "Button"
        func = cfg.Func or cfg.Callback or callback
    end
...
local function MakeLabel(section, text, wrap)
    -- Obsidian: AddLabel("text", wrap) → Brigida: AddParagraph
    if type(text) == "table" then
        -- format lama: AddLabel({ Text = ..., DoesWrap = ... })
        local cfg = text
        local el = section:AddParagraph({ Title = "", Content = cfg.Text or "" })
        local obj = {
            _el = el,
            SetText = function(self, t)
                if el.SetContent then el:SetContent(t)
                elseif el.SetTitle then el:SetTitle(t) end
            end,
        ...
        local el = section:AddParagraph({ Title = "", Content = text or "" })
        local obj = {
            _el = el,
            SetText = function(self, t)
                if el.SetContent then el:SetContent(t)
            ...
            AddKeyPicker = function(self, id, kbCfg)
                -- KeyPicker tidak ada di Brigida, return dummy
                local kbFlag = RegOption(id, kbCfg.Default or "")
                Library.ToggleKeybind = kbFlag
                return self
            end,
        }
        return obj
...
-- AddKeyPicker as standalone (chained from AddLabel)
-- sudah di-handle di MakeLabel
...
TierLabel = MakeLabel(GB.Information.Left.User, "<b>Type:</b> " .. initTier)
TimeLabel = MakeLabel(GB.Information.Left.User, "<b>Time Left:</b> " .. initTime)
...
MakeLabel(GB.Information.Left.User, "<b>Executor:</b> " .. executorDisplayName .. "\n<b>Status:</b> " .. statusText .. "\n" .. extraNote)

MakeButton(GB.Information.Left.Game, "Redeem All Codes", function()
...
MakeLabel(GB.Information.Right.Others, "- ⚠️ If some features are disabled, it is because your executor lacks the required functions.")

MakeButton(GB.Information.Right.Others, {
...
for i = 1, #PriorityTasks do
    MakeDropdown(GB.Priority.Left.Config, "SelectedPriority_" .. i, {
        Text    = "Priority " .. i,
...
-- // WEBHOOK TAB
-- // ============================================================
MakeInput(GB.Webhook.Left.Config, "WebhookURL", {
    Text        = "Webhook URL",
    Default     = "",
    Placeholder = "Enter Webhook URL...",
})

MakeInput(GB.Webhook.Left.Config, "UID", {
    Text        = "User ID",
    Default     = "",
    Placeholder = "Enter UID...",
})

MakeDropdown(GB.Webhook.Left.Config, "SelectedData", {
    Text    = "Select Data (s)",
...
MakeDropdown(GB.Webhook.Left.Config, "SelectedItemRarity", {
...
MakeToggle(GB.Webhook.Left.Config, "PingUser", {
    Text    = "Ping User",
    Default = false,
})

MakeToggle(GB.Webhook.Left.Config, "SendWebhook", {
    Text     = "Send Webhook",
    Default  = false,
    Disabled = not Support.Webhook,
})

MakeSlider(GB.Webhook.Left.Config, "WebhookDelay", {
...
MakeButton(GB.Webhook.Left.Config, "Test Webhook", function()
    PostToWebhook()
end)

-- // ============================================================
-- // MAIN TAB - Autofarm T1
-- // ============================================================
MakeDropdown(TB_Tabs.Autofarm.T1, "SelectedMob", {
    Text   = "Select Mob (s)",
    Values = Tables.MobList,
    Multi  = true,
})

MakeButton(TB_Tabs.Autofarm.T1, "Refresh", UpdateNPCLists)

MakeToggle(TB_Tabs.Autofarm.T1, "MobFarm", {
    Text    = "Autofarm Selected Mob",
    Default = false,
})

MakeToggle(TB_Tabs.Autofarm.T1, "AllMobFarm", {
    Text    = "Autofarm All Mobs",
    Default = false,
})

MakeDropdown(TB_Tabs.Autofarm.T1, "AllMobType", {
...
MakeToggle(TB_Tabs.Autofarm.T1, "LevelFarm", {
    Text    = "Autofarm Level",
    Default = false,
})

-- // MAIN TAB - Autofarm T2 (Boss)
MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedBosses", {
    Text   = "Select Bosses",
    Values = Tables.BossList,
    Multi  = true,
})

MakeToggle(TB_Tabs.Autofarm.T2, "BossesFarm", {
    Text    = "Autofarm Selected Boss",
    Default = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "AllBossesFarm", {
    Text    = "Autofarm All Bosses",
    Default = false,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedSummon", {
    Text   = "Select Summon Boss",
    Values = Tables.SummonList,
    Multi  = false,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedSummonDiff", {
    Text   = "Select Difficulty",
    Values = Tables.DiffList,
    Multi  = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "AutoSummon", {
    Text    = "Auto Summon",
    Default = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "SummonBossFarm", {
    Text    = "Autofarm Summon Boss",
    Default = false,
})

PityLabel = MakeLabel(TB_Tabs.Autofarm.T2, "<b>Pity:</b> 0/25")

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedBuildPity", {
    Text   = "Select Boss [Build Pity]",
    Values = Tables.AllBossList,
    Multi  = true,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedUsePity", {
    Text   = "Select Boss [Use Pity]",
    Values = Tables.AllBossList,
    Multi  = false,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedPityDiff", {
    Text   = "Select Difficulty [Use Pity]",
    Values = Tables.DiffList,
    Multi  = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "PityBossFarm", {
    Text    = "Autofarm Pity Boss",
    Default = false,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedOtherSummon", {
    Text   = "Select Summon Boss (Other)",
    Values = Tables.OtherSummonList,
    Multi  = false,
})

MakeDropdown(TB_Tabs.Autofarm.T2, "SelectedOtherSummonDiff", {
    Text   = "Select Difficulty",
    Values = Tables.DiffList,
    Multi  = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "AutoOtherSummon", {
    Text    = "Auto Summon",
    Default = false,
})

MakeToggle(TB_Tabs.Autofarm.T2, "OtherSummonFarm", {
    Text    = "Autofarm Summon Boss",
    Default = false,
})

-- // MAIN TAB - Autofarm T3 (Misc/Alt)
MakeDropdown(TB_Tabs.Autofarm.T3, "SelectedAltBoss", {
    Text   = "Select Boss",
    Values = Tables.AllBossList,
    Multi  = false,
})

MakeDropdown(TB_Tabs.Autofarm.T3, "SelectedAltDiff", {
    Text   = "Select Difficulty",
    Values = Tables.DiffList,
    Multi  = false,
})
...
for i = 1, 5 do
    MakeDropdown(TB_Tabs.Autofarm.T3, "SelectedAlt_" .. i, {
        Text    = "Select Alt #" .. i,
        Values  = {},
        Default = nil,
        Multi   = false,
...
MakeToggle(TB_Tabs.Autofarm.T3, "AltBossFarm", {
    Text    = "Auto Help Alt",
    Default = false,
})

-- // MAIN TAB - Autofarm T4 (Config)
MakeDropdown(TB_Tabs.Autofarm.T4, "SelectedWeaponType", {
    Text   = "Select Weapon Type",
    Values = Tables.Weapon,
    Multi  = true,
})

MakeSlider(TB_Tabs.Autofarm.T4, "SwitchWeaponCD", {
...
MakeToggle(TB_Tabs.Autofarm.T4, "SwitchWeapon", {
    Text    = "Auto Switch Weapon",
    Default = true,
})

MakeToggle(TB_Tabs.Autofarm.T4, "IslandTP", {
    Text    = "Island TP [Autofarm]",
    Default = true,
})

MakeSlider(TB_Tabs.Autofarm.T4, "IslandTPCD", {
    Text     = "Island TP CD",
    Default  = 0.67,
    Min      = 0,
    Max      = 2.5,
...
MakeSlider(TB_Tabs.Autofarm.T4, "TargetTPCD", {
    Text     = "Target TP CD",
    Default  = 0,
    Min      = 0,
    Max      = 5,
...
MakeSlider(TB_Tabs.Autofarm.T4, "TargetDistTP", {
...
MakeSlider(TB_Tabs.Autofarm.T4, "M1Speed", {
...
MakeDropdown(TB_Tabs.Autofarm.T4, "SelectedMovementType", {
...
MakeDropdown(TB_Tabs.Autofarm.T4, "SelectedFarmType", {
...
MakeSlider(TB_Tabs.Autofarm.T4, "Distance", {
    Text     = "Farm Distance",
    Default  = 12,
    Min      = 0,
    Max      = 30,
...
MakeSlider(TB_Tabs.Autofarm.T4, "TweenSpeed", {
    Text     = "Tween Speed",
    Default  = 160,
    Min      = 0,
    Max      = 500,
...
MakeToggle(TB_Tabs.Autofarm.T4, "InstaKill", {
    Text    = "Instant Kill",
    Default = false,
})

MakeDropdown(TB_Tabs.Autofarm.T4, "InstaKillType", {
    Text    = "Select Type",
    Values  = {"V1", "V2"},
    Default = "V1",
    Multi   = false,
...
MakeSlider(TB_Tabs.Autofarm.T4, "InstaKillHP", {
...
MakeInput(TB_Tabs.Autofarm.T4, "InstaKillMinHP", {
    Text        = "Min MaxHP for Insta-Kill",
    Default     = "100000",
    Placeholder = "Number..",
})

-- // MAIN TAB - MiscAuto T1 (Haki)
MakeToggle(TB_Tabs.MiscAuto.T1, "ObserHaki",    { Text = "Auto Observation Haki", Default = false })
MakeToggle(TB_Tabs.MiscAuto.T1, "ArmHaki",      { Text = "Auto Armament Haki",   Default = false })
MakeToggle(TB_Tabs.MiscAuto.T1, "ConquerorHaki",{ Text = "Auto Conqueror Haki",  Default = false })

-- // MAIN TAB - MiscAuto T2 (Skill)
MakeLabel(TB_Tabs.MiscAuto.T2, "Autofarm already has <b>auto-M1 built in</b>.\nYou do not need to enable this separately unless you have <b>any issues with the autofarm M1.</b>")

MakeToggle(TB_Tabs.MiscAuto.T2, "AutoM1",  { Text = "Auto Attack", Default = false })
MakeToggle(TB_Tabs.MiscAuto.T2, "KillAura",{ Text = "Kill Aura",   Default = false })

MakeSlider(TB_Tabs.MiscAuto.T2, "KillAuraCD",    { Text = "CD",    Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
MakeSlider(TB_Tabs.MiscAuto.T2, "KillAuraRange", { Text = "Range", Default = 200, Min = 0,   Max = 200, Rounding = 0 })

MakeLabel(TB_Tabs.MiscAuto.T2, "Mode:\n- <b>Normal:</b> Check skill cooldowns\n- <b>Instant:</b> No check (may affect performance when use in long time.)")

MakeDropdown(TB_Tabs.MiscAuto.T2, "SelectedSkills", {
    Text   = "Select Skills",
    Values = {"Z", "X", "C", "V", "F"},
    Multi  = true,
})

MakeDropdown(TB_Tabs.MiscAuto.T2, "AutoSkillType", {
    Text    = "Select Mode",
...
MakeToggle(TB_Tabs.MiscAuto.T2, "OnlyTarget",        { Text = "Target Only",        Default = false })
MakeToggle(TB_Tabs.MiscAuto.T2, "AutoSkill_BossOnly", { Text = "Use On Boss Only",   Default = false })

MakeSlider(TB_Tabs.MiscAuto.T2, "AutoSkill_BossHP", {
    Text     = "Boss HP%",
    Default  = 100,
    Min      = 1,
    Max      = 100,
...
MakeToggle(TB_Tabs.MiscAuto.T2, "AutoSkill", { Text = "Auto Use Skills", Default = false })

-- // MAIN TAB - MiscAuto T3 (Combo)
MakeLabel(TB_Tabs.MiscAuto.T3, "Example:\n- Z > X > C > 0.5 > V\n- Z, X, C, 0.5, V")
MakeLabel(TB_Tabs.MiscAuto.T3, "Mode:\n- Normal: Wait for input CD (ex: Z > 0.5)\n- Instant: Ignore input CD")

MakeInput(TB_Tabs.MiscAuto.T3, "ComboPattern", {
    Text        = "Combo Pattern",
    Default     = "Z > X > C > V > F",
    Placeholder = "combo..",
})

MakeDropdown(TB_Tabs.MiscAuto.T3, "ComboMode", {
    Text    = "Select Mode",
    Values  = {"Normal", "Instant"},
    Default = "Normal",
})

MakeToggle(TB_Tabs.MiscAuto.T3, "ComboBossOnly", { Text = "Boss Only",          Default = false })
MakeToggle(TB_Tabs.MiscAuto.T3, "AutoCombo",     {
...
-- // AUTOMATION TAB
-- // ============================================================
-- MiscAuto_Left T1 (Ascend)
MakeToggle(TB_Tabs.MiscAuto_Left.T1, "AutoAscend", { Text = "Auto Ascend", Default = false })

for i = 1, 10 do
    Tables.AscendLabels[i] = MakeLabel(TB_Tabs.MiscAuto_Left.T1, "")
    Tables.AscendLabels[i]:SetVisible(false)
end

-- MiscAuto_Left T2 (Rolls)
MakeLabel(TB_Tabs.MiscAuto_Left.T2, "- ⚠️ Increase delay based on your ping/internet speed.\n- ⚠️ Low delay settings are not recommended.")

MakeSlider(TB_Tabs.MiscAuto_Left.T2, "RollCD", { Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2 })

MakeDropdown(TB_Tabs.MiscAuto_Left.T2, "SelectedTrait", { Text = "Select Trait (s)", Values = Tables.TraitList, Multi = true })
MakeToggle(TB_Tabs.MiscAuto_Left.T2,  "AutoTrait",      { Text = "Auto Roll Trait",  Default = false })

MakeDropdown(TB_Tabs.MiscAuto_Left.T2, "SelectedRace", { Text = "Select Race (s)", Values = Tables.RaceList, Multi = true })
MakeToggle(TB_Tabs.MiscAuto_Left.T2,  "AutoRace",      { Text = "Auto Roll Race",  Default = false })

MakeDropdown(TB_Tabs.MiscAuto_Left.T2, "SelectedClan", { Text = "Select Clan (s)", Values = Tables.ClanList, Multi = true })
MakeToggle(TB_Tabs.MiscAuto_Left.T2,  "AutoClan",      { Text = "Auto Roll Clan",  Default = false })

-- MiscAuto_Left T3 (Trade)
MakeDropdown(TB_Tabs.MiscAuto_Left.T3, "SelectedTradePlr", {
    Text   = "Select Player",
    Values = {},
    Multi  = false,
})

MakeDropdown(TB_Tabs.MiscAuto_Left.T3, "SelectedTradeItems", {
    Text   = "Select Item (s)",
    Values = Tables.OwnedItem,
    Multi  = true,
})

MakeToggle(TB_Tabs.MiscAuto_Left.T3, "ReqTrade",      { Text = "Auto Send Request",   Default = false })
MakeToggle(TB_Tabs.MiscAuto_Left.T3, "ReqTradeAccept",{ Text = "Auto Accept Request", Default = false })
MakeToggle(TB_Tabs.MiscAuto_Left.T3, "AutoAccept",    { Text = "Auto Accept Trade",   Default = false })

-- Stats1 T1 (Level)
MakeDropdown(TB_Tabs.Stats1.T1, "SelectedStats", {
    Text   = "Select Stat (s)",
    Values = {"Melee", "Defense", "Sword", "Power"},
    Multi  = true,
})
MakeToggle(TB_Tabs.Stats1.T1, "AutoStats", { Text = "Auto UP Stats", Default = false })

-- Stats1 T2 (Gem)
MakeLabel(TB_Tabs.Stats1.T2, "- ⚠️ Increase delay based on your ping/internet speed.\n- ⚠️ Low delay settings are not recommended.\n- Reroll once for this to work.")

StatsLabel = MakeLabel(TB_Tabs.Stats1.T2, "N/A")

MakeDropdown(TB_Tabs.Stats1.T2, "SelectedGemStats", { Text = "Select Stat (s)",  Values = Tables.GemStat, Multi = true })
MakeDropdown(TB_Tabs.Stats1.T2, "SelectedRank",     { Text = "Select Rank (s)",  Values = Tables.GemRank, Multi = true })

MakeSlider(TB_Tabs.Stats1.T2, "StatsRollCD", { Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
MakeToggle(TB_Tabs.Stats1.T2, "AutoRollStats", { Text = "Auto Roll Stats", Default = false })

-- Stats1 T3 (Misc)
MakeToggle(TB_Tabs.Stats1.T3, "AutoSkillTree",      { Text = "Auto Skill Tree",            Default = false })
MakeToggle(TB_Tabs.Stats1.T3, "ArtifactMilestone",  { Text = "Auto Artifact Milestone",    Default = false })

-- Enchant T1
MakeDropdown(TB_Tabs.Enchant.T1, "SelectedEnchant", { Text = "Select Enchant",  Values = Tables.OwnedAccessory, Multi = true })
MakeToggle(TB_Tabs.Enchant.T1,  "AutoEnchant",      { Text = "Auto Enchant",     Default = false })
MakeToggle(TB_Tabs.Enchant.T1,  "AutoEnchantAll",   { Text = "Auto Enchant All", Default = false })

MakeDropdown(TB_Tabs.Enchant.T1, "SelectedBlessing", { Text = "Select Blessing",  Values = Tables.OwnedWeapon, Multi = true })
MakeToggle(TB_Tabs.Enchant.T1,  "AutoBlessing",      { Text = "Auto Blessing",     Default = false })
MakeToggle(TB_Tabs.Enchant.T1,  "AutoBlessingAll",   { Text = "Auto Blessing All", Default = false })

-- Enchant T2 (Passive)
SpecPassiveLabel = MakeLabel(TB_Tabs.Enchant.T2, "N/A")

MakeDropdown(TB_Tabs.Enchant.T2, "SelectedPassive", { Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons, Multi = true })
MakeDropdown(TB_Tabs.Enchant.T2, "SelectedSpec",    { Text = "Target Passives",   Values = Tables.SpecPassive,    Multi = true })

MakeSlider(TB_Tabs.Enchant.T2, "SpecRollCD", { Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
MakeToggle(TB_Tabs.Enchant.T2, "AutoSpec",   { Text = "Auto Reroll Passive", Default = false })

-- Enchant T3 (Config/Sliders dynamic)
MakeLabel(TB_Tabs.Enchant.T3, "- ⚠️ <b>Only adjust these values if you are an advanced user.\nWill stop once the selected passive reaches your set minimum value.</b>")

-- (Dynamic sliders will be added at runtime via UpdatePassiveSliders - kept same logic)

-- // ============================================================
-- // ARTIFACT TAB
-- // ============================================================
ArtifactLabel   = MakeLabel(GB.Artifact.Left.Status, "Status: N/A")
DustLabel       = MakeLabel(GB.Artifact.Left.Status, "Dust: N/A")
InvLabel_Helmet = MakeLabel(GB.Artifact.Left.Status, "Helmet: 0/500")
InvLabel_Gloves = MakeLabel(GB.Artifact.Left.Status, "Gloves: 0/500")
InvLabel_Body   = MakeLabel(GB.Artifact.Left.Status, "Body: 0/500")
InvLabel_Boots  = MakeLabel(GB.Artifact.Left.Status, "Boots: 0/500")

-- Artifact Lock
MakeDropdown(GB.Artifact.Right.Lock, "Lock_Type", { Text = "Artifact Type",    Values = {}, Multi = true })
MakeDropdown(GB.Artifact.Right.Lock, "Lock_Set",  { Text = "Artifact Set",     Values = {}, Multi = true })
MakeDropdown(GB.Artifact.Right.Lock, "Lock_MS",   { Text = "Main Stat Filter", Values = {}, Multi = true })
MakeDropdown(GB.Artifact.Right.Lock, "Lock_SS",   { Text = "Sub Stat Filter",  Values = {}, Multi = true })
MakeSlider(GB.Artifact.Right.Lock,   "Lock_MinSS",{ Text = "Min Sub-Stats", Default = 0, Min = 0, Max = 4, Rounding = 0 })
MakeToggle(GB.Artifact.Right.Lock,   "ArtifactLock", { Text = "Auto Lock", Default = false })

-- Artifact Delete
MakeDropdown(GB.Artifact.Right.Delete, "Del_Type",      { Text = "Artifact Type",      Values = {},                             Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_Set",       { Text = "Artifact Set",        Values = {},                             Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_MS_Helmet", { Text = "Main Stat [Helmet]",  Values = {"FlatDefense", "Defense"},     Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_MS_Gloves", { Text = "Main Stat [Gloves]",  Values = {"Damage"},                    Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_MS_Body",   { Text = "Main Stat [Body]",    Values = {},                             Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_MS_Boots",  { Text = "Main Stat [Boots]",   Values = {},                             Multi = true })
MakeDropdown(GB.Artifact.Right.Delete, "Del_SS",        { Text = "Sub Stat Filter",      Values = {},                             Multi = true })
MakeSlider(GB.Artifact.Right.Delete,   "Del_MinSS",     { Text = "Min Sub-Stats", Default = 0, Min = 0, Max = 4, Rounding = 0 })
MakeToggle(GB.Artifact.Right.Delete,   "ArtifactDelete",   { Text = "Auto Delete",           Default = false })
MakeToggle(GB.Artifact.Right.Delete,   "DeleteUnlock",     { Text = "Auto Delete Unlocked",  Default = false })

-- Artifact Upgrade
MakeSlider(GB.Artifact.Left.Upgrade, "UpgradeLimit", { Text = "Upgrade Limit", Default = 0, Min = 0, Max = 15, Rounding = 0 })
MakeDropdown(GB.Artifact.Left.Upgrade, "Up_MS",      { Text = "Main Stat Filter", Values = {}, Multi = true })
MakeToggle(GB.Artifact.Left.Upgrade,   "ArtifactUpgrade", { Text = "Auto Upgrade",         Default = false })
MakeToggle(GB.Artifact.Left.Upgrade,   "UpgradeStage",    { Text = "Upgrade in Stages",    Default = false })

-- Artifact Auto-Equip
MakeDropdown(GB.Artifact.Left.Equip, "Eq_Type", { Text = "Artifact Type",    Values = {}, Multi = true })
MakeDropdown(GB.Artifact.Left.Equip, "Eq_MS",   { Text = "Main Stat Filter", Values = {}, Multi = true })
MakeDropdown(GB.Artifact.Left.Equip, "Eq_SS",   { Text = "Sub Stat Filter",  Values = {}, Multi = true })
MakeToggle(GB.Artifact.Left.Equip,   "ArtifactEquip", { Text = "Auto Equip", Default = false })

-- // ============================================================
-- // DUNGEON TAB
-- // ============================================================
MakeLabel(TB_Tabs.Dungeon.T1, "BossRush & Infinite Tower Supported.")

MakeDropdown(TB_Tabs.Dungeon.T1, "SelectedDungeon", {
    Text   = "Select Dungeon",
    Values = Tables.DungeonList,
    Multi  = false,
})

MakeToggle(TB_Tabs.Dungeon.T1, "AutoDungeon",       { Text = "Auto Join Dungeon",   Default = false })
MakeToggle(TB_Tabs.Dungeon.T1, "AutoInfiniteTower", { Text = "Auto Infinite Tower", Default = false })

DungeonCount = MakeLabel(TB_Tabs.Dungeon.T2, "Dungeon Completed: N/A")

MakeDropdown(TB_Tabs.Dungeon.T2, "SelectedDiff", {
    Text   = "Select Difficulty",
    Values = {"Easy", "Medium", "Hard", "Extreme"},
    Multi  = false,
})

MakeToggle(TB_Tabs.Dungeon.T2, "AutoDiff",         { Text = "Auto Select Difficulty", Default = false })
MakeToggle(TB_Tabs.Dungeon.T2, "AutoReplay",        { Text = "Auto Replay",            Default = false })
MakeToggle(TB_Tabs.Dungeon.T2, "DungeonAutofarm",   { Text = "Autofarm Dungeon",        Default = false })
...
MakeToggle(GB.Player.Left.General, "Noclip",          { Text = "Noclip"              })
MakeToggle(GB.Player.Left.General, "AntiKnockback",   { Text = "Anti Knockback",   Default = false })
MakeToggle(GB.Player.Left.General, "Disable3DRender", { Text = "Disable 3D Rendering" })
AddSliderToggle({ Group = GB.Player.Left.General, Id = "Grav",     Text = "Gravity",         Default = 196, Min = 0,   Max = 500, Rounding = 1 })
AddSliderToggle({ Group = GB.Player.Left.General, Id = "Zoom",     Text = "Camera Zoom",     Default = 128, Min = 128, Max = 10000 })
AddSliderToggle({ Group = GB.Player.Left.General, Id = "FOV",      Text = "Field of View",   Default = 70,  Min = 30,  Max = 120  })
local FPS_T, FPS_S = AddSliderToggle({ Group = GB.Player.Left.General, Id = "LimitFPS", Text = "Set Max FPS", Disabled = not Support.FPS, Default = 60, Min = 5, Max = 360 })
MakeToggle(GB.Player.Left.General, "FPSBoost",       { Text = "FPS Boost"             })
MakeToggle(GB.Player.Left.General, "FPSBoost_AF",    { Text = "FPS Boost [Autofarm]"  })

MakeToggle(GB.Player.Left.Server, "AntiAFK",           { Text = "Anti AFK",              Default = true,  Disabled = not Support.Connections })
MakeToggle(GB.Player.Left.Server, "AntiKick",          { Text = "Anti Kick (Client)"     })
MakeToggle(GB.Player.Left.Server, "AutoReconnect",     { Text = "Auto Reconnect"         })
MakeToggle(GB.Player.Left.Server, "NoGameplayPaused",  { Text = "No Gameplay Paused"     })

MakeButton(GB.Player.Left.Server, "Serverhop", function()
    local Servers = game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")
end)
MakeButton(GB.Player.Left.Server, "Rejoin", function()
    Services.TeleportService:Teleport(game.PlaceId, Plr)
end)

MakeToggle(GB.Player.Left.Server, "AutoServerhop", { Text = "Auto Serverhop" })
MakeSlider(GB.Player.Left.Server, "AutoHopMins",   { Text = "Minutes", Default = 30, Min = 0, Max = 300 })

MakeToggle(GB.Player.Right.Game, "InstantPP",  { Text = "Instant Prompt" })
MakeToggle(GB.Player.Right.Game, "Fullbright", { Text = "Fullbright"     })
MakeToggle(GB.Player.Right.Game, "NoFog",      { Text = "No Fog"         })
AddSliderToggle({ Group = GB.Player.Right.Game, Id = "OverrideTime", Text = "Time Of Day", Default = 12, Min = 0, Max = 24, Rounding = 1 })

-- Safety (Panic keybind - Brigida punya AddKeybind)
MakeLabel(GB.Player.Right.Safety, "Panic")
GB.Player.Right.Safety:AddKeybind({
    Title   = "Panic Keybind",
    Value   = "P",
    Callback = function(key) end,
})
RegOption("PanicKeybind", "P")

MakeToggle(GB.Player.Right.Safety, "AutoKick", { Text = "Auto Kick", Default = true })
MakeDropdown(GB.Player.Right.Safety, "SelectedKickType", {
    Text    = "Select Type",
...
-- // TELEPORT TAB
-- // ============================================================
MakeDropdown(TB_Tabs.Waypoint.T1, "SelectedIsland", {
    Text     = "Select Island",
...
MakeDropdown(TB_Tabs.Waypoint.T2, "SelectedQuestNPC", {
    Text     = "Select NPC",
...
MakeButton(TB_Tabs.Waypoint.T2, "Teleport to Level Based Quest", function()
...
MakeDropdown(TB_Tabs.Waypoint.T3, "SelectedMiscNPC", {
...
MakeDropdown(TB_Tabs.Waypoint.T3, "SelectedMiscAllNPC", {
...
MakeDropdown(TB_Tabs.NPCs.T1, "SelectedMovesetNPC", {
    Text     = "Select NPC",
...
MakeDropdown(TB_Tabs.NPCs.T2, "SelectedMasteryNPC", {
    Text     = "Select NPC",
...
-- // MISC TAB - Merchant
-- // ============================================================
MerchantTimerLabel = MakeLabel(TB_Tabs.Merchant.T1, "Refresh: N/A")

MakeDropdown(TB_Tabs.Merchant.T1, "SelectedMerchantItems", {
    Text   = "Select Item (s)",
    Values = Tables.MerchantList,
    Multi  = true,
})
MakeToggle(TB_Tabs.Merchant.T1, "AutoMerchant", { Text = "Auto Buy Selected Items", Default = false })

MakeLabel(TB_Tabs.Merchant.T2, "Dungeon Merchant")
MakeDropdown(TB_Tabs.Merchant.T2, "SelectedDungeonMerchantItems", {
    Text   = "Select Item (s)",
    Values = Tables.DungeonMerchantList or {},
    Multi  = true,
})
MakeToggle(TB_Tabs.Merchant.T2, "AutoDungeonMerchant", { Text = "Auto Buy Dungeon Items", Default = false })

MakeLabel(TB_Tabs.Merchant.T3, "Able to causing performance issues.")
ValentineLabel = MakeLabel(TB_Tabs.Merchant.T3, "Hearts: N/A")
MakeDropdown(TB_Tabs.Merchant.T3, "SelectedValentineMerchantItems", {
    Text   = "Select Item (s)",
    Values = Tables.ValentineMerchantList,
    Multi  = true,
})
ValentinePriceLabel = MakeLabel(TB_Tabs.Merchant.T3, "Price: N/A")
MakeToggle(TB_Tabs.Merchant.T3, "AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

-- Merchant2
MakeLabel(TB_Tabs.Merchant2.T1, "Tower Merchant")
MakeDropdown(TB_Tabs.Merchant2.T1, "SelectedTowerMerchantItems", {
    Text   = "Select Item (s)",
    Values = Tables.InfiniteTowerMerchantList or {},
    Multi  = true,
})
MakeToggle(TB_Tabs.Merchant2.T1, "AutoTowerMerchant", { Text = "Auto Buy Tower Items", Default = false })

MakeLabel(TB_Tabs.Merchant2.T2, "Boss Rush Merchant")
MakeDropdown(TB_Tabs.Merchant2.T2, "SelectedBossRushMerchantItems", {
    Text   = "Select Item (s)",
    Values = Tables.BossRushMerchantList or {},
    Multi  = true,
})
MakeToggle(TB_Tabs.Merchant2.T2, "AutoBossRushMerchant", { Text = "Auto Buy Boss Rush Items", Default = false })

-- Misc1
MakeDropdown(TB_Tabs.Misc1.T1, "SelectedChests", {
    Text   = "Select Chest (s)",
    Values = Tables.Rarities,
    Multi  = true,
})
MakeToggle(TB_Tabs.Misc1.T1, "AutoChest", { Text = "Auto Open Chest", Default = false })

MakeDropdown(TB_Tabs.Misc1.T2, "SelectedCraftItems", {
    Text   = "Select Item (s) To Craft",
    Values = Tables.CraftItemList,
    Multi  = true,
})
MakeToggle(TB_Tabs.Misc1.T2, "AutoCraftItem", { Text = "Auto Craft Item", Default = false })

MakeToggle(TB_Tabs.Misc1.T3, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

-- // ============================================================
-- // MISC TAB - Puzzle & Questlines
-- // ============================================================
MakeButton(TB_Tabs.Puzzle.T1, { Text = "Complete Dungeon Puzzle", Disabled = not Support.Proximity, Func = function()
...
MakeButton(TB_Tabs.Puzzle.T1, { Text = "Complete Slime Key Puzzle", Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Slime")
end })

MakeButton(TB_Tabs.Puzzle.T1, { Text = "Complete Valentine Puzzle", Disabled = not Support.Proximity, Func = function()
    local Character = GetCharacter()
    ...
    for i = 1, 3 do
        ...
        if item then
            ...
            if prompt then
                ...
                fireproximityprompt(prompt)
                task.wait(0.5)
            else Library:Notify("No prompt found in " .. itemName, 2) end
        else Library:Notify(itemName .. " not found!", 2) end
    end
end })

MakeButton(TB_Tabs.Puzzle.T1, { Text = "Complete Demonite Puzzle", Disabled = not Support.Proximity, Func = function()
    UniversalPuzzleSolver("Demonite")
end })

MakeButton(TB_Tabs.Puzzle.T1, { Text = "Complete Hogyoku Puzzle", Disabled = not Support.Proximity, Func = function()
    local currentLevel = Plr.Data.Level.Value
...
MakeLabel(TB_Tabs.Puzzle.T2, "- ⚠️: Experimental feature. Deep testing required!\n- ⚠️: Make sure to store your race & clan before using this.\n- Dungeon tasks only make you join dungeon.\n- Feature will change some other features settings.\n- Report any bugs on discord server!")

MakeDropdown(TB_Tabs.Puzzle.T2, "SelectedQuestline", {
    Text   = "Select Questline",
    Values = Tables.QuestlineList,
    Multi  = false,
})

MakeDropdown(TB_Tabs.Puzzle.T2, "SelectedQuestline_Player", {
    Text   = "Select Player",
    Values = {},
    Multi  = false,
})

MakeDropdown(TB_Tabs.Puzzle.T2, "SelectedQuestline_DMGTaken", {
    Text   = "Select Mob [Take Damage]",
    Values = Tables.AllEntitiesList,
    Multi  = false,
})

MakeButton(TB_Tabs.Puzzle.T2, "Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)

MakeToggle(TB_Tabs.Puzzle.T2, "AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })

-- // ============================================================
-- // CONFIG TAB
-- // ============================================================
local MenuGroup = Tabs.Config:AddSection({ Title = "Menu", Tabbox = "Left" })

MakeToggle(MenuGroup, "AutoShowUI", { Text = "Auto Show UI", Default = true })

MakeToggle(MenuGroup, "KeybindMenuOpen", {
    Text     = "Open Keybind Menu",
    Default  = false,
    Callback = function(value) end,
})

MakeToggle(MenuGroup, "ShowCustomCursor", {
    Text     = "Custom Cursor",
    Default  = false,
...
MakeDropdown(MenuGroup, "NotificationSide", {
...
MakeDropdown(MenuGroup, "DPIDropdown", {
    Text    = "DPI Scale",
...
RegOption("MenuKeybind", "U")

MakeButton(MenuGroup, "Unload", function()
...
-- // ============================================================
-- // OnChanged CALLBACKS (sama persis seperti Obsidian)
-- // ============================================================
...
-- // POST SETUP
-- // ============================================================
-- Update dropdown values setelah init
...
task.spawn(function()
    ...
    -- Config autoload (Brigida handles via Config.AutoLoad = true di Window)
    task.wait(0.25)
...
elseif UIS.KeyboardEnabled then
    ...
    Library:Notify("Device detected: PC\nPress " .. (Options.MenuKeybind.Value or "U") .. " to show/hide UI.", 10)
end

task.spawn(function()
    task.wait(0.1)
    if Toggles.AutoShowUI and Toggles.AutoShowUI.Value == false then
        Library:Toggle()
    end
end)
