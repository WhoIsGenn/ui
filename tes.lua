...
fishingFeaturesSection:AddToggle({
    ...
    Callback = function(v)
        _G.AutoEquipRod = v
        if v then
            -- Equip rod langsung tanpa delay
            if not hasRod() then
                rod()
            end
...
-- Auto equip saat respawn (INSTAN)
game.Players.LocalPlayer.CharacterAdded:Connect(function()
    if _G.AutoEquipRod then
        task.wait(0.5)  -- Tunggu character load dikit
        if not hasRod() then
            rod()
        end
    end
...
local function legitcycle()
    -- Auto equip rod (INSTAN - tanpa nunggu)
    if _G.AutoEquipRod and not hasRod() then
        rod()
    end
...
legitFishingSection:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            -- Auto equip rod dulu sebelum mulai (INSTAN)
            if _G.AutoEquipRod and not hasRod() then
                rod()
            end
