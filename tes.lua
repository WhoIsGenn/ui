...
local function AutoParry()
    ...
    -- Cek team langsung, tidak pakai GetRole() yang bisa return "Lobby"
...
local function QTE_IsNeedleInZone(needleAngle, targetAngle)
    ...
    -- Offset 104-114 dari base code asli VD2
    -- Zone: needle harus ada di antara (Goal+104) dan (Goal+114)
    local s = (target + 104) % 360
    local e = (target + 114) % 360
...
local secESPGeneral = TabESP:AddSection("General")
secESPGeneral:AddToggle({
    Title = "Enable ESP",
...
secESPPlayers:AddToggle({
    ...
    Callback = function(v) Config.ESP_Killer = v; notifToggle("Killer ESP", v) end
})
secESPPlayers:AddToggle({
    Title = "Survivor ESP",
    Default = Config.ESP_Survivor,
    Callback = function(v) Config.ESP_Survivor = v; notifToggle("Survivor ESP", v) end
})
secESPPlayers:AddDropdown({
    Title = "Player Mode",
    Options = {"Draw Box", "Chams"},
    Default = Config.ESP_PlayerChams and "Chams" or "Draw Box",
...
secESPPlayers:AddToggle({
    ...
    Callback = function(v) Config.ESP_Skeleton = v; notifToggle("Skeleton", v) end
})
secESPPlayers:AddToggle({
...
secESPObjects:AddDropdown({
    Title = "Object Mode",
    Options = {"Draw Box", "Chams"},
    Default = Config.ESP_ObjectChams and "Chams" or "Draw Box",
...
local secGenerator = TabSURV:AddSection("Generator")
secGenerator:AddToggle({
    Title = "Auto Generator",
    Default = Config.AUTO_Generator,
    Callback = function(v) Config.AUTO_Generator = v; notifToggle("Auto Generator", v) end
})
secGenerator:AddDropdown({
    Title = "Gen Speed",
    Options = {"Fast", "Slow"},
    Default = Config.AUTO_GenMode,
    Callback = function(v) Config.AUTO_GenMode = v end
})
secGenerator:AddButton({
    Title = "Leave Generator",
    Callback = function() LeaveGenerator() end
})
secGenerator:AddButton({
    Title = "Stop Auto Generator",
    Callback = function() StopAutoGen() end
})
secGenerator:AddSlider({
    Title = "Leave Distance",
    Min = 10, Max = 30, Default = Config.AUTO_LeaveDist,
...
secSurvival:AddToggle({
    Title = "Flee Killer (Auto TP Away)",
    Default = Config.AUTO_TeleAway,
    Callback = function(v) Config.AUTO_TeleAway = v; notifToggle("Flee Killer", v) end
})
secSurvival:AddSlider({
    Title = "Flee Distance",
    Min = 20, Max = 80, Default = Config.AUTO_TeleAwayDist,
...
secSurvival:AddToggle({
    Title = "Anti Fail Gen (disable QTE)",
    Default = Config.ANTI_FailGen,
    Callback = function(v)
        Config.ANTI_FailGen = v
        if v then
            AntiFailGen_Apply(LocalPlayer.Character)
        else
            AntiFailGen_Restore()
        end
