...
local VelarisUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/WhoIsGenn/ui/refs/heads/main/victui.lua"))()
...
function Auto_Parry.Get_Ball()
    ...
    if bc then
        for _, b in pairs(bc:GetChildren()) do
            if b:IsA("BasePart") and (
                b:GetAttribute("realBall") or
                b:GetAttribute("target") or
                b:GetAttribute("Target") or
                b.Name:lower():find("ball")
            ) then
                return b
            end
        end
        -- Fallback: ambil BasePart apapun di folder balls
        for _, b in pairs(bc:GetChildren()) do
    ...
    if shared.Clicked_Target_Name then shared.Clicked_Target_Name = nil end
...
function Auto_Parry.CalculateParryCFrame(ball, targetPlayer)
    ...
    local mode = Config.ParryCurveDirection or "Camera"
    ...
    if targetPlayer and targetPlayer.PrimaryPart then
        targetPos = targetPlayer.PrimaryPart.Position
    elseif ball and ball.Parent then
        targetPos = ball.Position
    else
        targetPos = camPos + cam.CFrame.LookVector * 100
    end
    ...
    if mode == "Left" then
        return CFrame.new(camPos, camPos + (camRight * -1) * 100)
    elseif mode == "Right" then
        return CFrame.new(camPos, camPos + (camRight * 1) * 100)
    elseif mode == "Up" then
        return CFrame.new(camPos, camPos + (camUp * 1) * 100)
    elseif mode == "Down" then
        return CFrame.new(camPos, camPos + (camUp * -1) * 100)
    elseif mode == "Backwards" then
        return CFrame.new(camPos, camPos + (camLook * -1) * 100)
    elseif mode == "Random" then
        return CFrame.new(camPos, camPos + Vector3.new(math.random(-100, 100), math.random(-100, 100), math.random(-100, 100)))
...
for _, Function in getgc(true) do
    ...
    for _, value in debug.getupvalues(Function) do
        if type(value) == 'function' then _token = value; break end
    end
    if _token then break end
end

function _tokenize(_remote_uid)
    local time = tostring(math.floor(workspace_ref:GetServerTimeNow() * 100))
...
function _is_valid(args)
    ...
    if type(args[2]) ~= "string" or type(args[3]) ~= "string" then return false end
...
-- AUTO CAPTURE: extract parry remote + real args dari upvalues GC function
task.spawn(function()
    task.wait(1)
    if _captured then return end
    ...
    pcall(function()
        for _, func in ipairs(getgc(true)) do
            ...
            if not info:find("PRY", 1, true) then continue end

            -- Cari remote dan args dari upvalues
            local ups = debug.getupvalues(func)
            local foundRemote = nil
            local foundArgs = nil

            for _, up in ipairs(ups) do
                if typeof(up) == "Instance" and
                   (up:IsA("RemoteEvent") or up:IsA("RemoteFunction")) then
                    foundRemote = up
                end
                if type(up) == "table" and #up >= 8 and _is_valid(up) then
                    foundArgs = up
                end
            end

            -- Kalau ketemu remote tapi belum ada args, cari di constants
            if foundRemote and not foundArgs then
                local ok2, consts = pcall(getconstants, func)
                if ok2 and consts then
                    for _, c in ipairs(consts) do
                        if type(c) == "table" and #c >= 8 and _is_valid(c) then
                            foundArgs = c; break
                        end
                    end
                end
            ...
            -- Kalau ketemu keduanya, set langsung tanpa perlu FireServer
            if foundRemote and foundArgs then
                _reverted[foundRemote] = foundArgs
                _captured = { remote = foundRemote, args = foundArgs }
                print("[INVIS HUB] Auto-captured args:", foundRemote.Name)
                break
            elseif foundRemote then
                -- Ketemu remote tapi belum ada args — coba cari dari connections
                pcall(function()
                    local ok3, conns = pcall(getconnections, foundRemote.OnClientEvent)
                    if ok3 and conns then
                        for _, conn in ipairs(conns) do
                            if conn.Function then
                                local ups2 = debug.getupvalues(conn.Function)
                                for _, up2 in ipairs(ups2) do
                                    if type(up2) == "table" and #up2 >= 8 and _is_valid(up2) then
                                        _reverted[foundRemote] = up2
                                        _captured = { remote = foundRemote, args = up2 }
                                        print("[INVIS HUB] Auto-captured from connection:", foundRemote.Name)
                                        break
                                    end
                                end
                            end
                            if _captured then break end
                        end
                    end
                end)
                if not _captured then
                    print("[INVIS HUB] Remote found but args not captured yet:", foundRemote.Name)
                    _hook(foundRemote)
                end
            end
    ...
    -- Training ball: listen ball masuk workspace
    pcall(function()
        workspace.ChildAdded:Connect(function(child)
            if child.Name == "Balls" or child.Name == "TrainingBalls" then
                print("[INVIS HUB] Ball folder detected:", child.Name)
                -- Re-scan untuk capture
                if not _captured then
...
function Auto_Parry.FireParryRemote(isSpam)
    if not _reverted or next(_reverted) == nil then return end
    local ball = Auto_Parry.Get_Ball()
    local cam = workspace.CurrentCamera
    if not cam then return end

    local targetPlayer = Auto_Parry.GetTargetPlayer()
    local curveCF = Auto_Parry.CalculateParryCFrame(ball, targetPlayer)
            ...
            for _, character in ipairs(alive:GetChildren()) do
                if character:IsA("Model") and character.PrimaryPart then
                    if character.Name ~= LocalPlayer.Name then
                        Last_Positions_Cache[character.Name] = character.PrimaryPart.Position
                    end
                end
            end
            local myChar = LocalPlayer.Character
            local myHrp = myChar and (myChar.PrimaryPart or myChar:FindFirstChild("HumanoidRootPart"))
            if myHrp then Last_Positions_Cache[LocalPlayer.Name] = myHrp.Position end
        end
            ...
            task.spawn(function()
                ...
                pcall(function()
                    if _remote:IsA("RemoteEvent") then
                        _remote:FireServer(
                            _originalArgs[1], _originalArgs[2],
...
local function fireAbilityRemote()
    if not AbilityRemote or not AbilityRawFunc then
        pcall(function() ReplicatedStorage.Remotes.AbilityButtonPress:FireServer() end)
        return true
    end
    pcall(function()
        if AbilityRemote:IsA("RemoteEvent") then AbilityRawFunc(AbilityRemote)
        else AbilityRemote:InvokeServer() end
    end)
    return true
end
    ...
    local success, ready = pcall(function()
        ...
        if screenGui then
            ...
            if abilityBtn then
                ...
                if cdFrame and cdFrame.Visible then return false end
            end
        end
        return true
    end)
...
local function isAbilityOffCooldown(abilityName)
    ...
    return (tick() - (_abilityLastFire[abilityName] or 0)) >= timing.cooldown
end
...
local function getEquippedAbility()
    ...
    for name, _ in pairs(ABILITY_TIMINGS) do
        ...
        if abil and (abil:GetAttribute("Equipped") or abil.Enabled == true) then return name end
    end
    for _, abil in ipairs(abilities:GetChildren()) do
        if abil:GetAttribute("Equipped") or abil.Enabled == true then return abil.Name end
    end
...
local function smartAutoAbility()
    ...
    local isTargetingMe = (tostring(target or "") == LocalPlayer.Name)

    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local ballDist = 999
    if hrp and ball.Parent then ballDist = (ball.Position - hrp.Position).Magnitude end

    local shouldFire = false
    if timing.trigger == "target" then
        if isTargetingMe and (tick() - _lastBallTargetTime) >= timing.fireAt then shouldFire = true end
    elseif timing.trigger == "proximity" then
        if isTargetingMe and ballDist <= 45 and (tick() - _lastBallTargetTime) >= timing.fireAt then shouldFire = true end
    elseif timing.trigger == "clash" then
        if (tick() - _lastClashTime) <= 2 and (tick() - _lastClashTime) >= timing.fireAt then shouldFire = true end
    end
...
task.spawn(function()
    while true do
        ...
        if ball then
            local target = ball:GetAttribute("target") or ball:GetAttribute("Target")
            if tostring(target or "") == LocalPlayer.Name then
                if _lastBallTargetTime == 0 then _lastBallTargetTime = tick() end
            else
...
task.spawn(function()
    while true do
        task.wait(0.05)
        if Config.AutoAbility then smartAutoAbility() end
    end
end)
...
shared._BBAChaoticThread = task.spawn(function()
    while shared._InvisRunning do
        local autoSpamEnabled = Config.AutoSpam
...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    local autoParryEnabled = Config.AutoParry
    local autoSpamEnabled = Config.AutoSpam
    local distanceTiming = Config.DistanceTiming or 50
    ...
    for idx, Ball in ipairs(balls) do
        ...
        -- Training ball: kalau ga ada target attribute, anggap targeting player
        local isTrainingBall = (ballTargetStr == "" or ballTargetStr == "nil")
        local isTargetingMe = (ballTargetStr == LocalPlayer.Name or ballTargetStr == tostring(LocalPlayer)) or isTrainingBall
        ...
        if parryLocked[Ball] then
            ...
            if multiBallBurstCount > 1 then safeLockCooldown = 0.4 end
        ...
        if autoParryEnabled and allowedToHit then
            ...
            if reachTime <= 0.05 then triggerParry = true end
            ...
            if triggerParry then
                ...
                if bTarget then ZX_Parry.KeyTable = Ball:GetAttribute("realBall") or Ball.Name or bTarget end
...
local function applyLowGraphics(enable)
    ...
    if not enable then
        if lowGraphicsConn then lowGraphicsConn:Disconnect(); lowGraphicsConn = nil end
        return
    end
    ...
    if not lowGraphicsConn then
        lowGraphicsConn = workspace.DescendantAdded:Connect(function(obj)
            if Config.LowGraphicsEnabled then optimizeObject(obj) end
        end)
    end
end
...
local function ApplyHeadless(char)
    ...
    pcall(function()
        ...
        for _, child in ipairs(head:GetChildren()) do
            if child:IsA("Decal") then child.Transparency = isHeadless and 1 or 0 end
        end
    end)
end
...
local function ApplyKorblox(char)
    ...
    pcall(function()
        if char:FindFirstChild("RightLowerLeg") and char:FindFirstChild("RightFoot") then
            char:FindFirstChild("RightLowerLeg").Transparency = isKorblox and 1 or 0
            char:FindFirstChild("RightFoot").Transparency = isKorblox and 1 or 0
        elseif char:FindFirstChild("Right Leg") then
            char:FindFirstChild("Right Leg").Transparency = isKorblox and 1 or 0
        end
    end)
    if isKorblox then
        pcall(function()
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if not hum then return end
    task.wait(0.3)
    if isHeadless then ApplyHeadless(char) end
    if isKorblox then ApplyKorblox(char) end
end)
    ...
    while shared._InvisRunning do
        ...
        pcall(function()
            ...
            if ball and ball.Parent then
                ...
                if ballSpeed >= 2500 then Config.wowcurve = 0.82
                elseif ballSpeed >= 1000 then Config.wowcurve = 0.76
                elseif ballSpeed >= 470 then Config.wowcurve = 0.6
                else
...
-- ══════════════════════════════════════════════════════════════════════
-- ══════════════════════ VELARIS UI INTERFACE ══════════════════════════
-- ══════════════════════════════════════════════════════════════════════

local Window = VelarisUI:Window({
    Title = "INVIS HUB",
    Footer = "Paid but free",
    Color = "Cyan",
    Size = UDim2.fromOffset(620, 420),
    Search = true,
    Version = "1.0",
    Config = {
        ConfigFolder = "InvisHub/Config/",
...
-- ═══ TAB: COMBAT ═══════════════════════════════════════════════════════
local CombatTab = Window:AddTab({ Name = "Combat", Icon = "lucide:sword" })

local AutoParrySec = CombatTab:AddSection({ Title = "Auto Parry", Icon = "lucide:shield" })
...
AutoParrySec:AddDropdown({
    Title = "Curve Mode",
    Description = "Arah bola setelah parry (based on camera)",
    Flag = "parrycurve",
    Options = {"Camera", "Left", "Right", "Up", "Down", "Backwards", "Random", "Straight"},
    Default = "Camera",
    Multi = false,
    Callback = function(value) Config.ParryCurveDirection = value end,
})

AutoParrySec:AddSlider({
    Title = "Accuracy",
    Description = "Parry accuracy (1 = mepet, 100 = lebar)",
    Flag = "accuracy",
    Min = 1,
    Max = 100,
    Default = 50,
    Increment = 1,
    Callback = function(value) Config.DistanceTiming = value end,
})

local AutoSpamSec = CombatTab:AddSection({ Title = "Auto Spam", Icon = "lucide:zap" })

AutoSpamSec:AddToggle({
    Title = "Auto Spam Clash",
    Description = "Continuously spam parry during clash",
    Flag = "autospam",
    Default = false,
    Callback = function(value) Config.AutoSpam = value end,
})

AutoSpamSec:AddSlider({
    Title = "Spam Speed",
    Description = "Spam frequency (seconds)",
    Flag = "spamspeed",
    Min = 0.005,
    Max = 0.100,
    Default = 0.015,
    Increment = 0.005,
    Callback = function(value) Config.ManualSpamSpeed = value end,
})

local AbilitySec = CombatTab:AddSection({ Title = "Auto Ability", Icon = "lucide:sparkles" })

AbilitySec:AddToggle({
    Title = "Smart Auto Ability",
    Description = "Auto uses ability when ball targets you",
    Flag = "autoability",
    Default = false,
    Callback = function(value) Config.AutoAbility = value end,
})

AbilitySec:AddToggle({
    Title = "Special Skill Detection",
    Description = "Detect Tornado & special ball effects",
    Flag = "specialskill",
    Default = false,
    Callback = function(value) Config.SpecialSkillDetections = value end,
})

-- ═══ TAB: VISUALS ══════════════════════════════════════════════════════
local VisualsTab = Window:AddTab({ Name = "Visuals", Icon = "lucide:eye" })

local SkinSec = VisualsTab:AddSection({ Title = "Sword Skin Changer", Icon = "lucide:sword" })

SkinSec:AddToggle({
    Title = "Skin Changer",
    Description = "Enable sword skin changer",
    Flag = "skinchanger",
    Default = false,
    Callback = function(value)
...
SkinSec:AddInput({
    ...
    Description = "Enter sword name (e.g. DualPrince)",
...
local GraphicsSec = VisualsTab:AddSection({ Title = "Graphics", Icon = "lucide:monitor" })

GraphicsSec:AddToggle({
    Title = "Low Graphics Mode",
    Description = "Boost FPS & reduce lag",
    Flag = "lowgraphics",
    Default = false,
    Callback = function(value) applyLowGraphics(value) end,
})

local AppearanceSec = VisualsTab:AddSection({ Title = "Appearance", Icon = "lucide:user" })

AppearanceSec:AddButton({
...
-- ═══ TAB: CONFIG ═══════════════════════════════════════════════════════
local ConfigTab = Window:AddTab({ Name = "Config", Icon = "lucide:save" })
Window:AddConfigSection(ConfigTab, {
    Name = "Config Manager",
    FolderName = "InvisHub",
})

-- ═══ LIVE REMOTE CAPTURE NOTIFICATION ═════════════════════════════════
task.spawn(function()
    local notified = false
    while shared._InvisRunning do
        task.wait(1)
        if not notified and _reverted and next(_reverted) ~= nil then
            notified = true
            print("[INVIS HUB] Remote captured!")
            pcall(function()
                Window:Notify({
                    Title = "Auto Parry Ready",
                    Content = "Remote captured! Auto parry aktif.",
                    Duration = 4,
                })
            end)
        end
...
print("[INVIS HUB] Loaded with VelarisUI")
print("[INVIS HUB] Parry manual 1x untuk capture remote.")
