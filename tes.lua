...
local Library = {
    Notify     = function(self, msg, dur)
        if VelarisUI then pcall(function() VelarisUI:MakeNotify({ Title = "celina", Description = msg, Time = dur or 3 }) end) end
    end,
...
VelarisUI:MakeNotify({ Title = "celina", Description = "Script loaded!", Content = "Report bugs in Discord!", Color = "Success", Time = 5 })
...
if not eh_success then
    warn("[celina] ERROR: " .. tostring(err))
    if VelarisUI then
        pcall(function()
            VelarisUI:MakeNotify({ Title = "celina", Description = "ERROR: " .. tostring(err), Color = "Danger", Time = 6 })
        end)
    end
end
