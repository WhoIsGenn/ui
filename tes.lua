...
function Aimbot.Update(cam, screenSize, screenCenter)
    if not Config.AIM_Enabled and not Config.GUN_SilentAim then
        State.AimTarget = nil
        return
    end

    local gunMode = Config.GUN_SilentAim and not Config.AIM_Enabled
    if not gunMode and not State.AimHolding then
        State.AimTarget = nil
        return
    end
    ...
    -- Gun mode: kalau tidak ada target dalam FOV, lepas (tidak lock)
    if gunMode and not target then
        State.AimTarget = nil
        return
    end
...
local function MainLoop()
    ...
    if GUI.Drawings and GUI.Drawings.FOVCircle then
        if (Config.AIM_Enabled and Config.AIM_ShowFOV) or Config.GUN_SilentAim then
