...
local ParryTimer = 0
-- Track killer attack state
local KillerAttackDetected = false
local KillerAttackConn     = nil
local LastKillerTracked    = nil

local function AutoParry_HookKiller(killer)
    if not killer or not killer.Character then return end
    if LastKillerTracked == killer then return end
    LastKillerTracked = killer

    -- Disconnect old
    if KillerAttackConn then
        pcall(function() KillerAttackConn:Disconnect() end)
        KillerAttackConn = nil
    end

    -- Hook AnimationPlayed on killer Humanoid
    ...
    KillerAttackConn = animator.AnimationPlayed:Connect(function(track)
        -- Attack animations biasanya ada "attack", "swing", "lunge", "hit" di name/id
        local name = track.Name:lower()
        if name:find("attack") or name:find("swing") or name:find("lunge")
            or name:find("strike") or name:find("slash") or name:find("hit")
            or name:find("basic") or name:find("melee") then
            KillerAttackDetected = true
            -- Reset setelah window parry (0.8s)
...
local function AutoParry_Tick(dt)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            -- Hook killer untuk detect attack anim
            AutoParry_HookKiller(player)

            -- Parry hanya kalau:
            -- 1. Dalam jarak DAN killer sedang attack (anim detected)
            -- 2. Atau jarak sangat dekat (< 8) sebagai fallback
            local inRange     = dist <= Config.PARRY_Dist
            local veryClose   = dist <= 8
            local shouldParry = inRange and (KillerAttackDetected or veryClose)
...
local function AutoParry_Setup()
    ...
    ParryOnCooldown  = false
    KillerAttackDetected = false
    LastKillerTracked    = nil
    if KillerAttackConn then pcall(function() KillerAttackConn:Disconnect() end); KillerAttackConn = nil end
