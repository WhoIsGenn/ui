...
local Config = {
    ...
    GODMODE         = false,
    ANTI_FailGen    = false,
    ANTI_SkillCheck = false,  -- disable Skillcheck-player script di karakter   -- disable Skillcheck-gen script di karakter
    SURV_NoFall = false,
...
-- ================================================================
-- PERFECT SKILL CHECK
-- Hook SkillCheckEvent.OnClientEvent (server kirim saat QTE muncul)
-- Langsung FireServer SkillCheckResultEvent "success" → bypass needle
-- Ini paling reliable karena pakai remote langsung, bukan GUI monitor
-- ================================================================
local QTE_Conns     = {}
local QTE_WatchConn = nil
...
local function Godmode_Apply()
    ...
    GodmodeConn = hum.HealthChanged:Connect(function(health)
...
-- GUN AIMBOT (Twist of Fate)
-- Hook Gui-mob gun button: saat hold → aktifkan aimbot kamera ke killer
-- Saat lepas → tembak ke arah killer (arahkan camera dulu via aimbot)
local GunAimbotConn  = nil
local GunButtonConn1 = nil
local GunButtonConn2 = nil

local function GunAimbot_FindButton()
    local pg = LocalPlayer:FindFirstChild("PlayerGui")
    if not pg then return nil end
    -- Cari Gui-mob yang ada di Controls item gun (bukan parry)
    -- Twist of Fate biasanya punya Gui yang berbeda dari parry
    for _, gui in ipairs(pg:GetChildren()) do
        local gen = gui:FindFirstChild("Gen")
        if gen then
            local itemFrame = gen:FindFirstChild("ItemFrame")
            if itemFrame then
                -- Cari di semua item slot
                for _, slot in ipairs(itemFrame:GetChildren()) do
                    local controls = slot:FindFirstChild("Controls")
                    if controls then
                        local btn = controls:FindFirstChild("Gui-mob")
        ...
        -- Fallback: cari Controls langsung
...
local function GunSilentAim_Setup()
    -- Setup gun aimbot: hook Gui-mob hold/release
    -- Saat hold gun button → State.AimHolding = true (aimbot aktif)
    -- Saat lepas → State.AimHolding = false
    -- Aimbot.Update() di RenderStepped akan auto-aim kamera ke killer saat holding
    task.spawn(function()
        task.wait(2)  -- tunggu GUI load
        local pg = LocalPlayer:WaitForChild("PlayerGui")
        
        -- Monitor perubahan item yang di-equip via ChildAdded
        -- Tiap equip item baru, Controls.Gui-mob baru muncul
        local function hookGunButton(gui)
            gui.DescendantAdded:Connect(function(desc)
                if not Config.GUN_SilentAim then return end
                if desc:IsA("ImageButton") and desc.Name == "Gui-mob" then
                    -- Cek apakah ini gun button (parent chain ada "Twist" atau item gun)
                    local fullPath = desc:GetFullName()
                    -- Hook semua Gui-mob sebagai fallback
                    if GunButtonConn1 then pcall(function() GunButtonConn1:Disconnect() end) end
                    if GunButtonConn2 then pcall(function() GunButtonConn2:Disconnect() end) end
                    GunButtonConn1 = desc.MouseButton1Down:Connect(function()
                        if Config.GUN_SilentAim then
                            State.AimHolding = true
                        end
                    end)
                    GunButtonConn2 = desc.MouseButton1Up:Connect(function()
...
local function MainLoop()
    ...
    -- FOV Circle handled by dedicated Heartbeat loop (see Init)
...
local function Init()
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- Re-apply anti-scripts setelah sedikit delay
