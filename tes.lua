...
-- ================================================================
-- GENERATOR PROGRESS
-- Reuse billboard, update warna+teks saja = no flicker
-- ================================================================
local ESPGenerators  = {}
...
local function UpdateGeneratorProgress(gen)
    ...
    -- Reuse billboard, jangan recreate tiap tick
...
-- ================================================================
-- ESP REFRESH MAP (event-based only, jangan dipanggil tiap tick)
-- Highlight dibuat sekali, tidak destroy/recreate = no flicker
-- ================================================================
local function ESPRefreshMap()
    ...
    for _, obj in ipairs(Map:GetDescendants()) do
        ...
        elseif n == "Hook" then
            -- Selalu clear dulu, baru apply kalau toggle on
...
-- ================================================================
-- ESP UPDATE PLAYERS
-- Reuse BillboardGui, update text/warna saja = no flicker
-- ================================================================
local function ESPUpdatePlayers()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        -- Tentukan apakah player ini harus ditampilkan
        ...
        if shouldShow then
            ...
            -- Apply highlight ke seluruh character model
            ApplyHighlight(char, espColor, 0.6, 0)

            -- Nametag (reuse, jangan recreate)
            if hrp then
                local myRoot = GetCharacterRoot()
                ...
                if not tag then
                    ...
                    tag.Name = "_VHTag"; tag.AlwaysOnTop = true
                    tag.Size = UDim2.new(0, 130, 0, 40)
                    tag.StudsOffset = Vector3.new(0, 3.2, 0)
                    tag.Adornee = hrp; tag.Parent = hrp
                    local lbl = Instance.new("TextLabel")
                    lbl.Name = "_VHTagLabel"; lbl.Size = UDim2.new(1, 0, 1, 0)
                    lbl.BackgroundTransparency = 1; lbl.Font = Enum.Font.GothamBold
                    lbl.TextSize = 11; lbl.TextWrapped = true
                    lbl.TextStrokeTransparency = 0
                    lbl.TextStrokeColor3 = Color3.new(0, 0, 0)
                    lbl.Parent = tag
                end
                local lbl = tag:FindFirstChild("_VHTagLabel")
                if lbl then lbl.Text = txt; lbl.TextColor3 = espColor end
            end
        else
            -- Toggle off: hapus highlight & tag
...
SecESPObjects:AddToggle({Title="Generator",Default=Config.ESP_Generator,Callback=function(v)
    Config.ESP_Generator=v
    local m=workspace:FindFirstChild("Map")
    if m then
        for _, obj in ipairs(m:GetDescendants()) do
            if obj.Name=="Generator" then
                if v then ApplyHighlight(obj, ESPColors.Generator, 0.5, 0)
                else RemoveHighlight(obj) end
            end
        end
    end
end})
SecESPObjects:AddToggle({Title="Gate",Default=Config.ESP_Gate,Callback=function(v)
    Config.ESP_Gate=v
    local m=workspace:FindFirstChild("Map")
    if m then
        for _, obj in ipairs(m:GetDescendants()) do
            if obj.Name=="Gate" then
                if v then ApplyHighlight(obj, ESPColors.Gate, 0.6, 0)
                else RemoveHighlight(obj) end
            end
        end
    end
end})
SecESPObjects:AddToggle({Title="Hook",Default=Config.ESP_Hook,Callback=function(v)
    Config.ESP_Hook=v
    local m=workspace:FindFirstChild("Map")
    if m then
        for _, obj in ipairs(m:GetDescendants()) do
            if obj.Name=="Hook" then
                ...
                if mdl then
                    for _, p in ipairs(mdl:GetDescendants()) do
                        if p:IsA("MeshPart") then
                            if v then ApplyHighlight(p, ESPColors.Hook, 0.5, 0)
                            else RemoveHighlight(p) end
                        end
                    end
                else
                    if v then ApplyHighlight(obj, ESPColors.Hook, 0.5, 0)
                    else RemoveHighlight(obj) end
                end
            end
        end
...
SecESPObjects:AddToggle({Title="Pallet",Default=Config.ESP_Pallet,Callback=function(v)
    Config.ESP_Pallet=v
    local m=workspace:FindFirstChild("Map")
    if m then
        for _, obj in ipairs(m:GetDescendants()) do
            if obj.Name=="Pallet" or obj.Name=="Palletwrong" then
                if v then ApplyHighlight(obj, ESPColors.Pallet, 0.55, 0)
                else RemoveHighlight(obj) end
            end
        end
    end
end})
SecESPObjects:AddToggle({Title="Window",Default=Config.ESP_Window,Callback=function(v)
    Config.ESP_Window=v
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj.Name=="Window" then
            if v then ApplyHighlight(obj, ESPColors.Window, 0.55, 0)
            else RemoveHighlight(obj) end
        end
    end
end})
...
SecKillerCamera:AddToggle({Title="Third Person",Default=Config.CAM_ThirdPerson,Callback=function(v)
    ...
    if not v then ThirdPerson_Remove() end
    notif("Third Person",v)
end})
...
local function MainLoop()
    ...
    -- ESP player + gen progress update (throttled, no full map rescan)
...
local function AutoLoop()
    while not State.Unloaded do
        ...
        -- Auto gen remote
        if Config.AUTO_Generator and _AutoGenRemote and _AutoGenSkillRemote then
...
-- ================================================================
local function Init()
    -- Initial scan (satu kali saat load)
    ScanMap()
    ESPRefreshMap()
    ...
    Connections.Input = UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if input.KeyCode == Config.KEY_LeaveGen then
            local root = GetCharacterRoot()
            if root then root.CFrame = root.CFrame * CFrame.new(0,0,-Config.AUTO_LeaveDist) end
            return
        end
        if input.KeyCode == Config.KEY_StopGen  then Config.AUTO_Generator = false; return end
    ...
    -- Map added: scan + ESP refresh (event-based, bukan loop)
    ...
    -- Character respawn: scan + re-setup
    ...
    -- Background task: AutoGen remote scan (jarang, bukan tiap 0.1s)
    task.spawn(function()
        while not State.Unloaded do
            task.wait(2)
            if not Config.AUTO_Generator then continue end
            -- Cari remotes jika belum ada
