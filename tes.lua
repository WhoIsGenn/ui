...
SilentAim_Setup = function()
    ...
    pcall(function()
        ...
        mt.__namecall = newcclosure(function(self, ...)
            local method = getnamecallmethod()
            if method == "FireServer" and Config.SILENT_AIM then
                -- Cek via full path: Fire → Twist of Fate → Items → Remotes
                local isTarget = false
                pcall(function()
                    isTarget = self.Name == "Fire"
                        and self.Parent ~= nil
                        and self.Parent.Name == "Twist of Fate"
                        and self.Parent.Parent ~= nil
                        and self.Parent.Parent.Name == "Items"
                end)
                if isTarget then
                    local args = table.pack(...)
                    -- Debug: print semua args
                    local argTypes = {}
                    for i = 1, args.n do
                        argTypes[i] = typeof(args[i])
                    end
                    print("[SilentAim] FireServer args:", table.concat(argTypes, ", "))
                    
                    local gun, direction = args[1], args[2]
                    -- Pastikan args valid: gun=Instance, direction=Vector3
                    if typeof(gun) == "Instance" and typeof(direction) == "Vector3" then
                        ...
                        if target and target.Character and myRoot then
                            ...
                            if aimPart then
                                ...
                                print("[SilentAim] ->", target.Name, partName)
                            end
                        end
                        return oldNamecall(self, gun, direction)
                    end
                    -- Args tidak sesuai ekspektasi, pass original
                    return oldNamecall(self, ...)
                end
            end
            return oldNamecall(self, ...)
