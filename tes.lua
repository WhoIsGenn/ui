...
function Aimbot.AimAt(cam, targetPos)
    ...
    -- Set Scriptable agar bisa override camera CFrame
...
function Aimbot.Update(cam, screenSize, screenCenter)
    if not Config.AIM_Enabled then
        State.AimTarget = nil
        -- Restore camera type saat aimbot off
    ...
    if target and target.Character then
        ...
        -- Tidak ada target, restore camera
        if cam and cam.CameraType == Enum.CameraType.Scriptable then
            cam.CameraType = Enum.CameraType.Custom
        end
    end
...
local function Init()
    ...
    -- FOV Circle via Drawing (executor-based overlay)
    local fovOk, fovObj = pcall(function()
        local c = Drawing.new("Circle")
        c.Thickness = 2
        c.Color = Color3.fromRGB(0, 170, 255)
        c.Filled = false
        c.NumSides = 64
        c.Transparency = 1
        c.Visible = false
    ...
    if fovOk and fovObj then
        GUI.Drawings.FOVCircle = fovObj
    else
        -- Fallback: dummy object agar tidak error
        GUI.Drawings.FOVCircle = setmetatable({}, {
