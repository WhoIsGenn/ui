...
local function ReapplyFeatures()
    ...
    -- Fitur untuk semua role (Killer & Survivor)
    if Config.GODMODE then pcall(Godmode_Apply) end
    if Config.PERFECT_SkillCheck then task.spawn(PerfectSC_Setup) end
    ...
    -- Killer only
    if role == "Killer" then
        -- KillerSpeed/features akan handle sendiri via GetRole() check di MainLoop
    end
