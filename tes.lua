--// =========================================================
--// Oil Empire Admin Menu - Velaris UI (FIXED)
--// =========================================================
...
local function parseNumericValue(rawValue)
    if typeof(rawValue) == "number" then return rawValue end
...
local function textMatchesLocalPlayer(text)
    ...
    if normalized == UsernameLower or normalized == DisplayNameLower then return true end
    if normalized == ("owner: " .. UsernameLower) then return true end
    if normalized == ("owner: " .. DisplayNameLower) then return true end
    if normalized == (UsernameLower .. "'s base") then return true end
    if normalized == (DisplayNameLower .. "'s base") then return true end
    if string.find(normalized, UsernameLower, 1, true) then return true end
    if string.find(normalized, DisplayNameLower, 1, true) then return true end
    return false
end
...
local function getOwnerTextLabel(plot)
    ...
    if directLabel and directLabel:IsA("TextLabel") then return directLabel end
    for _, descendant in ipairs(main:GetDescendants()) do
        if descendant:IsA("TextLabel") then return descendant end
    end
    return nil
end
...
local function getTeleportPartFromBuilding(building)
    ...
    if primaryChild and primaryChild:IsA("BasePart") then return primaryChild end
    if building:IsA("BasePart") then return building end
    if building:IsA("Model") then
        if building.PrimaryPart then return building.PrimaryPart end
        local preferredNames = {"Main", "Base", "Root", "HumanoidRootPart", "Hitbox"}
...
local function returnToOwnedBase()
    ...
    if not ownedPlot then return false end
...
local function getBuildingPrimaryContainer(building)
    ...
    if namedPrimary then return namedPrimary end
    if building:IsA("Model") and building.PrimaryPart then return building.PrimaryPart end
    return nil
end

local function parseRefineryCapacityText(text)
    if typeof(text) ~= "string" then return nil, nil end
    ...
    if not currentAmount or not maxAmount then return nil, nil end
...
local function getRefineryAmounts(building)
    ...
    if not valueLabel then return nil, nil, nil end
    local rawText = valueLabel.Text
    local currentAmount, maxAmount = parseRefineryCapacityText(rawText)
    if not currentAmount or not maxAmount then return nil, nil, rawText end
...
local function updateBestRefineryESP()
    ...
    BestRefineryText.Text = string.format("Best Refinery: %s | %s | %s",
        tostring(bestPlot.Name), tostring(bestBuilding.Name),
...
local function shouldAutoSellNow()
    ...
    if not AutoSellLimitEnabled then return true end
    local gasPrice = getNumericGasPrice()
    if gasPrice == nil then return false end
        ...
        while AutoCollectGasEnabled and thisLoopId == AutoCollectLoopId do
            ...
            if ownedPlot then
                ...
                for _, info in ipairs(refineryBuildings) do
                    if not AutoCollectGasEnabled or thisLoopId ~= AutoCollectLoopId then break end
...
local function bindStealPrompt(prompt)
    ...
    if StealPromptConnections[prompt] then return end
    StealPromptConnections[prompt] = prompt.Triggered:Connect(function(player)
        if player ~= LocalPlayer then return end
        if not AutoReturnAfterStealEnabled then return end
        task.defer(function()
            task.wait(0.2)
            returnToOwnedBase()
        end)
...
local DrillNames = {
    ...
    "Reinforced Drill", "Industrial Drill", "Double Industrial Drill", "Turbo Drill",
    "Mega Drill", "Mega Emerald Drill", "Hell Drill", "Plasma Drill",
    "Huge Long Drill", "Mega Plasma Drill", "Multi Drill", "Lava Drill",
    "Ice Plasma Drill", "Crystal Drill", "Diamond Drill", "Ruby Drill"
}

local function purchaseDrill(drillName)
    local args = {"DrillShop", drillName}
    pcall(function() PurchaseRemote:FireServer(unpack(args)) end)
end
...
-- Info Labels (pake Paragraph, bukan AddLabel)
local secInfo = Tabs.Game:AddSection({ Title = "Info", open = false })

-- Update gas price dan plot status via paragraph
local function updateInfoParagraph()
    local numeric = getNumericGasPrice()
    local gasText = numeric and ("$" .. tostring(numeric)) or tostring(GasPriceValue.Value)
    local ownedPlot = findOwnedPlot()
    local plotText = ownedPlot and ownedPlot.Name or "not found"
    
    -- Hapus section lama dan buat baru (cara simpel update)
    -- Atau pake variable global
    if not secInfo.paragraph then
        secInfo.paragraph = secInfo:AddParagraph({
            Title = "Status",
            Content = "Gas Price: " .. gasText .. "\nOwned Plot: " .. plotText
        })
    else
        secInfo.paragraph:SetContent("Gas Price: " .. gasText .. "\nOwned Plot: " .. plotText)
    end
end

-- Panggil pertama kali dan setiap ada perubahan
updateInfoParagraph()
GasPriceValue:GetPropertyChangedSignal("Value"):Connect(updateInfoParagraph)
