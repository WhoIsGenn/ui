if getgenv().Gennskie_Running then
        ...
        if not _flags[k] then
            _flags[k] = { Value = false, _cb = {},
                ...
                SetValue  = function(self, v)
                    self.Value = v
                    for _, f in ipairs(self._cb) do pcall(f, v) end
                end,
            }
        end
        return _flags[k]
...
Options = setmetatable({}, {
    __index = function(_, k)
        if not _flags["o_"..k] then
            _flags["o_"..k] = { Value = nil, _cb = {}, _ref = nil,
                OnChanged  = function(self, fn) table.insert(self._cb, fn) end,
                SetValue   = function(self, v)
                    self.Value = v
                    for _, f in ipairs(self._cb) do pcall(f, v) end
                ...
                SetValues  = function(self, v)
                    self._vals = v
                    if self._ref and self._ref.SetValues then pcall(function() self._ref:SetValues(v) end) end
                end,
                SetText    = function(self, t)
                    if self._ref and self._ref.SetContent then pcall(function() self._ref:SetContent(t) end) end
                end,
                SetVisible = function(self, v)
                    if self._ref and self._ref.SetVisible then pcall(function() self._ref:SetVisible(v) end) end
                end,
            }
        end
        return _flags["o_"..k]
...
local Library = {
    Notify = function(self, msg, dur)
        pcall(function() VelarisUI:MakeNotify({ Title = "Victoria Hub", Description = tostring(msg), Time = dur or 3 }) end)
    end,
    Unload = function(self)
        if Window and Window.Destroy then pcall(function() Window:Destroy() end) end
    end,
    Toggle = function(self)
        if Window and Window.Toggle then pcall(function() Window:Toggle() end) end
    end,
...
function AddSliderToggle(cfg) end
function CreateSwitchGroup(sec, id, displayName, tableSource) end
...
local IslandCrystals = {
    ...
    ["HollowIsland"] = workspace:FindFirstChild("HollowIsland")       and workspace.HollowIsland:FindFirstChild("SpawnPointCrystal_HollowIsland"),
    ["WorldIsland"]  = workspace:FindFirstChild("WorldIsland")        and workspace.WorldIsland:FindFirstChild("SpawnPointCrystal_World"),
    -- Sea 2 (dari screenshot)
    ["BizarreIsland"]    = workspace:FindFirstChild("BizarreIsland")    and workspace.BizarreIsland:FindFirstChild("SpawnPointCrystal_Bizarre"),
    ["BluePlanetIsland"] = workspace:FindFirstChild("BluePlanetIsland") and workspace.BluePlanetIsland:FindFirstChild("SpawnPointCrystal_BluePlanet"),
    ["PunchIsland"]      = workspace:FindFirstChild("PunchIsland")      and workspace.PunchIsland:FindFirstChild("SpawnPointCrystal_Punch"),
    ["SlayerIsland"]     = workspace:FindFirstChild("SlayerIsland")     and workspace.SlayerIsland:FindFirstChild("SpawnPointCrystal_Slayer"),
    ["StarterIsland_Sea2"]= workspace:FindFirstChild("StarterIsland_Sea2") and workspace.StarterIsland_Sea2:FindFirstChild("SpawnPointCrystal_StarterSea2"),
}
...
local function UpdateNPCLists()
    ...
    local NPCConfigsModule = nil
    
    local function findModule(container, name)
        ...
        for _, child in ipairs(container:GetChildren()) do
            ...
            if child:IsA("Folder") then
                local found = findModule(child, name)
    ...
    NPCConfigsModule = findModule(RS, "NPCConfigs")
    
    if NPCConfigsModule then
        local success, result = pcall(require, NPCConfigsModule)
        if success and type(result) == "table" then
            for npcName, npcData in pairs(result) do
                if type(npcName) == "string" and npcName:match("%a") then
                    if not npcName:find("Get") and not npcName:find("Type") and not npcName:find("Config") then
                        local cleanName = npcName:gsub("%d+$", "")
                        local isBoss = cleanName:find("Boss") ~= nil
                        local isSpecial = table.find(specialMobs, cleanName) ~= nil
                        
                        if not isBoss or isSpecial then
                            if not currentList[cleanName] then
                                table.insert(Tables.MobList, cleanName)
                                currentList[cleanName] = true
                                
                                local closestIsland = "Starter"
                                if npcData and npcData.spawnLocation then
                                    closestIsland = npcData.spawnLocation
                                end
...
local function GetCurrentPity()
    local ok, current, max = pcall(function()
        local pityLabel = PGui.BossUI.MainFrame.BossHPBar:FindFirstChild("Pity")
        if not pityLabel then return 0, 25 end
        local c, m = pityLabel.Text:match("Pity: (%d+)/(%d+)")
        return tonumber(c) or 0, tonumber(m) or 25
    end)
    if ok and current then return current, max end
    return 0, 25
end
...
local function Func_KillAura()
    while Toggles.KillAura.Value do
        ...
        task.wait(tonumber(Options.KillAuraCD.Value) or 0.01)
    end
end
...
-- ============================================================
-- VELARIS UI (VelarisUI) - Clean conversion from Obsidian
-- ============================================================
...
-- ============================================================
local Tabs = {}
Tabs.Information = Window:AddTab({ Name = "Information", Icon = "lucide:info"          })
Tabs.Priority    = Window:AddTab({ Name = "Priority",    Icon = "lucide:arrow-up-down" })
Tabs.Main        = Window:AddTab({ Name = "Main",        Icon = "lucide:box"           })
Tabs.Automation  = Window:AddTab({ Name = "Automation",  Icon = "lucide:repeat-2"      })
Tabs.Artifact    = Window:AddTab({ Name = "Artifact",    Icon = "lucide:star"          })
Tabs.Dungeon     = Window:AddTab({ Name = "Dungeon",     Icon = "lucide:door-open"     })
Tabs.Player      = Window:AddTab({ Name = "Player",      Icon = "lucide:user"          })
Tabs.Teleport    = Window:AddTab({ Name = "Teleport",    Icon = "lucide:map-pin"       })
Tabs.Webhook     = Window:AddTab({ Name = "Webhook",     Icon = "lucide:send"          })
Tabs.Misc        = Window:AddTab({ Name = "Misc",        Icon = "lucide:layers"        })
Tabs.Config      = Window:AddTab({ Name = "Config",      Icon = "lucide:cog"           })

-- ============================================================
-- SECTIONS (Brigida: Tab:AddSection)
-- ============================================================
local S = {}

-- Information
S.InfoUser   = Tabs.Information:AddSection({ Title = "User",   Open = false })
S.InfoGame   = Tabs.Information:AddSection({ Title = "Game",   Open = false })
S.InfoOthers = Tabs.Information:AddSection({ Title = "Others", Open = false })

-- Priority
S.Priority = Tabs.Priority:AddSection({ Title = "Config", Open = false })

-- Main - Autofarm sections
S.AF_Farm   = Tabs.Main:AddSection({ Title = "Autofarm",       Open = false })
S.AF_Boss   = Tabs.Main:AddSection({ Title = "Boss",           Open = false })
S.AF_Alt    = Tabs.Main:AddSection({ Title = "Alt Help",        Open = false })
S.AF_Config = Tabs.Main:AddSection({ Title = "Farm Config",    Open = false })
S.Haki      = Tabs.Main:AddSection({ Title = "Haki",           Open = false })
S.Skill     = Tabs.Main:AddSection({ Title = "Skill",          Open = false })
S.Combo     = Tabs.Main:AddSection({ Title = "Combo",          Open = false })
S.SW_Title  = Tabs.Main:AddSection({ Title = "Switch - Title", Open = false })
S.SW_Rune   = Tabs.Main:AddSection({ Title = "Switch - Rune",  Open = false })
S.SW_Build  = Tabs.Main:AddSection({ Title = "Switch - Build", Open = false })

-- Automation
S.Ascend    = Tabs.Automation:AddSection({ Title = "Ascend",   Open = false })
S.Rolls     = Tabs.Automation:AddSection({ Title = "Rolls",    Open = false })
S.Trade     = Tabs.Automation:AddSection({ Title = "Trade",    Open = false })
S.StatLevel = Tabs.Automation:AddSection({ Title = "Stats - Level", Open = false })
S.StatGem   = Tabs.Automation:AddSection({ Title = "Stats - Gem",   Open = false })
S.StatMisc  = Tabs.Automation:AddSection({ Title = "Stats - Misc",  Open = false })
S.Enchant   = Tabs.Automation:AddSection({ Title = "Enchant",   Open = false })
S.Passive   = Tabs.Automation:AddSection({ Title = "Passive",   Open = false })
S.PassCfg   = Tabs.Automation:AddSection({ Title = "Passive Config", Open = false })

-- Artifact
S.ArtStatus  = Tabs.Artifact:AddSection({ Title = "Status",     Open = false })
S.ArtEquip   = Tabs.Artifact:AddSection({ Title = "Auto-Equip", Open = false })
S.ArtUpgrade = Tabs.Artifact:AddSection({ Title = "Upgrade",    Open = false })
S.ArtLock    = Tabs.Artifact:AddSection({ Title = "Lock",       Open = false })
S.ArtDelete  = Tabs.Artifact:AddSection({ Title = "Delete",     Open = false })

-- Dungeon
S.DungJoin   = Tabs.Dungeon:AddSection({ Title = "Dungeon",         Open = false })
S.DungBR     = Tabs.Dungeon:AddSection({ Title = "Boss Rush",        Open = false })
S.DungIT     = Tabs.Dungeon:AddSection({ Title = "Infinite Tower",   Open = false })
S.DungCD     = Tabs.Dungeon:AddSection({ Title = "Crystal Defense",  Open = false })
S.DungRaid   = Tabs.Dungeon:AddSection({ Title = "Raid",             Open = false })
S.DungCfg    = Tabs.Dungeon:AddSection({ Title = "Config",           Open = false })

-- Player
S.PlGeneral = Tabs.Player:AddSection({ Title = "General", Open = false })
S.PlServer  = Tabs.Player:AddSection({ Title = "Server",  Open = false })
S.PlGame    = Tabs.Player:AddSection({ Title = "Game",    Open = false })
S.PlSafety  = Tabs.Player:AddSection({ Title = "Safety",  Open = false })

-- Teleport
S.TpIsland  = Tabs.Teleport:AddSection({ Title = "Island",       Open = false })
S.TpQuest   = Tabs.Teleport:AddSection({ Title = "Quest NPC",    Open = false })
S.TpMisc    = Tabs.Teleport:AddSection({ Title = "Misc NPC",     Open = false })
S.TpMoveset = Tabs.Teleport:AddSection({ Title = "Moveset NPC",  Open = false })
S.TpMastery = Tabs.Teleport:AddSection({ Title = "Mastery NPC",  Open = false })

-- Webhook
S.Webhook = Tabs.Webhook:AddSection({ Title = "Config", Open = false })

-- Misc
S.MerchReg  = Tabs.Misc:AddSection({ Title = "Regular Merchant",   Open = false })
S.MerchDung = Tabs.Misc:AddSection({ Title = "Dungeon Merchant",   Open = false })
S.MerchVal  = Tabs.Misc:AddSection({ Title = "Valentine Merchant", Open = false })
S.MerchTwr  = Tabs.Misc:AddSection({ Title = "Tower Merchant",     Open = false })
S.MerchBR   = Tabs.Misc:AddSection({ Title = "Boss Rush Merchant", Open = false })
S.Chests    = Tabs.Misc:AddSection({ Title = "Chests",             Open = false })
S.Craft     = Tabs.Misc:AddSection({ Title = "Craft",              Open = false })
S.Notify    = Tabs.Misc:AddSection({ Title = "Notify",             Open = false })
S.Puzzles   = Tabs.Misc:AddSection({ Title = "Puzzles",            Open = false })
S.Questline = Tabs.Misc:AddSection({ Title = "Questlines",         Open = false })

-- Config
S.Menu = Tabs.Config:AddSection({ Title = "Menu", Open = false })

-- ============================================================
-- HELPER WRAPPERS
-- ============================================================
-- Brigida: Flag-based system via Toggles/Options tables
-- OnChanged sudah dihandle oleh game logic asli, kita hanya wrap UI

local function T(sec, id, cfg)
    sec:AddToggle({
        Title    = cfg.Text or id,
        Default  = cfg.Default or false,
        Locked   = cfg.Disabled or false,
        Flag     = id,
        Callback = function(v)
            Toggles[id].Value = v
            for _, f in ipairs(Toggles[id]._cb or {}) do pcall(f, v) end
...
local function D(sec, id, cfg)
    local el = sec:AddDropdown({
        ...
        Flag     = id,
        Callback = function(v)
            -- Convert Brigida array → dict for Multi dropdowns
            if cfg.Multi and type(v) == "table" then
                local isArray = (#v > 0)
                if isArray then
                    local dict = {}
            ...
            Options[id].Value = v
            for _, f in ipairs(Options[id]._cb or {}) do pcall(f, v) end
            if cfg.Callback then pcall(cfg.Callback, v) end
        end,
    })
    Options[id]._ref = el
    Options[id].SetValues = function(self, vals)
        if el and el.SetValues then pcall(function() el:SetValues(vals) end) end
    end
end

local function SL(sec, id, cfg)
    sec:AddSlider({
        Title     = cfg.Text or id,
        Min       = cfg.Min or 0,
        Max       = cfg.Max or 100,
        Default   = cfg.Default or 0,
        Increment = cfg.Rounding ~= nil and (10^-(cfg.Rounding)) or 1,
        Locked    = cfg.Disabled or false,
        Flag      = id,
        Callback  = function(v)
            Options[id].Value = v
...
local function B(sec, titleOrCfg, fn)
    local title, cb, locked
    if type(titleOrCfg) == "string" then
        title = titleOrCfg; cb = fn; locked = false
    else
        title  = titleOrCfg.Text or titleOrCfg.Title or "Button"
        cb     = titleOrCfg.Func or titleOrCfg.Callback or fn
        locked = titleOrCfg.Disabled or false
    end
    sec:AddButton({ Title = title, Locked = locked, Callback = cb or function() end })
end
...
-- AddSliderToggle helper (used in Player tab)
local function AST(cfg)
    T(cfg.Group, cfg.Id, { Text = cfg.Text, Default = cfg.DefaultToggle or false, Disabled = cfg.Disabled })
    SL(cfg.Group, cfg.Id.."Value", { Text = cfg.Text, Default = cfg.Default, Min = cfg.Min, Max = cfg.Max, Rounding = cfg.Rounding or 0, Disabled = cfg.Disabled })
end
...
TierLabel = L(S.InfoUser, "<b>Type:</b> " .. initTier)
TimeLabel = L(S.InfoUser, "<b>Time Left:</b> " .. initTime)

task.spawn(function()
    while task.wait(5) do
        if not getgenv().Gennskie_Running then break end
...
local extraNote  = isLimitedExecutor and "NOTE: May have bugs!" or "All features should work properly!"

L(S.InfoUser, "<b>Executor:</b> " .. executorDisplayName .. "\n<b>Status:</b> " .. statusText .. "\n" .. extraNote)

B(S.InfoGame, "Redeem All Codes", function()
...
L(S.InfoOthers, "⚠️ If some features are disabled, your executor lacks required functions.")
B(S.InfoOthers, { Text = "Join Discord Server", Func = function()
...
for i = 1, #PriorityTasks do
    D(S.Priority, "SelectedPriority_" .. i, { Text = "Priority " .. i, Values = PriorityTasks, Default = DefaultPriority[i] })
end

-- ============================================================
-- WEBHOOK TAB
-- ============================================================
I(S.Webhook, "WebhookURL", { Text = "Webhook URL", Placeholder = "Enter Webhook URL..." })
I(S.Webhook, "UID",        { Text = "User ID",     Placeholder = "Enter UID..." })
D(S.Webhook, "SelectedData", { Text = "Select Data (s)", Values = {"Name","Stats","New Items","All Items"}, Multi = true })
D(S.Webhook, "SelectedItemRarity", {
...
T(S.Webhook, "PingUser",    { Text = "Ping User",    Default = false })
T(S.Webhook, "SendWebhook", { Text = "Send Webhook", Default = false, Disabled = not Support.Webhook })
SL(S.Webhook, "WebhookDelay", { Text = "Send every [x] minutes", Default = 5, Min = 1, Max = 30, Rounding = 0 })
B(S.Webhook, "Test Webhook", function() PostToWebhook() end)

-- ============================================================
-- MAIN TAB - Autofarm
-- ============================================================
D(S.AF_Farm, "SelectedMob", { Text = "Select Mob (s)", Values = Tables.MobList, Multi = true })
B(S.AF_Farm, "Refresh", UpdateNPCLists)
T(S.AF_Farm, "MobFarm",    { Text = "Autofarm Selected Mob", Default = false })
T(S.AF_Farm, "AllMobFarm", { Text = "Autofarm All Mobs",     Default = false })
D(S.AF_Farm, "AllMobType", { Text = "Select Type [All Mob]", Values = {"Normal","Fast"}, Default = "Normal" })
T(S.AF_Farm, "LevelFarm",  { Text = "Autofarm Level",        Default = false })

-- Boss
D(S.AF_Boss, "SelectedBosses",     { Text = "Select Bosses",        Values = Tables.AllBossList, Multi = true })
T(S.AF_Boss, "BossesFarm",         { Text = "Autofarm Selected Boss", Default = false })
T(S.AF_Boss, "AllBossesFarm",      { Text = "Autofarm All Bosses",    Default = false })
D(S.AF_Boss, "SelectedSummon",     { Text = "Select Summon Boss",   Values = Tables.SummonList, Multi = false })
D(S.AF_Boss, "SelectedSummonDiff", { Text = "Select Difficulty",    Values = Tables.DiffList,   Multi = false })
T(S.AF_Boss, "AutoSummon",         { Text = "Auto Summon",           Default = false })
T(S.AF_Boss, "SummonBossFarm",     { Text = "Autofarm Summon Boss",  Default = false })
PityLabel = L(S.AF_Boss, "<b>Pity:</b> 0/25")
D(S.AF_Boss, "SelectedBuildPity",      { Text = "Select Boss [Build Pity]",    Values = Tables.AllBossList, Multi = true })
D(S.AF_Boss, "SelectedUsePity",        { Text = "Select Boss [Use Pity]",      Values = Tables.AllBossList, Multi = false })
D(S.AF_Boss, "SelectedPityDiff",       { Text = "Select Difficulty [Use Pity]",Values = Tables.DiffList,    Multi = false })
T(S.AF_Boss, "PityBossFarm",           { Text = "Autofarm Pity Boss",           Default = false })
D(S.AF_Boss, "SelectedOtherSummon",    { Text = "Select Summon Boss (Other)",   Values = Tables.OtherSummonList, Multi = false })
D(S.AF_Boss, "SelectedOtherSummonDiff",{ Text = "Select Difficulty",            Values = Tables.DiffList, Multi = false })
T(S.AF_Boss, "AutoOtherSummon",        { Text = "Auto Summon",                  Default = false })
T(S.AF_Boss, "OtherSummonFarm",        { Text = "Autofarm Summon Boss",         Default = false })

-- Alt Help
D(S.AF_Alt, "SelectedAltBoss", { Text = "Select Boss",       Values = Tables.AllBossList, Multi = false })
D(S.AF_Alt, "SelectedAltDiff", { Text = "Select Difficulty", Values = Tables.DiffList,    Multi = false })
for i = 1, 5 do
    D(S.AF_Alt, "SelectedAlt_"..i, { Text = "Select Alt #"..i, Values = {}, Multi = false })
end
T(S.AF_Alt, "AltBossFarm", { Text = "Auto Help Alt", Default = false })

-- Farm Config
D(S.AF_Config, "SelectedWeaponType",   { Text = "Select Weapon Type",   Values = Tables.Weapon, Multi = true })
SL(S.AF_Config, "SwitchWeaponCD",      { Text = "Switch Weapon Delay",  Default = 1,    Min = 1,   Max = 20,   Rounding = 0 })
T(S.AF_Config,  "SwitchWeapon",        { Text = "Auto Switch Weapon",   Default = true })
T(S.AF_Config,  "IslandTP",            { Text = "Island TP [Autofarm]", Default = true })
SL(S.AF_Config, "IslandTPCD",          { Text = "Island TP CD",         Default = 0, Min = 0,   Max = 2.5,  Rounding = 2 })
SL(S.AF_Config, "TargetTPCD",          { Text = "Target TP CD",         Default = 0,    Min = 0,   Max = 5,    Rounding = 2 })
SL(S.AF_Config, "TargetDistTP",        { Text = "Target Distance TP",   Default = 0,    Min = 0,   Max = 100,  Rounding = 0 })
SL(S.AF_Config, "M1Speed",             { Text = "M1 Attack Cooldown",   Default = 0,    Min = 0,   Max = 1,    Rounding = 2 })
D(S.AF_Config,  "SelectedMovementType",{ Text = "Select Movement Type", Values = {"Teleport","Tween"}, Default = "Teleport" })
D(S.AF_Config,  "SelectedFarmType",    { Text = "Select Farm Type",     Values = {"Behind","Above","Below"}, Default = "Above" })
SL(S.AF_Config, "Distance",            { Text = "Farm Distance",        Default = 7,    Min = 0,   Max = 30,   Rounding = 0 })
SL(S.AF_Config, "TweenSpeed",          { Text = "Tween Speed",          Default = 160,  Min = 0,   Max = 500,  Rounding = 0 })
T(S.AF_Config,  "InstaKill",           { Text = "Instant Kill",         Default = false })
D(S.AF_Config,  "InstaKillType",       { Text = "Select Type",          Values = {"V1","V2"}, Default = "V1" })
SL(S.AF_Config, "InstaKillHP",         { Text = "HP% For Insta-Kill",   Default = 90,   Min = 1,   Max = 100,  Rounding = 0 })
I(S.AF_Config,  "InstaKillMinHP",      { Text = "Min MaxHP for Insta-Kill", Placeholder = "Number..", Default = "100000" })

-- Haki
T(S.Haki, "ObserHaki",    { Text = "Auto Observation Haki", Default = false })
T(S.Haki, "ArmHaki",      { Text = "Auto Armament Haki",    Default = false })
T(S.Haki, "ConquerorHaki",{ Text = "Auto Conqueror Haki",   Default = false })

-- Skill
L(S.Skill, "Autofarm already has auto-M1 built in.")
T(S.Skill, "AutoM1",   { Text = "Auto Attack", Default = false })
T(S.Skill, "KillAura", { Text = "Kill Aura",   Default = false })
SL(S.Skill, "KillAuraCD",    { Text = "CD",    Default = 0.1, Min = 0.1, Max = 1,   Rounding = 2 })
SL(S.Skill, "KillAuraRange", { Text = "Range", Default = 200, Min = 0,   Max = 200, Rounding = 0 })
L(S.Skill, "Mode:\n- Normal: Check skill cooldowns\n- Instant: No check")
D(S.Skill, "SelectedSkills",    { Text = "Select Skills", Values = {"Z","X","C","V","F"}, Multi = true })
D(S.Skill, "AutoSkillType",     { Text = "Select Mode",   Values = {"Normal","Instant"}, Default = "Normal" })
T(S.Skill, "OnlyTarget",        { Text = "Target Only",     Default = false })
T(S.Skill, "AutoSkill_BossOnly",{ Text = "Use On Boss Only",Default = false })
SL(S.Skill, "AutoSkill_BossHP", { Text = "Boss HP%",        Default = 100, Min = 1, Max = 100, Rounding = 0 })
T(S.Skill, "AutoSkill",         { Text = "Auto Use Skills", Default = false })

-- Combo
L(S.Combo, "Example: Z > X > C > 0.5 > V")
L(S.Combo, "Mode:\n- Normal: Wait for input CD\n- Instant: Ignore input CD")
I(S.Combo, "ComboPattern", { Text = "Combo Pattern", Default = "Z > X > C > V > F", Placeholder = "combo.." })
D(S.Combo, "ComboMode",    { Text = "Select Mode",   Values = {"Normal","Instant"}, Default = "Normal" })
T(S.Combo, "ComboBossOnly",{ Text = "Boss Only",     Default = false })
T(S.Combo, "AutoCombo",    { Text = "Auto Skill Combo", Default = false,
...
-- ============================================================
-- OVERRIDE: Brigida syntax untuk AddSliderToggle dan CreateSwitchGroup
-- Game logic mendefinisikan ulang keduanya dengan Obsidian syntax,
-- kita override lagi di sini agar pakai Brigida
-- ============================================================
AddSliderToggle = function(cfg)
    T(cfg.Group, cfg.Id, { Text = cfg.Text, Default = cfg.DefaultToggle or false, Disabled = cfg.Disabled })
    SL(cfg.Group, cfg.Id.."Value", {
        Text = cfg.Text, Default = cfg.Default,
        Min = cfg.Min, Max = cfg.Max, Rounding = cfg.Rounding or 0, Disabled = cfg.Disabled
    })
end
...
CreateSwitchGroup = function(sec, id, displayName, tableSource)
    ...
    SL(sec, id.."_BossHPAmt",{ Text = "Change Until Boss HP%", Default = 15, Min = 0, Max = 100, Rounding = 0 })
end

-- Switch
CreateSwitchGroup(S.SW_Title, "Title", "Title", CombinedTitleList)
CreateSwitchGroup(S.SW_Rune,  "Rune",  "Rune",  Tables.RuneList)
CreateSwitchGroup(S.SW_Build, "Build", "Build", Tables.BuildList)

-- ============================================================
-- AUTOMATION TAB
-- ============================================================
T(S.Ascend, "AutoAscend", { Text = "Auto Ascend", Default = false })
for i = 1, 10 do
    Tables.AscendLabels[i] = L(S.Ascend, "")
    Tables.AscendLabels[i]:SetVisible(false)
end

L(S.Rolls, "⚠️ Increase delay based on your ping.\n⚠️ Low delay not recommended.")
SL(S.Rolls, "RollCD", { Text = "Roll Delay", Default = 0.3, Min = 0.01, Max = 1, Rounding = 2 })
D(S.Rolls, "SelectedTrait", { Text = "Select Trait (s)", Values = Tables.TraitList, Multi = true })
T(S.Rolls, "AutoTrait",     { Text = "Auto Roll Trait",  Default = false })
D(S.Rolls, "SelectedRace",  { Text = "Select Race (s)",  Values = Tables.RaceList,  Multi = true })
T(S.Rolls, "AutoRace",      { Text = "Auto Roll Race",   Default = false })
D(S.Rolls, "SelectedClan",  { Text = "Select Clan (s)",  Values = Tables.ClanList,  Multi = true })
T(S.Rolls, "AutoClan",      { Text = "Auto Roll Clan",   Default = false })

D(S.Trade, "SelectedTradePlr",   { Text = "Select Player",   Values = {}, Multi = false })
D(S.Trade, "SelectedTradeItems", { Text = "Select Item (s)", Values = Tables.OwnedItem, Multi = true })
T(S.Trade, "ReqTrade",       { Text = "Auto Send Request",   Default = false })
T(S.Trade, "ReqTradeAccept", { Text = "Auto Accept Request", Default = false })
T(S.Trade, "AutoAccept",     { Text = "Auto Accept Trade",   Default = false })

D(S.StatLevel, "SelectedStats", { Text = "Select Stat (s)", Values = {"Melee","Defense","Sword","Power"}, Multi = true })
T(S.StatLevel, "AutoStats",     { Text = "Auto UP Stats",   Default = false })

L(S.StatGem, "⚠️ Increase delay based on ping.\n- Reroll once for this to work.")
StatsLabel = L(S.StatGem, "N/A")
D(S.StatGem, "SelectedGemStats", { Text = "Select Stat (s)", Values = Tables.GemStat, Multi = true })
D(S.StatGem, "SelectedRank",     { Text = "Select Rank (s)", Values = Tables.GemRank, Multi = true })
SL(S.StatGem, "StatsRollCD",     { Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(S.StatGem, "AutoRollStats",    { Text = "Auto Roll Stats", Default = false })

T(S.StatMisc, "AutoSkillTree",     { Text = "Auto Skill Tree",         Default = false })
T(S.StatMisc, "ArtifactMilestone", { Text = "Auto Artifact Milestone", Default = false })

D(S.Enchant, "SelectedEnchant",  { Text = "Select Enchant",  Values = Tables.OwnedAccessory, Multi = true })
T(S.Enchant, "AutoEnchant",      { Text = "Auto Enchant",     Default = false })
T(S.Enchant, "AutoEnchantAll",   { Text = "Auto Enchant All", Default = false })
D(S.Enchant, "SelectedBlessing", { Text = "Select Blessing",  Values = Tables.OwnedWeapon,    Multi = true })
T(S.Enchant, "AutoBlessing",     { Text = "Auto Blessing",     Default = false })
T(S.Enchant, "AutoBlessingAll",  { Text = "Auto Blessing All", Default = false })

SpecPassiveLabel = L(S.Passive, "N/A")
D(S.Passive, "SelectedPassive", { Text = "Select Weapon (s)", Values = Tables.AllOwnedWeapons, Multi = true })
D(S.Passive, "SelectedSpec",    { Text = "Target Passives",   Values = Tables.SpecPassive,     Multi = true })
SL(S.Passive, "SpecRollCD",    { Text = "Roll Delay", Default = 0.1, Min = 0.01, Max = 1, Rounding = 2 })
T(S.Passive, "AutoSpec",       { Text = "Auto Reroll Passive", Default = false })

L(S.PassCfg, "⚠️ Advanced users only.\nWill stop once passive reaches min value.")

-- ShortMap + UpdatePassiveSliders
local ShortMap = {
...
local function UpdatePassiveSliders()
    ...
    for _, slider in pairs(Shared.SpecStatsSlider) do pcall(function() slider:SetVisible(false) end) end
        ...
        if isSelected then
            ...
            if data and data.Buffs then
                for statKey, range in pairs(data.Buffs) do
                    ...
                    local label = string.format("%s [%s]", Shorten(passiveName), Shorten(statKey))
                    if not Options[sliderId] then
                        SL(S.PassCfg, sliderId, { Text = label, Default = range[1], Min = range[1], Max = range[2], Rounding = 1 })
                        Shared.SpecStatsSlider[sliderId] = Options[sliderId]
                    else
                        Options[sliderId]:SetText(label)
                        if Shared.SpecStatsSlider[sliderId] then
                            pcall(function() Shared.SpecStatsSlider[sliderId]:SetVisible(true) end)
                        end
                    end
                end
            end
...
-- ARTIFACT TAB
-- ============================================================
ArtifactLabel   = L(S.ArtStatus, "Status: N/A")
DustLabel       = L(S.ArtStatus, "Dust: N/A")
InvLabel_Helmet = L(S.ArtStatus, "Helmet: 0/500")
InvLabel_Gloves = L(S.ArtStatus, "Gloves: 0/500")
InvLabel_Body   = L(S.ArtStatus, "Body: 0/500")
InvLabel_Boots  = L(S.ArtStatus, "Boots: 0/500")

D(S.ArtEquip, "Eq_Type", { Text = "Artifact Type",    Values = {}, Multi = true })
D(S.ArtEquip, "Eq_MS",   { Text = "Main Stat Filter", Values = {}, Multi = true })
D(S.ArtEquip, "Eq_SS",   { Text = "Sub Stat Filter",  Values = {}, Multi = true })
T(S.ArtEquip, "ArtifactEquip", { Text = "Auto Equip", Default = false })

SL(S.ArtUpgrade, "UpgradeLimit",    { Text = "Upgrade Limit",    Default = 0, Min = 0, Max = 15, Rounding = 0 })
D(S.ArtUpgrade,  "Up_MS",           { Text = "Main Stat Filter", Values = {}, Multi = true })
T(S.ArtUpgrade,  "ArtifactUpgrade", { Text = "Auto Upgrade",      Default = false })
T(S.ArtUpgrade,  "UpgradeStage",    { Text = "Upgrade in Stages", Default = false })

D(S.ArtLock, "Lock_Type",    { Text = "Artifact Type",    Values = {}, Multi = true })
D(S.ArtLock, "Lock_Set",     { Text = "Artifact Set",     Values = {}, Multi = true })
D(S.ArtLock, "Lock_MS",      { Text = "Main Stat Filter", Values = {}, Multi = true })
D(S.ArtLock, "Lock_SS",      { Text = "Sub Stat Filter",  Values = {}, Multi = true })
SL(S.ArtLock, "Lock_MinSS",  { Text = "Min Sub-Stats",    Default = 0, Min = 0, Max = 4, Rounding = 0 })
T(S.ArtLock, "ArtifactLock", { Text = "Auto Lock",        Default = false })

D(S.ArtDelete, "Del_Type",       { Text = "Artifact Type",       Values = {},                           Multi = true })
D(S.ArtDelete, "Del_Set",        { Text = "Artifact Set",         Values = {},                           Multi = true })
D(S.ArtDelete, "Del_MS_Helmet",  { Text = "Main Stat [Helmet]",   Values = {"FlatDefense","Defense"},    Multi = true })
D(S.ArtDelete, "Del_MS_Gloves",  { Text = "Main Stat [Gloves]",   Values = {"Damage"},                   Multi = true })
D(S.ArtDelete, "Del_MS_Body",    { Text = "Main Stat [Body]",     Values = {},                           Multi = true })
D(S.ArtDelete, "Del_MS_Boots",   { Text = "Main Stat [Boots]",    Values = {},                           Multi = true })
D(S.ArtDelete, "Del_SS",         { Text = "Sub Stat Filter",       Values = {},                           Multi = true })
SL(S.ArtDelete, "Del_MinSS",     { Text = "Min Sub-Stats",         Default = 0, Min = 0, Max = 4, Rounding = 0 })
T(S.ArtDelete, "ArtifactDelete", { Text = "Auto Delete",            Default = false })
T(S.ArtDelete, "DeleteUnlock",   { Text = "Auto Delete Unlocked",   Default = false })
...
-- Infinite Tower (Sea 1 - Tower Island, no difficulty)
L(S.DungIT, "Auto join & farm Infinite Tower.")
T(S.DungIT, "AutoJoinPortal_IT",  { Text = "Auto Join Portal", Default = false })
T(S.DungIT, "AutoStartWave_IT",   { Text = "Auto Start Wave",  Default = false })
T(S.DungIT, "AutoReplay_IT",      { Text = "Auto Replay",      Default = false })
T(S.DungIT, "DungeonAutofarm_IT", { Text = "Autofarm",         Default = false })

-- Crystal Defense (Sea 2, no difficulty)
L(S.DungCD, "Auto join & farm Crystal Defense.")
T(S.DungCD, "AutoJoinPortal_CD",  { Text = "Auto Join Portal", Default = false })
T(S.DungCD, "AutoStartWave_CD",   { Text = "Auto Start Wave",  Default = false })
T(S.DungCD, "AutoReplay_CD",      { Text = "Auto Replay",      Default = false })
T(S.DungCD, "DungeonAutofarm_CD", { Text = "Autofarm",         Default = false })

-- Dungeon (CidDungeon, RuneDungeon, DoubleDungeon)
L(S.DungJoin, "Auto join & farm dungeon.")
D(S.DungJoin, "SelectedDungeonPortal", { Text = "Select Dungeon", Values = {"Cid Dungeon","Rune Dungeon","Double Dungeon"}, Default = "Cid Dungeon", Multi = false })
T(S.DungJoin, "AutoJoinPortal_Dg", { Text = "Auto Join Portal",    Default = false })
T(S.DungJoin, "AutoVoteDiff_Dg",   { Text = "Auto Vote Difficulty", Default = false })
D(S.DungJoin, "SelectedDiff",      { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Medium", Multi = false })
T(S.DungJoin, "AutoReplay_Dg",     { Text = "Auto Replay",         Default = false })
T(S.DungJoin, "AutoDungeon",       { Text = "Autofarm",            Default = false })
DungeonCount = L(S.DungJoin, "Dungeon Completed: N/A")

-- Boss Rush (Sea 1 - Sailor Island, ada difficulty)
L(S.DungBR, "Auto join & Farm Boss Rush.")
T(S.DungBR, "AutoJoinPortal_BR",  { Text = "Auto Join Portal",    Default = false })
T(S.DungBR, "AutoVoteDiff_BR",    { Text = "Auto Vote Difficulty", Default = false })
D(S.DungBR, "SelectedBossRushDiff", { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Medium", Multi = false })
T(S.DungBR, "AutoReplay_BR",      { Text = "Auto Replay",  Default = false })
T(S.DungBR, "AutoBossRush",       { Text = "Autofarm",     Default = false })

-- Raid (Sea 2, ada difficulty)
L(S.DungRaid, "Auto join & farm Raid.")
T(S.DungRaid, "AutoJoinPortal_Raid", { Text = "Auto Join Portal",    Default = false })
T(S.DungRaid, "AutoVoteDiff_Raid",   { Text = "Auto Vote Difficulty", Default = false })
D(S.DungRaid, "SelectedRaidDiff",    { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Default = "Medium", Multi = false })
T(S.DungRaid, "AutoReplay_Raid",     { Text = "Auto Replay", Default = false })
T(S.DungRaid, "AutoRaid",            { Text = "Autofarm",    Default = false })

-- Config (mengikuti setting farm utama + tambahan dungeon)
L(S.DungCfg, "Farm config mengikuti Farm Config di tab Main.\nBerikut hanya override khusus dungeon.")
SL(S.DungCfg, "DungTP_CD",  { Text = "TP Cooldown after join", Default = 1.2, Min = 0.5, Max = 5, Rounding = 1 })

-- ============================================================
-- PLAYER TAB
-- ============================================================
AST({ Group = S.PlGeneral, Id = "WS",       Text = "WalkSpeed",     Default = 16,  Min = 16,  Max = 250  })
AST({ Group = S.PlGeneral, Id = "TPW",      Text = "TPWalk",        Default = 1,   Min = 1,   Max = 10,  Rounding = 1 })
AST({ Group = S.PlGeneral, Id = "JP",       Text = "JumpPower",     Default = 50,  Min = 0,   Max = 500  })
AST({ Group = S.PlGeneral, Id = "HH",       Text = "HipHeight",     Default = 2,   Min = 0,   Max = 10,  Rounding = 1 })
T(S.PlGeneral, "Noclip",          { Text = "Noclip"              })
T(S.PlGeneral, "AntiKnockback",   { Text = "Anti Knockback",   Default = false })
T(S.PlGeneral, "Disable3DRender", { Text = "Disable 3D Rendering" })
AST({ Group = S.PlGeneral, Id = "Grav",     Text = "Gravity",       Default = 196, Min = 0,   Max = 500, Rounding = 1 })
AST({ Group = S.PlGeneral, Id = "Zoom",     Text = "Camera Zoom",   Default = 128, Min = 128, Max = 10000 })
AST({ Group = S.PlGeneral, Id = "FOV",      Text = "Field of View", Default = 70,  Min = 30,  Max = 120  })
AST({ Group = S.PlGeneral, Id = "LimitFPS", Text = "Set Max FPS",   Default = 60,  Min = 5,   Max = 360, Disabled = not Support.FPS })
T(S.PlGeneral, "FPSBoost",    { Text = "FPS Boost"            })
T(S.PlGeneral, "FPSBoost_AF", { Text = "FPS Boost [Autofarm]" })

T(S.PlServer, "AntiAFK",          { Text = "Anti AFK",         Default = true, Disabled = not Support.Connections })
T(S.PlServer, "AntiKick",         { Text = "Anti Kick (Client)" })
T(S.PlServer, "AutoReconnect",    { Text = "Auto Reconnect"     })
T(S.PlServer, "NoGameplayPaused", { Text = "No Gameplay Paused", Default = true })
B(S.PlServer, "Serverhop", function()
    game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
end)
B(S.PlServer, "Rejoin", function() Services.TeleportService:Teleport(game.PlaceId, Plr) end)
T(S.PlServer, "AutoServerhop", { Text = "Auto Serverhop" })
SL(S.PlServer, "AutoHopMins",  { Text = "Minutes", Default = 30, Min = 0, Max = 300 })

T(S.PlGame, "InstantPP",  { Text = "Instant Prompt" })
T(S.PlGame, "Fullbright", { Text = "Fullbright"     })
T(S.PlGame, "NoFog",      { Text = "No Fog"         })
AST({ Group = S.PlGame, Id = "OverrideTime", Text = "Time Of Day", Default = 12, Min = 0, Max = 24, Rounding = 1 })

-- Safety: Panic keybind tidak ada di Brigida natively, pakai AddKeybind
S.PlSafety:AddKeybind({ Title = "Panic", Value = "P",
    Callback = function() PanicStop() end })
T(S.PlSafety, "AutoKick", { Text = "Auto Kick", Default = true })
D(S.PlSafety, "SelectedKickType", {
...
B(S.TpQuest, "Teleport to Level Based Quest", function()
...
D(S.TpMisc, "SelectedMiscAllNPC", {
    Text = "Select NPC [All NPCs]", Values = Tables.AllNPCList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

D(S.TpMoveset, "SelectedMovesetNPC", {
    Text = "Select NPC", Values = Tables.NPC_MovesetList, Multi = false,
    Callback = function(a) if a then SafeTeleportToNPC(tostring(a)) end end,
})

D(S.TpMastery, "SelectedMasteryNPC", {
...
-- MISC TAB
-- ============================================================
MerchantTimerLabel = L(S.MerchReg, "Refresh: N/A")
D(S.MerchReg, "SelectedMerchantItems", { Text = "Select Item (s)", Values = Tables.MerchantList, Multi = true })
T(S.MerchReg, "AutoMerchant",          { Text = "Auto Buy Selected Items", Default = false })

L(S.MerchDung, "Dungeon Merchant")
D(S.MerchDung, "SelectedDungeonMerchantItems", { Text = "Select Item (s)", Values = Tables.DungeonMerchantList or {}, Multi = true })
T(S.MerchDung, "AutoDungeonMerchant",          { Text = "Auto Buy Dungeon Items", Default = false })

L(S.MerchVal, "Able to cause performance issues.")
ValentineLabel = L(S.MerchVal, "Hearts: N/A")
D(S.MerchVal, "SelectedValentineMerchantItems", { Text = "Select Item (s)", Values = Tables.ValentineMerchantList, Multi = true })
ValentinePriceLabel = L(S.MerchVal, "Price: N/A")
T(S.MerchVal, "AutoValentineMerchant", { Text = "Auto Buy Selected Items", Default = false })

L(S.MerchTwr, "Tower Merchant")
D(S.MerchTwr, "SelectedTowerMerchantItems", { Text = "Select Item (s)", Values = Tables.InfiniteTowerMerchantList or {}, Multi = true })
T(S.MerchTwr, "AutoTowerMerchant",          { Text = "Auto Buy Tower Items", Default = false })

L(S.MerchBR, "Boss Rush Merchant")
D(S.MerchBR, "SelectedBossRushMerchantItems", { Text = "Select Item (s)", Values = Tables.BossRushMerchantList or {}, Multi = true })
T(S.MerchBR, "AutoBossRushMerchant",          { Text = "Auto Buy Boss Rush Items", Default = false })

D(S.Chests, "SelectedChests", { Text = "Select Chest (s)", Values = Tables.Rarities, Multi = true })
T(S.Chests, "AutoChest",      { Text = "Auto Open Chest",  Default = false })

D(S.Craft, "SelectedCraftItems", { Text = "Select Item (s) To Craft", Values = Tables.CraftItemList, Multi = true })
T(S.Craft, "AutoCraftItem",      { Text = "Auto Craft Item",          Default = false })

T(S.Notify, "AutoDeleteNotif", { Text = "Auto Hide Notification", Default = false })

B(S.Puzzles, { Text = "Complete Dungeon Puzzle",   Disabled = not Support.Proximity, Func = function()
...
B(S.Puzzles, { Text = "Complete Slime Key Puzzle", Disabled = not Support.Proximity, Func = function() UniversalPuzzleSolver("Slime") end })
B(S.Puzzles, { Text = "Complete Valentine Puzzle", Disabled = not Support.Proximity, Func = function()
...
B(S.Puzzles, { Text = "Complete Demonite Puzzle",  Disabled = not Support.Proximity, Func = function() UniversalPuzzleSolver("Demonite") end })
B(S.Puzzles, { Text = "Complete Hogyoku Puzzle",   Disabled = not Support.Proximity, Func = function()
    local lv = Plr.Data.Level.Value
    if lv >= 8500 then UniversalPuzzleSolver("Hogyoku")
    else Library:Notify("Level 8500 required! Current: "..lv, 4) end
end })

L(S.Questline, "⚠️ Experimental. Store race & clan before using.\nDungeon tasks only make you join dungeon.")
D(S.Questline, "SelectedQuestline",          { Text = "Select Questline",      Values = Tables.QuestlineList, Multi = false })
D(S.Questline, "SelectedQuestline_Player",   { Text = "Select Player",         Values = {}, Multi = false })
D(S.Questline, "SelectedQuestline_DMGTaken", { Text = "Select Mob [Take Damage]", Values = Tables.AllEntitiesList, Multi = false })
B(S.Questline, "Refresh", function()
    UpdateAllEntities()
    Options.SelectedQuestline_DMGTaken:SetValues(Tables.AllEntitiesList)
end)
T(S.Questline, "AutoQuestline", { Text = "Auto Questline [BETA]", Default = false })

-- ============================================================
-- CONFIG TAB
-- ============================================================
T(S.Menu, "AutoShowUI",       { Text = "Auto Show UI", Default = true })
T(S.Menu, "ShowCustomCursor", { Text = "Custom Cursor", Default = false, Callback = function(v) Library.ShowCustomCursor = v end })
D(S.Menu, "NotificationSide", { Text = "Notification Side", Values = {"Left","Right"}, Default = "Right",
    Callback = function(v) Library:SetNotifySide(v) end })
D(S.Menu, "DPIDropdown", { Text = "DPI Scale",
    Values = {"50%","75%","100%","125%","150%","175%","200%"}, Default = "100%",
    Callback = function(v) Library:SetDPIScale(tonumber(v:gsub("%%",""))) end })
S.Menu:AddKeybind({ Title = "Menu Keybind", Value = "U", Callback = function() Library:Toggle() end })
Options.MenuKeybind = Options.MenuKeybind or { Value = "U" }
B(S.Menu, "Unload", function()
    getgenv().Gennskie_Running = false
    Shared.Farm = false
    Cleanup(Connections)
    Cleanup(Flags)
    Library:Unload()
...
-- ============================================================
-- ============================================================
-- HELPER: Autofarm NPCs dari workspace (sama untuk semua dungeon)
-- ============================================================
-- ============================================================
...
-- Vote difficulty (dengan visible check + cooldown)
local function VoteDifficulty(difficulty, enabled)
    if not enabled then return false end
    if not isVoteFrameVisible() then return false end
    if tick() - _lastAction < 2 then return false end
    local dungeonUI = PGui:FindFirstChild("DungeonUI")
    if not dungeonUI then return false end
    local btnName = difficulty .. "DifficultyButton"
    for _, desc in ipairs(dungeonUI:GetDescendants()) do
        if desc.Name == btnName and desc.Visible then
            SafeClick(desc)
            _lastAction = tick()
            return true
        end
...
-- Start wave (visible check + cooldown)
local function StartWave(enabled)
    if not enabled then return false end
    if tick() - _lastAction < 3 then return false end
    local dungeonUI = PGui:FindFirstChild("DungeonUI")
    if dungeonUI then
        local content = dungeonUI:FindFirstChild("ContentFrame", true)
        local btn = content and content:FindFirstChild("StartButton", true)
...
-- Auto replay (visible check + cooldown)
local function ReplayDungeon(enabled)
    if not enabled then return false end
    if tick() - _lastAction < 3 then return false end
    local dungeonUI = PGui:FindFirstChild("DungeonUI")
    if dungeonUI then
        local replayFrame = dungeonUI:FindFirstChild("ReplayDungeonFrameVisibleOnlyWhenClearingDungeon", true)
        if replayFrame and replayFrame.Visible then
            local btn = replayFrame:FindFirstChild("ReplayDungeonButton", true)
            if btn and btn.Visible then
                SafeClick(btn)
                _lastAction = tick()
                return true
...
-- Join portal (cek tidak sedang di dungeon)
local function JoinPortal(dungeonType, enabled)
    if not enabled then return false end
    local dungeonUI = PGui:FindFirstChild("DungeonUI")
    local inDungeon = dungeonUI
        and dungeonUI:FindFirstChild("LeaveButton", true)
        and dungeonUI:FindFirstChild("LeaveButton", true).Visible
    if inDungeon then return false end
    pcall(function()
        local remote = RS:FindFirstChild("Remotes") and RS.Remotes:FindFirstChild("RequestDungeonPortal")
        if remote then remote:FireServer(dungeonType) end
    end)
    task.wait(1.2)
    local portalUI = PGui:FindFirstChild("DungeonPortalActionUI")
    if portalUI then
        local btn = portalUI:FindFirstChild("StartButton", true)
        if btn and btn.Visible then SafeClick(btn); return true end
    end
    return false
end

-- Autofarm NPCs di workspace (mengikuti setting dari Farm Config di Main Tab)
-- Autofarm NPCs di workspace (mengikuti setting dari Farm Config di Main Tab)
local function DungeonAutofarm()
    -- Cek apapun toggle DungeonAutofarm_* yang aktif
    local isActive = false
    local dungeonAutofarmToggles = {"DungeonAutofarm_IT", "DungeonAutofarm_CD", "DungeonAutofarm_Dg", "DungeonAutofarm_BR", "DungeonAutofarm_Raid"}
    for _, toggleName in ipairs(dungeonAutofarmToggles) do
    ...
    -- ============================================================
    -- PAKAI SETTING DARI FARM CONFIG (MAIN TAB)
    -- ============================================================
    local distance = tonumber(Options.Distance and Options.Distance.Value) or 12
    local movementType = Options.SelectedMovementType and Options.SelectedMovementType.Value or "Teleport"
    local tweenSpeed = tonumber(Options.TweenSpeed and Options.TweenSpeed.Value) or 160
    local farmType = Options.SelectedFarmType and Options.SelectedFarmType.Value or "Behind"
    ...
    -- Farm Type (Behind, Above, Below)
    if farmType == "Above" then
        finalPos = tPos + Vector3.new(0, distance, 0)
        finalDestination = CFrame.new(finalPos, finalPos + Vector3.new(0, -1, 0))
    elseif farmType == "Below" then
        finalPos = tPos + Vector3.new(0, -distance, 0)
        finalDestination = CFrame.new(finalPos, finalPos + Vector3.new(0, 1, 0))
    else -- "Behind"
        finalPos = tPos - direction * distance
        finalDestination = CFrame.lookAt(finalPos, tPos)
        -- Cegah terlalu dekat (cuma untuk Behind)
        if (finalPos - tPos).Magnitude < 3 then
            finalPos = tPos - direction * 5
            finalDestination = CFrame.lookAt(finalPos, tPos)
        end
    end
    ...
    if movementType == "Teleport" then
        pcall(function() root.CFrame = finalDestination end)
    else
        local distToMove = (myPos - finalPos).Magnitude
        if distToMove > 0.5 then
            local duration = distToMove / tweenSpeed
            local tween = game:GetService("TweenService"):Create(
                root, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = finalDestination}
            )
            tween:Play()
            task.wait(duration)
        end
    ...
    -- Attack
    local m1Delay = tonumber(Options.M1Speed and Options.M1Speed.Value) or 0.1
    if tick() - (Shared.LastM1 or 0) >= m1Delay then
        if Toggles.SwitchWeapon and Toggles.SwitchWeapon.Value then
            EquipWeapon()
        end
...
-- ============================================================
-- DUNGEON LOOP (INDEPENDEN - TIDAK BERGANTUNG TOGGLE ENABLE)
-- ============================================================
task.spawn(function()
    while getgenv().Gennskie_Running do
        task.wait(0.5)
        
        -- ========== UPDATE SHARED.TARGET UNTUK AUTO SKILL & KILL AURA ==========
        -- Cari target terdekat untuk auto skill & kill aura
        local function getNearestDungeonTarget()
            ...
            local best, bestDist = nil, 200 -- range kill aura
            for _, obj in ipairs(npcFolder:GetDescendants()) do
                if obj:IsA("Model") then
                    local hum = obj:FindFirstChildOfClass("Humanoid")
        ...
        -- Isi Shared.Target buat Auto Skill & Kill Aura (selama di dungeon)
        local inDungeon = PGui:FindFirstChild("DungeonUI") 
            and PGui.DungeonUI:FindFirstChild("LeaveButton", true) 
            and PGui.DungeonUI:FindFirstChild("LeaveButton", true).Visible
        
        if inDungeon then
            Shared.Target = getNearestDungeonTarget()
        else
            Shared.Target = nil
        end

        -- INFINITE TOWER
        if Toggles.AutoJoinPortal_IT and Toggles.AutoJoinPortal_IT.Value or
           Toggles.AutoStartWave_IT and Toggles.AutoStartWave_IT.Value or
           Toggles.AutoReplay_IT and Toggles.AutoReplay_IT.Value or
           Toggles.DungeonAutofarm_IT and Toggles.DungeonAutofarm_IT.Value then
            
            JoinPortal("InfiniteTower", Toggles.AutoJoinPortal_IT and Toggles.AutoJoinPortal_IT.Value)
            StartWave(Toggles.AutoStartWave_IT and Toggles.AutoStartWave_IT.Value)
            ReplayDungeon(Toggles.AutoReplay_IT and Toggles.AutoReplay_IT.Value)
            if Toggles.DungeonAutofarm_IT and Toggles.DungeonAutofarm_IT.Value then 
                DungeonAutofarm() 
            end
        end

        -- CRYSTAL DEFENSE
        if Toggles.AutoJoinPortal_CD and Toggles.AutoJoinPortal_CD.Value or
           Toggles.AutoStartWave_CD and Toggles.AutoStartWave_CD.Value or
           Toggles.AutoReplay_CD and Toggles.AutoReplay_CD.Value or
           Toggles.DungeonAutofarm_CD and Toggles.DungeonAutofarm_CD.Value then
            
            JoinPortal("CrystalDefense", Toggles.AutoJoinPortal_CD and Toggles.AutoJoinPortal_CD.Value)
            StartWave(Toggles.AutoStartWave_CD and Toggles.AutoStartWave_CD.Value)
            ReplayDungeon(Toggles.AutoReplay_CD and Toggles.AutoReplay_CD.Value)
            if Toggles.DungeonAutofarm_CD and Toggles.DungeonAutofarm_CD.Value then 
                DungeonAutofarm() 
            end
        end

        -- DUNGEON (Cid / Rune / Double)
        if Toggles.AutoJoinPortal_Dg and Toggles.AutoJoinPortal_Dg.Value or
           Toggles.AutoVoteDiff_Dg and Toggles.AutoVoteDiff_Dg.Value or
           Toggles.AutoReplay_Dg and Toggles.AutoReplay_Dg.Value or
           Toggles.DungeonAutofarm_Dg and Toggles.DungeonAutofarm_Dg.Value then
            
            local dungMap = {
                ["Cid Dungeon"]    = "CidDungeon",
                ["Rune Dungeon"]   = "RuneDungeon",
                ["Double Dungeon"] = "DoubleDungeon",
            }
            local portalName = dungMap[Options.SelectedDungeonPortal and Options.SelectedDungeonPortal.Value or ""] or "CidDungeon"
            
            JoinPortal(portalName, Toggles.AutoJoinPortal_Dg and Toggles.AutoJoinPortal_Dg.Value)
            VoteDifficulty(Options.SelectedDiff and Options.SelectedDiff.Value or "Medium", Toggles.AutoVoteDiff_Dg and Toggles.AutoVoteDiff_Dg.Value)
            ReplayDungeon(Toggles.AutoReplay_Dg and Toggles.AutoReplay_Dg.Value)
            if Toggles.DungeonAutofarm_Dg and Toggles.DungeonAutofarm_Dg.Value then 
                DungeonAutofarm() 
            end
        end

        -- BOSS RUSH
        if Toggles.AutoJoinPortal_BR and Toggles.AutoJoinPortal_BR.Value or
           Toggles.AutoVoteDiff_BR and Toggles.AutoVoteDiff_BR.Value or
           Toggles.AutoReplay_BR and Toggles.AutoReplay_BR.Value or
           Toggles.DungeonAutofarm_BR and Toggles.DungeonAutofarm_BR.Value then
            
            JoinPortal("BossRush", Toggles.AutoJoinPortal_BR and Toggles.AutoJoinPortal_BR.Value)
            VoteDifficulty(Options.SelectedBossRushDiff and Options.SelectedBossRushDiff.Value or "Medium", Toggles.AutoVoteDiff_BR and Toggles.AutoVoteDiff_BR.Value)
            ReplayDungeon(Toggles.AutoReplay_BR and Toggles.AutoReplay_BR.Value)
            if Toggles.DungeonAutofarm_BR and Toggles.DungeonAutofarm_BR.Value then 
                DungeonAutofarm() 
            end
        end

        -- RAID
        if Toggles.AutoJoinPortal_Raid and Toggles.AutoJoinPortal_Raid.Value or
           Toggles.AutoVoteDiff_Raid and Toggles.AutoVoteDiff_Raid.Value or
           Toggles.AutoReplay_Raid and Toggles.AutoReplay_Raid.Value or
           Toggles.DungeonAutofarm_Raid and Toggles.DungeonAutofarm_Raid.Value then
            
            JoinPortal("Raid", Toggles.AutoJoinPortal_Raid and Toggles.AutoJoinPortal_Raid.Value)
            VoteDifficulty(Options.SelectedRaidDiff and Options.SelectedRaidDiff.Value or "Medium", Toggles.AutoVoteDiff_Raid and Toggles.AutoVoteDiff_Raid.Value)
            ReplayDungeon(Toggles.AutoReplay_Raid and Toggles.AutoReplay_Raid.Value)
            if Toggles.DungeonAutofarm_Raid and Toggles.DungeonAutofarm_Raid.Value then 
                DungeonAutofarm() 
            end
        end
    end
...
-- OnChanged CALLBACKS (game logic original - tidak diubah)
-- ============================================================
...
-- Dungeon: dihandle oleh unified loop (task.spawn di atas), tidak perlu Thread() terpisah
...
Toggles.TPW:OnChanged(function(v)
    Options.TPWValue:SetVisible(Toggles.TPW.Value)
...
Options.LimitFPSValue:OnChanged(function()
    if Toggles.LimitFPS.Value then setfpscap(Options.LimitFPSValue.Value) end
end)
Toggles.LimitFPS:OnChanged(function(v)
    Options.LimitFPSValue:SetVisible(Toggles.LimitFPS.Value)
...
-- Main farm loop & attack loop (dari game logic asli - tidak diubah)
task.spawn(function()
    while true do
        task.wait()
        if Shared.AltActive then continue end
...
task.spawn(function()
    while task.wait() do
        ...
        if isPityReady then
            ...
            if t then foundTask=true; Shared.Target=t; Shared.TargetValid=true; UpdateSwitchState(t,fType); ExecuteFarmLogic(t,isl,fType) end
        end
        if not foundTask then
            for i = 1, #PriorityTasks do
                ...
                if t then
                    foundTask=true; Shared.Target=(typeof(t)=="Instance") and t or nil
                    Shared.TargetValid=true; UpdateSwitchState(t,fType)
...
elseif UIS.KeyboardEnabled then
    ...
    Library:Notify("Device: PC - Press " .. (Options.MenuKeybind and Options.MenuKeybind.Value or "U") .. " to show/hide UI.", 10)
end
...
VelarisUI:MakeNotify({ Title = "Victoria Hub", Description = "Script loaded!", Color = "Success", Time = 3 })
...
if not eh_success then
    ...
    pcall(function()
        if VelarisUI then
            VelarisUI:MakeNotify({ Title = "Victoria Hub", Description = "ERROR: "..tostring(err), Color = "Danger", Time = 6 })
        end
    end)
end
