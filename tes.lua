...
local function StopAutoGen()
    -- Matiin flag loop gen dulu (loop generator cek Config.AUTO_Generator)
    ...
    pcall(function()
        ...
        -- Reset state humanoid agar bisa gerak normal lagi
...
local function LeaveGenerator()
    -- Cari gen terdekat lalu push humanoid menjauhi gen via WalkDirection
    -- Tidak TP, agar tidak nyangkut
    local char = LocalPlayer.Character
    if not char then return false end
    local root = char:FindFirstChild("HumanoidRootPart")
    local humanoid = char:FindFirstChildOfClass("Humanoid")
    ...
    if not nearestGen then return false end

    -- Arah lari: menjauhi gen
    ...
    -- Paksa humanoid move ke arah itu sebentar lalu stop
...
-- Auto Parry: hook BasicAttack OnClientEvent, fire parry remote saat diserang
local LastParryTime = 0
local ParryAttackConn = nil

local function AutoParry_DoParry()
    ...
    pcall(function()
        ...
        local parryRemote = dagger:FindFirstChild("parry")
        if parryRemote then
...
local function AutoParry_Setup()
    ...
    pcall(function()
        ...
        local basicAttack = attacks:FindFirstChild("BasicAttack")
        if not basicAttack then return end
        -- Saat server kasih tau kita kena serangan, langsung parry
        ParryAttackConn = basicAttack.OnClientEvent:Connect(function()
            AutoParry_DoParry()
        end)
    end)
end
...
-- Cek apakah needle sudah masuk sweet-spot goal
-- Offset 104–114 derajat adalah zona sukses berdasarkan contoh user
local function QTE_NeedleInGoal()
    local lr = QTEHandler.Line.Rotation % 360
    local gr = QTEHandler.Goal.Rotation % 360
    local gs = (gr + 104) % 360
    local ge = (gr + 114) % 360
    if gs > ge then
...
local function QTE_StopMonitoring()
    if QTEHandler.HeartbeatConn then
        pcall(function() QTEHandler.HeartbeatConn:Disconnect() end)
        QTEHandler.HeartbeatConn = nil
    end
end

local function QTE_OnHeartbeat()
    ...
    if not QTEHandler.Line or not QTEHandler.Goal then
        QTE_StopMonitoring(); return
    end
    if QTE_NeedleInGoal() then
        QTE_PressSpace()
        QTE_StopMonitoring()
    end
end

local function QTE_OnVisChanged()
    if not Config.AUTO_SkillCheck then return end
    if GetRole() ~= "Survivor" then return end
    if QTEHandler.Check and QTEHandler.Check.Visible then
        if not QTEHandler.HeartbeatConn then
            QTEHandler.HeartbeatConn = RunService.Heartbeat:Connect(QTE_OnHeartbeat)
        end
    else
        QTE_StopMonitoring()
    end
...
local function SetupSkillCheckMonitor()
    -- Disconnect lama dulu
    QTE_StopMonitoring()
    if QTEHandler.VisConn then
        pcall(function() QTEHandler.VisConn:Disconnect() end)
        QTEHandler.VisConn = nil
    end

    -- Coba WaitForChild dengan timeout; kalau GUI belum ada saat Init(), retry lewat task.spawn
    task.spawn(function()
        pcall(function()
            local pg    = LocalPlayer:WaitForChild("PlayerGui", 10)
            if not pg then return end
            local gui   = pg:WaitForChild("SkillCheckPromptGui", 15)
            ...
            QTEHandler.Check = check
            QTEHandler.Line  = line
            QTEHandler.Goal  = goal

            -- Hook visibility change
            QTEHandler.VisConn = check:GetPropertyChangedSignal("Visible"):Connect(QTE_OnVisChanged)

            -- Kalau sudah visible saat setup (edge case), langsung mulai
...
local function Init()
    ...
    -- Setup parry hook di awal (akan aktif saat Config.AUTO_Parry = true)
    task.spawn(function()
        task.wait(2) -- tunggu remotes ready
        AutoParry_Setup()
    end)
