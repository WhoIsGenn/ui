...
local function TriggerMobileButton(PG)
    ...
    if b and b:IsA("GuiObject") then
        -- Method 1: firesignal Down+Up (paling cepat)
        pcall(function()
            firesignal(b.MouseButton1Down)
            firesignal(b.MouseButton1Up)
        end)
        -- Method 2: SendTouchEvent sebagai fallback
        pcall(function()
            local p2, s2, ins = b.AbsolutePosition, b.AbsoluteSize, GuiService:GetGuiInset()
            local cx, cy = p2.X + s2.X/2 + ins.X, p2.Y + s2.Y/2 + ins.Y
...
-- PARRY FOV drawings (world-space circle, ikut tesvide.lua: Y-3)
local PFOV_N      = 32
local PFOVOutline = {}
local PFOVGlow    = {}
for i = 1, PFOV_N do
    ...
    gl.Thickness = 6; gl.Color = Color3.fromRGB(80,160,255); gl.Transparency = 0.4; gl.Visible = false
    PFOVGlow[i] = gl
end

-- Highlight disk: Part dengan Transparency < 1 agar Highlight bekerja
local ParryDiskPart      = nil
local ParryDiskHighlight = nil

local function ParryDisk_GetOrCreate()
    if ParryDiskPart and ParryDiskPart.Parent then return ParryDiskPart end
    local d = Config.PARRY_Dist * 2
    ParryDiskPart              = Instance.new("Part")
    ParryDiskPart.Name         = "_VHParryDisk"
    ParryDiskPart.Anchored     = true
    ParryDiskPart.CanCollide   = false
    ParryDiskPart.CanTouch     = false
    -- PENTING: tidak boleh 1, Highlight butuh Part yang visible secara render
    ParryDiskPart.Transparency = 0.999
    ParryDiskPart.Shape        = Enum.PartType.Cylinder
    ParryDiskPart.Size         = Vector3.new(0.1, d, d)
    ParryDiskPart.Parent       = workspace

    ParryDiskHighlight                     = Instance.new("Highlight")
    ParryDiskHighlight.Name                = "_VHParryHL"
    ParryDiskHighlight.DepthMode           = Enum.HighlightDepthMode.AlwaysOnTop
    ParryDiskHighlight.Adornee             = ParryDiskPart
    ParryDiskHighlight.OutlineTransparency = 1      -- tidak ada outline dari Highlight itu sendiri
    ParryDiskHighlight.FillTransparency    = 0.55
    ParryDiskHighlight.Parent              = ParryDiskPart
    return ParryDiskPart
end
...
local function ParryDisk_Remove()
    ...
    for i = 1, PFOV_N do PFOVOutline[i].Visible = false; PFOVGlow[i].Visible = false end
end

-- Raycast ke bawah untuk dapat Y lantai yang akurat
local function GetFloorY(pos, charToIgnore)
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Blacklist
    params.FilterDescendantsInstances = charToIgnore and {charToIgnore} or {}
    local result = workspace:Raycast(pos, Vector3.new(0, -10, 0), params)
    if result then
        return result.Position.Y + 0.05  -- sedikit di atas lantai
    end
    return pos.Y - 3  -- fallback kalau tidak kena lantai
end

local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        ParryDisk_Remove(); return
    end
    local root = GetCharacterRoot()
    if not root then ParryDisk_Remove(); return end

    -- Dapat Y lantai yang akurat lewat raycast dari kaki
    local feetPos = root.Position - Vector3.new(0, 2.5, 0)
    local floorY  = GetFloorY(feetPos, LocalPlayer.Character)

    -- Update posisi disk
    local disk = ParryDisk_GetOrCreate()
    local d    = Config.PARRY_Dist * 2
    if disk.Size.Y ~= d then disk.Size = Vector3.new(0.1, d, d) end
    disk.CFrame = CFrame.new(root.Position.X, floorY, root.Position.Z)
               * CFrame.Angles(0, 0, math.rad(90))

    -- Update warna highlight
    local col = ParryOnCooldown and Color3.fromRGB(255,60,60) or Color3.fromRGB(80,160,255)
    if ParryDiskHighlight then
        ParryDiskHighlight.FillColor = col
    end

    -- World-space line segments di floorY (bukan hardcoded Y-3)
    local r  = Config.PARRY_Dist
    local ox = root.Position.X
    local oz = root.Position.Z
    local oc = ParryOnCooldown and Color3.fromRGB(255,80,80) or Color3.fromRGB(255,255,255)
    local gc = ParryOnCooldown and Color3.fromRGB(255,60,60) or Color3.fromRGB(80,160,255)
    for i = 1, PFOV_N do
        local a1 = (i-1)/PFOV_N * math.pi*2
        local a2 =  i   /PFOV_N * math.pi*2
        local s1, v1 = WorldToScreen(Vector3.new(ox+math.cos(a1)*r, floorY, oz+math.sin(a1)*r))
        local s2, v2 = WorldToScreen(Vector3.new(ox+math.cos(a2)*r, floorY, oz+math.sin(a2)*r))
        local vis = v1 and v2
        PFOVOutline[i].From=s1; PFOVOutline[i].To=s2; PFOVOutline[i].Color=oc; PFOVOutline[i].Visible=vis
        PFOVGlow[i].From=s1;    PFOVGlow[i].To=s2;    PFOVGlow[i].Color=gc;    PFOVGlow[i].Visible=vis
    end
end
...
local function Unload()
    ...
    for i = 1, PFOV_N do
        pcall(function() PFOVOutline[i]:Remove() end)
        pcall(function() PFOVGlow[i]:Remove() end)
    end
