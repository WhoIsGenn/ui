...
local function GetVeilRemote()
    ...
    local remotes = ReplicatedStorage:FindFirstChild("Remotes"); if not remotes then return nil end
    local killers = remotes:FindFirstChild("Killers"); if not killers then return nil end
    local veil = killers:FindFirstChild("Veil"); if not veil then return nil end
    ...
    if not fireRemote then
        for _, child in pairs(veil:GetChildren()) do
            if string.find(string.lower(child.Name), "fire") or string.find(string.lower(child.Name), "throw") then
                fireRemote = child; break
            end
        end
    end
    State.VeilRemote = fireRemote; return fireRemote
end
...
local function HookVeilRemote()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "FireServer" and self == remote and SilentAimVeil.Enabled then
            ...
            if target then
                local args = {...}; local myRoot = GetHRP(LocalPlayer.Character)
                if myRoot then
                    ...
                    if #args >= 1 then args[1] = dir; return oldNamecall(self, table.unpack(args)) end
                end
            end
        end
        return oldNamecall(self, ...)
...
local function startSkillCheck()
    ...
    Connections.SkillHeartbeat = RunService.RenderStepped:Connect(function()
        ...
        local startRange = (gr + 108) % 360; local endRange = (gr + 120) % 360
