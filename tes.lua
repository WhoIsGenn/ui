...
local function AutoParry_HookAttackRemote()
    if #KillerHitConns > 0 then return end
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    ...
    -- HealthChanged: reactive fallback
    local c1 = hum.HealthChanged:Connect(function(newHealth)
        ...
        if newHealth < LastHealth then
            ...
            task.delay(0.35, function() KillerAttackDetected = false end)
        end
...
-- Cek killer velocity approaching (dipanggil di Tick)
...
local function Godmode_Apply()
    ...
    -- Re-hook saat karakter ganti (map swap)
    ...
    GodmodeLoop = RunService.Heartbeat:Connect(function()
        ...
        -- Block hook progress
        pcall(function() c:SetAttribute("HookedProgress", 0) end)
        pcall(function() c:SetAttribute("HookProgressDepleting", false) end)
    end)
end
            ...
            local visConn = Check:GetPropertyChangedSignal("Visible"):Connect(function()
                ...
                if Check.Visible then
                    ...
                    hbConn = RunService.Heartbeat:Connect(function()
                        ...
                        if inGoal then
                            pcall(function()
                                -- Mobile: coba tap GUI button skill check
                                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                                local scGui = pg and pg:FindFirstChild("SkillCheckPromptGui")
                                local btn = scGui and scGui:FindFirstChild("Check")
                                    and scGui.Check:FindFirstChildWhichIsA("GuiButton", true)
                                if btn then
                                    -- Tap button langsung
...
-- ================================================================
-- GUN SILENT AIM (Twist of Fate)
-- hookmetamethod & hookfunction tidak work di Delta (block FireServer)
-- Approach: rotate camera CFrame ke arah target sesaat sebelum game
-- membaca direction bullet (game pakai LookVector camera sebagai dir)
-- ================================================================
local GunSilentAimConn = nil

local function GunSilentAim_GetTarget()
    local myChar = LocalPlayer.Character
    local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
...
local GunSilentAim_OrigCFrame = nil
local GunSilentAim_Snapping   = false

local function GunSilentAim_Setup()
    if GunSilentAimConn then return end
    -- Setiap frame: kalau toggle aktif, snap camera ke target
    -- Saat nembak game baca camera LookVector sebagai bullet direction
    GunSilentAimConn = RunService.RenderStepped:Connect(function()
        if not Config.GUN_SilentAim then return end
        local cam    = workspace.CurrentCamera
        local target = GunSilentAim_GetTarget()
        if not target or not cam then return end
        local myChar = LocalPlayer.Character
        local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
        if not myRoot then return end
        -- Snap camera CFrame agar LookVector mengarah ke target
        -- Posisi camera tidak berubah, hanya rotasi
        local camPos = cam.CFrame.Position
        local targetPos = target.Position + Vector3.new(0, 0.5, 0)
        local newCF = CFrame.new(camPos, targetPos)
        -- Simpan CFrame asli sebelum di-snap (untuk restore)
        GunSilentAim_OrigCFrame = cam.CFrame
        cam.CFrame = newCF
    end)
    print("[SilentAim] Camera snap aktif")
end

local function GunSilentAim_Stop()
    if GunSilentAimConn then
        GunSilentAimConn:Disconnect()
        GunSilentAimConn = nil
    end
    -- Restore camera
    if GunSilentAim_OrigCFrame then
        local cam = workspace.CurrentCamera
        if cam then cam.CFrame = GunSilentAim_OrigCFrame end
        GunSilentAim_OrigCFrame = nil
    end
end

local secGun = TabAIM:AddSection("Gun Aimbot (Twist of Fate)")
secGun:AddToggle({
    Title = "Gun Silent Aim (Twist of Fate)",
    Default = Config.GUN_SilentAim,
    Callback = function(v)
        Config.GUN_SilentAim = v
        if v then task.spawn(GunSilentAim_Setup) else GunSilentAim_Stop() end
        notifToggle("Gun Silent Aim", v)
    end
})
secGun:AddDropdown({
    Title = "Aim Part",
...
local function Init()
    ...
    task.spawn(function()
        ...
        -- Parry circle: lingkaran di bawah karakter, radius = PARRY_Dist
        ...
        parryCircle.Color        = Color3.fromRGB(255, 200, 0)
        parryCircle.Visible      = false
        GUI.Drawings.ParryCircle = parryCircle
        ...
        RunService.Heartbeat:Connect(function()
            ...
            -- Parry Circle: proyeksi lingkaran 3D di bawah karakter ke layar
            local parryCircle = GUI.Drawings.ParryCircle
            if parryCircle then
                local showParry = Config.AUTO_Parry
                if showParry then
                    local char = LocalPlayer.Character
                    local root = char and char:FindFirstChild("HumanoidRootPart")
                    if root and cam then
                        -- Project titik bawah karakter ke screen
                        local basePos = root.Position - Vector3.new(0, 3, 0)
                        local screenPos, onScreen = cam:WorldToViewportPoint(basePos)
                        -- Hitung radius screen dari PARRY_Dist (3D → 2D approximate)
                        -- Ambil titik di edge lingkaran, project ke screen
                        local edgePos3D = basePos + Vector3.new(Config.PARRY_Dist, 0, 0)
                        local edgeScreen, _ = cam:WorldToViewportPoint(edgePos3D)
                        local screenRadius = math.abs(edgeScreen.X - screenPos.X)
                        if onScreen and screenRadius > 5 then
                            parryCircle.Position = Vector2.new(screenPos.X, screenPos.Y)
