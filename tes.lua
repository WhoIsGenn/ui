...
-- Hook __namecall, intercept FireServer dari Fire remote
-- Ganti direction vector ke arah killer/target
-- ================================================================
local GunSilentAimHooked = false
local OriginalNamecall   = nil

local function GunSilentAim_GetTargetDir(selfRemote)
    -- Cari killer/target terdekat
    local myChar = LocalPlayer.Character
    local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player == LocalPlayer then continue end
        local isTarget = Config.GUN_SilentAim and (
            (Config.AIM_TargetRole == "Killer" and IsKiller(player)) or
            (Config.AIM_TargetRole == "Survivor" and not IsKiller(player))
        )
        if not isTarget then continue end
        if not player.Character then continue end
        local part = player.Character:FindFirstChild(Config.GUN_AimPart)
    ...
    -- Hitung unit direction vector dari gun origin ke target
...
local function GunSilentAim_Setup()
    if GunSilentAimHooked then return end
    local mt = getrawmetatable and getrawmetatable(game)
    if not mt then
        warn("[SilentAim] getrawmetatable not available")
        return
    end
    if not setreadonly then
        warn("[SilentAim] setreadonly not available")
        return
    end
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "FireServer" and Config.GUN_SilentAim then
            local args = {...}
            -- Format: Fire:FireServer("EmperorGun", dirX, dirY, dirZ)
            if type(args[1]) == "string" and args[1]:find("Gun") and type(args[2]) == "number" then
                local dir = GunSilentAim_GetTargetDir(self)
                if dir then
                    return OriginalNamecall(self, args[1], dir.X, dir.Y, dir.Z)
                end
            end
        end
    ...
    setreadonly(mt, true)
    GunSilentAimHooked = true
    print("[SilentAim] Hooked successfully")
end
...
secGun:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then task.spawn(GunSilentAim_Setup) end
...
local function Init()
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- (Skillcheck-gen / Skillcheck-player baru ada beberapa detik setelah spawn)
