-- [[ VICTORIA HUB FISH IT - FINAL VERSION ]] --
-- Version: 1.0.0
...
local rodOptions = {"Luck Rod", "Carbon Rod", "Grass Rod", "Ice Rod", "Lucky Rod"}
...
rodSection:AddButton({
    Title = "Buy Rod",
    Callback = function()
        if selectedRod and R[selectedRod] then
            pcall(function() 
                net["RF/PurchaseFishingRod"]:InvokeServer(R[selectedRod]) 
            end)
            notif("Purchased: " .. selectedRod, 3, Color3.fromRGB(0, 255, 0))
        end
    end
})
...
local miscSection = Tab7:AddSection("Miscellaneous", false)
...
-- ==================== FORCE CLOSE ALL SECTIONS ====================
-- Function to close all sections
local function closeAllSections()
    -- Wait a bit for UI to load
    task.wait(1)
    
    -- Try to find and close all section buttons
    pcall(function()
        -- This is a hacky way, but VictUI doesn't expose section objects
        for _, v in pairs(game:GetService("CoreGui"):GetDescendants()) do
            if v:IsA("TextButton") and (v.Text == "▼" or v.Text == "▲") then
                -- If section is open (showing ▼), click to close it
                if v.Text == "▼" then
