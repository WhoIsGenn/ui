...
local Config = {
    ...
    PARRY_Dist      = 15,
    PARRY_Mode      = "Animation",   -- "Animation" | "BasicAttack" | "Distance"
    PARRY_Delay     = 0.0,           -- detik delay setelah deteksi sebelum fire
    GUN_SilentAim   = false,
    GUN_AimPart     = "HumanoidRootPart",
    ANTI_FailGen    = false,   -- disable Skillcheck-gen script di karakter
    SURV_NoFall = false,
...
-- ================================================================
-- AUTO PARRY - Event-based multi-mode detection
-- Mode "Animation" : detect attack animation dari killer (paling akurat)
-- Mode "BasicAttack": hook OnClientEvent dari BasicAttack remote
-- Mode "Distance"  : polling distance, fire saat killer dekat (fallback)
-- ================================================================
local LastParryTime    = 0
local ParryRemote      = nil
local ParryConns       = {}  -- semua connections parry

local ATTACK_ANIMS = {
    -- ID animasi attack umum di VD, tambah kalau ketemu lebih
    "rbxassetid://", -- placeholder, di-scan saat runtime
}
...
local function AutoParry_Fire()
    ...
    -- Cek killer masih dalam range sebelum fire
    local root = GetCharacterRoot()
    if not root then return end
    for _, p in ipairs(Players:GetPlayers()) do
...
local function AutoParry_DisconnectAll()
    for _, c in ipairs(ParryConns) do
        pcall(function() c:Disconnect() end)
    end
    ParryConns = {}
end

-- Mode: Animation - hook AnimationTrack.Playing di setiap killer
local function AutoParry_HookKillerAnim(killerChar)
    local animator = killerChar:FindFirstChildOfClass("Animator")
                  or (killerChar:FindFirstChild("Humanoid") and
                      killerChar:FindFirstChild("Humanoid"):FindFirstChildOfClass("Animator"))
    if not animator then return end

    -- Hook setiap AnimationTrack yang play
    local c = animator.AnimationPlayed:Connect(function(track)
        if not Config.AUTO_Parry then return end
        if Config.PARRY_Mode ~= "Animation" then return end
        -- Cek apakah ini attack animation
        -- Heuristik: nama track atau speed tinggi = attack
        local name = track.Name:lower()
        local isAttack = name:find("attack") or name:find("swing")
                      or name:find("hit") or name:find("lunge")
                      or name:find("strike") or name:find("slash")
        if isAttack then
            -- Fire sedikit delay agar timing pas
            task.delay(Config.PARRY_Delay, AutoParry_Fire)
        end
    end)
    table.insert(ParryConns, c)
end

-- Mode: BasicAttack event - hook OnClientEvent
local function AutoParry_HookBasicAttack()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    -- OnClientEvent dikirim ke semua client saat ada serangan
    local ok, c = pcall(function()
        return ba.OnClientEvent:Connect(function(...)
            if not Config.AUTO_Parry then return end
            if Config.PARRY_Mode ~= "BasicAttack" then return end
            task.delay(Config.PARRY_Delay, AutoParry_Fire)
        end)
    end)
    if ok and c then table.insert(ParryConns, c) end
end

-- Setup semua hooks berdasarkan mode
local function AutoParry_Setup()
    AutoParry_DisconnectAll()
    ParryRemote = nil

    if Config.PARRY_Mode == "Animation" then
        -- Hook semua killer yang sudah ada + yang baru join
        for _, p in ipairs(Players:GetPlayers()) do
            if IsKiller(p) and p.Character then
                AutoParry_HookKillerAnim(p.Character)
            end
        end
        local c1 = Players.PlayerAdded:Connect(function(p)
            if not Config.AUTO_Parry then return end
            local c2 = p.CharacterAdded:Connect(function(char)
                if IsKiller(p) then
                    task.wait(1)
                    AutoParry_HookKillerAnim(char)
                end
            end)
            table.insert(ParryConns, c2)
        end)
        table.insert(ParryConns, c1)

    elseif Config.PARRY_Mode == "BasicAttack" then
        AutoParry_HookBasicAttack()
    end
    -- Mode "Distance": tidak perlu setup, ditangani AutoLoop
end

local function AutoParry()
    if not Config.AUTO_Parry then return end
    if Config.PARRY_Mode ~= "Distance" then return end
    -- Distance mode: polling dari AutoLoop (sama seperti sebelumnya)
    local team = LocalPlayer.Team
    if not team or team.Name ~= "Survivors" then return end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if killerRoot then
                if (killerRoot.Position - root.Position).Magnitude <= Config.PARRY_Dist then
                    local remote = AutoParry_GetRemote()
                    if remote then
                        pcall(function() remote:FireServer() end)
                        LastParryTime = tick()
...
-- =========================================================
-- SKILL CHECK SYSTEM
local QTE_Conns = {}  -- {hb=HeartbeatConn, vis=VisConn}

local function QTE_PressSpace()
...
-- QTE offset: zona perfect = (Goal + offset_start) sampai (Goal + offset_end)
-- Dari VD2 base: 104-114. Bisa di-adjust kalau masih miss.
local QTE_OFFSET_START = 104
local QTE_OFFSET_END   = 114

local function QTE_IsNeedleInZone(needleAngle, targetAngle)
    local needle = needleAngle % 360
    local target = targetAngle % 360
    local s = (target + QTE_OFFSET_START) % 360
    local e = (target + QTE_OFFSET_END)   % 360
    if s > e then return needle >= s or needle <= e end
    return needle >= s and needle <= e
end

-- Kirim SkillCheckResultEvent langsung ke server sebagai fallback
-- Ini bypass input simulation sama sekali
local function QTE_FireDirect(mode)
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if not remotes then return end
        -- Generator skillcheck result
        local gen = remotes:FindFirstChild("Generator")
        if gen then
            local ev = gen:FindFirstChild("SkillCheckResultEvent")
            if ev then ev:FireServer("success", 1) end
        end
        -- Healing skillcheck result  
        local heal = remotes:FindFirstChild("Healing")
        if heal then
            local ev = heal:FindFirstChild("SkillCheckResultEvent")
            if ev then ev:FireServer("success", 1) end
        end
    end)
end
...
local function QTE_StopMonitoring()
    for _, c in ipairs(QTE_Conns) do
        if c.hb  then pcall(function() c.hb:Disconnect()  end); c.hb  = nil end
        if c.vis then pcall(function() c.vis:Disconnect() end); c.vis = nil end
    end
    QTE_Conns = {}
end

local function QTE_HookGui(guiName)
    task.spawn(function()
        -- Poll sampai GUI ada, max 30 detik
        local pg = LocalPlayer:WaitForChild("PlayerGui", 10)
        if not pg then return end
        local gui, check, line, goal
        local t = 0
        while t < 30 do
            gui = pg:FindFirstChild(guiName)
            if gui then
                -- Coba "Check" dulu (gen GUI), lalu FirstChildOfClass Frame (heal GUI)
                check = gui:FindFirstChild("Check")
                if not check then
                    for _, v in ipairs(gui:GetChildren()) do
                        if v:IsA("Frame") and v:FindFirstChild("Line") and v:FindFirstChild("Goal") then
                            check = v; break
                        end
                    end
                end
                line  = check and check:FindFirstChild("Line")
                goal  = check and check:FindFirstChild("Goal")
                if line and goal then break end
            end
            task.wait(0.5)
            t = t + 0.5
        end
        if not line or not goal then
            warn("QTE: GUI '"..guiName.."' not found or missing Line/Goal")
            return
        end

        local c = { hb = nil, vis = nil }
        table.insert(QTE_Conns, c)

        local function startHB()
            if c.hb then return end  -- sudah jalan
            c.hb = RunService.Heartbeat:Connect(function()
                if not Config.AUTO_SkillCheck then
                    if c.hb then c.hb:Disconnect(); c.hb = nil end
                    return
                end
                -- Stop kalau frame tidak visible lagi
                if not check.Visible then
                    if c.hb then c.hb:Disconnect(); c.hb = nil end
                    return
                end
                if QTE_IsNeedleInZone(line.Rotation, goal.Rotation) then
                    -- Coba dua cara: direct remote fire + VIM input
                    QTE_FireDirect()
                    QTE_PressSpace()
                    if c.hb then c.hb:Disconnect(); c.hb = nil end
                end
        ...
        c.vis = check:GetPropertyChangedSignal("Visible"):Connect(function()
            if not Config.AUTO_SkillCheck then return end
            if check.Visible then startHB() else stopHB() end
        end)

        -- Kalau saat hook GUI sudah visible, langsung mulai
        if check.Visible and Config.AUTO_SkillCheck then
            startHB()
        end
    end)
end
...
local AntiFailDescConn    = nil  -- DescendantAdded connection

local function AntiFailGen_DisableScript(scr)
    if scr and scr.Name == "Skillcheck-gen" then
...
local function AntiFailGen_Apply(char)
    if not char then return end
    -- Disconnect hook lama
    if AntiFailDescConn then
        pcall(function() AntiFailDescConn:Disconnect() end)
        AntiFailDescConn = nil
    end
    -- Disable kalau sudah ada
    local scr = char:FindFirstChild("Skillcheck-gen")
    if scr then AntiFailGen_DisableScript(scr) end
    -- Hook DescendantAdded: disable setiap kali Skillcheck-gen di-add
    -- (game bisa re-add script ini setelah lobby → round baru)
    AntiFailDescConn = char.DescendantAdded:Connect(function(desc)
        if not Config.ANTI_FailGen then
            AntiFailDescConn:Disconnect(); AntiFailDescConn = nil; return
        end
        AntiFailGen_DisableScript(desc)
    end)
end
...
local function SetupSkillCheckMonitor()
    QTE_StopMonitoring()
    QTE_HookGui("SkillCheckPromptGui")
    QTE_HookGui("SkillCheckPromptGui-con")
end
...
-- Hook remote Fire sebelum dikirim, redirect target ke killer
local GunSilentHooked = false
...
local function GunSilentAim_GetTarget()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and IsKiller(p) and p.Character then
            local part = p.Character:FindFirstChild(Config.GUN_AimPart)
...
local function GunSilentAim_Setup()
    if GunSilentHooked then return end
    pcall(function()
        local remotes  = ReplicatedStorage:WaitForChild("Remotes", 10)
        if not remotes then return end
        local items    = remotes:WaitForChild("Items", 10)
        if not items then return end
        local tof      = items:WaitForChild("Twist of Fate", 10)
        if not tof then return end
        local fireRE   = tof:WaitForChild("Fire", 10)
        if not fireRE then return end

        -- Hook FireServer: redirect arg pertama (target CFrame/pos) ke killer
        local oldFire = fireRE.FireServer
        fireRE.FireServer = newcclosure(function(self, ...)
            if Config.GUN_SilentAim then
                local target = GunSilentAim_GetTarget()
                if target then
                    -- Kirim posisi killer sebagai target
                    return oldFire(self, target.CFrame, ...)
                end
            end
            return oldFire(self, ...)
        end)
        GunSilentHooked = true
    end)
end

local secGun = TabAIM:AddSection("Gun Silent Aim (Twist of Fate)")
secGun:AddToggle({
    Title = "Gun Silent Aim",
    Default = Config.GUN_SilentAim,
    Callback = function(v)
        Config.GUN_SilentAim = v
        if v then GunSilentAim_Setup() end
...
secSurvival:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then AntiFailGen_Apply(LocalPlayer.Character) else AntiFailGen_Restore() end
        notifToggle("Anti Fail Gen", v)
    end
})
secSurvival:AddToggle({
    Title = "Perfect Skill Check (auto tap)",
    Default = Config.AUTO_SkillCheck,
    Callback = function(v)
        Config.AUTO_SkillCheck = v
        if v then SetupSkillCheckMonitor() end
        notifToggle("Skill Check", v)
    end
})
secSurvival:AddToggle({
    Title = "Auto Parry",
...
secSurvival:AddDropdown({
    Title = "Parry Mode",
    Options = {"Animation", "BasicAttack", "Distance"},
    Default = Config.PARRY_Mode,
    Callback = function(v)
        Config.PARRY_Mode = v
        if Config.AUTO_Parry then AutoParry_Setup() end
    end
})
secSurvival:AddSlider({
    Title = "Parry Distance",
    Min = 5, Max = 25, Default = Config.PARRY_Dist,
    Callback = function(v) Config.PARRY_Dist = v end
})
secSurvival:AddSlider({
    Title = "Parry Delay (ms)",
    Min = 0, Max = 500, Default = math.floor(Config.PARRY_Delay * 1000),
    Callback = function(v) Config.PARRY_Delay = v / 1000 end
})
secSurvival:AddToggle({
...
local function MainLoop()
    ...
    -- FOV Circle
    if GUI.Drawings and GUI.Drawings.FOVCircle then
        if Config.AIM_Enabled and Config.AIM_ShowFOV then
            ...
            GUI.Drawings.FOVCircle.Color = State.AimTarget and Color3.fromRGB(0, 220, 120) or Color3.fromRGB(0, 170, 255)
    ...
    -- Auto Gen hint (Drawing text, still useful)
    if GUI.Drawings and GUI.Drawings.AutoGenHint then
        if Config.AUTO_Generator then
            GUI.Drawings.AutoGenHint.Text = "AUTO GEN  |  [Q] Leave  [X] Stop"
...
Unload = function()
    ...
    -- Cleanup parry hook
    if ParryAttackConn then
        pcall(function() ParryAttackConn:Disconnect() end)
        ParryAttackConn = nil
    end
...
local function Init()
    ...
    GUI.Drawings.FOVCircle.Color = Color3.fromRGB(0, 170, 255)
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- Delay sebelum mulai track KillerBaseSpeed
        -- Game butuh waktu untuk set WalkSpeed killer yang benar
        task.delay(1.5, function()
            if not State.Unloaded then
                NoSlowdownReady = true
            end
        end)
        -- AntiFailGen: re-apply ke karakter baru
        if Config.ANTI_FailGen then
