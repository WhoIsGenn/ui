...
local Config = {
    ...
    PARRY_Dist = 15,
    PARRY_FacingCheck = true,   -- killer harus menghadap kita
    PARRY_Strictness = 0.3,     -- 0=semua arah, 1=harus persis menghadap (dot threshold)
...
local function AutoParry_FindBtn()
    ...
    for _, gui in ipairs(pg:GetChildren()) do
        local controls = gui:FindFirstChildWhichIsA("Frame", true) and gui:FindFirstChild("Controls", true)
        if controls then
        ...
        if checkObj then
            local conn = checkObj:GetAttributeChangedSignal("action"):Connect(function()
                ...
                if myRoot and killerRoot then
                    ...
                    if dist <= Config.PARRY_Dist and KillerIsFacingMe(killerRoot, myRoot) then
...
-- Cek apakah killer menghadap kita (dot product LookVector vs arah ke player)
local function KillerIsFacingMe(killerRoot, myRoot)
    if not Config.PARRY_FacingCheck then return true end  -- skip kalau off
    local toMe = (myRoot.Position - killerRoot.Position)
    if toMe.Magnitude < 0.01 then return true end
    -- Pakai LookVector killer
    local killerLook = killerRoot.CFrame.LookVector
    local dot = killerLook:Dot(toMe.Unit)
    -- dot > threshold: killer menghadap kita
    -- Strictness 0 = threshold -0.3 (hampir semua arah)
    -- Strictness 1 = threshold 0.7 (harus persis menghadap)
    local threshold = -0.3 + (Config.PARRY_Strictness * 1.0)
...
local function AutoParry_Tick(dt)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            -- Facing check: killer harus menghadap kita
            local isFacing = KillerIsFacingMe(kr, root)
            local shouldParry = isFacing and (KillerAttackDetected or CheckKillerApproach(kr, root))
            if shouldParry then
                local fired = false
...
-- Simulate skill check input via semua method yang ada
local function SkillCheck_Fire()
    ...
    -- Method 2: VirtualInputManager SendKeyEvent Space (fallback)
    pcall(function()
    ...
    -- Method 3: SendMouseButton pada action button di Controls GUI
    pcall(function()
        local vim = game:GetService("VirtualInputManager")
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        if not pg then return end
        for _, gui in ipairs(pg:GetChildren()) do
            -- Cari Controls frame di semua GUI
...
local function SetupSkillCheckMonitor()
    pcall(function()
        ...
        local function tryConnect()
            ...
            if not needle or not target then return false end
...
secSurvival:AddToggle({ Title = "Parry Facing Check", Default = Config.PARRY_FacingCheck,
    Callback = function(v) Config.PARRY_FacingCheck = v end })
secSurvival:AddSlider({ Title = "Parry Strictness (0=loose, 10=strict)", Min = 0, Max = 10, Default = math.floor(Config.PARRY_Strictness * 10),
    Callback = function(v) Config.PARRY_Strictness = v / 10 end })
