...
local ParryHighlightCircle = nil  -- highlight circle di bawah kaki
...
local function CreateParryFOVCircle()
    ...
    -- Highlight Circle (filled)
    ParryHighlightCircle = Drawing.new("Circle")
    ParryHighlightCircle.Filled = true
    ParryHighlightCircle.Thickness = 0
    ParryHighlightCircle.Color = Color3.fromRGB(0, 255, 0)
    ParryHighlightCircle.Transparency = 0.7
    ParryHighlightCircle.NumSides = 64
    ParryHighlightCircle.Visible = false
end
...
local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        ...
        if ParryHighlightCircle then ParryHighlightCircle.Visible = false end
        return
    end
    ...
    if onScreen then
        ...
        radius = math.clamp(radius, 20, 300)
        ...
        -- Highlight circle (filled)
        ParryHighlightCircle.Position = screenPos
        ParryHighlightCircle.Radius = radius
        ParryHighlightCircle.Visible = true
        
        -- Warna berdasarkan cooldown
        if ParryOnCooldown then
            ParryFOVCircle.Color = Color3.fromRGB(255, 50, 50)
            ParryHighlightCircle.Color = Color3.fromRGB(255, 50, 50)
            ParryHighlightCircle.Transparency = 0.7
        else
            ParryFOVCircle.Color = Color3.fromRGB(0, 255, 50)
            ParryHighlightCircle.Color = Color3.fromRGB(0, 255, 50)
            ParryHighlightCircle.Transparency = 0.5
        end
    else
        ParryFOVCircle.Visible = false
        ParryHighlightCircle.Visible = false
