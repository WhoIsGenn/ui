-- ============================================
-- 🔧 FIX UNTUK SEMUA ERROR
-- ============================================

-- 1. Fix getnilinstances (buat executor lemah)
getnilinstances = getnilinstances or function() return {} end

-- 2. Fix setreadonly (buat executor lemah)
if not setreadonly then
    setreadonly = function(t, v)
        if debug and debug.setreadonly then
            debug.setreadonly(t, v)
        end
        return t
    end
end

-- 3. Fix newcclosure (buat executor lemah)
if not newcclosure then
    newcclosure = function(f) return f end
end

-- 4. Fix getrawmetatable (biar ga nil)
...
-- 5. Fix Library:GetCustomIcon (biar ga error)
if Library and not Library.GetCustomIcon then
    Library.GetCustomIcon = function(id)
        if Library.Icons and Library.Icons[id] then
            return Library.Icons[id]
        end
        return {
            Url = "",
...
-- 6. Fix MakeDraggable (kalo error)
if Library and not Library.MakeDraggable then
    Library.MakeDraggable = function(frame, dragButton, useScale)
...
-- 7. Fix warn kalo ga ada (buat executor lemah)
if not warn then
    warn = print
end

-- 8. Fix task (buat executor lama)
if not task then
    task = {
        wait = wait,
        delay = delay,
...
-- ============================================
-- FIX UNTUK LIBRARY (TARUH SETELAH LOAD LIBRARY)
-- ============================================
...
if Library and not Library.GetCustomIcon then
    Library.GetCustomIcon = function(id)
        ...
        return {Url = "", ImageRectOffset = Vector2.new(0, 0), ImageRectSize = Vector2.new(0, 0)}
    end
end

-- 2. Fix MakeDraggable (kalo error)
if Library and not Library.MakeDraggable then
    Library.MakeDraggable = function(frame, dragButton, useScale)
        local dragging = false
        local dragStart = Vector2.new()
