...
local ParryFiredAt  = {}  -- [UserId] = tick() kapan terakhir fire ke killer ini
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if kr and (kr.Position - root.Position).Magnitude <= Config.PARRY_Dist then
                ...
                local lastFired = ParryFiredAt[pid] or 0
                -- Fire tiap 1.5 detik selama killer dalam range
                -- Server punya parry window ~0.5-1 detik per attack cycle
                -- 1.5 detik = cukup jarang untuk tidak di-rate limit, cukup sering untuk catch attack
                if now - lastFired >= 1.5 then
                    local remote = AutoParry_GetRemote()
                    if remote then
                        pcall(function() remote:FireServer() end)
                        ParryFiredAt[pid] = now
                    end
                end
                return
            end
...
local function Init()
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- Apply semua anti-script yang aktif
