...
local ParryTimer = 0
-- Track killer attack via Remotes.Attacks.hit
local KillerAttackDetected = false
local KillerHitConn        = nil

local function AutoParry_HookAttackRemote()
    if KillerHitConn then return end  -- already hooked
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        ...
        KillerHitConn = hitRemote.OnClientEvent:Connect(function(...)
            if not Config.AUTO_Parry then return end
            -- hit event fired = killer is attacking RIGHT NOW
...
local function AutoParry_Setup()
    ...
    ParryOnCooldown      = false
    KillerAttackDetected = false
    if KillerHitConn then pcall(function() KillerHitConn:Disconnect() end); KillerHitConn = nil end
