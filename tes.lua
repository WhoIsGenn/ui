...
local AutoAttackBtn = nil

local function AutoAttack_GetBtn()
    if AutoAttackBtn and AutoAttackBtn.Parent then return AutoAttackBtn end
    local pg = LocalPlayer:FindFirstChild("PlayerGui")
    if not pg then return nil end
    -- Cari di semua GUI killer (Killer-mob, Slasher-mob, dll)
    for _, gui in ipairs(pg:GetChildren()) do
        local controls = gui:FindFirstChild("Controls")
        if controls then
            local btn = controls:FindFirstChild("attack")
            if btn then AutoAttackBtn = btn; return btn end
        end
    end
    return nil
end
        ...
        if player ~= LocalPlayer and player.Character then
            ...
            if targetRoot then
                ...
                if dist <= Config.AUTO_AttackRange then
                    -- firesignal tombol attack GUI killer
                    local btn = AutoAttack_GetBtn()
                    if btn then
