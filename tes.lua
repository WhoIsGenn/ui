...
T(S.DungIT, "AutoInfiniteTower",  { Text = "Enable Auto Infinite Tower", Default = false })
...
T(S.DungCD, "AutoCrystalDefense", { Text = "Enable Auto Crystal Defense", Default = false })
...
T(S.DungJoin, "DungeonAutofarm_Dg", { Text = "Autofarm",              Default = false })
T(S.DungJoin, "AutoDungeon",        { Text = "Enable Auto Dungeon",    Default = false })
...
T(S.DungBR, "AutoBossRush",       { Text = "Enable Auto Boss Rush", Default = false })
...
T(S.DungRaid, "AutoRaid",             { Text = "Enable Auto Raid",    Default = false })
...
local function VoteDifficulty(difficulty, enabled)
    ...
    if tick() - _lastAction < 0 then return false end
    local dungeonUI = PGui:FindFirstChild("DungeonUI")
    if not dungeonUI then return false end
...
-- Autofarm NPCs di workspace (universal untuk semua dungeon)
local function DungeonAutofarm(activeToggleName)
    if not Toggles[activeToggleName] or not Toggles[activeToggleName].Value then return end
    local char = GetCharacter()
    local root = char and char:FindFirstChild("HumanoidRootPart")
    ...
    local finalPos = tPos + (root.Position - tPos).Unit * dist
    if (root.Position - finalPos).Magnitude > 3 then
        ...
        if Options.SelectedMovementType and Options.SelectedMovementType.Value == "Teleport" then
            ...
            local tween = game:GetService("TweenService"):Create(
                root, TweenInfo.new(dur, Enum.EasingStyle.Linear), {CFrame = dest})
            tween:Play(); task.wait(dur)
        end
        root.AssemblyLinearVelocity  = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
    end
...
task.spawn(function()
    while getgenv().Gennskie_Running do
        ...
        -- INFINITE TOWER (Sea 1 - Tower Island, no difficulty)
        if Toggles.AutoInfiniteTower.Value then
            JoinPortal("InfiniteTower", Toggles.AutoJoinPortal_IT.Value)
            StartWave(Toggles.AutoStartWave_IT.Value)
            ReplayDungeon(Toggles.AutoReplay_IT.Value)
            if Toggles.DungeonAutofarm_IT.Value then DungeonAutofarm("AutoInfiniteTower") end
        end

        -- CRYSTAL DEFENSE (Sea 2, no difficulty)
        if Toggles.AutoCrystalDefense.Value then
            JoinPortal("CrystalDefense", Toggles.AutoJoinPortal_CD.Value)
            StartWave(Toggles.AutoStartWave_CD.Value)
            ReplayDungeon(Toggles.AutoReplay_CD.Value)
            if Toggles.DungeonAutofarm_CD.Value then DungeonAutofarm("AutoCrystalDefense") end
        end
        ...
        if Toggles.AutoDungeon.Value then
            ...
            JoinPortal(portalName, Toggles.AutoJoinPortal_Dg.Value)
            VoteDifficulty(Options.SelectedDiff and Options.SelectedDiff.Value or "Medium", Toggles.AutoVoteDiff_Dg.Value)
            ReplayDungeon(Toggles.AutoReplay_Dg.Value)
            if Toggles.DungeonAutofarm_Dg.Value then DungeonAutofarm("AutoDungeon") end
        end

        -- BOSS RUSH (Sea 1 - Sailor Island, ada difficulty)
        if Toggles.AutoBossRush.Value then
            JoinPortal("BossRush", Toggles.AutoJoinPortal_BR.Value)
            VoteDifficulty(Options.SelectedBossRushDiff and Options.SelectedBossRushDiff.Value or "Medium", Toggles.AutoVoteDiff_BR.Value)
            ReplayDungeon(Toggles.AutoReplay_BR.Value)
            if Toggles.DungeonAutofarm_BR.Value then DungeonAutofarm("AutoBossRush") end
        end

        -- RAID (Sea 2, ada difficulty)
        if Toggles.AutoRaid.Value then
            JoinPortal("Raid", Toggles.AutoJoinPortal_Raid.Value)
            VoteDifficulty(Options.SelectedRaidDiff and Options.SelectedRaidDiff.Value or "Medium", Toggles.AutoVoteDiff_Raid.Value)
            ReplayDungeon(Toggles.AutoReplay_Raid.Value)
            if Toggles.DungeonAutofarm_Raid.Value then DungeonAutofarm("AutoRaid") end
        end
    end
end)
