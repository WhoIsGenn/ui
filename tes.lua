...
-- ================================================================
-- ANTI FAKE PARRY (Validasi Attack Real)
-- ================================================================

-- Cek apakah killer beneran bisa kena lo (valid attack)
local function IsValidAttack(killerChar, myChar)
    ...
    -- 1. CEK JARAK (lebih ketat)
    local dist = (killerRoot.Position - myRoot.Position).Magnitude
    if dist > Config.PARRY_Dist then return false end
    
    -- 2. CEK ARAH KILLER (apakah dia menghadap lo)
    local killerLook = killerRoot.CFrame.LookVector
    local toTarget = (myRoot.Position - killerRoot.Position).Unit
    local dotProduct = killerLook:Dot(toTarget)
    
    -- Killer harus menghadap lo (dot > 0.5 = 60 derajat)
    if dotProduct < 0.5 then 
        return false  -- Killer ga ngadep lo, fake attack
    end
    
    -- 3. CEK APAKAH ADA PART KILLER YANG NYENTUH LO (hitbox check)
    local attackParts = {
        killerChar:FindFirstChild("Right Arm"),
        killerChar:FindFirstChild("Left Arm"),
        killerChar:FindFirstChild("RightHand"),
        killerChar:FindFirstChild("LeftHand"),
        killerChar:FindFirstChild("Weapon"),
        killerChar:FindFirstChild("Tool"),
    }
    
    for _, part in ipairs(attackParts) do
        if part and part:IsA("BasePart") then
            local partDist = (part.Position - myRoot.Position).Magnitude
            if partDist < 6 then  -- Jarak efektif kena
                return true
            end
        end
    end
    
    -- 4. FALLBACK: cek jarak biasa (tapi lebih ketat)
    if dist < 8 then  -- Jarak sangat dekat, pasti kena
        return true
    end
...
local AttackAnimIDs = {
    ...
    ["117042998468241"] = true, ["129918027564423"] = true,
    ["133963973694098"] = true, ["95934119190708"] = true,
    ["118907603246885"] = true, ["77081789642514"] = true,
    ["78432063483146"] = true, ["80411309607666"] = true,
...
local NonAttackAnimIDs = {
    ["101784373049485"] = true, ["104239995665623"] = true,
    ["109066149291691"] = true, ["110953720370369"] = true,
    ["111427918159250"] = true, ["117224999672195"] = true,
    ["118699522268698"] = true, ["122986861455212"] = true,
    ["123782306962803"] = true, ["124191224140066"] = true,
    ["136859656743697"] = true, ["137688077908355"] = true,
    ["137846825408335"] = true, ["138045669415653"] = true,
    ["139198068127517"] = true, ["139610361987372"] = true,
    ["139928639611415"] = true, ["75258958842388"] = true,
    ["80105342981313"] = true, ["84093948968516"] = true,
    ["86266790353635"] = true, ["90249435310475"] = true,
    ["92098503722633"] = true, ["93136435416899"] = true,
    ["94067810090105"] = true, ["96744338559260"] = true,
    ["102182386301796"] = true, ["113499071528107"] = true,
    ["126100203042329"] = true, ["128387952281975"] = true,
    ["131476715474323"] = true, ["138125499040825"] = true,
    ["70746483345907"] = true, ["76294518257930"] = true,
    ["79376988328260"] = true, ["89642871504538"] = true,
    ["90374658251379"] = true, ["91224543667492"] = true,
    ["84440437648153"] = true,
}
...
-- ✅ MODIFIKASI: AutoParry_TryFire dengan validasi anti fake
local function AutoParry_TryFire()
    if not Config.AUTO_Parry then return end
    if ParryOnCooldown then return end
    if GetRole() ~= "Survivor" then return end
    
    -- ✅ VALIDASI: Cek apakah ada killer yang beneran attack ke lo
    local hasValidKiller = false
    for _, player in ipairs(Players:GetPlayers()) do
        if IsKiller(player) and player.Character then
            if IsValidAttack(player.Character, LocalPlayer.Character) then
                hasValidKiller = true
                break
            end
        end
    ...
    -- Kalo ga ada killer yang valid, skip parry (biar ga kena fake)
    if not hasValidKiller then return end
    ...
    task.spawn(function()
        ...
        local conn1 = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and kr then
                ...
                -- ✅ Validasi tambahan: cek arah killer
                local killerLook = kr.CFrame.LookVector
                local toTarget = (myRoot.Position - kr.Position).Unit
                local dotProduct = killerLook:Dot(toTarget)
                
                if dist <= Config.PARRY_Dist and dotProduct > 0.5 then
                    AutoParry_TryFire()
                end
            end
        end)
        ...
        local conn2 = RunService.Heartbeat:Connect(function()
            ...
            -- ✅ Validasi arah killer
            local killerLook = kr.CFrame.LookVector
            local toTarget = (myRoot.Position - kr.Position).Unit
            local dotProduct = killerLook:Dot(toTarget)
            if dotProduct < 0.5 then return end
...
local function Fullbright_On()
    ...
    Lighting.Ambient        = Color3.fromRGB(178, 178, 178)  -- tidak 255 penuh, cukup bright tanpa overexpose
    Lighting.OutdoorAmbient = Color3.fromRGB(178, 178, 178)
    Lighting.Brightness     = 1.5
    Lighting.ShadowSoftness = 0
    Lighting.GlobalShadows  = false

    -- Disable hanya effect yang bikin berat: blur dan depth of field
    -- Biarkan bloom/colorCorrection untuk tidak mengubah terlalu banyak
    for _, v in ipairs(Lighting:GetChildren()) do
...
local function Fullbright_Off()
    ...
    -- Restore state effect ke aslinya
