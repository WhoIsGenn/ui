...
local PFOVBg = Drawing.new("Circle")
PFOVBg.NumSides = 48
PFOVBg.Filled = true
PFOVBg.Color = Color3.fromRGB(255, 255, 255)
PFOVBg.Transparency = 0.85
PFOVBg.Visible = false
...
local function MainLoop()
    ...
    if Config.AUTO_Parry and Config.PARRY_FOV then
        ...
        if myRoot then
            ...
            local screenPts = {}
            for i = 1, PFOV_N do
                local a = (i-0.5)/PFOV_N * math.pi * 2
                local sp, onSc = WorldToScreen(Vector3.new(ox+math.cos(a)*r, baseY, oz+math.sin(a)*r))
                screenPts[i] = onSc and sp or nil
            end
            -- Render outline
            for i = 1, PFOV_N do
            ...
            -- Background: fill pakai triangle fan dari center ke tiap segment
            local centerSP, centerOn = WorldToScreen(Vector3.new(ox, baseY, oz))
            if centerOn then
                PFOVBg.Position = centerSP
                -- Estimasi radius screen dari titik pertama yang visible
                for i = 1, PFOV_N do
                    if screenPts[i] then
                        PFOVBg.Radius = (screenPts[i] - centerSP).Magnitude
                        break
                    end
                end
                PFOVBg.Visible = true
...
Unload = function()
    ...
    pcall(function() PFOVBg:Remove() end)
    for i = 1, 4 do
