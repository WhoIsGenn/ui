...
local ParryTimer = 0
-- Track killer attack via attribute changes + HealthChanged
local KillerAttackDetected = false
local KillerHitConns       = {}
local LastHealth           = 100
local LastSpeedBoost       = 1
...
local function AutoParry_HookAttackRemote()
    ...
    LastSpeedBoost = char:GetAttribute("speedboost") or 1

    -- HealthChanged: kena hit = parry window (reactive, tapi masih useful)
    ...
    -- AttributeChanged: speedboost naik = killer swing animation start
    -- Ini lebih awal dari HealthChanged
    local c2 = char.AttributeChanged:Connect(function(attr)
        if not Config.AUTO_Parry then return end
        if attr == "speedboost" then
            local val = char:GetAttribute("speedboost") or 1
            -- speedboost naik ke 1.2+ = killer attacking
            if val > 1.1 and val > LastSpeedBoost then
                KillerAttackDetected = true
...
local function AutoParry_Tick(dt)
    ...
    -- Hook attack remote kalau belum
    AutoParry_HookAttackRemote()

    if not KillerAttackDetected then return end

    local root = GetCharacterRoot()
    if not root then return end

    -- Cari killer terdekat dalam range
    for _, player in ipairs(Players:GetPlayers()) do
...
local function GunSilentAim_Setup()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "FireServer" and Config.GUN_SilentAim then
            -- Cek apakah ini Fire remote dari Twist of Fate
            -- Format: Fire:FireServer("gun", dirX, dirY, dirZ)
            local args = {...}
            if args[1] == "gun" and type(args[2]) == "number" then
                local dir = GunSilentAim_GetTargetDir(self)
                if dir then
                    return OriginalNamecall(self, "gun", dir.X, dir.Y, dir.Z)
                end
            end
        end
