...
local function TriggerMobileButton(PG)
    -- Coba firesignal dulu (paling cepat, tidak ada cursor)
    ...
    -- Fallback: cari button lewat Controls tree
    for _, gui in ipairs(PG:GetChildren()) do
        local function findControls(parent, depth)
            if depth > 5 then return nil end
            for _, child in ipairs(parent:GetChildren()) do
                if child.Name == "Controls" then return child end
                local f = findControls(child, depth + 1)
                if f then return f end
            end
        end
        local controls = findControls(gui, 0)
        if controls then
            for _, btnName in ipairs({"check", "action", "Action", "skillcheck", "SkillCheck"}) do
...
-- PARRY FOV drawings (world-space circle, bukan screen-space)
local PFOV_N        = 32
local PFOVOutline   = {}  -- putih tipis
local PFOVGlow      = {}  -- biru tebal transparan
for i = 1, PFOV_N do
    ...
    gl.Thickness = 5;   gl.Color = Color3.fromRGB(80, 160, 255);  gl.Transparency = 0.5; gl.Visible = false
    PFOVGlow[i] = gl
end
...
local function UpdateParryFOV()
    if not Config.AUTO_Parry or not Config.PARRY_FOV then
        for i = 1, PFOV_N do
            PFOVOutline[i].Visible = false
            PFOVGlow[i].Visible    = false
        end
        return
    end
    local root = GetCharacterRoot()
    ...
    local baseY = root.Position.Y - 2.8  -- level lantai kaki
    local ox, oz = root.Position.X, root.Position.Z
    -- Warna berubah saat cooldown
    ...
    for i = 1, PFOV_N do
        ...
        local p1 = Vector3.new(ox + math.cos(a1)*r, baseY, oz + math.sin(a1)*r)
        local p2 = Vector3.new(ox + math.cos(a2)*r, baseY, oz + math.sin(a2)*r)
        local s1, v1 = WorldToScreen(p1)
        local s2, v2 = WorldToScreen(p2)
        local vis = v1 and v2
        PFOVOutline[i].From = s1; PFOVOutline[i].To = s2
        PFOVOutline[i].Color = oc; PFOVOutline[i].Visible = vis
        PFOVGlow[i].From = s1; PFOVGlow[i].To = s2
        PFOVGlow[i].Color = gc; PFOVGlow[i].Visible = vis
    end
end
