...
local ParryWasInRange = {}  -- track killer mana yang sudah dalam range
...
local function AutoParry()
    if not Config.AUTO_Parry then
        ParryWasInRange = {}
        return
    end
    local team = LocalPlayer.Team
    if not team or team.Name ~= "Survivors" then
        ParryWasInRange = {}
        return
    end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if kr then
                ...
                local wasIn = ParryWasInRange[pid]

                if dist <= Config.PARRY_Dist then
                    if not wasIn then
                        -- Killer BARU masuk range → fire parry
                        ParryWasInRange[pid] = true
                    ...
                    -- Killer keluar range → reset state biar bisa fire lagi
                    ParryWasInRange[pid] = nil
                end
            end
        end
    end
...
-- ================================================================
-- QTE / PERFECT SKILL CHECK
-- Cara kerja:
-- 1. Watch PlayerGui.ChildAdded untuk detect GUI masuk kapanpun
-- 2. Saat check.Visible = true, langsung FireServer "success"
--    tanpa perlu deteksi needle (bypass sepenuhnya)
-- 3. Juga coba VirtualInputManager sebagai backup
-- ================================================================
local QTE_WatchConn  = nil  -- PlayerGui.ChildAdded watcher
local QTE_GuiConns   = {}   -- per-GUI connections
...
local function QTE_IsNeedleInZone(needleAngle, targetAngle)
    -- Offset 104-114 dari VD2 base (belum dikonfirmasi, jaga-jaga)
    local needle = needleAngle % 360
    local target = targetAngle % 360
    local s = (target + 100) % 360
    local e = (target + 120) % 360
    if s > e then return needle >= s or needle <= e end
    return needle >= s and needle <= e
end

-- Fire SkillCheckResultEvent langsung - ini yang paling reliable
-- isGen = true → fire gen remote, false → fire heal remote
local function QTE_FireResult(isGen)
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if not remotes then return end
        if isGen then
            local ev = remotes:FindFirstChild("Generator")
            ev = ev and ev:FindFirstChild("SkillCheckResultEvent")
            if ev then ev:FireServer("success", 1) end
        else
            local ev = remotes:FindFirstChild("Healing")
            ev = ev and ev:FindFirstChild("SkillCheckResultEvent")
            if ev then ev:FireServer("success", 1) end
        end
    end)
...
local function QTE_StopMonitoring()
    for _, c in ipairs(QTE_Conns) do
        if c.hb  then pcall(function() c.hb:Disconnect()  end) end
        if c.vis then pcall(function() c.vis:Disconnect() end) end
    end
    QTE_Conns = {}
end

-- Hook satu GUI instance
local function QTE_HookOne(gui, isGen)
    -- Cari Check frame (gen) atau frame lain yang punya Line+Goal (heal)
    ...
    local function startHB()
        if c.hb then return end
        c.hb = RunService.Heartbeat:Connect(function()
            if not Config.AUTO_SkillCheck or not check.Visible then
                c.hb:Disconnect(); c.hb = nil; return
            end
            if QTE_IsNeedleInZone(line.Rotation, goal.Rotation) then
                QTE_FireResult(isGen)
                QTE_PressSpace()
                c.hb:Disconnect(); c.hb = nil
            end
        end)
    end
    ...
    if check.Visible and Config.AUTO_SkillCheck then startHB() end
    return true
end

local function SetupSkillCheckMonitor()
    QTE_StopMonitoring()
    if QTE_WatchConn then
        pcall(function() QTE_WatchConn:Disconnect() end)
        QTE_WatchConn = nil
    end
    ...
    -- Hook GUI yang sudah ada sekarang
    local gen  = pg:FindFirstChild("SkillCheckPromptGui")
    local heal = pg:FindFirstChild("SkillCheckPromptGui-con")
    if gen  then QTE_HookOne(gen,  true)  end
    if heal then QTE_HookOne(heal, false) end

    -- Watch ChildAdded: GUI mungkin baru masuk saat round mulai
    QTE_WatchConn = pg.ChildAdded:Connect(function(child)
        if not Config.AUTO_SkillCheck then return end
        local name = child.Name
        if name == "SkillCheckPromptGui" then
            task.wait(0.1)  -- tunggu children load
            QTE_HookOne(child, true)
        elseif name == "SkillCheckPromptGui-con" then
            task.wait(0.1)
            QTE_HookOne(child, false)
        end
    end)
end
