...
local function SuspendAllFeatures()
    -- Stop semua active functions tanpa toggle Config (agar state tersimpan)
    pcall(Godmode_Remove)
    pcall(AntiScript_Restore)
    pcall(PerfectSC_Stop)
    pcall(AutoParry_Cleanup)
    pcall(UpdateSpeed_Restore)   -- restore speed ke normal
    pcall(function()
...
local function ReapplyFeatures()
    -- Dipanggil setelah masuk match, semua fungsi sudah terdefinisi
    task.spawn(function()
        local role = GetRole()

        -- Semua role
        if Config.GODMODE then pcall(Godmode_Apply) end
        if Config.PERFECT_SkillCheck then
            pcall(function() PerfectSC_Setup() end)
        end
            ...
            if Config.ANTI_FailGen or Config.ANTI_SkillCheck then
                task.delay(2, function()
                    if not State.Unloaded and GetRole() == "Survivor" then
                        pcall(function() AntiScript_Apply(LocalPlayer.Character) end)
                    end
                end)
            end
            if Config.AUTO_Parry then
                task.delay(1, function()
                    if not State.Unloaded then
                        pcall(function() AutoParry_HookAttackRemote() end)
                    end
                end)
            end
        end
...
local function RoleManager_Start()
    if RoleManagerConn then
        pcall(function() RoleManagerConn:Disconnect() end)
    end
    ...
    RoleManagerConn = LocalPlayer:GetPropertyChangedSignal("Team"):Connect(function()
        ...
        if newRole == "Spectator" then
            -- Masuk lobby → suspend semua
            task.spawn(SuspendAllFeatures)
        elseif prevRole == "Spectator" then
...
local function PerfectSC_ConnectGui()
    task.spawn(function()
        pcall(function()
            ...
            local CheckGui  = PlayerGui:WaitForChild("SkillCheckPromptGui", 15)
            ...
            local visConn = Check:GetPropertyChangedSignal("Visible"):Connect(function()
                ...
                if Check.Visible then
                    ...
                    hbConn = RunService.Heartbeat:Connect(function()
                        ...
                        if inGoal then
                            pcall(function()
                                -- Game pakai CheckInterractable attribute "action"
                                -- Source confirmed dari Animations-new decompile
                                ...
                                if checkObj then
                                    ...
                                    -- Fallback: tap action button di Controls
                                    local pg = LocalPlayer:FindFirstChild("PlayerGui")
                                    if pg then
                                        for _, gui in ipairs(pg:GetChildren()) do
            ...
            -- Watch kalau GUI di-recreate saat map swap
            local watchConn = PlayerGui.ChildAdded:Connect(function(child)
                if not PerfectSCActive then return end
            ...
            table.insert(PerfectSCConns, watchConn)
        end)
    end)
end
...
local function BeatGameSurvivor() end  -- stub
local function BeatGameKiller() end  -- stub
local function UpdateFog() end  -- stub
...
local function Init()
    ...
    Connections.CharacterRespawn = LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- Re-hook AutoParry attack detection on new character
        if Config.AUTO_Parry then
            for _, c in ipairs(KillerHitConns) do pcall(function() c:Disconnect() end) end
            KillerHitConns = {}
            task.delay(1, function()
                if not State.Unloaded then AutoParry_HookAttackRemote() end
            end)
        end
        if Config.GODMODE then Godmode_Apply() end
        -- Anti script: delay karena Skillcheck-gen/player muncul beberapa detik setelah spawn
        if Config.ANTI_FailGen or Config.ANTI_SkillCheck then
            task.delay(2, function()
                if not State.Unloaded then AntiScript_Apply(char) end
            end)
        end
        -- Perfect Skill Check: reconnect GUI setelah map swap
        if Config.PERFECT_SkillCheck then
            task.delay(2, function()
                if not State.Unloaded then PerfectSC_Setup() end
            end)
        end
    end)
