...
local Config = {
    ...
    AIM_Enabled = false,
    AIM_AutoMode = true,   -- Auto aim saat aiming (mobile) vs hold RMB (PC)
    AIM_UseRMB = true,
    AIM_FOV = 120,
    AIM_Smooth = 0.3,
    AIM_TargetPart = "Head",
...
function Aimbot.Update(cam, screenSize, screenCenter)
    ...
    -- Mobile: aktif saat karakter Aiming (hold tombol gun)
    -- PC: aktif saat hold RMB
    local isAiming = false
    if Config.AIM_AutoMode then
        -- Auto mode: aktif saat character Aiming (hold tombol gun di mobile)
        local char = LocalPlayer.Character
        isAiming = char ~= nil and (char:GetAttribute("Aiming") == true)
    else
        -- Manual mode: hold RMB
        isAiming = State.AimHolding
    end
...
secAimbot:AddToggle({
    Title = "Auto Mode (Mobile - saat aim gun)",
    Default = Config.AIM_AutoMode,
    Callback = function(state)
        Config.AIM_AutoMode = state
        notifToggle("Auto Aim Mode", state)
    end
})

secAimbot:AddToggle({
    Title = "Use RMB (PC only)",
