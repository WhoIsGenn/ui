...
local Config = {
    ...
    PARRY_Dist = 13,
    PARRY_Dist = 13,                 -- Parry Detection Range (studs)
    PARRY_FaceKiller = 0.9,         -- Face Killer Sensitivity (0-1)
    PARRY_AimStrict = 1.3,
    PARRY_FOV  = false,
...
-- Face Killer Sensitivity & Aim Strictness
-- ================================================================
...
-- ================================================================
-- DETEKSI FACE KILLER (SENSITIVITY)
-- ================================================================
local function IsKillerFacingMe(killerChar, myPos)
    local killerHead = killerChar:FindFirstChild("Head")
    if not killerHead then 
        killerHead = killerChar:FindFirstChild("HumanoidRootPart")
        if not killerHead then return true end
    end
    
    local killerLook = killerHead.CFrame.LookVector
    local toMe = (myPos - killerHead.Position).Unit
    local dotProduct = killerLook:Dot(toMe)
    
    local threshold = Config.PARRY_FaceKiller or 0.9
    
    return dotProduct > threshold
end

-- ================================================================
-- DETEKSI AIM STRICTNESS (Arah Weapon/Right Arm)
-- ================================================================
local function IsWeaponFacingMe(killerChar, myPos)
    local weapon = killerChar:FindFirstChild("Weapon") or 
                   killerChar:FindFirstChild("Right Arm") or
                   killerChar:FindFirstChild("Tool")
    
    if not weapon then return true end
    
    local weaponPos = weapon.Position
    local weaponDir = weapon.CFrame.LookVector
    local toMe = (myPos - weaponPos).Unit
    local dotProduct = weaponDir:Dot(toMe)
    
    local rawStrict = Config.PARRY_AimStrict or 1.3
    local threshold = math.min(rawStrict / 2, 0.99)
...
local function AutoParry_TryFire(killerChar)
    ...
    -- 2. CEK FACE KILLER
    if not IsKillerFacingMe(killerChar, myRoot.Position) then
        return
    end
    
    -- 3. CEK AIM STRICTNESS (arah weapon)
    if not IsWeaponFacingMe(killerChar, myRoot.Position) then
        return
    end
...
secParry:AddSlider({ 
    ...
    Increment = 0.01,
    Default = 0.9,
    Callback = function(v) Config.PARRY_FaceKiller = v end 
})
