...
local ParryFiredAt = {}  -- [UserId] = tick() saat terakhir fire
local PARRY_CD     = 51  -- ~50 detik cooldown parry di game + 1 detik buffer
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if kr and (kr.Position - root.Position).Magnitude <= Config.PARRY_Dist then
                ...
                -- Hanya fire kalau cooldown 50 detik sudah habis
                if now - (ParryFiredAt[pid] or 0) >= PARRY_CD then
