...
-- ================================================================
-- ESP UPDATE PLAYERS
-- Highlight dibuat sekali di CharacterAdded, update warna saja di sini
-- Nametag: reuse BillboardGui, update text saja = no flicker
-- ================================================================
        ...
        if killer then
            if Config.ESP_Killer then
                ...
                if hrp then
                    ...
                    local col = ESPColors.Killer
                    if hooked then col = Color3.fromRGB(255,182,193)
                    elseif hum and hum.Health < hum.MaxHealth then
                        col = knocked and Color3.fromRGB(200,100,0) or Color3.fromRGB(230,220,0)
                    end
                    local myRoot = GetCharacterRoot()
                    ...
                    if Config.ESP_Names    then table.insert(lines, player.Name) end
                    if Config.ESP_Distance then table.insert(lines, "["..dist.."m]") end
                    if Config.ESP_Health and hum then table.insert(lines, "HP:"..math.floor(GetHealthPercent(hum)*100).."%") end
                    ...
                    if not tag then
                        ...
                        tag.Size=UDim2.new(0,130,0,36); tag.StudsOffset=Vector3.new(0,3.2,0)
                        tag.Adornee=hrp; tag.Parent=hrp
                        local t=Instance.new("TextLabel")
                        t.Name="_VHTagLabel"; t.Size=UDim2.new(1,0,1,0)
                        t.BackgroundTransparency=1; t.Font=Enum.Font.GothamBold
                        t.TextSize=11; t.TextWrapped=true
                        t.TextStrokeTransparency=0; t.TextStrokeColor3=Color3.new(0,0,0)
                        t.Parent=tag
                    end
