...
local function SafeLoop(name, func)
    return function()
        ...
        if not success then
            VelarisUI:Notify("Error in ["..name.."]: "..tostring(err), 10)
...
Remotes.StockUpdate.OnClientEvent:Connect(function(itemName, stockLeft)
    ...
    if stockLeft == 0 then
        VelarisUI:Notify("[MERCHANT] Bought: " .. tostring(itemName), 2)
    end
end)
...
local function SafeTeleportToNPC(targetName, customMap)
    ...
    if target then
        ...
        VelarisUI:Notify("NPC not found: " .. tostring(actualName), 3)
    end
end
...
local function UniversalPuzzleSolver(puzzleType)
    ...
    VelarisUI:Notify("Starting " .. puzzleType .. " Puzzle...", 5)
    ...
    for i, islandOrPiece in ipairs(piecesToCollect) do
        ...
        if piece then
            ...
            if prompt then
                fireproximityprompt(prompt)
                VelarisUI:Notify(string.format("Collected Piece %d/%d", i, #piecesToCollect), 2)
                task.wait(1.5)
            else
                VelarisUI:Notify("Found piece but no interaction prompt was detected.", 3)
            end
        else
            VelarisUI:Notify("Failed to find piece " .. i .. " on " .. tostring(tpTarget or "Island"), 3)
        end
    end
    VelarisUI:Notify(puzzleType .. " Puzzle Completed!", 5)
end
...
local function AutoRollStatsLoop()
    ...
    if not hasStat or not hasRank then
        VelarisUI:Notify("Error: Select at least one Stat and one Rank first!", 5)
    ...
    while Toggles.AutoRollStats.Value do
            ...
            if selectedStats[statName] then
                ...
                if currentData then
                    ...
                    if not selectedRanks[currentRank] then
                        ...
                        if not success then
                            VelarisUI:Notify("ERROR: " .. tostring(err):gsub("<", "["), 5)
                        end
        ...
        if workDone then
            VelarisUI:Notify("Successfully rolled selected stats.", 5)
            Toggles.AutoRollStats:SetValue(false)
            break
        end
    ...
    while task.wait() do
        if Toggles.AutoTrait.Value then
            ...
            if selected[currentTrait] then
                VelarisUI:Notify("Success! Got Trait: " .. currentTrait, 5)
        ...
        if Toggles.AutoRace.Value then
            ...
            if selected[currentRace] then
                VelarisUI:Notify("Success! Got Race: " .. currentRace, 5)
        ...
        if Toggles.AutoClan.Value then
            ...
            if selected[currentClan] then
                VelarisUI:Notify("Success! Got Clan: " .. currentClan, 5)
...
local Window = VelarisUI:Window({
    Title = "Sailor Piece",
    Footer = "VictUI Migration",
    Content = "Sailor Piece",
    Color = "Default",
    Version = 1.0,
    ["Tab Width"] = 120,
    Image = "101833678008843",
    Configname = "SailorPiece",
    Uitransparent = 0.15,
    ShowUser = true,
    Search = true,
    Animation = true,
...
local function CreateSectionProxy(section)
    ...
    function proxy:AddInput(id, data)
        return section:AddInput({
            Title = data.Text or id,
...
local function CreateTabProxy(tab)
    local proxy = {}

    function proxy:AddLeftGroupbox(name)
        local sec = tab:AddSection({
            Title = name
        })

        return CreateSectionProxy(sec)
    end

    function proxy:AddRightGroupbox(name)
        local sec = tab:AddSection({
            Title = name
        })
...
local Tabs = {
    Information = CreateTabProxy(Window:AddTab({
        Name = "Information",
        Icon = "lucide:info",
    })),

    Priority = CreateTabProxy(Window:AddTab({
        Name = "Priority",
        Icon = "lucide:arrow-up-down",
    })),

    Main = CreateTabProxy(Window:AddTab({
        Name = "Main",
        Icon = "lucide:box",
    })),

    Automation = CreateTabProxy(Window:AddTab({
        Name = "Automation",
        Icon = "lucide:repeat-2",
    })),

    Artifact = CreateTabProxy(Window:AddTab({
        Name = "Artifact",
        Icon = "lucide:gem",
    })),

    Dungeon = CreateTabProxy(Window:AddTab({
        Name = "Dungeon",
        Icon = "lucide:door-open",
    })),

    Player = CreateTabProxy(Window:AddTab({
        Name = "Player",
        Icon = "lucide:user",
    })),

    Teleport = CreateTabProxy(Window:AddTab({
        Name = "Teleport",
        Icon = "lucide:map-pin",
    })),

    Webhook = CreateTabProxy(Window:AddTab({
        Name = "Webhook",
        Icon = "lucide:send",
    })),

    Misc = CreateTabProxy(Window:AddTab({
        Name = "Misc",
        Icon = "lucide:apple",
    })),

    Config = CreateTabProxy(Window:AddTab({
        Name = "Config",
...
GB.Information.Left.Game:AddButton("Redeem All Codes", function()
    ...
    for codeName, data in pairs(allCodes) do
        ...
        if playerLevel >= levelReq then
            ...
            VelarisUI:Notify("Attempt to redeem code: " .. codeName .. rewardText, 8)
            
            Remotes.UseCode:InvokeServer(codeName)
            
            task.wait(2)
        else
            VelarisUI:Notify(string.format("Not enough requirement for: %s (Req. Lvl %d)", codeName, levelReq), 4)
        end
    end
end)
...
TB_Tabs.MiscAuto.T3:AddToggle("AutoCombo", {
    ...
    Callback = function(state)
        if state and Toggles.AutoSkill.Value then
            ...
            VelarisUI:Notify("NOTICE: Auto Skill disabled for this to works properly.", 3)
        end
    end
})
...
TB_Tabs.Waypoint.T1:AddDropdown("SelectedIsland", {
    ...
    if a ~= nil then
        ...
        VelarisUI:Notify("Please select a island to teleport.", 2)
    end
    end
})
...
TB_Tabs.Waypoint.T2:AddButton({
    ...
    Func = function()
        ...
        if not distance then
            VelarisUI:Notify("someting wrong..", 2)
            return
        end
...
TB_Tabs.Puzzle.T1:AddButton({
    ...
    Func = function()
        ...
        if currentLevel >= 5000 then
            ...
            VelarisUI:Notify("Level 5000 required! Current: " .. currentLevel, 3)
        end
    end
})
        ...
        for i = 1, 3 do
            ...
            if item then
                ...
                if prompt then
                    ...
                    VelarisUI:Notify("No prompt found in " .. itemName, 2)
                end
            else
                VelarisUI:Notify(itemName .. " not found!", 2)
            end
        end
    end
})
...
TB_Tabs.Puzzle.T1:AddButton({
    ...
    Func = function()
        ...
        if currentLevel >= 8500 then
            ...
            VelarisUI:Notify("Level 8500 required! Current: " .. currentLevel, 4)
        end
    end
})
...
Remotes.UpAscend.OnClientEvent:Connect(function(data)
    ...
    if data.allMet then
        VelarisUI:Notify("All requirements met! Attempt to ascend into: " .. data.nextRankName, 5)
...
task.spawn(function()
    while true do
        ...
        if not success then
            VelarisUI:Notify("ERROR: " .. tostring(err), 10)
        end
    end
end)
    ...
    while task.wait(1) do
        ...
        if root and not Shared.MovingIsland then
            ...
            if pos.Y > 5000 or math.abs(pos.X) > 10000 or math.abs(pos.Z) > 10000 then
                ...
                VelarisUI:Notify("Something wrong, attempt to reset..", 5)
...
VelarisUI:Notify("Script loaded.", 2)
VelarisUI:Notify("Report bug and give suggestion in Discord!", 5)

end)

if not eh_success then
    VelarisUI:Notify("ERROR: " .. tostring(err), 4)
end
