...
-- Version: 1.0.1 - Fixed Dropdown Refresh & Notif Spam

-- ==================== FIRST LOAD FLAG (MUST BE FIRST!) ====================
_G.VictoriaFirstLoad = true
...
legitFishingSection:AddToggle({
    ...
    Callback = function(v)
        ...
        -- Matikan instant fishing kalo nyala
        if _G.InstantFishing then
            _G.InstantFishing = false
            if fishThread then
...
local function lempar()
    safeCall("lempar", function()
        net["RF/RequestFishingMinigameStarted"]:InvokeServer(-139.63, 0.996, -1766532005.497)
    end)
...
instantFishingSection:AddToggle({
    ...
    Callback = function(v)
        ...
        -- Matikan legit fishing kalo nyala
        if _G.LegitFishing then
            _G.LegitFishing = false
            if fishThread then
                task.cancel(fishThread)
                fishThread = nil
...
local IslandLocations = {
    ...
    ["Kohana Volcano"] = CFrame.new(-511.570, 18.036, 155.349, 0.259, 0.000, 0.966, 0.000, 1.000, 0.000, -0.966, 0.000, 0.259),
    ...
    ["Lava Basin"] = CFrame.new(891.992, 73.058, -10178.758, -0.045, -0.000, -0.999, -0.000, 1.000, -0.000, 0.999, 0.000, -0.045),
    ["Volcano Cavern"] = CFrame.new(1129.736, 86.611, -10208.650, -0.144, 0.000, -0.990, 0.000, 1.000, 0.000, 0.990, -0.000, -0.144),
    ["Heartfelt Island"] = CFrame.new(1111.694, 4.846, 2718.758, -0.025, -0.000, -1.000, -0.000, 1.000, -0.000, 1.000, 0.000, -0.025),
}
...
local function optimizeParticle(obj)
    pcall(function()
...
local function optimizeSound(obj)
    pcall(function()
...
local function applyBoost()
    ...
    -- 4. OPTIMIZE ALL OBJECTS (Background processing with batches)
    task.spawn(function()
        local descendants = game:GetDescendants()
        local batchSize = 200 -- Process 200 per batch
        ...
        for i = 1, #descendants, batchSize do
            ...
            task.wait() -- Yield after each batch
        end
    end)
    
    -- 5. MONITOR NEW OBJECTS
    if monitorConnection then
        monitorConnection:Disconnect()
    end
    
    monitorConnection = workspace.DescendantAdded:Connect(function(obj)
        if not FPSBoost then return end
        
        task.spawn(function()
            task.wait(0.1) -- Delay sedikit biar object loaded
    ...
    -- 6. MONITOR PLAYER CHARACTER
    local player = game.Players.LocalPlayer
    if player.Character then
        for _, obj in pairs(player.Character:GetDescendants()) do
            optimizeParticle(obj)
            optimizeLight(obj)
            optimizeSound(obj)
        end
    end
    ...
    for obj, properties in pairs(OriginalValues) do
        if obj and (obj.Parent or obj == game:GetService("Lighting") or obj == settings().Rendering) then
            for property, value in pairs(properties) do
                pcall(function()
                    if property == "Decoration" and sethiddenproperty then
                        sethiddenproperty(obj, property, value)
                    else
                        obj[property] = value
                    end
                end)
...
local configManagerSection = Tab9:AddSection("Config Manager")
...
local function getConfigPath(configName)
    return "VictoriaHub/Config/" .. configName .. ".json"
end

local function refreshConfigList()
    _G.ConfigList = {} -- Kosongkan dulu
    if listfiles then
        local files = listfiles("VictoriaHub/Config")
        for _, file in ipairs(files) do
            if file:match("%.json$") then
                local name = file:match("([^/\\]+)%.json$")
                if name then
                    table.insert(_G.ConfigList, name)
                end
            end
        end
    end
    
    -- Jika list kosong, kasih placeholder
    if #_G.ConfigList == 0 then
        _G.ConfigList = {"No configs found"}
    end
    
    return _G.ConfigList
end

local function saveAutoloadSetting(configName)
    if writefile then
        writefile("VictoriaHub/autoload.txt", configName or "")
    end
end

local function loadAutoloadSetting()
    if readfile and isfile and isfile("VictoriaHub/autoload.txt") then
        local content = readfile("VictoriaHub/autoload.txt")
...
configManagerSection:AddInput({
    Title = "Config Name",
    Default = "",
    Placeholder = "Write you input there",
    Callback = function(v)
        currentInputName = v
    end
})
...
local selectConfigDropdown
selectConfigDropdown = configManagerSection:AddDropdown({
    Title = "Select Config",
    Options = {"Select Option"},
    Default = "Select Option",
    Callback = function(v)
        if v ~= "Select Option" and v ~= "No configs found" then
            _G.CurrentConfig = v
        end
    end
})
...
configManagerSection:AddButton({
    ...
    Callback = function()
        local configName = currentInputName ~= "" and currentInputName or _G.CurrentConfig
        
        if configName == "" or configName == "Select Option" or configName == "No configs found" then
            notif("Please enter a config name!", 3, Color3.fromRGB(255, 0, 0))
            return
        end
        
        SaveConfig(configName)
        _G.CurrentConfig = configName
        
        -- Refresh dropdown
        refreshConfigList()
        selectConfigDropdown:Refresh(_G.ConfigList, true)
...
configManagerSection:AddButton({
    ...
    Callback = function()
        if _G.CurrentConfig == "" or _G.CurrentConfig == "Select Option" or _G.CurrentConfig == "No configs found" then
            notif("Please select a config first!", 3, Color3.fromRGB(255, 0, 0))
            return
        end
        
        local loaded = LoadConfigFromFile(_G.CurrentConfig)
        if loaded then
            notif("Config Loaded: " .. _G.CurrentConfig, 3, Color3.fromRGB(0, 255, 0))
        else
            notif("Failed to load config!", 3, Color3.fromRGB(255, 0, 0))
        end
    end
})
...
configManagerSection:AddButton({
    ...
    Callback = function()
        if _G.CurrentConfig == "" or _G.CurrentConfig == "Select Option" or _G.CurrentConfig == "No configs found" then
            notif("Please select a config to delete!", 3, Color3.fromRGB(255, 0, 0))
            return
        end
        
        local filePath = getConfigPath(_G.CurrentConfig)
        
        if delfile and isfile and isfile(filePath) then
            delfile(filePath)
            
            -- Clear autoload jika config yang dihapus adalah autoload
            if _G.AutoloadConfig == _G.CurrentConfig then
                _G.AutoloadConfig = ""
                saveAutoloadSetting("")
            end
            ...
            notif("Config Deleted Successfully", 3, Color3.fromRGB(0, 255, 0))
        else
            notif("Config file not found!", 3, Color3.fromRGB(255, 0, 0))
        end
    end
})
...
configManagerSection:AddButton({
    Title = "Set Autoload",
    Description = "Set selected config to load on startup",
    Callback = function()
        if _G.CurrentConfig == "" or _G.CurrentConfig == "Select Option" or _G.CurrentConfig == "No configs found" then
            notif("Please select a config first!", 3, Color3.fromRGB(255, 0, 0))
            return
        ...
        _G.AutoloadConfig = _G.CurrentConfig
        saveAutoloadSetting(_G.CurrentConfig)
        notif("Autoload Set: " .. _G.CurrentConfig, 3, Color3.fromRGB(0, 255, 0))
    end
})
...
configManagerSection:AddButton({
    ...
    Callback = function()
        refreshConfigList()
        selectConfigDropdown:Refresh(_G.ConfigList, true)
        notif("Config list refreshed", 3, Color3.fromRGB(0, 170, 255))
    end
})
...
configManagerSection:AddButton({
    ...
    Description = "Disable autoload on startup",
    Callback = function()
        _G.AutoloadConfig = ""
        saveAutoloadSetting("")
        notif("Autoload Cleared", 3, Color3.fromRGB(0, 255, 0))
    end
})

-- RESET ALL ELEMENTS TO DEFAULT BUTTON
configManagerSection:AddButton({
    Title = "Reset All Elements to Default",
    Description = "Reset all settings to default values",
    Callback = function()
        -- Reset semua toggle ke false
        _G.AutoFishing = false
        _G.AutoEquipRod = false
        _G.InstantDelay = 0.65
        _G.LegitDelay = 5.5
        
        notif("All settings reset to default", 3, Color3.fromRGB(255, 170, 0))
    end
})
    ...
    Callback = function()
        ...
        if readfile and isfile and isfile(filePath) then
            ...
            if setclipboard then
                ...
                notif("Config exported to clipboard!", 3, Color3.fromRGB(0, 255, 0))
            else
                notif("Clipboard not supported!", 3, Color3.fromRGB(255, 0, 0))
            end
        else
            notif("Config file not found!", 3, Color3.fromRGB(255, 0, 0))
        end
...
task.spawn(function()
    task.wait(0.5) -- Wait untuk UI load
    
    -- Load autoload setting
    _G.AutoloadConfig = loadAutoloadSetting()
    ...
    if _G.AutoloadConfig ~= "" then
        task.wait(1)
        local loaded = LoadConfigFromFile(_G.AutoloadConfig)
        if loaded then
            _G.CurrentConfig = _G.AutoloadConfig
            notif("Autoload: " .. _G.AutoloadConfig, 3, Color3.fromRGB(0, 255, 170))
        else
            _G.AutoloadConfig = ""
            saveAutoloadSetting("")
        end
...
-- DISABLE FIRST LOAD FLAG AFTER 2 SECONDS
task.delay(2, function()
    _G.VictoriaFirstLoad = false
end)

-- WELCOME NOTIFICATION
task.delay(2.5, function()
    notif("Victoria Hub Loaded!", 3, Color3.fromRGB(0, 170, 255))
end)
