...
-- Key improvement: fire langsung dari event, tidak tunggu tick loop
-- ================================================================
...
-- Fire parry LANGSUNG — zero delay
...
local function AutoParry_HookAttackRemote()
    ...
    -- Method 1: CheckInterractable attribute killer → fire LANGSUNG (paling cepat)
    local function hookKillerCheckAttr(char, p)
        local checkObj = char:FindFirstChild("CheckInterractable")
        if not checkObj then return end
        local conn = checkObj:GetAttributeChangedSignal("action"):Connect(function()
            ...
            if myRoot and killerRoot then
                if (killerRoot.Position - myRoot.Position).Magnitude <= Config.PARRY_Dist then
                    AutoParry_TryFire()
                end
            end
        end)
    ...
    -- Method 2: AnimationPlayed → fire LANGSUNG (hapus task.wait(0.5))
    local function hookKillerAnim(char, p)
        -- Tidak ada delay — hook segera setelah char tersedia
        if not char or not char.Parent then return end
        -- Tunggu Humanoid kalau belum ada (max 2 detik)
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not hum then
            local waited = 0
            repeat task.wait(0.1); waited = waited + 0.1
                hum = char:FindFirstChildOfClass("Humanoid")
            until hum or waited >= 2
        end
        ...
        local conn = animator.AnimationPlayed:Connect(function(track)
            ...
            if myRoot and killerRoot then
                ...
                if dist <= threshold then
                    if isConfirmedAttack then
                        AutoParry_TryFire()  -- fire LANGSUNG
                    else
    ...
    -- Method 3: Remote OnClientEvent → fire LANGSUNG
    ...
    local function watchPlayer(p)
        ...
        local function hookChar(char)
            ...
            -- Spawn hookKillerAnim tanpa delay artificial
...
local function CheckKillerApproach(killerRoot, myRoot)
    ...
    if toMe.Magnitude > Config.PARRY_Dist then return false end
...
-- Tick: per-frame (RenderStepped) untuk velocity/approach detect
-- Event methods sudah fire langsung, tick sebagai safety net approach detection
local function AutoParry_Tick(dt)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and IsKiller(player) and player.Character then
            ...
            if (kr.Position - root.Position).Magnitude > Config.PARRY_Dist then continue end
            -- Velocity approach: kalau killer lunge cepat ke arah kita, fire langsung
            if KillerAttackDetected or CheckKillerApproach(kr, root) then
                AutoParry_TryFire()
                return
            end
        end
...
local function AutoParry_Setup()
    ...
    -- RenderStepped: lebih responsif untuk timing-sensitive parry
...
secSurvival:AddToggle({ Title = "Perfect Skill Check", Default = Config.PERFECT_SkillCheck,
    Callback = function(v)
        Config.PERFECT_SkillCheck = v
        if v then pcall(PerfectSC_Setup) else pcall(PerfectSC_Stop) end
        notifToggle("Perfect Skill Check", v)
    end })
