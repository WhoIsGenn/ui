...
D(S.DungJoin, "SelectedDiff",     { Text = "Select Difficulty", Values = {"Easy","Medium","Hard","Extreme"}, Multi = false })
...
T(S.DungBR, "AutoVoteStart",   { Text = "Auto Vote/Start", Default = false })
T(S.DungBR, "DungeonAutofarm", { Text = "Autofarm",        Default = false })

-- Infinite Tower
L(S.DungIT, "Auto join & farm Infinite Tower.")
T(S.DungIT, "AutoInfiniteTower", { Text = "Auto Infinite Tower", Default = false })
T(S.DungIT, "AutoVoteStart",     { Text = "Auto Vote/Start",     Default = false })
T(S.DungIT, "DungeonAutofarm",   { Text = "Autofarm",            Default = false })

-- Crystal Defense
L(S.DungCD, "Auto join & farm Crystal Defense.\n(Same as Infinite Tower)")
T(S.DungCD, "AutoCrystalDefense", { Text = "Auto Crystal Defense", Default = false })
T(S.DungCD, "AutoVoteStart",      { Text = "Auto Vote/Start",      Default = false })
T(S.DungCD, "DungeonAutofarm",    { Text = "Autofarm",             Default = false })

-- Raid
L(S.DungRaid, "Auto join & farm Raid.\n(Same as Boss Rush)")
T(S.DungRaid, "AutoRaid",      { Text = "Auto Raid",       Default = false })
T(S.DungRaid, "AutoVoteStart", { Text = "Auto Vote/Start", Default = false })
T(S.DungRaid, "DungeonAutofarm", { Text = "Autofarm",      Default = false })
...
local function Func_AutoBossRush()
    while Toggles.AutoBossRush.Value do
        ...
        local bossRushUI = PGui:FindFirstChild("BossRushUI")
        if not bossRushUI then task.wait(1); continue end

        local mainFrame = bossRushUI:FindFirstChild("MainFrame")
        if not mainFrame then task.wait(1); continue end

        if mainFrame.Visible then
            -- Ada di lobby BossRush, auto vote/start
            if Toggles.AutoVoteStart.Value then
                local holder = mainFrame:FindFirstChild("Frame", true)
                holder = holder and holder:FindFirstChild("Content", true)
                holder = holder and holder:FindFirstChild("Holder", true)
                local spawnFrame = holder and holder:FindFirstChild("SpawnBossRushPortalFrame")
                local spawnBtn = spawnFrame and spawnFrame:FindFirstChild("SpawnPortalButton")
                if spawnBtn then
                    pcall(function() fire_event(spawnBtn.MouseButton1Click) end)
                    task.wait(1)
                end
            end
        else
            -- Belum di lobby, TP ke Sailor lalu open BossRush
            if Shared.Island ~= "Sailor" then
                Remotes.TP_Portal:FireServer("Sailor")
                Shared.Island = "Sailor"
                task.wait(2.5)
            end
            if tick() - (Shared.LastDungeon or 0) > 10 then
...
-- ============================================================
-- AUTO CRYSTAL DEFENSE (sama seperti Infinite Tower)
-- ============================================================
local function Func_AutoCrystalDefense()
    while Toggles.AutoCrystalDefense.Value do
        task.wait(0.5)

        local cdUI = PGui:FindFirstChild("CrystalDefenseUI")
        if not cdUI then task.wait(1); continue end

        local mainFrame = cdUI:FindFirstChild("MainFrame")
        if not mainFrame then task.wait(1); continue end
        ...
        if mainFrame.Visible then
            ...
            if Toggles.AutoVoteStart.Value then
                ...
                holder = holder and holder:FindFirstChild("Content", true)
                holder = holder and holder:FindFirstChild("Holder", true)
                local spawnFrame = holder and holder:FindFirstChild("SpawnCrystalDefensePortalFrame")
                local spawnBtn = spawnFrame and spawnFrame:FindFirstChild("SpawnPortalButton")
                if spawnBtn then
                    pcall(function() fire_event(spawnBtn.MouseButton1Click) end)
                    task.wait(1)
            ...
            -- TP ke TowerIsland dulu
            if Shared.Island ~= "TowerIsland" then
                Remotes.TP_Portal:FireServer("TowerIsland")
                Shared.Island = "TowerIsland"
                task.wait(2.5)
            end
            -- Open CrystalDefense via NPC
            local npc = NPCs.Merchant and NPCs.Merchant.InfiniteTower
            if npc then
                local prompt = npc:FindFirstChild("HumanoidRootPart")
                    and npc.HumanoidRootPart:FindFirstChild("ProximityPrompt")
                if prompt then
...
-- ============================================================
-- AUTO RAID (sama seperti BossRush)
-- ============================================================
local function Func_AutoRaid()
    while Toggles.AutoRaid.Value do
        task.wait(0.5)

        local raidUI = PGui:FindFirstChild("RaidUI")
        if not raidUI then task.wait(1); continue end

        local mainFrame = raidUI:FindFirstChild("MainFrame")
        if not mainFrame then task.wait(1); continue end
        ...
        if mainFrame.Visible then
            if Toggles.AutoVoteStart.Value then
                ...
                holder = holder and holder:FindFirstChild("Content", true)
                holder = holder and holder:FindFirstChild("Holder", true)
                local spawnFrame = holder and holder:FindFirstChild("SpawnRaidPortalFrame")
                local spawnBtn = spawnFrame and spawnFrame:FindFirstChild("SpawnPortalButton")
                if spawnBtn then
                    pcall(function() fire_event(spawnBtn.MouseButton1Click) end)
                    task.wait(1)
            ...
            -- TP ke Boss island untuk Raid
            if Shared.Island ~= "Boss" then
                Remotes.TP_Portal:FireServer("Boss")
                Shared.Island = "Boss"
                task.wait(2.5)
            end
            if tick() - (Shared.LastRaid or 0) > 10 then
                Remotes.OpenDungeon:FireServer("Raid")
                Shared.LastRaid = tick()
                task.wait(1)
            end
        end
    end
...
-- ============================================================
-- AUTO VOTE START (Dungeon - klik tombol Vote/Start)
-- ============================================================
        ...
        pcall(function()
            ...
            if dungUI then
                local voted = dungUI:FindFirstChild("ContentFrame", true)
                    and dungUI.ContentFrame:FindFirstChild("PeopleVoted", true)
                    and dungUI.ContentFrame.PeopleVoted:FindFirstChild("ButtonFrame", true)
                    and dungUI.ContentFrame.PeopleVoted.ButtonFrame:FindFirstChild("Voted")
                if voted and voted.Visible then
                    pcall(function() fire_event(voted.MouseButton1Click) end)
                end
            end
        end)
    end
