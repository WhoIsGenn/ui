...
-- =========================
-- FORWARD DECLARATION (FIX ERROR LINE 1)
-- =========================
local function createTradeUI() end

-- =========================
-- LOAD SERVICES (ASYNC SAFE)
-- =========================
task.spawn(function()
    local RS = game:GetService("ReplicatedStorage")
    ...
    if not ok1 or not ok2 then
        warn("[AutoTrade] Failed to load ItemUtility / Replion")
        return
    end
...
local function sendTrade(targetPlayer, uuid)
    ...
    return pcall(function()
        if uuid then
            ...
            net["RF/InitiateTrade"]:InvokeServer(targetPlayer.UserId)
        end
    end)
end
