...
-- AUTO PARRY
-- Fire parry:FireServer() saat killer dalam attack range
...
local function AutoParry_Fire()
    ...
    pcall(function()
        ...
        local parryRE = dagger and dagger:FindFirstChild("parry")
        if parryRE then parryRE:FireServer() end
    end)
end
...
local function AutoParry_Setup()
    ...
    ParryAttackConn = RunService.Heartbeat:Connect(function()
        if not Config.AUTO_Parry then
            ParryAttackConn:Disconnect(); ParryAttackConn = nil; return
        end
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and IsKiller(p) and p.Character then
                ...
                if kr and (kr.Position - root.Position).Magnitude <= 8 then
                    AutoParry_Fire()
                    return
                end
            end
...
local function AutoParry()
    if Config.AUTO_Parry and GetRole() == "Survivor" then
        if not ParryAttackConn then AutoParry_Setup() end
