...
local Config = {
    AutoParry = false,
    ParryMode = "Time",          -- 🚀 Default Time Mode
    TargetTime = 0.5,            -- 🚀 Default 0.5 detik
    DistanceTiming = 100, 
...
local function SafeGetEmergencyDistance(speed)
    ...
    if type(GetEmergencyDistance) == "function" then
        ...
        if success and type(result) == "number" then
            local cap = (speed < 200) and 55 or 40
            return math.min(math.max(result, math.clamp(8 + (speed * 0.09), 10, cap)), cap)
        end
    end
...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    local parryMode = cfg.ParryMode or "Time"
    local distanceTiming = cfg.DistanceTiming or 100
    local targetTime = cfg.TargetTime or 0.5
        ...
        if autoParryEnabled and allowedToHit then
            ...
            if Speed > 470 then
                ...
                if parryMode == "Curve" then
                    local dynamicReachTime
                    if Speed < 100 then dynamicReachTime = 0.55
                    elseif Speed < 300 then dynamicReachTime = 0.42
                    elseif Speed < 600 then dynamicReachTime = 0.32
                    else dynamicReachTime = 0.25 end
                    
                    local dynamicDot = (Speed < 200) and -0.55 or -0.40
                    local dynamicDist = (Speed < 200) and 55 or 40
                    
                    local maxCurveDist = (Speed > 470) and 35 or 25
                    ...
                    if bypassAntiCurve then
                        ...
                        if Distance <= strikeZone or reachTime <= dynamicReachTime then
                            if dotProduct > dynamicDot or Distance <= dynamicDist then
                                triggerParry = true
                            end
                        end
                    end
                elseif parryMode == "Distance" then
                    ...
                    if Distance <= distanceThreshold or reachTime <= dynamicReachTime or Distance <= emergencyDist then
                        triggerParry = true
                    end
                elseif parryMode == "Time" then
                    -- 🚀 TIME MODE: Waktu-based + slow bonus untuk bola lambat
                    local baseTime = targetTime
                    if baseTime < 0.1 then baseTime = 0.5 end
                    ...
                    local dynamicTimeWindow = baseTime + slowBonus + (pingSeconds * 1.25)
                    
                    if dynamicTimeWindow < 0.35 then
                        dynamicTimeWindow = 0.35
                    end
                    
                    if reachTime <= dynamicTimeWindow then
                        triggerParry = true
                    end
                end
            end
...
-- ═══ ADVANCED HUMAN EMULATION AUTO PLAY BOT ═══
local currentMode = 1
local modeTimer = 0
...
-- 🚀 PARRY MODE DROPDOWN
createSelect("Parry Mode", Config.ParryMode or "Time", {
    "Time", "Curve", "Distance"
}, combatScroll, function(selected)
    Config.ParryMode = selected
    if type(CustomNotify) == "function" then
        CustomNotify("⚙️ Parry Mode: " .. selected, 2)
    end
end)
...
createLabel("Time Mode: Reaction Time (detik sebelum bola sampai)", combatScroll)
createTextBox("Target Time (0.3-1.0)", tostring(Config.TargetTime), combatScroll, function(val)
    local n = tonumber(val)
    if n and n > 0 and n < 3 then 
        Config.TargetTime = n 
        CustomNotify("⏱️ Target Time: " .. tostring(n) .. "s", 2) 
    else
        CustomNotify("❌ Invalid (0.1-2.0)", 2)
    end
end)
