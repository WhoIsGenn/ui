...
local System = {
    __properties = {
        ...
        __predictive_parry_enabled = false
    },
    __config = {
        __detections = {
...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry=RunService.PreSimulation:Connect(function()
        ...
        for _,ball in pairs(balls) do
            ...
            local range_offset = System.__properties.__parry_range_offset or 0
            local speed_scale = math.clamp(1 - (speed / 200), 0.1, 1)
            parry_accuracy = parry_accuracy + (range_offset * speed_scale)

            local predictive_trigger = false
            if System.__properties.__predictive_parry_enabled and speed > 0 then
                local ping_sec = Stats.Network.ServerStatsItem['Data Ping']:GetValue() / 1000
                local reach_time = distance / speed
                predictive_trigger = (ball_target == LocalPlayer.Name) and (reach_time <= (ping_sec * 1.5 + 0.08))
            end

            if ball_target==LocalPlayer.Name and (distance <= parry_accuracy or predictive_trigger) then
        ...
        if training_ball then
            ...
            if zoomies then
                ...
                if not System.__properties.__training_parried then
                    ...
                    local speed_divisor=(2.4+capped_speed_diff*0.002)*System.__properties.__divisor_multiplier
                    local parry_accuracy=ping_threshold+math.max(speed/speed_divisor,9.5)
                    local t_range_offset = System.__properties.__parry_range_offset or 0
                    local t_speed_scale = math.clamp(1 - (speed / 200), 0.1, 1)
                    parry_accuracy = parry_accuracy + (t_range_offset * t_speed_scale)
                    local t_predictive = false
                    if System.__properties.__predictive_parry_enabled and speed > 0 then
                        local t_ping_sec = Stats.Network.ServerStatsItem['Data Ping']:GetValue() / 1000
                        local t_reach_time = distance / speed
                        t_predictive = (ball_target == LocalPlayer.Name) and (t_reach_time <= (t_ping_sec * 1.5 + 0.08))
                    end
                    if ball_target==LocalPlayer.Name and (distance <= parry_accuracy or t_predictive) then
                        if getgenv().AutoParryMode=="Keypress" then System.parry.keypress()
                        else System.parry.execute_action() end
...
AutoParryGroup:create_slider("Kittylol_AutoParry_RangeOffset", {
    title = "Parry Range",
    minimum = 1,
    maximum = 10,
    default = 1,
    rounding = true,
    callback = function(value)
        System.__properties.__parry_range_offset = value - 1
    end
})

AutoParryGroup:create_toggle("Kittylol_AutoParry_Predictive", {
    title = "Predictive Parry",
    default = false,
    callback = function(state)
        System.__properties.__predictive_parry_enabled = state
    end
})
