...
local Config = {
    AutoParry = false,
    DistanceTiming = 50,          -- Accuracy (1-100)
    ParryRange = 1.0,              -- Parry Range multiplier (1.0 - 10.0)
...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    local autoSpamEnabled = Config.AutoSpam
    local distanceTiming = Config.DistanceTiming or 50       -- Accuracy
    local parryRangeMult = Config.ParryRange or 1.0          -- Parry Range multiplier
    ...
    for idx, Ball in ipairs(balls) do
        ...
        if autoParryEnabled and allowedToHit then
            ...
            -- ═══ ACCURACY (timing dasar dari ping + speed) ═══
            ...
            -- ═══ PARRY RANGE MULTIPLIER (0.1 - 10.0) ═══
            -- Multiplier ini ngali accuracy buat dapetin range final
            local parryAccuracy = accuracy * parryRangeMult

            if reachTime <= 0.05 then triggerParry = true end

            if Speed > 470 then
                local triggerTimeThreshold = pingSeconds + 0.34
                if reachTime <= triggerTimeThreshold or Distance <= parryAccuracy then
                    triggerParry = true
                end
            else
...
AutoParrySec:AddSlider({
    Title = "Accuracy",
    Description = "Timing accuracy (1 = mepet, 100 = balance)",
    Flag = "accuracy",
    Min = 1,
    Max = 100,
    Default = 50,
...
AutoParrySec:AddSlider({
    Title = "Parry Range",
    Description = "Multiplier radius parry (1.0 = normal, 10.0 = 10x lebih jauh)",
    Flag = "parryrange",
    Min = 1.0,
    Max = 10.0,
    Default = 1.0,
