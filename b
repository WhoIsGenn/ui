...
if not Window then
    error("[Victoria] Vict:Window() return nil — cek VictUI library lu!")
end

-- ==================== NOTIF HELPER (FIX) ====================
local function notif(msg, delay, color)
...
if not Tab1 then warn("[Victoria] AddTab 'Auto Parry' gagal") end

-- SECTION: Main
local mainSection = Tab1:AddSection({ Title = "Main" })
...
-- SECTION: Protections
local protSection = Tab1:AddSection({ Title = "Protections" })
...
-- SECTION: Detections
local detectionSection = Tab1:AddSection({ Title = "Detections" })
...
if not Tab2 then warn("[Victoria] AddTab 'Curve' gagal") end

local curveSection = Tab2:AddSection({ Title = "Curve Mode" })
...
if not Tab3 then warn("[Victoria] AddTab 'Visuals' gagal") end

local visualSection = Tab3:AddSection({ Title = "Dynamic Hitbox Visual" })
...
if not Tab4 then warn("[Victoria] AddTab 'Misc' gagal") end

local miscSection = Tab4:AddSection({ Title = "Controls" })
...
miscSection:AddButton({
    Title    = "Test Notify",
    Callback = function()
        notif("Victoria Bladeball v2.0 aktif!", 3)
    end
})

-- ==================== FINAL ====================
task.delay(1.5, function()
    notif("Victoria Blade Ball v2.0 loaded!", 4)
end)
