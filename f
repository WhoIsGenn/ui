...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry=RunService.PreSimulation:Connect(function()
        ...
        for _,ball in pairs(balls) do
            ...
            local parry_accuracy=ping_threshold+base_accuracy+range_bonus
...
local function CreateSmallActionUI(kind)
    ...
    local dragStart
    local startPos
    local isMobile = System.__properties.__is_mobile
    local dragThreshold = isMobile and 4 or 6
    local dragInput = nil
    ...
    connections.inputBegan = UserInputService.InputBegan:Connect(function(input)
        ...
        local eligible = isMobile and isOnFrame(input.Position) or isOnBorder(input.Position)
    ...
    connections.inputEnded = UserInputService.InputEnded:Connect(function(input)
        if input == dragInput
            ...
            if isMobile and not dragging and (os.clock() - touchStartTime) < 0.2 then
                doButtonAction()
            end
            dragging = false
            dragCandidate = false
...
local function CreateCurveUI()
    ...
    local dragInput
    local dragCandidate = false
    local curveDragThreshold = System.__properties.__is_mobile and 5 or 3
    local curveTouchStartTime = 0
    ...
    CurveConnections.inputChanged = UserInputService.InputChanged:Connect(function(input)
        ...
        local delta = input.Position - dragStart
        if not dragging then
            if math.abs(delta.X) < curveDragThreshold and math.abs(delta.Y) < curveDragThreshold then return end
            dragging = true
        end
        Main.Position = UDim2.new(
            startPos.X.Scale, startPos.X.Offset + delta.X,
    ...
    CurveConnections.inputEnded = UserInputService.InputEnded:Connect(function(input)
        if input == dragInput
            or input.UserInputType == Enum.UserInputType.MouseButton1
            or input.UserInputType == Enum.UserInputType.Touch then
            if System.__properties.__is_mobile and not dragging and (os.clock() - curveTouchStartTime) < 0.2 then
                if expanded then
                    task.spawn(closeUI)
                else
                    task.spawn(openUI)
...
-- ═══ VISUAL: UNLOCK ALL ══════════════════════════════════════════
local VisualUnlockGroup = Visual:create_group("Unlock All", "left")
VisualUnlockGroup:create_toggle("unlock_all", {
    title = "Unlock All",
    default = false,
    callback = function(state)
...
-- ═══ VISUAL: SWORD CHANGER (from parrytes) ════════════════════════
local VisualSwordGroup = Visual:create_group("Sword Changer", "right")
do
    local _sc_rs = ReplicatedStorage
    local _sc_swordInstancesModule = _sc_rs:WaitForChild("Shared", 9e9)
        :WaitForChild("ReplicatedInstances", 9e9)
        :WaitForChild("Swords", 9e9)
    local _sc_swordInstances = nil
    local _sc_swordsController = nil

    task.spawn(function()
        local ok, si = pcall(require, _sc_swordInstancesModule)
        if ok then _sc_swordInstances = si end

        task.spawn(function()
            while task.wait() and not _sc_swordsController do
                local ok2, conns = pcall(getconnections, _sc_rs.Remotes.FireSwordInfo.OnClientEvent)
                if ok2 and conns then
                    for _, v in ipairs(conns) do
                        if v.Function and islclosure and islclosure(v.Function) then
                            local ok3, up = pcall(getupvalues, v.Function)
                            if ok3 and #up == 1 and type(up[1]) == "table" then
                                _sc_swordsController = up[1]; break
                            end
                        end
                    end
                end
        ...
        local function _sc_getSlashName(swordName)
            if not _sc_swordInstances then return "SlashEffect" end
            local ok4, sln = pcall(function() return _sc_swordInstances:GetSword(swordName) end)
            return (ok4 and sln and sln.SlashName) or "SlashEffect"
        end

        local function _sc_refreshSlashName()
            local fxName = getgenv().swordFX ~= "" and getgenv().swordFX or getgenv().swordModel
            getgenv().slashName = fxName ~= "" and _sc_getSlashName(fxName) or "SlashEffect"
        end

        local function _sc_setSword()
            if not getgenv().skinChanger then return end
            if not LocalPlayer.Character then return end
            if not _sc_swordInstances then return end
            pcall(function()
                local f = rawget(_sc_swordInstances, "EquipSwordTo")
                if type(f) == "function" then
                    local ups = getupvalues(f)
                    for i = 1, #ups do if type(ups[i]) == "boolean" then setupvalue(f, i, false); break end end
                end
            end)
            pcall(function() _sc_swordInstances:EquipSwordTo(LocalPlayer.Character, getgenv().swordModel) end)
            task.spawn(function()
                local attempts = 0
                while not _sc_swordsController and attempts < 20 do task.wait(0.5); attempts += 1 end
                if not _sc_swordsController then return end
                pcall(function()
                    if _sc_swordsController.SetSword then
                        _sc_swordsController:SetSword(getgenv().swordAnimations ~= "" and getgenv().swordAnimations or getgenv().swordModel)
                    end
                end)
                pcall(function()
                    local targetSword = getgenv().swordFX ~= "" and getgenv().swordFX or getgenv().swordModel
                    if _sc_rs.Remotes:FindFirstChild("FireSwordInfo") then _sc_rs.Remotes.FireSwordInfo:FireServer(targetSword) end
                    if _sc_swordsController.currentSword ~= nil then _sc_swordsController.currentSword = targetSword end
                    if _sc_swordsController.SwordFX ~= nil then _sc_swordsController.SwordFX = targetSword end
                end)
            end)
        end

        local _sc_hookedFuncs = {}
        task.spawn(function()
            while task.wait(1) do
                local ok5, conns = pcall(getconnections, _sc_rs.Remotes.ParrySuccessAll.OnClientEvent)
                if ok5 and type(conns) == "table" then
                    for _, v in ipairs(conns) do
                        local func = v.Function
                        if func and not _sc_hookedFuncs[func] then
                            if isourclosure and isourclosure(func) then
                                _sc_hookedFuncs[func] = true
                            else
                                _sc_hookedFuncs[func] = true
                                v:Disable()
                                local targetFunc = func
                                ...
                                _sc_hookedFuncs[ourFunc] = true
                                _sc_rs.Remotes.ParrySuccessAll.OnClientEvent:Connect(ourFunc)
                            end
                        end
                    end
                end
        ...
        getgenv().updateSword = function() _sc_refreshSlashName(); _sc_setSword() end
        ...
        task.spawn(function()
            while task.wait(1) do
                if getgenv().skinChanger and getgenv().swordModel ~= "" then
                    local char = LocalPlayer.Character
                    if char then
                        if LocalPlayer:GetAttribute("CurrentlyEquippedSword") ~= getgenv().swordModel then _sc_setSword() end
                        if not char:FindFirstChild(getgenv().swordModel) then _sc_setSword() end
    ...
    VisualSwordGroup:create_toggle("Kittylol_Visual_SkinChanger", {
        title = "Skin Changer",
        default = false,
        callback = function(state)
            getgenv().skinChanger = state
            if getgenv().setSkinChangerToggleUI then getgenv().setSkinChangerToggleUI(state) end
            pcall(function() if state and getgenv().updateSword then getgenv().updateSword() end end)
        end,
    })

    VisualSwordGroup:create_input("Kittylol_Visual_SwordModel", {
        title = "Sword Name",
        ...
        callback = function(value)
            ...
            pcall(function() if getgenv().updateSword then getgenv().updateSword() end end)
        end,
    })
end

-- ═══ VISUAL: HEADLESS & KORBLOX (from parrytes) ═══════════════════
local VisualAppearanceGroup = Visual:create_group("Appearance", "left")
do
    local _ap_isHeadless = false
    local _ap_isKorblox = false

    local function _ap_applyHeadless(char)
        if not char then return end
        local head = char:FindFirstChild("Head")
        if not head then return end
        pcall(function()
            head.Transparency = _ap_isHeadless and 1 or 0
            for _, child in ipairs(head:GetChildren()) do
                if child:IsA("Decal") then child.Transparency = _ap_isHeadless and 1 or 0 end
            end
        end)
    end

    local function _ap_applyKorblox(char)
        if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not hum then return end
        pcall(function()
            if char:FindFirstChild("RightLowerLeg") and char:FindFirstChild("RightFoot") then
                char:FindFirstChild("RightLowerLeg").Transparency = _ap_isKorblox and 1 or 0
                char:FindFirstChild("RightFoot").Transparency = _ap_isKorblox and 1 or 0
            elseif char:FindFirstChild("Right Leg") then
                char:FindFirstChild("Right Leg").Transparency = _ap_isKorblox and 1 or 0
            end
        end)
        if _ap_isKorblox then
            pcall(function()
    ...
    LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        if not hum then return end
        task.wait(0.3)
        if _ap_isHeadless then _ap_applyHeadless(char) end
        if _ap_isKorblox then _ap_applyKorblox(char) end
    end)

    VisualAppearanceGroup:create_toggle("Kittylol_Visual_Headless", {
        title = "Headless",
        default = false,
        callback = function(state)
            _ap_isHeadless = state
            System.__properties.__headless_enabled = state
            _ap_applyHeadless(LocalPlayer.Character)
        end,
    })

    VisualAppearanceGroup:create_toggle("Kittylol_Visual_Korblox", {
        title = "Korblox Leg",
        default = false,
        callback = function(state)
            _ap_isKorblox = state
            System.__properties.__korblox_enabled = state
            _ap_applyKorblox(LocalPlayer.Character)
        end,
    })
end
