...
local _Win = Library:Window({
    Title         = "VictoriaHub",
    Footer        = "autoparry",
    Content       = "VictoriaHub",
    Color         = Color3.fromRGB(0, 170, 210),
    ...
    Search        = true,
    Config = {
        ConfigFolder  = "VictoriaHub/Config/" .. tostring(game.PlaceId) .. "/",
...
if not _tokenFound then
    Notify("VictoriaHub", "Remote not found! Re-inject vÃ  cháº¡y láº¡i.", 5)
    return
end
...
task.spawn(function()
    ...
    if _capturedRemote then
        Notify("VictoriaHub", "Anti-cheat Bypassed", 3)
    end
end)
...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry=RunService.PreSimulation:Connect(function()
        ...
        for _,ball in pairs(balls) do
            ...
            local pingRaw=Stats.Network.ServerStatsItem['Data Ping']:GetValue()
            local ping=pingRaw/10
            local pingSeconds=math.clamp(pingRaw/1000, 0.01, 0.35)
            local speed=velocity.Magnitude

            -- Direction check: is ball heading toward player?
            local toPlayer=(playerPos-ballPos)
            local dotProduct=speed>0.1 and velocity.Unit:Dot(toPlayer.Unit) or 0

            -- Time-to-reach: how many seconds until ball arrives
            local time_to_reach=speed>0.1 and (distance/speed) or 999

            -- Trigger window: ping + execution buffer + range-offset bonus time
            local range_offset=System.__properties.__parry_range_offset
            local exec_buffer=0.09 + (range_offset/10)*0.12
            local trigger_window=pingSeconds+exec_buffer

            -- Legacy distance fallback for very slow balls or edge cases
            local ping_threshold=math.clamp(ping/10, 5, 17)
            local capped_speed_diff=math.min(math.max(speed-9.5,0),850)
            local speed_divisor=(2.4+capped_speed_diff*0.0016)*System.__properties.__divisor_multiplier
            local base_accuracy=math.max(speed/speed_divisor, 9.5)
            -- Wider range bonus: scale up to 3x base_accuracy at offset=10
            local range_bonus=(range_offset/10)*(base_accuracy*3.0)
            local distance_accuracy=ping_threshold+base_accuracy+range_bonus

            -- Trajectory prediction: project ball position forward by ping time,
            -- check if it lands within a capture radius around the player
            local capture_radius = 8 + (range_offset/10)*22
            local predicted_pos = ballPos + velocity * (pingSeconds + exec_buffer)
            local predicted_dist = (playerPos - predicted_pos).Magnitude
            local trajectory_capture = predicted_dist <= capture_radius

            -- Primary: trajectory-based trigger (ball heading to player AND arrives soon OR predicted to hit)
            -- Secondary: legacy distance trigger (catches edge cases, slow balls)
            local trajectory_trigger=((dotProduct>0.25) and (time_to_reach<=trigger_window)) or trajectory_capture
            local distance_trigger=(distance<=distance_accuracy)
            local parry_accuracy=distance_accuracy -- kept for ability check below
            local should_parry=trajectory_trigger or distance_trigger

            -- Anywhere-ball detection: if range_offset >= 1, also parry any ball whose
            -- predicted path passes within capture_radius of player regardless of target attribute
            local anywhere_parry = (range_offset >= 1) and trajectory_capture
            ...
            if (ball_target==LocalPlayer.Name and should_parry) or anywhere_parry then
            ...
            if ball_target==LocalPlayer.Name and should_parry then
        ...
        if training_ball then
            ...
            if zoomies then
                ...
                if not System.__properties.__training_parried then
                    ...
                    local velocity=zoomies.VectorVelocity
                    local tbPlayerPos=LocalPlayer.Character and LocalPlayer.Character.PrimaryPart and LocalPlayer.Character.PrimaryPart.Position or Vector3.zero
                    local tbBallPos=training_ball.Position
                    local distance=(tbPlayerPos-tbBallPos).Magnitude
                    local speed=velocity.Magnitude
                    local pingRaw=Stats.Network.ServerStatsItem['Data Ping']:GetValue()
                    local ping=pingRaw/10
                    local pingSeconds=math.clamp(pingRaw/1000, 0.01, 0.35)
                    local tbToPlayer=(tbPlayerPos-tbBallPos)
                    local tbDot=speed>0.1 and velocity.Unit:Dot(tbToPlayer.Unit) or 0
                    local tbTTR=speed>0.1 and (distance/speed) or 999
                    local tbExecBuf=0.09
                    local tbWindow=pingSeconds+tbExecBuf
                    local tb_range_offset=System.__properties.__parry_range_offset
                    local ping_threshold=math.clamp(ping/10, 5, 17)
                    local capped_speed_diff=math.min(math.max(speed-9.5,0),850)
                    local speed_divisor=(2.4+capped_speed_diff*0.0016)*System.__properties.__divisor_multiplier
                    local base_accuracy=math.max(speed/speed_divisor,9.5)
                    local range_bonus=(tb_range_offset/10)*(base_accuracy*3.0)
                    local distance_accuracy=ping_threshold+base_accuracy+range_bonus
                    local tb_capture_radius=8+(tb_range_offset/10)*22
                    local tb_predicted=tbBallPos+zoomies.VectorVelocity*(pingSeconds+tbExecBuf)
                    local tb_pred_dist=(tbPlayerPos-tb_predicted).Magnitude
                    local tb_traj=((tbDot>0.25) and (tbTTR<=tbWindow)) or (tb_pred_dist<=tb_capture_radius)
                    local tb_dist=(distance<=distance_accuracy)
                    local tb_should_parry=tb_traj or tb_dist
                    if ball_target==LocalPlayer.Name and tb_should_parry then
                        if getgenv().AutoParryMode=="Keypress" then System.parry.keypress()
                        else System.parry.execute_action() end
...
local function stopManualKeyboardSpam()
    if KeyboardSettings.ManualSpamMode == "Hold to Spam" and System.__properties.__manual_spam_enabled then
        ...
        if _G.VictoriaHub then _G.VictoriaHub.manualSpamEnabled = false end
        System.manual_spam.stop()
...
local function toggleManualKeyboardSpam()
    ...
    _G.VictoriaHub = _G.VictoriaHub or {}
    _G.VictoriaHub.manualSpamEnabled = state
...
local function CreateKeyboardUI()
    ...
    gui.Name = "VictoriaHub_KeyboardUI"
        ...
        elseif input.KeyCode == KeyboardSettings.ManualSpam then
            if KeyboardSettings.ManualSpamMode == "Press Once to Spam" then
                ...
                if not System.__properties.__manual_spam_enabled then
                    ...
                    _G.VictoriaHub = _G.VictoriaHub or {}
                    _G.VictoriaHub.manualSpamEnabled = true
                    System.manual_spam.start()
                end
            end
        end
...
local function CreateSmallActionUI(kind)
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = isTrigger and "VictoriaHub_TriggerUI" or "VictoriaHub_SpamUI"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.Parent = CoreGui
    ...
    if isTrigger then
        ...
        _G.manualSpamEnabled = false
        _G.VictoriaHub = _G.VictoriaHub or {}
        _G.VictoriaHub.manualSpamEnabled = false
    end
    ...
    local function isOnFrame(inputPosition)
        ...
        return x >= 0 and x <= absoluteSize.X and y >= 0 and y <= absoluteSize.Y
    end
    ...
    connections.inputBegan = UserInputService.InputBegan:Connect(function(input)
        ...
        if not isOnFrame(input.Position) then
            dragCandidate = false
            return
        end
        dragCandidate = true
    ...
    connections.inputChanged = UserInputService.InputChanged:Connect(function(input)
        ...
        if not dragging then
            -- threshold harus dilewati dulu baru drag aktif (biar tap tetap jalan)
    ...
    connections.inputEnded = UserInputService.InputEnded:Connect(function(input)
        if input == dragInput
            ...
            -- jika dragging aktif, konsumsi event biar tap button tidak ikut trigger
...
-- ══════════════════════════════════════════════════
-- VelarisUI: Tab & Section setup
-- ══════════════════════════════════════════════════
local CombatTab = _Win:AddTab({ Name = "Combat", Icon = "76499042599127" })

-- ── Auto Parry ──
local AutoParrySec = CombatTab:AddSection({ Title = "Auto Parry", Open = false })

AutoParrySec:AddToggle({
    Flag    = "VictoriaHub_AutoParry",
    Title   = "Auto Parry",
    Default = false,
...
AutoParrySec:AddDropdown({
    Flag    = "VictoriaHub_AutoParry_Mode",
    Title   = "Parry Mode",
...
AutoParrySec:AddSlider({
    Flag    = "VictoriaHub_AutoParry_Accuracy",
    Title   = "Accuracy",
    Min     = 1,
    Max     = 100,
    Default = 50,
...
AutoParrySec:AddSlider({
    Flag      = "VictoriaHub_AutoParry_Range",
    Title     = "Parry Range",
    Min       = 1,
    Max       = 10,
    Default   = 1,
    Increment = 0.1,
    Callback  = function(value)
        System.__properties.__parry_range_offset = value
    end,
})

AutoParrySec:AddToggle({
    Flag    = "VictoriaHub_AutoParry_RandomAccuracy",
...
AutoParrySec:AddToggle({
    Flag    = "VictoriaHub_AutoParry_AutoAbility",
    Title   = "Auto Ability",
    Default = false,
    Callback = function(state)
...
-- ── Curve UI ──
local CurveUISec = CombatTab:AddSection({ Title = "Curve UI", Open = false })

CurveUISec:AddToggle({
    Flag    = "VictoriaHub_CurveUI",
    Title   = "Curve UI",
    Default = false,
    Callback = function(state)
        if state then CreateCurveUI(); Notify("Curve UI", "ON", 2)
        else DestroyCurveUI(); Notify("Curve UI", "OFF", 2) end
    end,
})

CurveUISec:AddToggle({
    Flag    = "VictoriaHub_CurveUI_Random",
    Title   = "Random Curve",
    Default = false,
    Callback = function(state)
        if state then
...
-- ── Keyboard ──
local KeyboardSec = CombatTab:AddSection({ Title = "Keyboard", Open = false })

KeyboardSec:AddToggle({
    Flag    = "VictoriaHub_Keyboard_ShowUI",
    Title   = "Keyboard UI",
    Default = false,
    Callback = function(state)
        if state then CreateKeyboardUI(); Notify("Keyboard", "ON", 2)
        else DestroyKeyboardUI(); Notify("Keyboard", "OFF", 2) end
    end,
})

-- ── Triggerbot ──
local TriggerbotSec = CombatTab:AddSection({ Title = "Triggerbot", Open = false })

TriggerbotSec:AddToggle({
    Flag    = "VictoriaHub_Triggerbot",
    Title   = "Triggerbot UI",
    Default = false,
    Callback = function(state)
        if state then CreateTriggerUI(); Notify("Triggerbot", "ON", 2)
        else DestroyTriggerUI(); Notify("Triggerbot", "OFF", 2) end
    end,
})

-- ── Detection ──
local DetectSec = CombatTab:AddSection({ Title = "Detection", Open = false })

DetectSec:AddToggle({ Flag="VictoriaHub_Detect_Infinity",     Title="Infinity",        Default=false, Callback=function(s) System.__config.__detections.__infinity=s end })
DetectSec:AddToggle({ Flag="VictoriaHub_Detect_Deathslash",   Title="Death Slash",     Default=false, Callback=function(s) System.__config.__detections.__deathslash=s end })
DetectSec:AddToggle({ Flag="VictoriaHub_Detect_Timehole",     Title="Time Hole",       Default=false, Callback=function(s) System.__config.__detections.__timehole=s end })
DetectSec:AddToggle({ Flag="VictoriaHub_Detect_Slashesoffury",Title="Slashes Of Fury", Default=false, Callback=function(s) System.__config.__detections.__slashesoffury=s end })
DetectSec:AddToggle({ Flag="VictoriaHub_Detect_Phantom",      Title="Phantom",         Default=false, Callback=function(s) System.__config.__detections.__phantom=s end })

-- ── Auto Spam ──
local AutoSpamSec = CombatTab:AddSection({ Title = "Auto Spam", Open = false })

AutoSpamSec:AddToggle({
    Flag    = "VictoriaHub_AutoSpam",
    Title   = "Auto Spam",
    Default = false,
    Callback = function(state)
        System.__properties.__auto_spam_enabled = state
...
AutoSpamSec:AddDropdown({
    Flag    = "VictoriaHub_AutoSpam_Mode",
    Title   = "Spam Mode",
    Options = {"Remote", "Keypress"},
    Default = "Remote",
...
AutoSpamSec:AddToggle({
    Flag    = "VictoriaHub_AutoSpam_AnimationFix",
    Title   = "Animation Fix",
    Default = true,
    Callback = function(state) getgenv().AutoSpamAnimationFix = state end,
})

AutoSpamSec:AddToggle({
    Flag    = "VictoriaHub_AutoSpam_Notify",
    Title   = "Notify",
    Default = false,
    Callback = function(state) getgenv().AutoSpamNotify = state end,
})

AutoSpamSec:AddSlider({
    Flag    = "VictoriaHub_AutoSpam_Rate",
    Title   = "Spam Rate",
    Min     = 60,
    Max     = 5000,
    Default = 240,
...
-- ── Manual Spam ──
local ManualSpamSec = CombatTab:AddSection({ Title = "Manual Spam", Open = false })

ManualSpamSec:AddToggle({
    Flag    = "VictoriaHub_ManualSpam_ShowUI",
...
ManualSpamSec:AddDropdown({
    Flag    = "VictoriaHub_ManualSpam_Mode",
    Title   = "Spam Mode",
    Options = {"Remote", "Keypress"},
    Default = "Remote",
...
ManualSpamSec:AddToggle({
    Flag    = "VictoriaHub_ManualSpam_AnimationFix",
    Title   = "Animation Fix",
    Default = true,
    Callback = function(state) getgenv().ManualSpamAnimationFix = state; macroAnimFix = state end,
})
...
local Visual  = _Win:AddTab({ Name = "Visual",  Icon = "7733774602" })
local MiscTab = _Win:AddTab({ Name = "Misc",    Icon = "132243429647479" })
...
local World    = _Win:AddTab({ Name = "World",    Icon = "7733954760" })
local Immortal = _Win:AddTab({ Name = "Immortal", Icon = "7733765398" })
local ImmortalSec = Immortal:AddSection({ Title = "Immortal", Open = false })

local ConfigTab = _Win:AddTab({ Name = "Config", Icon = "lucide:folder" })
Library:AddConfigSection(ConfigTab, {
    Name       = "Configuration",
    FolderName = "VictoriaHub",
})
...
local function ClearKittyWorldEffects()
    for _,object in ipairs(game:GetService("Lighting"):GetChildren()) do
        if object.Name=="VictoriaHubSky" or object.Name=="VictoriaHubAtmosphere" or object.Name=="VictoriaHubColorCorrection" or object.Name=="VictoriaHubBloom" then
            object:Destroy()
        end
    end
end
...
local function ApplyWorldPreset(preset)
    ...
    atmosphere.Name="VictoriaHubAtmosphere"
    ...
    correction.Name="VictoriaHubColorCorrection"
...
local WorldSkyChangerSec = World:AddSection({ Title = "Sky Changer", Open = false })
...
WorldSkyChangerSec:AddToggle({
    Flag    = "world_sky_changer",
    Title   = "Sky Changer",
    Default = false,
    Callback = function(value)
        WorldEnabled=value
        if value then
            for _,preset in ipairs(WorldPresets) do
                if preset.Name==WorldSelected then ApplyWorldPreset(preset); break end
            end
        else
            ClearKittyWorldEffects()
        end
...
WorldSkyChangerSec:AddDropdown({
    Flag    = "world_sky_preset",
    Title   = "Sky Preset",
    Options = WorldNames,
    ...
    Callback = function(value)
        ...
        if WorldEnabled then
            for _,preset in ipairs(WorldPresets) do
                if preset.Name==value then ApplyWorldPreset(preset); break end
            end
        end
    end,
})
...
-- ══ VISUAL TAB: Unlock All ══
local VisualUnlockSec = Visual:AddSection({ Title = "Unlock", Open = false })
VisualUnlockSec:AddToggle({
    Flag    = "unlock_all_visual",
    Title   = "Unlock All",
    Default = false,
    Callback = function(state)
        getgenv().unlockAllEnabled = state
        if state and getgenv().__runUnlockAll then task.spawn(getgenv().__runUnlockAll) end
    end,
})

-- ══ VISUAL TAB: Low Graphics (from parrytes) ══
local _kLowGraphicsEnabled = false
local _kLowGraphicsConn = nil
local function _kApplyLowGraphics(enable)
    _kLowGraphicsEnabled = enable
    if not enable then
        if _kLowGraphicsConn then _kLowGraphicsConn:Disconnect(); _kLowGraphicsConn = nil end
        return
    end
    local function _kOptimizeObj(obj)
        pcall(function()
            if obj:IsA("BasePart") then obj.CastShadow = false
            elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Smoke")
                or obj:IsA("Sparkles") or obj:IsA("Fire") or obj:IsA("Beam") then
                obj.Enabled = false
    ...
    pcall(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do _kOptimizeObj(obj) end
    ...
    if not _kLowGraphicsConn then
        _kLowGraphicsConn = workspace.DescendantAdded:Connect(function(obj)
            if _kLowGraphicsEnabled then _kOptimizeObj(obj) end
        end)
    end
end

-- ══ VISUAL TAB: Headless & Korblox (from parrytes) ══
local _kIsHeadless = false
local _kIsKorblox  = false

local function _kApplyHeadless(char)
    if not char then return end
    local head = char:FindFirstChild("Head"); if not head then return end
    pcall(function()
        head.Transparency = _kIsHeadless and 1 or 0
        for _, c in ipairs(head:GetChildren()) do
            if c:IsA("Decal") then c.Transparency = _kIsHeadless and 1 or 0 end
        end
    end)
end

local function _kApplyKorblox(char)
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    pcall(function()
        local rll = char:FindFirstChild("RightLowerLeg")
        local rft = char:FindFirstChild("RightFoot")
        local rleg = char:FindFirstChild("Right Leg")
        if rll and rft then
            rll.Transparency = _kIsKorblox and 1 or 0
            rft.Transparency = _kIsKorblox and 1 or 0
        elseif rleg then
            rleg.Transparency = _kIsKorblox and 1 or 0
        end
    end)
    if _kIsKorblox then
        pcall(function()
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    task.wait(0.3)
    if _kIsHeadless then _kApplyHeadless(char) end
    if _kIsKorblox  then _kApplyKorblox(char)  end
end)

-- ══ VISUAL TAB: Skin Changer backend (from parrytes) ══
getgenv()._kSkinChanger     = false
getgenv()._kSwordModel      = ""
getgenv()._kSwordAnimations = ""
getgenv()._kSwordFX         = ""

task.spawn(function()
    local rs = ReplicatedStorage
    local swordInstancesInst = rs:WaitForChild("Shared",9e9):WaitForChild("ReplicatedInstances",9e9):WaitForChild("Swords",9e9)
    local swordInstances = require(swordInstancesInst)
    local _kSwordsCtrl
    task.spawn(function()
        while task.wait() and not _kSwordsCtrl do
            local ok, conns = pcall(getconnections, rs.Remotes.FireSwordInfo.OnClientEvent)
    ...
    local _kSlashCache = {}
    local function _kGetSlash(n)
        if _kSlashCache[n] then return _kSlashCache[n] end
        local ok, sl = pcall(function() return swordInstances:GetSword(n) end)
        local name = (ok and sl and sl.SlashName) or "SlashEffect"
        _kSlashCache[n] = name; return name
    end
    local function _kRefreshSlash()
        local fx = getgenv()._kSwordFX ~= "" and getgenv()._kSwordFX or getgenv()._kSwordModel
        getgenv()._kSlashName = (fx ~= "" and _kGetSlash(fx)) or "SlashEffect"
    end
    local function _kSetSword()
        if not getgenv()._kSkinChanger or not LocalPlayer.Character then return end
        pcall(function()
            local f = rawget(swordInstances, "EquipSwordTo")
        ...
        pcall(function() swordInstances:EquipSwordTo(LocalPlayer.Character, getgenv()._kSwordModel) end)
        task.spawn(function()
            local att = 0
            while not _kSwordsCtrl and att < 20 do task.wait(0.5); att = att + 1 end
            if not _kSwordsCtrl then return end
            pcall(function()
                if _kSwordsCtrl.SetSword then
                    _kSwordsCtrl:SetSword(getgenv()._kSwordAnimations ~= "" and getgenv()._kSwordAnimations or getgenv()._kSwordModel)
                end
            end)
            pcall(function()
                local tgt = getgenv()._kSwordFX ~= "" and getgenv()._kSwordFX or getgenv()._kSwordModel
                if rs.Remotes:FindFirstChild("FireSwordInfo") then rs.Remotes.FireSwordInfo:FireServer(tgt) end
                if _kSwordsCtrl.currentSword ~= nil then _kSwordsCtrl.currentSword = tgt end
                if _kSwordsCtrl.SwordFX ~= nil then _kSwordsCtrl.SwordFX = tgt end
            end)
        end)
    end
    getgenv()._kUpdateSword = function() _kRefreshSlash(); _kSetSword() end
    local _kHookedFX = {}
    task.spawn(function()
        while task.wait(1) do
            ...
            if ok and type(conns) == "table" then
                for _, v in ipairs(conns) do
                    local func = v.Function
                    if func and not _kHookedFX[func] then
                        if isourclosure and isourclosure(func) then
                            _kHookedFX[func] = true
                        else
                            _kHookedFX[func] = true; v:Disable()
                            local tF = func
                            rs.Remotes.ParrySuccessAll.OnClientEvent:Connect(function(...)
                                local args = {...}
                                if tostring(args[4]) == LocalPlayer.Name and getgenv()._kSkinChanger then
                                    local fxS = getgenv()._kSwordFX ~= "" and getgenv()._kSwordFX or getgenv()._kSwordModel
                                    _kRefreshSlash(); args[1] = getgenv()._kSlashName; args[3] = fxS
                                end
                                if setthreadidentity then pcall(setthreadidentity, 2) end
    ...
    task.spawn(function()
        while task.wait(1) do
            if getgenv()._kSkinChanger and getgenv()._kSwordModel ~= "" then
                local char = LocalPlayer.Character
                if char then
                    if LocalPlayer:GetAttribute("CurrentlyEquippedSword") ~= getgenv()._kSwordModel then _kSetSword() end
                    if not char:FindFirstChild(getgenv()._kSwordModel) then _kSetSword() end
                    for _, v in char:GetChildren() do
                        if v:IsA("Model") and v.Name ~= getgenv()._kSwordModel then v:Destroy() end
                        task.wait()
                    end
                end
            end
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        if getgenv()._kSkinChanger then
            getgenv()._kSkinChanger = false; task.wait(1.5)
            getgenv()._kSkinChanger = true; task.wait(0.5)
            pcall(function() getgenv()._kUpdateSword() end)
        end
    end)
end)

-- ══ VISUAL TAB: UI groups ══
local VisualGraphicsSec = Visual:AddSection({ Title = "Graphics", Open = false })
VisualGraphicsSec:AddToggle({
    Flag    = "kk_low_graphics",
    Title   = "Low Graphics Mode",
    Default = false,
    Callback = function(value) _kApplyLowGraphics(value) end,
})

local VisualAppearanceSec = Visual:AddSection({ Title = "Appearance", Open = false })
VisualAppearanceSec:AddButton({
    Title = "Toggle Headless",
    Callback = function()
        _kIsHeadless = not _kIsHeadless
        _kApplyHeadless(LocalPlayer.Character)
    end,
})
VisualAppearanceSec:AddButton({
    Title = "Toggle Korblox",
    Callback = function()
        _kIsKorblox = not _kIsKorblox
        _kApplyKorblox(LocalPlayer.Character)
    end,
})

local VisualSkinSec = Visual:AddSection({ Title = "Skin Changer", Open = false })
VisualSkinSec:AddToggle({
    Flag    = "kk_skin_changer",
    Title   = "Skin Changer",
    Default = false,
    Callback = function(value)
        getgenv()._kSkinChanger = value
        pcall(function() getgenv()._kUpdateSword() end)
    end,
})
VisualSkinSec:AddInput({
    Flag        = "kk_sword_name",
    Title       = "Sword Name",
    Placeholder = "e.g. DualPrince",
    Default     = "",
    Callback    = function(value)
        getgenv()._kSwordModel      = value
        getgenv()._kSwordAnimations = value
        getgenv()._kSwordFX         = value
        pcall(function() getgenv()._kUpdateSword() end)
    end,
})

local VisualLeft = Visual:AddSection({ Title = "Ability ESP", Open = false })
do
    local Visual_Data = {}
...
local VisualRight = Visual:AddSection({ Title = "Ball Statistic", Open = false })
do
...
-- ── No Render ──
local NoRenderSec = MiscTab:AddSection({ Title = "No Render", Open = false })
NoRenderSec:AddToggle({
    Flag    = "VictoriaHub_Misc_NoRender",
    Title   = "No Render",
    Default = false,
    Callback = function(state)
...
-- ── Emotes ──
local EmotesSec = MiscTab:AddSection({ Title = "Emotes", Open = false })
EmotesSec:AddToggle({
    Flag    = "VictoriaHub_Misc_Emotes",
    Title   = "Emotes",
    Default = false,
    Callback = function(value)
        ...
        if value then
            ...
            if selected_animation then animation_system.play(selected_animation) end
        else
...
EmotesSec:AddToggle({
    Flag    = "VictoriaHub_Misc_Emotes_AutoStop",
    Title   = "Auto Stop",
    Default = false,
    Callback = function(value) getgenv().AutoStop = value end,
})
EmotesSec:AddDropdown({
    Flag    = "VictoriaHub_Misc_Emotes_Selected",
    Title   = "Emote Type",
    Options = emotes_data,
    Default = selected_animation,
    Callback = function(value)
        selected_animation = value
        if getgenv().Animations then animation_system.play(value) end
    end,
})

-- ── FOV ──
local FOVSec = MiscTab:AddSection({ Title = "FOV", Open = false })
FOVSec:AddToggle({
    Flag    = "VictoriaHub_Misc_FOV",
    Title   = "FOV",
    Default = false,
    Callback = function(value)
...
FOVSec:AddSlider({
    Flag    = "VictoriaHub_Misc_FOV_Value",
    Title   = "Camera FOV",
    Min     = 50,
    Max     = 120,
    Default = 70,
...
-- ── Character ──
local CharacterSec = MiscTab:AddSection({ Title = "Character", Open = false })
CharacterSec:AddToggle({ Flag="VictoriaHub_Misc_Character",          Title="Character",     Default=false, Callback=function(v) CharacterBackendSetEnabled(v) end })
CharacterSec:AddToggle({ Flag="VictoriaHub_Misc_Character_InfiniteJump", Title="Infinite Jump", Default=false, Callback=function(v) CharacterBackendSetInfiniteJump(v) end })
CharacterSec:AddToggle({
    Flag="VictoriaHub_Misc_Character_Spin", Title="Spin", Default=false,
    Callback=function(v)
        getgenv().SpinbotCheckboxEnabled=v
        if not v and getgenv().CharacterModifierEnabled then
            local c=LocalPlayer.Character
            if c and c:FindFirstChild("Humanoid") and getgenv().OriginalValues then
                c.Humanoid.AutoRotate=getgenv().OriginalValues.AutoRotate or true
            end
        end
    end,
})
CharacterSec:AddSlider({ Flag="VictoriaHub_Misc_Character_SpinSpeed", Title="Spin Speed",   Min=1,  Max=50,    Default=5,     Callback=function(v) getgenv().CustomSpinSpeed=v end })
CharacterSec:AddToggle({
    Flag="VictoriaHub_Misc_Character_WalkSpeed", Title="Walk Speed", Default=false,
    Callback=function(v)
        getgenv().WalkspeedCheckboxEnabled=v
        if not v and getgenv().CharacterModifierEnabled then
            local c=LocalPlayer.Character
            if c and c:FindFirstChild("Humanoid") and getgenv().OriginalValues then
                c.Humanoid.WalkSpeed=getgenv().OriginalValues.WalkSpeed or 16
            end
        end
    end,
})
CharacterSec:AddSlider({
    Flag="VictoriaHub_Misc_Character_WalkSpeedValue", Title="Walk Speed Value", Min=16, Max=500, Default=36,
    Callback=function(v)
...
CharacterSec:AddToggle({
    Flag="VictoriaHub_Misc_Character_JumpPower", Title="Jump Power", Default=false,
    Callback=function(v)
        getgenv().JumpPowerCheckboxEnabled=v
        if not v and getgenv().CharacterModifierEnabled then
            local c=LocalPlayer.Character
            if c and c:FindFirstChild("Humanoid") and getgenv().OriginalValues then
                local h=c.Humanoid
                if h.UseJumpPower then h.JumpPower=getgenv().OriginalValues.JumpPower or 50
                else h.JumpHeight=getgenv().OriginalValues.JumpHeight or 7.2 end
            end
        end
    end,
})
CharacterSec:AddSlider({
    Flag="VictoriaHub_Misc_Character_JumpPowerValue", Title="Jump Power Value", Min=50, Max=200, Default=50,
    Callback=function(v)
        ...
        if getgenv().CharacterModifierEnabled and getgenv().JumpPowerCheckboxEnabled then
            ...
            if c and c:FindFirstChild("Humanoid") then
                local h=c.Humanoid
                if h.UseJumpPower then h.JumpPower=v else h.JumpHeight=v*0.144 end
            end
        end
    end,
})
CharacterSec:AddToggle({
    Flag="VictoriaHub_Misc_Character_Gravity", Title="Gravity", Default=false,
    Callback=function(v)
        getgenv().GravityCheckboxEnabled=v
        if not v and getgenv().CharacterModifierEnabled then workspace.Gravity=196.2 end
    end,
})
CharacterSec:AddSlider({
    Flag="VictoriaHub_Misc_Character_GravityValue", Title="Gravity Value", Min=0, Max=400, Default=196.2,
    Callback=function(v)
...
CharacterSec:AddToggle({
    Flag="VictoriaHub_Misc_Character_HipHeight", Title="Hip Height", Default=false,
    Callback=function(v)
        getgenv().HipHeightCheckboxEnabled=v
        if not v and getgenv().CharacterModifierEnabled then
            local c=LocalPlayer.Character
            if c and c:FindFirstChild("Humanoid") and getgenv().OriginalValues then
                c.Humanoid.HipHeight=getgenv().OriginalValues.HipHeight or 0
            end
        end
    end,
})
CharacterSec:AddSlider({
    Flag="VictoriaHub_Misc_Character_HipHeightValue", Title="Hip Height Value", Min=-5, Max=20, Default=0,
    Callback=function(v)
        getgenv().CustomHipHeight=v
        if getgenv().CharacterModifierEnabled and getgenv().HipHeightCheckboxEnabled then
            local c=LocalPlayer.Character
            if c and c:FindFirstChild("Humanoid") then c.Humanoid.HipHeight=v end
        end
    end,
})

-- ── Immortal ──
ImmortalSec:AddToggle({
    Flag    = "VictoriaHub_immortal",
    Title   = "VictoriaHub Immortal",
    Default = false,
    Callback = function(state)
        if state then startBackgroundOrbit() else stopBackgroundOrbit() end
    end,
})

print("VictoriaHub  1/3")
print("VictoriaHub  2/3")
print("VictoriaHub  3/3")
print("VictoriaHub  success")
