...
local function UpdateBossDropdowns()
  ...
  --  table.sort(Tables.MobList)
    -- Hanya update list tanpa reset selection yang sudah dipilih user
...
local function EquipCorrectWeapon()
    ...
    -- Sudah pegang senjata yang benar?
    local currentTool = char:FindFirstChildOfClass("Tool")
    if currentTool then
        local cType = (GetToolTypeFromModule and GetToolTypeFromModule(currentTool.Name)) or "Melee"
        if not hasFilter or enabledTypes[cType] then return end
...
-- ============================================================
-- MAIN LOOP: FARM + KILL AURA (SATU LOOP, SATU TOGGLE)
-- ============================================================
task.spawn(function()
    while getgenv().celina_Running do
        task.wait(0.05)

        -- ---- KILL AURA (gabung, nyala kalau toggle KillAura ON) ----
        if Toggles.KillAura and Toggles.KillAura.Value then
            local char = GetCharacter()
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
        ...
        -- ---- FARM (nyala kalau salah satu toggle farm ON) ----
        local farmActive = Toggles.MobFarm.Value    or Toggles.AllMobFarm.Value
                        or Toggles.BossesFarm.Value or Toggles.AllBossesFarm.Value
        ...
        -- Island TP via portal jika perlu
        if Toggles.IslandTP and Toggles.IslandTP.Value then
            local neededIsland = GetIslandForTarget(target:GetPivot().Position, target.Name)
            if neededIsland ~= Shared.Island then
                TeleportToIsland(neededIsland)
        ...
        -- Cek ulang setelah TP
        hum = target:FindFirstChildOfClass("Humanoid")
        if not hum or hum.Health <= 0 or not target.Parent then
            Shared.Target = nil; continue
        end

        -- Cek range → gerak atau serang
        local root  = char:FindFirstChild("HumanoidRootPart")
        local tRoot = target:FindFirstChild("HumanoidRootPart")
        if root and tRoot then
            local attackRange = (tonumber(Options.Distance and Options.Distance.Value) or 12) + 3
