...
-- ═══ AUTO-CAPTURE REMOTE VIA KEYPRESS F (SEKALI SAJA) ═════════════════
task.spawn(function()
    task.wait(2)
    pcall(function()
        if keypress then
            ...
            print("[INVIS HUB] Keypress F sent to capture remote.")
        else
            warn("[INVIS HUB] keypress tidak tersedia di executor ini.")
        end
    end)
end)
...
if _reverted and next(_reverted) ~= nil then
    print("[INVIS HUB] Remote parry captured successfully!")
else
    warn("[INVIS HUB] Remote belum ke-capture. Mungkin butuh parry manual 1x.")
end
...
shared._PhysicsBind = RunService.RenderStepped:Connect(function()
    ...
    local autoSpamEnabled = Config.AutoSpam
    local distanceTiming = Config.DistanceTiming or 100
    ...
    for idx, Ball in ipairs(balls) do
        ...
        if autoParryEnabled and allowedToHit then
            ...
            if Speed > 470 then
                ...
                if reachTime <= triggerTimeThreshold or Distance <= emergencyDist then triggerParry = true end
            else
                ...
                if bypassAntiCurve then
                    ...
                    if Speed < 130 then
                        dynamicReachTime = 0.50
                    elseif Speed < 200 then
                        dynamicReachTime = 0.38
                    end

                    if Distance <= strikeZone and reachTime <= dynamicReachTime then
